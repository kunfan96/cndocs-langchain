...
description: "使用 LangChain Python 与 DuckDuckGo 搜索工具集成。"
---
...
默认情况下，结果以原始搜索结果的键值对逗号分隔字符串形式返回。您也可以通过设置 `output_format="list"` 将搜索结果作为列表返回，或通过设置 `output_format="json"` 将其作为 JSON 字符串返回。
...
您也可以仅搜索新闻文章。使用关键字 `backend="news"`
...
您也可以直接将自定义的 `DuckDuckGoSearchAPIWrapper` 传递给 `DuckDuckGoSearchResults`，以对搜索结果提供更多控制。
