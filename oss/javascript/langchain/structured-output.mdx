...
结构化输出允许 agents 以特定的、可预测的格式返回数据。您将获得类型化的结构化数据，而不是解析自然语言响应。

<Tip>
本页涵盖了使用 `createAgent` 的 agents 的结构化输出。要直接在模型上使用结构化输出（在 agents 之外），请参阅 [模型 - 结构化输出](/oss/javascript/langchain/models#structured-output)。
</Tip>

LangChain 的预构建 ReAct agent `createAgent` 自动处理结构化输出。用户设置他们期望的结构化输出模式，当模型生成结构化数据时，它会被捕获、验证并在 agent 状态的 `structuredResponse` 键中返回。
...
控制 agent 如何返回结构化数据。您可以提供 Zod 对象或 JSON schema。默认情况下，agent 使用工具调用策略，其中输出由附加的工具调用创建。某些模型支持原生结构化输出，在这种情况下，agent 将改为使用该策略。
...
<Tip>
    如果使用 `langchain>=1.1`，对原生结构化输出功能的支持将从模型的 [配置文件数据](/oss/javascript/langchain/models#model-profiles) 中动态读取。如果数据不可用，请使用其他条件或手动指定：
    ```typescript
...
## 提供者策略 (Provider strategy)

一些模型提供者通过其 API 原生支持结构化输出（例如 OpenAI, xAI (Grok), Gemini, Anthropic (Claude)）。当可用时，这是最可靠的方法。
...
提供者原生的结构化输出提供了高可靠性和严格的验证，因为模型提供者强制执行模式。尽量使用它。


<Note>
    如果提供者原生支持您选择的模型的结构化输出，则编写 `responseFormat: contactInfoSchema` 与 `responseFormat: providerStrategy(contactInfoSchema)` 在功能上是等效的。

    在任何一种情况下，如果不支持结构化输出，agent 将回退到工具调用策略。
</Note>
...
<ParamField path="options.handleError">
    Options 参数，包含一个可选的 `handleError` 参数，用于自定义错误处理策略。
...
`toolMessageContent` 参数允许您自定义在生成结构化输出时出现在对话历史记录中的消息：
...
通过工具调用生成结构化输出时，模型可能会犯错。LangChain 提供了智能重试机制来自动处理这些错误。

#### 多个结构化输出错误

当模型错误地调用多个结构化输出工具时，agent 在 [`ToolMessage`](https://reference.langchain.com/javascript/langchain-core/messages/ToolMessage) 中提供错误反馈并提示模型重试：
...
您可以使用 `handleErrors` 参数自定义错误的处理方式：
