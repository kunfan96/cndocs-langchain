...
本指南提供了开始使用 `GreenNodeEmbeddings` 的指南。它使您能够通过生成文本的高质量向量表示，使用各种内置连接器或您自己的自定义数据源执行语义文档搜索。
...
GreenNode 需要 API 密钥进行身份验证，可以在初始化时作为 `api_key` 参数提供，也可以设置为环境变量 `GREENNODE_API_KEY`。您可以通过在 [GreenNode Serverless AI](https://aiplatform.console.greennode.ai/playground) 注册帐户来获取 API 密钥。
...
嵌入模型在检索增强生成（RAG）工作流中扮演关键角色，它支持内容的索引及其高效检索。
