---
title: "ChatCohere 集成"
description: "使用 LangChain JavaScript 与 ChatCohere 聊天模型进行集成。"
---

[Cohere](https://cohere.com/) 是一家加拿大初创公司，提供自然语言处理模型，帮助企业改善人机交互。

这将帮助您开始使用 `ChatCohere` [聊天模型](/oss/javascript/langchain/models)。有关所有 `ChatCohere` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-cohere/ChatCohere)。
...
前往 [Cohere 网站](https://dashboard.cohere.com/welcome/register) 注册 Cohere 并生成 API 密钥。完成此操作后，设置 `COHERE_API_KEY` 环境变量：
...
<CodeGroup>
  ```bash npm npm install @langchain/cohere @langchain/core ``` ```bash yarn
  yarn add @langchain/cohere @langchain/core ``` ```bash pnpm pnpm add
...
API 还允许连接其他非静态文档。
一个例子是他们的 `web-search` 连接器，它允许您传递一个查询，API 将在网络上搜索相关文档。
下面的示例演示了如何使用此功能。
...
我们可以在 `additional_kwargs` 对象中看到 API 请求执行了以下操作：

- 执行了一个搜索查询，将结果数据存储在 `searchQueries` 和 `searchResults` 字段中。在 `searchQueries` 字段中，我们看到他们重新表述了我们的查询以获得更好的结果。
- 根据搜索查询生成了三个文档。
- 生成了一个引用列表。
- 根据上述操作和内容生成了最终响应。
...
<div className="source-links">
  <Callout icon="edit">
    [在 GitHub
    上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/javascript/integrations/chat/cohere.mdx)
