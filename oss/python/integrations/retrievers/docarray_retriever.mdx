...
>[DocArray](https://github.com/docarray/docarray) 是一个多功能、开源的工具，用于管理您的多模态数据。它允许您以任何方式塑造数据，并提供灵活性，可以使用各种文档索引后端来存储和搜索数据。此外，它甚至更进一步 - 您可以利用您的 `DocArray` 文档索引来创建一个 `DocArrayRetriever`，并构建出色的 LangChain 应用程序！

本笔记本分为两个部分。[第一部分](#document-index-backends)介绍了所有五个支持的文档索引后端。它提供了设置和索引每个后端的指导，并指导您如何构建一个 `DocArrayRetriever` 来查找相关文档。
...
在开始构建索引之前，定义文档模式非常重要。这决定了您的文档将包含哪些字段以及每个字段将保存什么类型的数据。

在本演示中，我们将创建一个包含 'title' (字符串)、'title_embedding' (numpy 数组)、'year' (整数) 和 'color' (字符串) 的模式。
...
在此了解更多：[docs.docarray.org/user_guide/storing/index_in_memory/](https://docs.docarray.org/user_guide/storing/index_in_memory/)
...
在此了解更多：[docs.docarray.org/user_guide/storing/index_hnswlib/](https://docs.docarray.org/user_guide/storing/index_hnswlib/)
...
在此了解更多：[docs.docarray.org/user_guide/storing/index_weaviate/](https://docs.docarray.org/user_guide/storing/index_weaviate/)

```python
# 与其他后端相比，Weaviate 后端有一个小差异。
# 在这里，您需要使用 'is_embedding=True' 来“标记”用于向量搜索的字段。
# 因此，让我们为 Weaviate 创建一个新的模式来满足此要求。
...
在 [DocArray Elasticsearch 索引文档](https://docs.docarray.org/user_guide/storing/index_elastic/) 中了解更多
...
在 [DocArray Qdrant 索引文档](https://docs.docarray.org/user_guide/storing/index_qdrant/) 中了解更多
