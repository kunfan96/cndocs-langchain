...
本 notebook 介绍如何开始使用 `MLX` 大语言模型作为聊天模型。
...
2. 使用 `ChatMLX` 类，使这些 LLM 能够与 LangChain 的 [Chat Messages](https://python.langchain.com/docs/modules/model_io/chat/#messages) 抽象层对接。
...
这里我们将测试 `gemma-2b-it` 作为零样本 `ReAct` Agent。以下示例取自[此处](https://python.langchain.com/docs/modules/agents/agent_types/react#using-chat-models)。
