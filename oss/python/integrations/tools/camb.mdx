---
title: "CAMB AI 集成"
description: "使用 LangChain Python 集成 CAMB AI 多语言音频和本地化工具。"
---

[CAMB AI](https://camb.ai) 提供支持 140 多种语言的多语言音频和本地化服务，包括文本转语音、翻译、转录、语音克隆和音频生成。
...
| `CambVoiceListTool` | 列出可用的 TTS 语音 | 语音列表的 JSON |
| `CambVoiceCloneTool` | 从 2 秒以上的音频样本克隆语音 | 新的语音 ID |
...
设置 LangSmith 以获得一流的可观测性/<Tooltip tip="记录模型执行的每个步骤以进行调试和改进">跟踪</Tooltip>您的工具调用也是有帮助的（但不是必需的）。要启用自动跟踪，请设置您的 [LangSmith](/langsmith/home) API 密钥：
...
您可以使用 `CambToolkit` 一次获取所有工具，或者实例化单个工具。
...
在 140 多种语言之间翻译文本，可选正式程度控制：

```python
from langchain_camb import CambTranslationTool

# 语言代码（完整列表请参阅下面的语言代码部分）
LANGUAGES = {
    "english": 1,
    "spanish": 54,
    "french": 76,
...
转录音频，将其翻译，并用另一种语言生成语音 - 这是配音工作流程的基础：
...
您可以将 CAMB AI 工具与 LangGraph 代理结合使用，创建强大的多语言 AI 助手：
