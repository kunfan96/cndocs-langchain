---
title: "Clarifai 集成"
description: "使用 LangChain Python 与 Clarifai 向量存储集成。"
---

>[Clarifai](https://www.clarifai.com/) 是一个 AI 平台，提供完整的 AI 生命周期，涵盖数据探索、数据标注、模型训练、评估和推理。上传输入后，Clarifai 应用程序可用作向量数据库。

本笔记本展示了如何使用与 `Clarifai` 向量数据库相关的功能。示例演示了文本语义搜索能力。Clarifai 还支持使用图像、视频帧进行语义搜索，以及本地化搜索（参见 [Rank](https://docs.clarifai.com/api-guide/search/rank)）和属性搜索（参见 [Filter](https://docs.clarifai.com/api-guide/search/filter)）。
...
设置将上传文本数据的用户 ID 和应用 ID。注意：创建该应用程序时，请选择一个合适的基础工作流来索引您的文本文档，例如 Language-Understanding 工作流。
...
# 不使用元数据上传（不推荐）- 因为将无法根据元数据执行搜索操作。
# 自定义 input_id（可选）
response = clarifai_vector_db.add_texts(texts=texts)
```

您可以通过以下方式创建 clarifai 向量数据库存储并将所有输入直接摄取到您的应用中：
...
```python
# 通过利用元数据过滤器，您可以在应用内进行许多强大的过滤。
# 这将把相似性查询限制为仅具有键 "source" 匹配值 "book 1" 的文本
...
创建一个 clarifai 向量数据库类，并将所有文档摄取到 clarifai 应用中。
...
在 Clarifai 内部，我们拥有出色的工具，可以通过 API 或 UI 向应用程序（本质上是项目）添加数据。大多数用户在与 LangChain 交互之前已经完成了此操作，因此本示例将使用现有应用中的数据执行搜索。请查看我们的 [API 文档](https://docs.clarifai.com/api-guide/data/create-get-update-delete) 和 [UI 文档](https://docs.clarifai.com/portal-guide/data)。然后，Clarifai 应用程序可用于语义搜索以查找相关文档。
