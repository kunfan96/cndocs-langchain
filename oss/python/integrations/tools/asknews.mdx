---
title: "Asknews 集成"
description: "使用 LangChain Python 与 Asknews 工具集成。"
---

> [AskNews](https://asknews.app) 通过单个自然语言查询，为任何大语言模型注入最新的全球新闻（或历史新闻）。具体来说，AskNews 每天通过翻译、摘要、实体提取和索引到热向量数据库和冷向量数据库中，丰富超过 30 万篇文章。AskNews 将这些向量数据库放在一个低延迟端点上供您使用。当您查询 AskNews 时，您会得到一个经过提示优化的字符串，其中包含所有最相关的丰富信息（例如实体、分类、翻译、摘要）。这意味着您无需管理自己的新闻 RAG，也无需担心如何以简洁的方式将新闻信息恰当地传达给您的大语言模型。
> AskNews 也致力于透明度，这就是为什么我们的覆盖范围受到监控，并且在数百个国家、13 种语言和 5 万个来源中实现多元化。如果您想跟踪我们的来源覆盖范围，可以访问我们的[透明度仪表板](https://asknews.app/en/transparency)。
...
我们还需要设置 AskNews API 凭据，可以在 [AskNews 控制台](https://my.asknews.app)获取。
...
这里我们展示如何将其作为代理的一部分使用。我们使用 OpenAI Functions Agent，因此需要设置并安装所需的依赖项。我们还将使用 [LangSmith Hub](https://smith.langchain.com/hub) 来拉取提示，因此也需要安装它。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/tools/asknews.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>