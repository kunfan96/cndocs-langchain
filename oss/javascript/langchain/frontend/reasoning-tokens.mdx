---
title: 推理令牌
description: 在可折叠块中展示模型的思考与推理过程
---

推理令牌揭示了像 OpenAI 的 o1/o3 和 Anthropic 的 Claude（具有扩展思考能力）等高级模型的内部思考过程。这些模型生成结构化的内容块，将推理与最终答案分离开来，使您能够构建展示模型*如何*得出其响应的用户界面。
...
这些内容作为 `AIMessage` 中的类型化内容块传递，可通过 `contentBlocks` 属性访问：
...
单个消息可能包含多个推理块（例如，如果模型暂停其推理，生成部分文本，然后继续推理）。将它们连接起来即可获得完整的思考过程。
...
`ThinkingBubble` 在一个视觉上独特的可折叠容器中呈现推理令牌。用户可以展开它以查看完整的思考过程，或折叠它以专注于最终答案。
...
<Tip>
在流式传输期间，默认保持 ThinkingBubble 折叠状态，仅显示旋转器。在流式传输过程中展开可能会导致布局抖动，因为新令牌会不断到达。让用户在推理阶段完成后再展开。
</Tip>
...
并非每条 AI 消息都会包含推理块。当 `contentBlocks` 仅包含文本块时，渲染标准消息气泡，不包含 ThinkingBubble。
...
- **视觉区分**：使用不同的颜色、边框或背景，确保推理永远不会与实际答案混淆
