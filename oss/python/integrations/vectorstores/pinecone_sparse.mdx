...
description: "使用 LangChain Python 与 Pinecone (Sparse) 向量存储进行集成。"
---
...
index_name = "langchain-sparse-vector-search"  # 根据需要更改
...
我们可以使用 `delete` 方法从向量存储中删除记录，并提供要删除的文档 ID 列表。
...
将文档加载到向量存储后，我们很可能已准备好开始查询。在 LangChain 中有多种方法可以实现这一点。

首先，我们将看到如何通过直接使用 `similarity_search` 方法查询我们的 `vector_store` 来执行简单的向量搜索：
...
我们还可以向查询添加[元数据过滤](https://docs.pinecone.io/guides/data/understanding-metadata#metadata-query-language)，以根据各种条件限制搜索范围。让我们尝试一个简单的过滤器，将搜索限制为仅包含 `source=="social"` 的记录：
...
比较这些结果时，我们可以看到我们的第一个查询返回了一个来自 `"website"` 来源的不同记录。在我们后来的、经过过滤的查询中，情况不再如此。

### 相似性搜索和分数

我们还可以在搜索时返回相似性分数，结果是一个 `(document, score)` 元组列表。其中 `document` 是一个 LangChain [`Document`](https://reference.langchain.com/python/langchain-core/documents/base/Document) 对象，包含我们的文本内容和元数据。
...
在我们的链和代理中，我们通常将向量存储用作 `VectorStoreRetriever`。要创建它，我们使用 `as_retriever` 方法：
...
[API 参考](https://reference.langchain.com/python/langchain-pinecone/vectorstores_sparse/PineconeSparseVectorStore)

稀疏嵌入：
[API 参考](https://reference.langchain.com/python/langchain-pinecone/embeddings/PineconeSparseEmbeddings)

---

<div className="source-links">
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/vectorstores/pinecone_sparse.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
