...
description: "使用 LangChain JavaScript 集成 TogetherAIEmbeddings 嵌入模型。"
---

本指南将帮助您开始使用 LangChain 的 [嵌入模型](/oss/javascript/integrations/embeddings) TogetherAIEmbeddings。有关 `TogetherAIEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-community/embeddings/togetherai/TogetherAIEmbeddings)。

## 概述

### 集成详情

| 类 | 包 | 本地 | [Python 支持](https://python.langchain.com/docs/integrations/embeddings/together/) | 下载量 | 版本 |
| :--- | :--- | :---: | :---: |  :---: | :---: |
...
要访问 TogetherAI 嵌入模型，您需要创建一个 TogetherAI 账户，获取 API 密钥，并安装 `@langchain/community` 集成包。

### 凭证

您可以 [注册 Together 账户并创建 API 密钥](https://api.together.xyz/)。完成此操作后，设置 `TOGETHER_AI_API_KEY` 环境变量：

```bash
export TOGETHER_AI_API_KEY="your-api-key"
```

如果您想获得模型调用的自动跟踪，也可以取消下方注释来设置您的 [LangSmith](/langsmith/home) API 密钥：
...
嵌入模型通常用于检索增强生成（RAG）流程中，既作为索引数据的一部分，也用于后续检索。有关更详细的说明，请参阅 [**学习** 标签](/oss/javascript/learn/) 下的 RAG 教程。
...
有关所有 `TogetherAIEmbeddings` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/javascript/langchain-community/embeddings/togetherai/TogetherAIEmbeddings)。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/javascript/integrations/embeddings/togetherai.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>