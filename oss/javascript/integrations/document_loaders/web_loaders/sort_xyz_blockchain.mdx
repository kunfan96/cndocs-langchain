...
/**
 * 请参阅 https://docs.sort.xyz/docs/api-keys 获取您的免费 Sort API 密钥。
 * 请参阅 https://docs.sort.xyz 了解可用查询的更多信息。
 * 请参阅 https://docs.sort.xyz/reference 了解 Sort REST API 的更多信息。
 */
...
export const run = async () => {
  // 初始化 OpenAI 模型。使用 /examples 中 .env 文件里的 OPENAI_API_KEY
  ...
  */
  ...
  const nftPrompt =
    "用以下 JSON 文档中的属性，用一个 4 句话的故事来描述这个角色。";
  ...
  */
  ...
  const latestPrompt =
    "仅用每个文档 4 句话来描述以下 JSON 文档。尽可能包含更多细节。";
  ...
  /*
    使用原始 SQL 和 NFT 索引加载特定 NFT 的元数据。请参阅 https://docs.sort.xyz 了解如何构建 SQL。
  */
  ...
  const sqlPrompt =
    "用以下 JSON 文档中的属性，为一个新咖啡店写一则广告来描述这个角色。";
