...
请按照此处 [HanaRdfGraph](/oss/javascript/integrations/graphs/sap_hana_rdf_graph) 的步骤了解有关创建 `HanaRdfGraph` 实例的更多信息。
...
**前提条件**：
您必须拥有启用了**三元组存储**功能的 SAP HANA Cloud 实例。
...
const query = "哪些演员出演了《银翼杀手》？";
// const query = "数据中有哪些电影？"
// const query = "基努·里维斯和凯瑞-安·莫斯共同出演了哪些电影？"
// const query = "数据中有哪些电影类型？"
// const query = "哪两种电影类型最常被分配？"
// const query = "《银翼杀手》的演员们出生在哪里？"
// const query = "哪些演员在同一部电影中合作过，并且出生在同一个城市？"
...
出演《银翼杀手》的演员包括 Morgan Paull、William Sanderson、James Hong、Brion James、M. Emmet Walsh、Joe Turkel、Daryl Hannah、Joanna Cassidy、Rutger Hauer、Hy Pyke、Sean Young 和 Edward James Olmos。
```

## 底层发生了什么？

1. **SPARQL 生成**
   该链使用您的 Turtle 格式本体（`graph.getSchema()`）和用户问题，通过 `SPARQL_GENERATION_SELECT_PROMPT` 调用 LLM。然后 LLM 会发出一个针对您的模式量身定制的有效 `SELECT` 查询。
2. **预处理与执行**
   * **提取与清理**：从 LLM 的响应中提取原始 SPARQL 文本。
   * **注入图上下文**：如果缺少 `FROM <graph_uri>`，则添加，并确保声明了常见前缀（`rdf:`、`rdfs:`、`owl:`、`xsd:`）。
   * **在 HANA 上运行**：通过 `HanaRdfGraph.query()` 在您的命名图上执行最终确定的查询。
3. **答案制定**
   返回的 CSV（或 Turtle）结果再次输入 LLM——这次使用 `SPARQL_QA_PROMPT`。LLM 会严格基于检索到的数据生成一个简洁、人类可读的答案，而不会产生幻觉。
