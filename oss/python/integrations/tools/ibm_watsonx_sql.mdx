---
title: "IBM watsonx.ai SQL 集成"
description: "使用 LangChain Python 集成 IBM watsonx.ai SQL 工具。"
---

本示例展示如何使用 `langchain-ibm` 的 `watsonx.ai` SQL 数据库 Toolkit（基于 Flight 服务）。

<Warning>
构建 SQL 数据库的问答系统需要执行模型生成的 SQL 查询，这本身存在安全风险。请确保数据库连接权限始终尽可能限制在智能体所需的最小范围内。这可以降低（但不能完全消除）构建模型驱动系统的风险。
</Warning>
...
要访问 `langchain-ibm` SQL 数据库 toolkit，你需要创建一个 IBM watsonx.ai 账户、获取 API 密钥，并安装 `langchain-ibm` 集成包。

### 凭据

此单元格定义了使用 watsonx SQL 数据库 Toolkit 所需的 WML 凭据。
...
要设置 SQL 数据库 toolkit，必须先实例化 `WatsonxSQLDatabase` 类，该类通过 Flight SQL 客户端从 IBM watsonx.ai 数据库连接资产中获取必要信息。有关以编程方式创建数据库连接资产的更多详情，请参阅 watsonx.ai Python SDK [文档](https://ibm.github.io/watsonx-ai-python-sdk/v1.4.7/autoai_working_with_dataconnection.html#connection-asset-databaselocation)。
...
对于某些需求，可以选择将 IBM 的 [`APIClient`](https://ibm.github.io/watsonx-ai-python-sdk/base.html#apiclient) 对象传入 `WatsonxSQLDatabase` 类。
...
有关 `WatsonxSQLDatabaseToolkit` 所有功能和配置的详细文档，请参阅 [API 参考](https://reference.langchain.com/python/integrations/langchain_ibm/WatsonxSQLDatabaseToolkit/)。
