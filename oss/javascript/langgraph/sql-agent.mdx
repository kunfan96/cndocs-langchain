...
LangChain 提供了内置的 [agent](/oss/javascript/langchain/agents) 实现，使用 [LangGraph](/oss/javascript/langgraph/overview) 原语实现。如果需要更深入的自定义，可以直接在 LangGraph 中实现 agent。本指南演示了 SQL agent 的示例实现。你可以在[这里](/oss/javascript/langchain/sql-agent)找到使用更高级 LangChain 抽象构建 SQL agent 的教程。

<Warning>
构建 SQL 数据库的问答系统需要执行模型生成的 SQL 查询。这样做存在固有风险。确保你的数据库连接权限始终尽可能地缩小范围以满足 agent 的需求。这将减轻（但不能消除）构建模型驱动系统的风险。
</Warning>

[预构建 agent](/oss/javascript/langchain/sql-agent) 让我们快速入门，但我们依赖系统提示来约束其行为——例如，我们指示 agent 始终从"list tables"工具开始，并在执行查询之前始终运行 query-checker 工具。
...
- LangGraph [Graph API](/oss/javascript/langgraph/graph-api)，包括 state、nodes、edges 和 conditional edges。
...
我们将使用 `@langchain/classic/sql_db` 模块中提供的便捷 SQL 数据库包装器来与数据库交互。该包装器提供了执行 SQL 查询和获取结果的简单接口：
...
我们现在可以使用 [Graph API](/oss/javascript/langgraph/graph-api) 将这些步骤组装成工作流。我们在查询生成步骤定义一个[条件边](/oss/javascript/langgraph/graph-api#conditional-edges)，如果生成了查询将路由到查询检查器，如果没有工具调用则结束，这样 LLM 已经对查询提供了响应。
...
<Tip>
查看上述运行的 [LangSmith trace](https://smith.langchain.com/public/a6a96896-686a-4040-b9b5-28d701453d6f/r)。
</Tip>
...
在这里，我们利用 LangGraph 的[人机协作](/oss/javascript/langgraph/interrupts)功能在执行 SQL 查询之前暂停运行并等待人工审核。使用 LangGraph 的[持久化层](/oss/javascript/langgraph/persistence)，我们可以无限期地暂停运行（或至少在持久化层存活期间）。
...
让我们现在重新组装我们的图。我们将用人工审核替换程序化检查。注意，我们现在包含一个 [checkpointer](/oss/javascript/langgraph/persistence)；这是暂停和恢复运行所必需的。
