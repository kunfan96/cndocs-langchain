...
当在工具调用操作期间向模型传递不匹配、不足或过多的 [`ToolMessage`](https://reference.langchain.com/javascript/classes/_langchain_core.messages.ToolMessage.html) 对象时，会发生此错误。

此错误源于一个基本要求：带有 `tool_calls` 的 assistant 消息必须后跟响应每个 `tool_call_id` 的工具消息。

当模型返回带有工具调用的 [`AIMessage`](https://reference.langchain.com/javascript/classes/_langchain_core.messages.AIMessage.html) 时，你必须为每个工具调用提供恰好一个对应的 [`ToolMessage`](https://reference.langchain.com/javascript/classes/_langchain_core.messages.ToolMessage.html)，并具有匹配的 `tool_call_id` 值。
...
* **计算匹配对**：确保前置 [`AIMessage`](https://reference.langchain.com/javascript/classes/_langchain_core.messages.AIMessage.html) 中的每个工具调用都有一个 [`ToolMessage`](https://reference.langchain.com/javascript/classes/_langchain_core.messages.ToolMessage.html)
