...
[Playground](/langsmith/prompt-engineering-concepts#playground) 支持多种模型提供商。您可以选择提供商，配置首选设置，并将这些配置保存以在多个提示中重复使用。

请参阅此页面了解可用提供商及其配置选项的列表：
...
在使用此模型之前，请确保您已拥有 [AWS 凭证或 IAM 角色](https://docs.aws.amazon.com/bedrock/latest/userguide/security-iam.html)。
...
使用 IAM 可信实体身份验证，您需要在 AWS 账户中创建一个 IAM 角色，并允许 LangSmith 假设该角色。LangSmith 中不存储任何访问密钥。相反，LangSmith 使用 [AWS STS](https://docs.aws.amazon.com/STS/latest/APIReference/welcome.html) 在每次请求时假设该角色。
...
<Tip>
您可以在 [LangSmith 工作区设置](https://smith.langchain.com/settings) 中找到您的工作区 ID。
</Tip>

3. 在 LangSmith Playground 中，展开 Bedrock 提供商下的 **IAM 可信实体** 部分，并输入您创建的角色的 ARN。

有关信任策略的更多详细信息，请参阅 [AWS 文档](https://aws.amazon.com/blogs/security/how-to-use-trust-policies-with-iam-roles/)。

#### 访问密钥

或者，您可以使用 AWS 访问密钥（`AWS_ACCESS_KEY_ID` 和 `AWS_SECRET_ACCESS_KEY`）进行身份验证。在 Playground 的 Bedrock 提供商配置中输入这些密钥。此方法设置更简单，但安全性较低，因为它需要存储长期有效的凭证。
...
有关当前可用模型列表，请参阅 [AWS Bedrock 文档](https://docs.aws.amazon.com/bedrock/latest/userguide/models-supported.html)。
...
#### 对于 Anthropic 模型
使用 Anthropic 配置（参见下文 [Anthropic](#anthropic) 部分）。
...
在使用此模型之前，请确保您已拥有 [Anthropic API 密钥](https://console.anthropic.com/settings/keys)。
...
有关当前可用模型列表，请参阅 [Anthropic 文档](https://docs.anthropic.com/claude/docs/models-overview)。
...
| **Top K** | 1+ | 可选 | 限制为前 K 个令牌（取消选中以使用模型默认值） |
...
| 参数 | 范围 | 描述 |
|-----------|-------|-------------|
| **Enable Extended Thinking** | 开关 | 显示/隐藏思考过程 |
| **Budget Tokens** | 1+ | 思考的最大令牌数（默认：1024） |
...
在使用此模型之前，请确保您已拥有 [Azure OpenAI 凭证](https://learn.microsoft.com/en-us/azure/ai-services/openai/quickstart)（端点 + API 密钥）。
...
模型可用性因 Azure 区域而异，使用前需要部署。

有关当前可用模型列表，请参阅 [Azure OpenAI 文档](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/models)。
...
| 参数 | 范围 | 描述 |
|-----------|-------|-------------|
| **Temperature** | 0.0 - 2.0 | 控制随机性。值越低 = 越专注，值越高 = 越有创意。 |
| **Max Output Tokens** | 1+ | 响应的最大长度 |
| **Top P** | 0.0 - 1.0 | 核采样阈值。Temperature 的替代方案。 |
| **Presence Penalty** | -2.0 - 2.0 | 惩罚新主题（正值）或鼓励它们（负值） |
| **Frequency Penalty** | -2.0 - 2.0 | 惩罚重复（正值）或允许重复（负值） |
| **Seed** | 整数 | 用于可重现的输出 |

#### 高级参数

**Reasoning Effort:** 在推理优化模型（o 系列和较新的 GPT 模型）上可用。

**Service Tier:** 在较新的模型上可用。
...
在使用此模型之前，请确保您已拥有 [DeepSeek API 密钥](https://platform.deepseek.com/api_keys)。

### 可用模型

DeepSeek 提供通用模型、推理优化模型（R 系列）和编码专用模型。

有关当前可用模型列表，请参阅 [DeepSeek 文档](https://platform.deepseek.com/api-docs/)。
...
在使用此模型之前，请确保您已拥有 [Fireworks API 密钥](https://fireworks.ai/api-keys)。
...
有关当前可用模型列表，请参阅 [Fireworks 模型文档](https://docs.fireworks.ai/models)。
...
在使用此模型之前，请确保您已拥有 [Google AI API 密钥](https://aistudio.google.com/app/apikey)。

### 可用模型

Google 提供多个层级（Ultra、Pro、Flash）的 Gemini 模型，针对不同用例进行了优化。

有关当前可用模型列表，请参阅 [Google Gemini 文档](https://ai.google.dev/models/gemini)。
...
在使用此模型之前，请确保您已拥有 [Google Cloud 凭证](https://cloud.google.com/vertex-ai/docs/authentication)。

### 可用模型

Google 提供多个层级（Ultra、Pro、Flash）的 Gemini 模型，针对不同用例进行了优化，以及通过 Vertex AI 提供的其他模型。

有关当前可用模型列表，请参阅 [Vertex AI 文档](https://cloud.google.com/vertex-ai/docs/generative-ai/models)。
...
在使用此模型之前，请确保您已拥有 [Groq API 密钥](https://console.groq.com/keys)。

### 可用模型

Groq 为流行的开源模型提供高速推理，包括 Llama、Mixtral 和 Gemma 变体。

有关当前可用模型列表，请参阅 [Groq 模型文档](https://console.groq.com/docs/models)。
...
在使用此模型之前，请确保您已拥有 [Mistral AI API 密钥](https://console.mistral.ai/api-keys/)。

### 可用模型

Mistral 提供多个层级（Large、Medium、Small）的模型，针对不同的性能和成本要求进行了优化。

有关当前可用模型列表，请参阅 [Mistral 文档](https://docs.mistral.ai/platform/endpoints/)。
...
在使用此模型之前，请确保您已拥有 [OpenAI API 密钥](https://platform.openai.com/api-keys) 或 [Azure OpenAI 凭证](https://learn.microsoft.com/en-us/azure/ai-services/openai/quickstart)。
...
有关当前可用模型列表，请参阅 [OpenAI 文档](https://platform.openai.com/docs/models)。
...
在使用此模型之前，请确保您已拥有 [xAI API 密钥](https://console.x.ai/)。

### 可用模型

xAI 提供多种尺寸的 Grok 模型，适用于不同的用例。

有关当前可用模型列表，请参阅 [xAI 文档](https://docs.x.ai/docs)。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/langsmith/playground-model-providers.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>