...
LangChain 提供了内置的 [代理](/oss/javascript/langchain/agents) 实现，这些实现使用 [LangGraph](/oss/javascript/langgraph/overview) 原语构建。如果需要更深层次的定制，可以直接在 LangGraph 中实现代理。本指南演示了一个 SQL 代理的示例实现。有关实际介绍，请参阅 [使用更高级的 LangChain 抽象构建 SQL 代理](/oss/javascript/langchain/sql-agent)。

<Warning>
构建 SQL 数据库的问答系统需要执行模型生成的 SQL 查询。这样做存在固有风险。请确保您的数据库连接权限始终根据代理的需求尽可能严格地限定范围。这将减轻（但不能消除）构建模型驱动系统的风险。
</Warning>

[预构建代理](/oss/javascript/langchain/sql-agent) 让我们可以快速开始，但我们依赖系统提示来约束其行为——例如，我们指示代理始终从“列出表”工具开始，并在执行查询之前始终运行查询检查器工具。

我们可以通过定制代理在 LangGraph 中强制执行更高级别的控制。在这里，我们实现一个简单的 ReAct 代理设置，为特定的工具调用设置专用节点。我们将使用与预构建代理相同的 [状态]。
...
- [人机回环](/oss/javascript/langgraph/interrupts) 流程
...
const getSchemaTool = tool(
  ...
  {
    name: "sql_db_schema",
    description: "此工具的输入是表的逗号分隔列表，输出是这些表的模式和示例行。请务必先调用 sql_db_list_tables 以确保表确实存在！示例输入：table1, table2, table3",
    schema: z.object({
      table_names: z.string().describe("表名的逗号分隔列表"),
    }),
  }
);
...
sql_db_schema: 此工具的输入是表的逗号分隔列表，输出是这些表的模式和示例行。请务必先调用 sql_db_list_tables 以确保表确实存在！示例输入：table1, table2, table3
...
将这些步骤放在专用节点中，让我们可以 (1) 在需要时强制工具调用，以及 (2) 定制与每个步骤关联的提示。
...
您是一个设计用于与 SQL 数据库交互的代理。
给定一个输入问题，创建一个语法正确的 ${dialect}
查询来运行，然后查看查询结果并返回答案。除非
用户指定了他们希望获取的特定示例数量，否则始终将
您的查询限制在最多 ${topK} 个结果。

您可以按相关列对结果进行排序，以返回数据库中最有趣的
示例。切勿查询特定表的所有列，
仅根据问题请求相关列。
...
我们现在可以使用 [图 API](/oss/javascript/langgraph/graph-api) 将这些步骤组装成一个工作流。我们在查询生成步骤定义一个 [条件边](/oss/javascript/langgraph/graph-api#conditional-edges)，如果生成了查询，则路由到查询检查器，或者如果没有工具调用存在则结束，这样 LLM 就会向查询提供响应。
...
<Tip>
请参阅 [LangSmith 追踪](https://smith.langchain.com/public/a6a96896-686a-4040-b9b5-28d701453d6f/r) 了解上述运行。
</Tip>


## 6. 实现人机回环审查

在执行代理的 SQL 查询之前检查是否有任何意外操作或低效操作是明智的。

这里我们利用 LangGraph 的 [人机回环](/oss/javascript/langgraph/interrupts) 功能，在执行 SQL 查询之前暂停运行并等待人工审查。使用 LangGraph 的 [持久化层](/oss/javascript/langgraph/persistence)，我们可以无限期地暂停运行（或至少在持久化层存活期间）。

让我们将 `sql_db_query` 工具包装在一个接收人工输入的节点中。我们可以使用 [中断](/oss/javascript/langgraph/interrupts) 函数来实现这一点。下面，我们允许输入来批准工具调用、编辑其参数或提供用户反馈。
...
<Note>
上述实现遵循了更广泛的 [人机回环](/oss/javascript/langgraph/interrupts) 指南中的 [工具中断示例](/oss/javascript/langgraph/interrupts#interrupts-in-tools)。有关详细信息和替代方案，请参阅该指南。
</Note>

现在让我们重新组装我们的图。我们将用人机审查替换程序化检查。请注意，我们现在包含了一个 [检查点](/oss/javascript/langgraph/persistence)；这是暂停和恢复运行所必需的。
...
有关详细信息，请参阅 [人机回环指南](/oss/javascript/langgraph/interrupts)。
