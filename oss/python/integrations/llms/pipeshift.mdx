...
本指南将帮助您开始使用 Pipeshift 补全模型（LLM）与 LangChain。有关 `Pipeshift` 功能和配置选项的详细文档，请参阅 [API 参考](https://dashboard.pipeshift.com/docs)。
...
要访问 Pipeshift 模型，您需要创建 Pipeshift 账户、获取 API 密钥，并安装 `langchain-pipeshift` 集成包。

### 凭证

前往 [Pipeshift](https://dashboard.pipeshift.com) 注册并生成 API 密钥。完成后设置 PIPESHIFT_API_KEY 环境变量：
...
如需自动追踪模型调用，也可以设置 [LangSmith](/langsmith/home) API 密钥（取消注释以下代码）：
...
有关 `Pipeshift` 所有功能和配置的详细文档，请访问 API 参考：[dashboard.pipeshift.com/docs](https://dashboard.pipeshift.com/docs)
