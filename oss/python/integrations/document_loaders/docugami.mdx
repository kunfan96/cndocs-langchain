...
3. 获取一些已处理文档的文档集和文档 ID，如这里所述：[help.docugami.com/home/docugami-api](https://help.docugami.com/home/docugami-api)

```python
# 您需要 dgml-utils 包来使用 DocugamiLoader（如果您不使用 poetry，请直接运行 pip install，无需 "poetry run"）
...
2. 添加您的文档（PDF、DOCX 或 DOC），并允许 Docugami 将它们摄取并聚类成相似文档集，例如 NDA、租赁协议和服务协议。系统没有固定的文档类型支持，创建的集群取决于您的特定文档，您可以稍后[更改文档集分配](https://help.docugami.com/home/working-with-the-doc-sets-view)。
3. 通过开发者游乐场为您的工作区创建访问令牌。[详细说明](https://help.docugami.com/home/docugami-api)
4. 探索 [Docugami API](https://api-docs.docugami.com) 以获取您处理过的文档集 ID 列表，或特定文档集的文档 ID。
5. 使用下面详述的 DocugamiLoader 来获取文档的丰富语义块。
6. 可选地，构建并发布一个或多个[报告或摘要](https://help.docugami.com/home/reports)。这有助于 Docugami 根据您的偏好改进语义 XML 并添加更好的标签，然后这些标签将作为元数据添加到 DocugamiLoader 输出中。使用自查询检索器等技术进行高精度文档问答。

## 与其他分块技术的优势

文档的适当分块对于文档检索至关重要。存在许多分块技术，包括依赖空白的简单技术以及基于字符长度的递归分块拆分。Docugami 提供了一种不同的方法：

1. **智能分块：** Docugami 将每个文档分解为层次化语义 XML 树，包含不同大小的块，从单个单词或数值到整个部分。这些块遵循文档的语义轮廓，提供了比任意长度或简单基于空白的分块更有意义的表示。
2. **语义注释：** 块使用语义标签进行注释，这些标签在文档集中是一致的，便于在多个文档之间进行一致的层次化查询，即使它们的编写和格式不同。例如，在租赁协议集中，您可以轻松识别关键条款，如房东、租户或续订日期，以及更复杂的信息，如任何分租条款的措辞或特定司法管辖区在终止条款中是否有例外部分。
3. **结构化表示：** 此外，XML 树指示每个文档的结构轮廓，使用属性表示标题、段落、列表、表格和其他常见元素，并且在所有支持的文档格式（如扫描的 PDF 或 DOCX 文件）中一致地执行此操作。它适当处理长篇文档特征，如页眉/页脚或多列流，以实现干净的文本提取。
4. **附加元数据：** 如果用户一直在使用 Docugami，块还会附加额外的元数据。此附加元数据可用于高精度文档问答，不受上下文窗口限制。请参阅下面的详细代码演练。
...
如果设置了 DOCUGAMI_API_KEY 环境变量，则无需显式将其传递给加载器，否则您可以将其作为 `access_token` 参数传递。
...
每个 [`Document`](https://reference.langchain.com/python/langchain-core/documents/base/Document)（实际上是实际 PDF、DOC 或 DOCX 的块）的 `metadata` 包含一些有用的附加信息：
...
4. **tag：** 块的语义标签，使用各种生成和提取技术。更多详细信息请参见：[github.com/docugami/DFM-benchmarks](https://github.com/docugami/DFM-benchmarks)

您可以通过在 `DocugamiLoader` 实例上设置以下属性来控制分块行为：

1. 您可以设置最小和最大块大小，系统会尝试遵守这些大小并尽量减少截断。您可以设置 `loader.min_text_length` 和 `loader.max_text_length` 来控制这些。
2. 默认情况下，仅返回块的文本。但是，Docugami 的 XML 知识图包含额外的丰富信息，包括块内实体的语义标签。如果您希望返回的块上附加 XML 元数据，请设置 `loader.include_xml_tags = True`。
...
您可以像标准加载器一样使用 Docugami Loader 进行多文档文档问答，尽管它提供了遵循文档自然轮廓的更好块。有许多关于如何执行此操作的优秀教程，例如[这个](https://www.youtube.com/watch?v=3yPBVii7Ct0)。我们可以使用相同的代码，但使用 `DocugamiLoader` 进行更好的分块，而不是使用基本拆分技术直接加载文本或 PDF 文件。
...
加载器返回的文档已经拆分，因此我们不需要使用文本拆分器。可选地，我们可以使用每个文档上的元数据，例如结构或标签属性，进行任何所需的后处理。

我们将直接使用 `DocugamiLoader` 的输出，以通常的方式设置检索 QA 链。
...
## 使用 docugami 知识图进行高精度文档问答

大型文档的一个问题是，问题的正确答案可能取决于文档中相距较远的块。典型的分块技术，即使有重叠，也难以向 LLM 提供足够的上下文来回答此类问题。随着即将出现的非常大的上下文 LLM，可能将大量令牌（甚至整个文档）塞入上下文中，但这仍然会在某些时候达到限制，特别是对于非常长的文档或大量文档。

例如，如果我们提出一个更复杂的问题，要求 LLM 利用文档不同部分的块，即使是 OpenAI 强大的 LLM 也无法正确回答。
...
乍一看，答案似乎合理，但它是错误的。如果您仔细查看此答案的源块，您会发现文档的分块最终没有将房东名称和可出租面积放在同一上下文中，并产生了不相关的块，因此答案是错误的（应为 **13,500 平方英尺**）

Docugami 可以在这里提供帮助。如果用户一直在[使用 Docugami](https://help.docugami.com/home/reports)，块会使用不同的技术创建的附加元数据进行注释。稍后将添加更多技术方法。
...
让我们再次运行相同的问题。它返回正确的结果，因为所有块都具有元数据键/值对，这些键/值对携带有关文档的关键信息，即使此信息在物理上距离用于生成答案的源块非常远。
...
这次答案是正确的，因为自查询检索器在元数据的房东属性上创建了一个过滤器，正确过滤到专门关于 DHA Group 房东的文档。结果源块都与此房东相关，这提高了答案的准确性，即使房东没有直接在包含正确答案的特定块中提及。

# 高级主题：使用文档知识图层次结构进行小到大检索

文档本质上是半结构化的，DocugamiLoader 能够导航文档的语义和结构轮廓，以在返回的块上提供父块引用。这对于例如使用多向量检索器进行[小到大](https://www.youtube.com/watch?v=ihSiRrOUwmg)检索很有用。
...
loader.max_text_length = (
    1024 * 8
)  # 8K 字符大约 2K 令牌（参考：https://help.openai.com/en/articles/4936856-what-are-tokens-and-how-to-count-them）
loader.include_project_metadata_in_doc_metadata = (
    False  # 不过滤向量元数据，因此移除以减轻向量负担
)
