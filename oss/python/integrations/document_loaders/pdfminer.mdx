...
description: "使用 LangChain Python 集成 PDFMinerLoader 文档加载器。"
---

本指南提供了 `PDFMiner` [文档加载器](https://python.langchain.com/docs/concepts/document_loaders) 的快速入门概述。有关所有 __ModuleName__Loader 特性和配置的详细文档，请参阅 [API 参考](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.pdf.PDFMinerLoader.html)。
...
使用 PDFMinerLoader 无需任何凭据

如果您希望获得模型调用的自动最优追踪，也可以通过取消注释以下代码来设置您的 [LangSmith](/langsmith/home) API 密钥：
...
默认情况下，PDFMinerLoader 将按页分割 PDF。
...
这有助于将文本按语义分割为各个部分，因为输出的 HTML 内容可以通过 `BeautifulSoup` 解析，以获取更结构化和丰富的信息，例如字体大小、页码、PDF 页眉/页脚等。
...
有关 `PDFMinerLoader` 所有特性和配置的详细文档，请参阅 API 参考：[python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.pdf.PDFMinerLoader.html](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.pdf.PDFMinerLoader.html)
