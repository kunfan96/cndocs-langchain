...
[**Functional API**](/oss/javascript/langgraph/functional-api) 允许你用最少的代码更改为应用程序添加 LangGraph 的关键功能——[持久化](/oss/javascript/langgraph/persistence)、[记忆](/oss/javascript/langgraph/add-memory)、[人工介入](/oss/javascript/langgraph/interrupts)和[流式传输](/oss/javascript/langgraph/streaming)。
...
**Functional API** 和 [**Graph API**](/oss/javascript/langgraph/graph-api) 可以在同一应用程序中一起使用，因为它们共享相同的底层运行时。
...
**Functional API** 使用与 **Graph API** 相同的流式传输机制。请阅读[**流式传输指南**](/oss/javascript/langgraph/streaming)部分了解更多详情。
...
Functional API 支持使用 [`interrupt`](https://reference.langchain.com/javascript/functions/_langchain_langgraph.index.interrupt.html) 函数和 `Command` 原语的[人工介入](/oss/javascript/langgraph/interrupts)工作流。
...
[interrupt()](/oss/javascript/langgraph/interrupts#pause-using-interrupt) 在任务内部调用，使人工可以审核和编辑前一个任务的输出。先前任务的结果——在本例中是 `step_1`——被持久化，因此在 [`interrupt`](https://reference.langchain.com/javascript/functions/_langchain_langgraph.index.interrupt.html) 之后不会再次运行。
...
注意，我们在 `step_1` 之后通过 [`interrupt`](https://reference.langchain.com/javascript/functions/_langchain_langgraph.index.interrupt.html) 暂停了。中断提供了恢复运行的说明。要恢复，我们发出一个包含 `human_feedback` 任务期望数据的 [`Command`](/oss/javascript/langgraph/interrupts#resuming-interrupts)。
...
要在执行前审核工具调用，我们添加一个调用 [`interrupt`](/oss/javascript/langgraph/interrupts#pause-using-interrupt) 的 `review_tool_call` 函数。当此函数被调用时，执行将暂停直到我们发出命令恢复它。

给定一个工具调用，我们的函数将 [`interrupt`](https://reference.langchain.com/javascript/functions/_langchain_langgraph.index.interrupt.html) 以供人工审核。此时我们可以：
...
短期记忆允许在同一 **thread id** 的不同**调用**之间存储信息。有关更多详情，请参阅[短期记忆](/oss/javascript/langgraph/functional-api#short-term-memory)。
...
使用 Functional API 和 [`InMemorySaver`](https://reference.langchain.com/javascript/classes/_langchain_langgraph-checkpoint.MemorySaver.html) 检查点器的简单聊天机器人示例。
...
[长期记忆](/oss/javascript/concepts/memory#long-term-memory)允许在不同的 **thread id** 之间存储信息。这对于在一个对话中学习关于给定用户的信息并在另一个对话中使用它很有用。

## 工作流

* [工作流和 agent](/oss/javascript/langgraph/workflows-agents) 指南提供了更多使用 Functional API 构建工作流的示例。
