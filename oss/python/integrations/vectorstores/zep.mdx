...
> 回忆、理解并从聊天记录中提取数据。为个性化 AI 体验提供动力。
...
在下面的示例中，我们使用 Zep 的自动嵌入功能，该功能在 Zep 服务器上使用低延迟嵌入模型自动嵌入文档。

## 注意

- 这些示例使用 Zep 的异步接口。通过从方法名中移除 `a` 前缀来调用同步接口。
- 如果您传入一个 [`Embeddings`](https://reference.langchain.com/python/langchain-core/embeddings/embeddings/Embeddings) 实例，Zep 将使用它来嵌入文档，而不是自动嵌入。
  您还必须将文档集合设置为 `isAutoEmbedded === false`。
- 如果您将集合设置为 `isAutoEmbedded === false`，则必须传入一个 [`Embeddings`](https://reference.langchain.com/python/langchain-core/embeddings/embeddings/Embeddings) 实例。
...
使用元数据过滤器缩小结果范围。首先，加载另一本书："福尔摩斯历险记"
