...
暂停智能体执行，以便在工具调用执行前进行人工批准、编辑或拒绝。[Human in the Loop](/oss/javascript/langchain/human-in-the-loop)对于以下情况很有用：
...
<Warning>
  Human in the
  Loop中间件需要一个[检查点](/oss/javascript/langgraph/persistence#checkpoints)来在中断期间维护状态。
</Warning>
...
<Tip>
  ...
  Loop文档](/oss/javascript/langchain/human-in-the-loop)。
</Tip>

<Callout icon="player-play" iconType="solid">
  观看此[视频指南](https://www.youtube.com/watch?v=tdOeUVERukA)演示Human in the
  Loop中间件的行为。
</Callout>
...
<ParamField
  body="retryOn"
  type="((error: Error) => boolean) | (new (...args: any[]) => Error)[]"
  default="() => true"
>
...
<ParamField
  body="onFailure"
  type="'error' | 'continue' | ((error: Error) => string)"
  default="continue"
>
