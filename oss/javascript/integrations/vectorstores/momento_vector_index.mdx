...
[MVI](https://gomomento.com)：为您数据打造的最高效、最易用的无服务器向量索引。要开始使用 MVI，只需注册一个账户。无需处理基础设施、管理服务器或担心扩展问题。MVI 是一项可自动扩展以满足您需求的服务。无论是在 Node.js、浏览器还是边缘环境，Momento 都能为您提供支持。
...
与上面类似，此示例演示了使用 `fromDocuments` 方法实例化向量存储并索引文档。
如果索引不存在，则会创建它。如果索引已存在，则文档将被添加到现有索引中。
