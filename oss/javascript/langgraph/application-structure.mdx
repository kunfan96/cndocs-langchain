...
本指南展示了应用程序的典型结构，并展示了如何提供所需的配置以便通过 [LangSmith Deployment](/langsmith/deployments) 部署应用程序。

<Info>
LangSmith Deployment 是一个用于部署和扩展 LangGraph 代理的托管平台。它处理基础设施、扩展和运维问题，以便您可以直接从仓库部署有状态、长运行的代理。在 [部署文档](/langsmith/deployments) 中了解更多信息。
</Info>
...
使用 [LangGraph 配置文件](#configuration-file-concepts) 中的 `graphs` 键指定在部署的 LangGraph 应用程序中可用的图。

您可以在配置文件中指定一个或多个图。每个图由一个名称（应唯一）和一个路径标识，路径可以是：(1) 编译后的图，或 (2) 定义图的函数。
