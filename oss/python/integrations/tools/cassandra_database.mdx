...
>`Cassandra 数据库`工具包使 AI 工程师能够高效地将代理与 Cassandra 数据集成，提供以下功能：
>
...
`Cassandra 查询语言 (CQL)` 是与 Cassandra 数据库交互的主要*以人为中心*的方式。虽然在生成查询时提供了一定的灵活性，但它需要了解 Cassandra 数据建模的最佳实践。LLM 函数调用赋予代理推理并选择工具以满足请求的能力。使用 LLM 的代理在选择适当的工具包或工具链时，应使用 Cassandra 特定的逻辑进行推理。这减少了当 LLM 被迫提供自上而下解决方案时引入的随机性。您希望 LLM 拥有完全不受限制的数据库访问权限吗？嗯，可能不希望。为此，我们提供了一个提示，用于在为代理构建问题时使用：
...
连接通过 `cassio` 使用 `auto=True` 参数进行，笔记本使用 OpenAI。您应相应地创建一个 `.env` 文件。
...
cassandra_db_select_table_data -
    ...
    使用 WHERE 子句指定使用主键的查询谓词。空白谓词将返回所有行。尽可能避免这种情况。
    使用 limit 指定要返回的行数。空白限制将返回所有行。
```
...
input = (
    QUERY_PATH_PROMPT
    + "\n\n以下是您的任务：查找电子邮件地址为 'patrick@datastax.com' 的用户上传到 langchain_agent_test 键空间的所有视频。"
)
...
遵循此查询路径，我们发现 user_id 为 `522b1fe2-2e36-4cef-a667-cd4237d08b89` 的用户至少上传了一个标题为 'DataStax Academy'、描述为 'DataStax Academy is a free resource for learning Apache Cassandra.' 的视频。此视频的 video_id 为 `27066014-bad7-9f58-5a30-f63fe03718f6`。如果有更多视频，可以使用相同的查询来检索它们，必要时可能需要增加限制。
