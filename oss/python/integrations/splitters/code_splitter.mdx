...
[RecursiveCharacterTextSplitter](https://reference.langchain.com/python/langchain-text-splitters/character/RecursiveCharacterTextSplitter) 包含预构建的分隔符列表，这些分隔符对于在特定编程语言中[分割文本](/oss/python/integrations/splitters/)非常有用。
...
要实例化一个为特定语言定制的分割器，请将枚举中的一个值传递给
```python
