...
> [制表符分隔值 (TSV)](https://en.wikipedia.org/wiki/Tab-separated_values) 文件是一种简单的、基于文本的文件格式，用于存储表格数据。[3] 记录由换行符分隔，记录中的值由制表符分隔。

## `UnstructuredTSVLoader`

你也可以使用 `UnstructuredTSVLoader` 加载表格。使用 `UnstructuredTSVLoader` 的一个优点是，如果你在 `"elements"` 模式下使用它，表格的 HTML 表示将可在元数据中获取。
