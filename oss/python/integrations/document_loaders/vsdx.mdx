...
> [Visio 文件](https://fr.wikipedia.org/wiki/Microsoft_Visio)（扩展名为 .vsdx）与 Microsoft Visio 相关联，这是一款图表创建软件。它存储有关图表结构、布局和图形元素的信息。此格式便于在商业、工程和计算机科学等领域创建和共享可视化内容。

一个 Visio 文件可以包含多个页面。其中一些页面可能作为其他页面的背景，并且这可以跨越多个层级。此**加载器**从每个页面及其关联页面中提取文本内容，从而能够提取每个页面的所有可见文本，类似于 OCR 算法所做的工作。

**警告**：只有扩展名为 **.vsdx** 的 Visio 文件与此加载器兼容。扩展名为 .vsd 等的文件不兼容，因为它们无法转换为压缩 XML。
