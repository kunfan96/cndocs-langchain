...
本指南提供了通过 CLOVA Studio 快速上手 Naver HyperCLOVA X [聊天模型](https://python.langchain.com/docs/concepts/chat_models) 的概览。有关 ChatClovaX 所有功能和配置的详细文档，请参阅 [API 参考](https://guide.ncloud-docs.com/docs/clovastudio-dev-langchain)。

[CLOVA Studio](http://clovastudio.ncloud.com/) 提供多种聊天模型。您可以在 CLOVA Studio 指南[文档](https://guide.ncloud-docs.com/docs/clovastudio-model)中找到有关最新模型的信息，包括费用、上下文窗口和支持的输入类型。
...
3. 创建您要使用的模型的 CLOVA Studio 测试应用或服务应用（参见[此处](https://guide.ncloud-docs.com/docs/clovastudio-playground-testapp)）。
...
CLOVA Studio 支持工具调用（也称为"[函数调用](https://api.ncloud-docs.com/docs/clovastudio-chatcompletionsv3-fc)"），允许您描述工具及其参数，并让模型返回包含要调用的工具及其输入的 JSON 对象。这对于构建使用工具的链和 Agent，以及从模型中获取结构化输出非常有用。

**注意**：在 CLOVA Studio 中使用工具调用功能时，应将 `max_tokens` 设置为大于 1024 的值。
...
注意 `AIMessage` 具有 [`tool_calls`](https://reference.langchain.com/python/langchain/messages/#langchain.messages.AIMessage.tool_calls) 属性。该属性以标准 `ToolCall` 格式存储，与模型提供商无关。
...
对于支持的模型，当启用[思考](https://api.ncloud-docs.com/docs/clovastudio-chatcompletionsv3-thinking)功能时（默认启用），模型将输出得出最终答案的逐步推理过程。
...
您可以通过将 `task_id` 以 `ft:{task_id}` 的格式传递给 `model` 参数来调用微调模型。
