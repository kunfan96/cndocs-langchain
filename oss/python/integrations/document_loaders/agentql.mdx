...
[AgentQL](https://www.agentql.com/) 的文档加载器使用 [AgentQL 查询](https://docs.agentql.com/agentql-query)从任意网页中提取结构化数据。AgentQL 可跨多种语言和网页使用，不会随时间和变化而失效。

## 概述

`AgentQLLoader` 需要以下两个必填参数：

- `url`：您要提取数据的网页 URL。
- `query`：要执行的 AgentQL 查询。详细了解[如何编写 AgentQL 查询](https://docs.agentql.com/agentql-query)，或在 [AgentQL Playground](https://dev.agentql.com/playground) 中测试。

以下参数为可选项：

- `api_key`：您在 [dev.agentql.com](https://dev.agentql.com) 获取的 AgentQL API key。**`可选`。**
- `timeout`：请求超时前等待的秒数。**默认为 `900`。**
- `is_stealth_mode_enabled`：是否启用实验性反机器人规避策略。此功能可能并非在所有网站上始终有效。启用此模式后数据提取可能需要更长时间。**默认为 `False`。**
- `wait_for`：提取数据前等待页面加载的秒数。**默认为 `0`。**
- `is_scroll_to_bottom_enabled`：是否在提取数据前滚动到页面底部。**默认为 `False`。**
- `mode`：`"standard"` 使用深度数据分析，而 `"fast"` 以牺牲部分分析深度换取速度，适用于大多数用例。[在此指南中了解更多关于模式的信息。](https://docs.agentql.com/accuracy/standard-mode) **默认为 `"fast"`。**
- `is_screenshot_enabled`：是否在提取数据前截图。以 Base64 字符串形式返回在 'metadata' 中。**默认为 `False`。**

AgentQLLoader 使用 AgentQL 的 [REST API](https://docs.agentql.com/rest-api/api-reference) 实现。
...
要使用 AgentQL 文档加载器，您需要配置 `AGENTQL_API_KEY` 环境变量，或使用 `api_key` 参数。您可以从我们的[开发者门户](https://dev.agentql.com)获取 API key。
...
`AgentQLLoader` 目前每次只加载一个 [`Document`](https://reference.langchain.com/python/langchain-core/documents/base/Document)。因此，`load()` 和 `lazy_load()` 的行为相同：
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/document_loaders/agentql.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
