...
description: "使用 LangChain Python 集成 ManticoreSearch VectorStore。"
---

[ManticoreSearch](https://manticoresearch.com/) 是一款开源搜索引擎，提供快速、可扩展且用户友好的功能。它起源于 [Sphinx Search](http://sphinxsearch.com/) 的一个分支，并在此基础上融入了现代搜索引擎特性和改进。ManticoreSearch 以其强大的性能和易于集成到各类应用中而著称。

ManticoreSearch 近期引入了[向量搜索功能](https://manual.manticoresearch.com/dev/Searching/KNN)，从搜索引擎版本 6.2 开始支持，且仅在安装了 [manticore-columnar-lib](https://github.com/manticoresoftware/columnar) 包的情况下可用。该功能是一项重大进步，可基于向量相似度执行搜索。

目前，向量搜索功能仅在搜索引擎的开发（dev）版本中可用。因此，必须使用开发版 [manticoresearch-dev](https://pypi.org/project/manticoresearch-dev/) Python 客户端才能有效使用此功能。
