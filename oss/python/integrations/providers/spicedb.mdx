...
> [SpiceDB](https://authzed.com/spicedb) 是一个开源的、基于图的授权系统，灵感来源于 Google Zanzibar。它为您的应用程序提供细粒度的、基于关系的访问控制。
...
`SpiceDBRetriever` 将任何 LangChain 检索器包装上 SpiceDB 授权过滤，移除用户无权访问的文档。
...
`SpiceDBAuthFilter` 是一个与 LCEL 兼容的可运行对象，用于链中的授权。`SpiceDBAuthLambda` 是一个轻量级包装器，可与 `RunnableLambda` 一起使用。
