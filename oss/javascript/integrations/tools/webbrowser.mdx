---
title: "Web 浏览器集成"
description: "使用 LangChain JavaScript 与 Web 浏览器工具集成。"
---

Webbrowser 工具使您的代理能够访问网站并提取信息。它被描述为代理的

```
当您需要在网页上查找内容或总结网页时非常有用。输入应为一个逗号分隔的列表，包含“有效的 URL（包括协议）”、“您想在页面上查找的内容”或用于总结的空字符串。
```

它公开了两种操作模式：

- 当代理仅使用 URL 调用时，它会生成网站内容的摘要
- 当代理使用 URL 和要查找内容的描述调用时，它将使用内存中的向量存储来查找最相关的片段并总结这些片段

## 设置

要使用 Webbrowser 工具，您需要安装依赖项：
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/javascript/integrations/tools/webbrowser.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
