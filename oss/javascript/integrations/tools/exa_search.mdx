...
description: "使用 LangChain JavaScript 与 ExaSearchResults 工具集成。"
---

Exa（前身为 Metaphor Search）是一个专为 LLM 使用而设计的搜索引擎。使用自然语言查询在互联网上搜索文档，然后从所需文档中检索已清理的 HTML 内容。

与基于关键词的搜索（如 Google）不同，Exa 的神经搜索功能使其能够语义理解查询并返回相关文档。例如，我们可以搜索 `"fascinating article about cats"` 并比较 Google 和 Exa 的搜索结果。Google 会根据关键词“fascinating”提供经过 SEO 优化的列表文章。而 Exa 则直接有效。

本页介绍如何将 `ExaSearchResults` 与 LangChain 结合使用。

## 概述

### 集成详情

| 类 | 包 | 可序列化 | [Python 支持](https://python.langchain.com/docs/integrations/tools/exa_search/) | 版本 |
| :--- | :--- | :---: | :---: | :---: |
...
首先，获取 Exa API 密钥并将其添加为环境变量。通过[注册 Exa](https://dashboard.exa.ai/login) 可获得每月 1000 次免费搜索。

```typescript
process.env.EXASEARCH_API_KEY="your-api-key"
```

设置 LangSmith 以获得最佳可观测性也很有帮助（但不是必需的）：
...
我们可以将工具用于链中，首先将其绑定到一个[工具调用模型](/oss/javascript/langchain/tools)，然后调用它：
...
我们可以创建使用 `ExaRetriever` 和 `createRetrieverTool` 的 LangChain 工具。使用这些工具，我们可以构建一个简单的搜索代理，可以回答有关任何主题的问题。

我们将使用 LangGraph 创建代理。确保已安装 `@langchain/langgraph`：
...
有关所有 `ExaSearchResults` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-exa/ExaSearchResults)

---

<div className="source-links">
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/javascript/integrations/tools/exa_search.mdx) 或[提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
