---
title: 消息队列
description: 队列化多个消息并在代理顺序处理时进行管理
---
...
`useStream` 钩子公开了一个 `queue` 属性，该属性提供对挂起消息的实时可见性：
...
构建一个 `QueueList` 组件，显示每条挂起的消息并带有取消按钮。这使用户能够看到等待的内容，并能够移除不再需要的项目。
...
当用户想要开始新的对话时，使用 `switchThread(null)` 创建一个新线程。这将清除当前消息历史记录和队列。
...
- **限制队列大小**：虽然客户端对队列大小没有硬性限制，但请注意，非常大的队列会降低用户体验。考虑在队列超过合理阈值（例如 10 个项目）时显示警告。
- **显示队列位置**：为每个排队项目编号，以便用户了解处理顺序。
- **保留输入焦点**：提交后保持输入字段聚焦，以便用户可以立即输入下一条消息。
- **动画过渡**：当项目开始处理时，平滑地将它们从队列面板移动到消息列表。
- **优雅地处理错误**：如果排队消息失败，请显示错误而不阻止后续队列条目。
