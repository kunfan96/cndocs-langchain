...
代理可以调用外部工具，如天气 API、计算器、网络搜索、数据库查询等。结果以原始 JSON 格式返回。此模式展示了如何为代理进行的每个工具调用渲染结构化、类型安全的 UI 卡片，包括加载状态和错误处理。
...
一个 AI 消息可能触发多个工具调用，而您的聊天可能包含许多 AI 消息。为了在每个消息下渲染正确的工具卡片，请通过匹配 `call.id` 与消息的 `tool_calls` 数组来过滤：
...
<Note>
工具调用会原地更新。相同的 `call.id` 会从 `"pending"` 转换为 `"completed"`（或 `"error"`），因此您的 UI 会使用新状态重新渲染相同的组件。
</Note>
