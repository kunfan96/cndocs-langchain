...
本指南提供了快速入门 `LinkupSearchTool` [工具](/oss/python/langchain/tools)的概览。
...
要使用 Linkup 提供商，您需要一个有效的 API 密钥，您可以通过[注册](https://app.linkup.so/sign-up)来获取。要运行以下示例，您还需要一个 OpenAI API 密钥。
...
这里我们展示如何实例化一个 LinkupSearchTool 实例，使用
