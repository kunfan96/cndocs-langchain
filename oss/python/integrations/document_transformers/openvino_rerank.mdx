---
title: "OpenVINO 重排序集成"
description: "使用 LangChain Python 集成 OpenVINO 重排序文档转换器。"
---

[OpenVINO™](https://github.com/openvinotoolkit/openvino) 是一个用于优化和部署 AI 推理的开源工具套件。OpenVINO™ Runtime 支持多种硬件[设备](https://github.com/openvinotoolkit/openvino?tab=readme-ov-file#supported-hardware-matrix)，包括 x86 和 ARM CPU 以及 Intel GPU。它有助于提升计算机视觉、自动语音识别、自然语言处理及其他常见任务中的深度学习性能。

Hugging Face 重排序模型可以通过 `OpenVINOReranker` 类由 OpenVINO 提供支持。如果您有 Intel GPU，可以指定 `model_kwargs={"device": "GPU"}` 在其上运行推理。
...
让我们从初始化一个简单的向量存储检索器开始，并存储 2023 年国情咨文演讲（分块处理）。我们可以将检索器设置为检索较多数量（20 篇）的文档。
...
现在让我们使用 `OpenVINOReranker` 作为压缩器，将基础检索器封装到 `ContextualCompressionRetriever` 中。
...
重排序后，排名前 4 的文档与基础检索器检索到的前 4 篇文档有所不同。
