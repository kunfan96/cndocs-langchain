---
title: "Naver 搜索集成"
description: "使用 LangChain Python 与 Naver 搜索工具集成。"
---

Naver 搜索工具提供了一个简单的界面来搜索 Naver 并获取结果。
...
**搜索**：Naver 搜索工具提供了一个简单的界面来搜索 Naver 并获取结果。

## 设置

### 设置 API 凭证

要使用 Naver 搜索，您需要获取 API 凭证。请按照以下步骤操作：

登录 [Naver 开发者门户](https://developers.naver.com/main/)。
创建一个新应用并启用搜索 API。
从“应用列表”部分获取您的 **NAVER_CLIENT_ID** 和 **NAVER_CLIENT_SECRET**。

### 设置环境变量

获取凭证后，在脚本中将它们设置为环境变量：
...
Naver 搜索工具可以集成到 LangChain 代理中，以执行更复杂的任务。下面我们演示如何设置一个可以搜索 Naver 获取当前信息的代理。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/tools/naver_search.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>