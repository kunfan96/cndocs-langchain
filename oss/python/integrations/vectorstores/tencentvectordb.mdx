---
title: "腾讯云向量数据库集成"
description: "使用 LangChain Python 集成腾讯云向量数据库向量存储。"
---

>[腾讯云向量数据库](https://cloud.tencent.com/document/product/1709) 是一款全托管、自研的企业级分布式数据库服务，专为存储、检索和分析多维向量数据而设计。该数据库支持多种索引类型和相似度计算方法。单个索引可支持高达十亿级别的向量规模，并能支持百万级 QPS 和毫秒级查询延迟。腾讯云向量数据库不仅可以为大模型提供外部知识库以提高大模型响应的准确性，还可广泛应用于推荐系统、NLP 服务、计算机视觉和智能客服等 AI 领域。

本笔记本展示了如何使用与腾讯向量数据库相关的功能。

运行前，您应拥有一个[数据库实例](https://cloud.tencent.com/document/product/1709/95101)。
...
以下代码展示了两种嵌入文档的方式，您可以通过注释掉其中一种来选择使用：

```python
## 您可以使用 LangChain 嵌入模型，例如 OpenAIEmbeddings：
...
现在我们可以创建一个 TencentVectorDB 实例，您必须至少提供 `embeddings` 或 `t_vdb_embedding` 参数中的一个。如果两者都提供了，将使用 `embeddings` 参数：
...
现在我们将创建一个新的带有元数据的 TencentVectorDB 集合，并演示如何根据元数据过滤搜索结果：
