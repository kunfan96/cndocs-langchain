...
本指南将帮助您开始使用 IMAP [检索器](/oss/python/integrations/retrievers)。`ImapRetriever` 支持从 IMAP 服务器搜索和检索电子邮件，并将其作为 LangChain `Document` 对象返回。
...
若要使用 docling 进行完整的文档处理（DOCX、PPTX 等）（未经测试）：
...
要使用 `ImapRetriever`，您需要使用 `ImapConfig` 配置您的 IMAP 服务器详情：
...
- **verify_cert**: 对于自签名证书，设置为 `False`（不建议在生产环境中使用）
- **k**: 要检索的文档数量
...
- `"full_content"`：使用 docling 从办公文档中完整提取（需要 `[docling]` 额外依赖）
...
与其他检索器一样，`ImapRetriever` 可以通过链集成到 LLM 应用程序中。以下是一个完整示例，它使用 LLM 生成 IMAP 查询并根据邮件内容回答问题：
...
# 使用链
TODO = full_chain.invoke("请根据主题中包含 URGENT 的邮件创建一个待办事项列表")
print(TODO)
```

### 清理测试环境

如果您使用了 GreenMail 测试容器，请在测试后清理它：
