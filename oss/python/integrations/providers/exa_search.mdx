---
title: "Exa 集成"
description: "使用 LangChain Python 与 Exa 进行集成。"
---

>[Exa](https://exa.ai/) 是面向 AI 和开发者的知识 API。
>

## 安装与配置

`Exa` 集成存在于其独立的 [合作伙伴包](https://pypi.org/project/langchain-exa/) 中。您可以使用以下命令进行安装：

```python
pip install -qU langchain-exa
```

要使用该包，您还需要将 `EXA_API_KEY` 环境变量设置为您自己的 Exa API 密钥。

## 检索器

您可以在标准的检索流程中使用 [`ExaSearchRetriever`](/oss/python/integrations/tools/exa_search#using-exasearchretriever)。您可以按如下方式导入它。

请参阅 [使用示例](/oss/python/integrations/tools/exa_search)。
...
您可以将 Exa 用作智能体工具，具体说明请参考 [Exa 工具调用文档](/oss/python/integrations/tools/exa_search#use-within-an-agent)。

请参阅 [使用示例](/oss/python/integrations/tools/exa_search)。

### ExaFindSimilarResults

一个查询 Metaphor Search API 并返回 JSON 数据的工具。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/providers/exa_search.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
