...
>[阿里云 MaxCompute](https://www.alibabacloud.com/product/maxcompute)（原名 ODPS）是一个通用、全托管、多租户的大规模数据仓库处理平台。MaxCompute 支持多种数据导入方案和分布式计算模型，使用户能够高效查询海量数据集、降低生产成本并确保数据安全。

`MaxComputeLoader` 允许你执行 MaxCompute SQL 查询，并将结果加载为每行一个文档。
...
要实例化加载器，你需要一个要执行的 SQL 查询、你的 MaxCompute 端点和项目名称，以及你的访问 ID 和密钥访问密钥。访问 ID 和密钥访问密钥可以通过 `access_id` 和 `secret_access_key` 参数直接传入，也可以设置为环境变量 `MAX_COMPUTE_ACCESS_ID` 和 `MAX_COMPUTE_SECRET_ACCESS_KEY`。
...
你可以使用 `page_content_columns` 和 `metadata_columns` 参数来配置哪些列子集应加载为文档的内容，哪些应加载为元数据。
