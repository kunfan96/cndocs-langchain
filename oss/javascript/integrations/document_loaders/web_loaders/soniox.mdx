...
您也可以使用 `two_way` 翻译类型同时转录和翻译两种语言。了解有关 [Soniox 翻译](https://soniox.com/docs/stt/async/async-translation) 的更多信息。
...
语言提示**不会限制**识别——它们只会**偏向**指定的语言，同时仍然允许检测其他语言（如果存在）。
...
有关更多详细信息，请参阅 [Soniox 语言提示文档](https://soniox.com/docs/stt/concepts/language-hints)。
...
有关更多详细信息，请参阅 [Soniox 上下文文档](https://soniox.com/docs/stt/concepts/context)。
...
浏览[文档](https://soniox.com/docs/stt/api-reference/transcriptions/create_transcription)以获取支持选项的完整列表。
...
元数据包括转录文本、说话人信息（如果启用了区分）、语言信息（如果启用了识别）、翻译数据（如果启用了翻译）以及时间信息。
