---
title: "PGVector 集成"
description: "使用 LangChain Python 与 PGVector 向量存储集成。"
---

> 这是使用 `postgres` 作为后端并利用 `pgvector` 扩展的 LangChain 向量存储抽象的实现。

代码位于名为 [langchain-postgres](https://github.com/langchain-ai/langchain-postgres/) 的集成包中。
...
目前**没有机制**支持在模式变更时轻松进行数据迁移。因此，向量存储的任何模式变更都需要用户重新创建表并重新添加文档。
如果这是一个顾虑，请使用其他向量存储。如果不是，则此实现应该适合你的使用场景。
...
运行本笔记本不需要任何凭据，只需确保你已下载 `langchain-postgres` 包并正确启动了 postgres 容器。

如果你希望获得最佳的模型调用自动追踪，也可以取消以下注释来设置你的 [LangSmith](/langsmith/home) API 密钥：
...
有关可在 `PGVector` 向量存储上执行的不同搜索的完整列表，请参阅 [API 参考](https://python.langchain.com/api_reference/postgres/vectorstores/langchain_postgres.vectorstores.PGVector.html)。
...
有关所有 __ModuleName__VectorStore 功能和配置的详细文档，请参阅 API 参考：[python.langchain.com/api_reference/postgres/vectorstores/langchain_postgres.vectorstores.PGVector.html](https://python.langchain.com/api_reference/postgres/vectorstores/langchain_postgres.vectorstores.PGVector.html)
