...
Valthera 是一个开源框架，使 LLM 代理能够以更有意义的方式与用户互动。它基于 BJ Fogg 的行为模型 (B=MAT) 构建，并利用来自多个来源（如 HubSpot、PostHog 和 Snowflake）的数据，在触发操作之前评估用户的**动机**和**能力**。
...
在本节中，我们实例化核心组件。首先，我们创建一个数据聚合器来组合来自自定义连接器的数据。然后，我们配置动机和能力的评分指标。
...
# 您现在可以通过调用 data_aggregator.get_user_context(user_id) 来获取统一的用户数据
```
