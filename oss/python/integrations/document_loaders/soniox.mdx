...
示例：如何使用 `SonioxDocumentLoader` 转录音频文件，并使用 LLM 生成摘要。
...
Soniox 自动检测并转录 [60 多种语言](https://soniox.com/docs/stt/concepts/supported-languages) 的语音。当您知道音频中可能出现哪些语言时，提供 `language_hints` 可以通过偏向这些语言来提高识别准确性。

语言提示**不会限制**识别——它们只是**偏向**指定的语言，同时仍然允许检测其他语言（如果存在）。
...
提供特定领域的[上下文](https://soniox.com/docs/stt/concepts/context)以提高转录准确性。上下文帮助模型理解您的领域、识别重要术语并应用自定义词汇。
...
您也可以使用 `two_way` 翻译类型同时转录和翻译两种语言。更多信息，请参阅[异步翻译](https://soniox.com/docs/stt/async/async-translation)。
...
| `file_data` | `bytes` | 否\* | `None` | 要转录的音频文件的二进制数据 |
| `file_url` | `str` | 否\* | `None` | 要转录的音频文件的 URL |
| `api_key` | `str` | 否 | `SONIOX_API_KEY` 环境变量 | Soniox API 密钥 |
| `base_url` | `str` | 否 | `https://api.soniox.com/v1` | API 基础 URL（参见[区域端点][endpoints]） |
| `options` | `SonioxTranscriptionOptions` | 否 | `SonioxTranscriptionOptions()` | 转录选项 |
| `polling_interval_seconds` | `float` | 否 | `1.0` | 状态轮询之间的时间（秒） |
| `timeout_seconds` | `float` | 否 | `300.0` (5 分钟) | 等待转录完成的最长时间 |
| `http_request_timeout_seconds` | `float` | 否 | `60.0` | 单个 HTTP 请求的超时时间 |

\* 您必须**恰好指定一个**：`file_path`、`file_data` 或 `file_url`。
...
| `language_hints_strict` | `bool` | 强制执行严格语言提示 |
| `enable_speaker_diarization` | `bool` | 启用说话人识别 |
| `enable_language_identification` | `bool` | 启用语言检测 |
| `translation` | `TranslationConfig` | 翻译配置 |
| `context` | `StructuredContext` | 用于提高准确性的上下文 |
| `client_reference_id` | `str` | 您记录的自定义参考 ID |
| `webhook_url` | `str` | 用于完成通知的 Webhook URL |
| `webhook_auth_header_name` | `str` | Webhook 的自定义身份验证标头名称 |
| `webhook_auth_header_value` | `str` | Webhook 的自定义身份验证标头值 |

浏览 [API 文档](https://soniox.com/docs/stt/api-reference/transcriptions/create_transcription) 以获取支持选项的完整列表。

[models]: https://soniox.com/docs/stt/models

### 返回值

`lazy_load()` 和 `alazy_load()` 方法生成单个 `Document` 对象：
...
Document(
    ...
    metadata={
        ...
        "transcription_id": str,  # 唯一的转录 ID
        "audio_duration_ms": int,  # 音频持续时间（毫秒）
        "model": str,  # 用于转录的模型
        "created_at": str,  # ISO 8601 时间戳
        "tokens": list[dict],  # 详细的令牌级信息
    }
)
```

元数据中的 `tokens` 数组包含每个转录单词的详细信息：

- `text`: 转录的文本
- `start_ms`: 开始时间（毫秒）
- `end_ms`: 结束时间（毫秒）
- `speaker`: 说话人 ID（如果启用了区分），例如 `"1"`、`"2"` 等
- `language`: 检测到的语言（如果启用了识别），例如 `"en"`、`"fr"` 等
- `translation_status`: 翻译状态（`"original"`、`"translated"` 或 `"none"`）

了解有关 [Soniox API 参考](https://soniox.com/docs/stt/api-reference/transcriptions/get_transcription_transcript) 的更多信息。
