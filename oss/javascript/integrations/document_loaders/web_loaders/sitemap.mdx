...
本笔记本介绍如何使用 [`SitemapLoader`](https://reference.langchain.com/javascript/langchain-community/document_loaders/web/sitemap/SitemapLoader) 类将站点地图加载到 `Document` 中。
...
传入的 URL 必须包含站点地图的 `.xml` 路径，否则将默认在 URL 后附加 `/sitemap.xml`。
...
或者，如果您只想加载站点地图而不加载站点地图中每个页面的内容，可以使用 `parseSitemap` 方法：
