...
`NVIDIARAGRetriever` 需要一个正在运行的 NVIDIA RAG Blueprint 服务器。请参阅 [NVIDIA RAG Blueprint 文档](https://docs.nvidia.com/rag/latest/index.html) 了解部署说明。默认情况下，服务器监听 `http://localhost:8081`，并期望其向量数据库中至少有一个已摄取的集合。
...
| 参数 | 类型 | 默认值 | 描述 |
|-----------|------|---------|-------------|
| `base_url` | `str` | — | RAG Blueprint 服务器的基本 URL |
