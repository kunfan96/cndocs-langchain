---
title: "VoyageAI 重排序集成"
description: "使用 LangChain Python 集成 VoyageAI 重排序文档转换器。"
---

>[Voyage AI](https://www.voyageai.com/) 提供尖端的嵌入/向量化模型。

本 notebook 演示如何在检索器中使用 [Voyage AI 的重排序接口](https://api.voyageai.com/v1/rerank)。这是基于 ContextualCompressionRetriever 的思路构建的。
...
让我们从初始化一个简单的向量存储检索器开始，并存储 2023 年国情咨文演讲（分块处理）。我们可以将检索器设置为检索较多数量（20 篇）的文档。您可以使用以下任意嵌入模型（[来源](https://docs.voyageai.com/docs/embeddings)）：
...
现在让我们使用 `ContextualCompressionRetriever` 封装基础检索器，并使用 Voyage AI 重排序器对返回结果进行重排序。您可以使用以下任意重排序模型（[来源](https://docs.voyageai.com/docs/reranker)）：
