---
title: "Friendli 集成"
description: "使用 LangChain Python 与 Friendli LLM 进行集成。"
---

> [Friendli](https://friendli.ai/) 通过可扩展、高效的部署选项，增强 AI 应用性能并优化成本节约，专为高需求 AI 工作负载量身定制。

本教程将指导你完成将 `Friendli` 与 LangChain 集成的过程。
...
默认模型是 `meta-llama-3.1-8b-instruct`。你可以在 [friendli.ai/docs](https://friendli.ai/docs/guides/serverless_endpoints/pricing#text-generation-models) 查看可用模型。
...
你可以使用 `invoke`、`batch`、`generate` 和 `stream` 功能。
...
你也可以使用所有异步 API 功能：`ainvoke`、`abatch`、`agenerate` 和 `astream`。
