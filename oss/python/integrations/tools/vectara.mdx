...
[Vectara](https://vectara.com/) 是面向关键业务应用的可信 AI 助手与代理平台，专注于企业级可靠性。更多[详情](../providers/vectara.ipynb)。

[Vectara](https://vectara.com/) 提供了多种可与 LangChain 配合使用的工具。

- **VectaraSearch**：对您的语料库进行语义搜索
- **VectaraRAG**：使用 RAG 生成摘要
- **VectaraIngest**：将文档摄入您的语料库
- **VectaraAddFiles**：上传文件

## 设置

要使用 `Vectara Tools`，首先需要安装合作伙伴包。
...
1. 如果您还没有账户，请[注册](https://www.vectara.com/integrations/langchain)免费 Vectara 试用。
2. 在您的账户中，可以创建一个或多个语料库。每个语料库代表一个存储文本数据的区域，用于存放从输入文档摄入的内容。点击 **"Create Corpus"** 按钮创建语料库，然后为其提供名称和描述。可选地，您可以定义过滤属性并应用一些高级选项。点击已创建的语料库，即可在顶部看到其名称和语料库 ID。
3. 接下来，您需要创建 API 密钥来访问语料库。在语料库视图中点击 **"Access Control"** 标签页，然后点击 **"Create API Key"** 按钮。为密钥命名，并选择仅查询或查询+索引权限。点击"Create"，即可获得一个有效的 API 密钥。请妥善保管此密钥。

要将 LangChain 与 Vectara 配合使用，您需要以下两个值：`corpus_key` 和 `api_key`。
您可以通过两种方式向 LangChain 提供 `VECTARA_API_KEY`：

## 实例化

1. 在您的环境中包含以下变量：`VECTARA_API_KEY`。

   例如，可以使用 os.environ 和 getpass 按如下方式设置这些变量：
...
首先，我们将《国情咨文》文本加载到 Vectara 中。

请注意，我们使用 `VectaraAddFiles` 工具，无需在本地进行任何处理或分块——Vectara 接收文件内容后会自动完成所有必要的预处理、分块和嵌入操作，将其存入知识库。

此示例使用 .txt 文件，但同样适用于许多其他[文件类型](https://docs.vectara.com/docs/api-reference/indexing-apis/file-upload/file-upload-filetypes)。
...
- 我们启用摘要功能，指定希望 LLM 选取前 7 个匹配块并以英文作答

使用此配置，让我们创建一个封装完整 Vectara RAG 流水线的 LangChain 工具 `VectaraRAG` 对象：
...
`VectaraSearch` 工具可以直接作为检索器使用。

在这种情况下，它与其他任何 LangChain 检索器的行为相同。此模式主要用于语义搜索，在此情况下我们禁用摘要功能：
...
您可以将 Vectara 工具与其他 LangChain 组件链式组合。以下示例展示了如何：

- 设置 ChatOpenAI 模型进行额外处理
- 创建自定义提示词模板以满足特定摘要需求
- 使用 LangChain 的 Runnable 接口将多个组件链接在一起
...
以下代码演示如何将 Vectara 工具与 LangChain 配合使用来创建代理。
...
`VectaraIngest` 工具允许您直接将文本内容摄入 Vectara 语料库。当您有文本内容想要添加到语料库而无需先创建文件时，此工具非常有用。
...
有关 Vectara [工具](https://github.com/vectara/langchain-vectara/blob/main/libs/vectara/langchain_vectara/tools.py)实现的详细信息，请参阅源代码。
