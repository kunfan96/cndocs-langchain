...
[CopilotKit](https://www.copilotkit.ai/) 提供了一个完整的 React 聊天运行时，当您希望代理返回**结构化 UI 载荷**而不仅仅是纯文本时，它与 LangGraph 配合得尤其出色。在此模式中，您的 LangGraph 部署同时提供图 API 和自定义 CopilotKit 端点，而前端则将助手消息解析为动态 React 组件。
...
关键思想是 LangGraph 部署不仅提供图服务。它还可以加载一个 HTTP 应用，这允许您在部署本身旁边挂载额外的路由。
...
这个中间件使得前端的 `useAgentContext({ description: "output_schema", ... })` 变得有用。CopilotKit 运行时转发该模式，而代理将其转换为模型必须遵循的结构化输出契约。
