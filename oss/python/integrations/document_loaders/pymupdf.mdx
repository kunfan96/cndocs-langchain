...
本笔记本提供了快速入门 `PyMuPDF` [文档加载器](https://python.langchain.com/docs/concepts/document_loaders) 的概览。有关所有 __ModuleName__Loader 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/python/langchain-community/document_loaders/pdf/PyMuPDFLoader)。
...
如果您想获得模型调用的自动化一流跟踪，也可以取消注释以下内容来设置您的 [LangSmith](https://docs.smith.langchain.com/) API 密钥：
...
您可以调整这些函数，以在 *html*、*markdown* 或 *text* 之间选择提取图像的输出格式。

结果插入在页面文本的最后和倒数第二段之间。
...
因此，将解析逻辑与加载逻辑解耦会很有帮助，这样无论数据如何加载，都可以更轻松地重用给定的解析器。
...
有关所有 `PyMuPDFLoader` 功能和配置的详细文档，请访问 API 参考：[API 参考](https://reference.langchain.com/python/langchain-community/document_loaders/pdf/PyMuPDFLoader)
