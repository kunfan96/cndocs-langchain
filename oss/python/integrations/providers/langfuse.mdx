...
> **什么是 Langfuse？** [Langfuse](https://langfuse.com) 是一个开源的 LLM 工程平台，帮助团队追踪 API 调用、监控性能，并调试 AI 应用中的问题。

## 追踪 LangChain

[Langfuse 追踪](https://langfuse.com/docs/tracing)通过 LangChain Callbacks（[Python](https://python.langchain.com/docs/how_to/#callbacks)、[JS](https://js.langchain.com/docs/how_to/#callbacks)）与 LangChain 集成。因此，Langfuse SDK 会自动为 LangChain 应用的每次运行创建嵌套追踪，让您可以记录、分析和调试 LangChain 应用。

您可以通过（1）构造函数参数或（2）环境变量来配置集成。在 [cloud.langfuse.com](https://cloud.langfuse.com) 注册或[自托管 Langfuse](https://langfuse.com/self-hosting) 以获取凭据。
...
本节演示如何使用 [Langfuse](https://langfuse.com/docs) 通过 [LangChain 集成](https://langfuse.com/docs/integrations/langchain/tracing)调试、分析和迭代 LangGraph 应用。

### 初始化 langfuse

**注意：** 您需要运行至少 Python 3.11（[GitHub Issue](https://github.com/langfuse/langfuse/issues/1926)）。

使用项目设置中 Langfuse UI 的 [API 密钥](https://langfuse.com/faq/all/where-are-langfuse-api-keys)初始化 Langfuse 客户端，并将其添加到您的环境中。
...
首先创建一个 `StateGraph`。`StateGraph` 对象将我们的聊天机器人结构定义为状态机。我们将添加节点来表示 LLM 和聊天机器人可调用的函数，以及边来指定机器人如何在这些函数之间转换。
...
现在，我们将 [LangChain 的 Langfuse 回调处理器](https://langfuse.com/docs/integrations/langchain/tracing) 添加到追踪中：`config={"callbacks": [langfuse_handler]}`
...
![Langfuse 中对话应用的追踪视图](https://langfuse.com/images/cookbook/integration-langgraph/integration_langgraph_chatapp_trace.png)

- 查看[完整 notebook](https://langfuse.com/docs/integrations/langchain/example-python-langgraph) 获取更多示例。
- 要了解如何评估 LangGraph 应用的性能，请查看 [LangGraph 评估指南](https://langfuse.com/docs/integrations/langchain/example-langgraph-agents)。

---

<div className="source-links">
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/providers/langfuse.mdx) 或 [提交 issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
