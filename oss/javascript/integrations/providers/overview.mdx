...
LangChain 与各种聊天与嵌入模型、工具与工具包、文档加载器、向量存储等进行了集成。

**提供商** 是 LangChain 与之集成以访问 AI 功能（如聊天模型、嵌入和向量存储）的第三方服务或平台。这些提供商拥有独立的 `langchain-provider` 包，以改进版本控制、依赖管理和测试。
...
查看[所有提供商](/oss/javascript/integrations/providers/all_providers)或使用搜索字段搜索提供商。
