...
目前，**没有机制**支持在架构更改时轻松迁移数据。向量存储中的任何架构更改都将要求用户重新创建表并重新添加文档。
如果这是一个问题，请使用其他向量存储。如果不是，此实现应该适合您的用例。
...
运行此笔记本不需要任何凭证，只需确保您已下载 `langchain-postgres` 包并正确启动了 postgres 容器。
...
有关所有 PGVector VectorStore 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/python/langchain-postgres/vectorstores/PGVector)
