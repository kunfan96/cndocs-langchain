...
sidebarTitle: "聊天"
description: "使用 LangChain JavaScript 与 ChatAnthropic 聊天模型集成。"
---

[Anthropic](https://www.anthropic.com/) 是一家 AI 安全与研究公司。他们是 Claude 的创造者。

这将帮助您开始使用 `ChatAnthropic` [聊天模型](/oss/javascript/langchain/models)。有关所有 `ChatAnthropic` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-anthropic/ChatAnthropic)。
...
您需要注册并获取 [Anthropic API 密钥](https://www.anthropic.com/)，并安装 `@langchain/anthropic` 集成包。

### 凭证

前往 [Anthropic 网站](https://www.anthropic.com/) 注册 Anthropic 并生成 API 密钥。完成此操作后，设置 `ANTHROPIC_API_KEY` 环境变量：
...
需要注意的是，Anthropic 模型与大多数其他模型的一个关键区别在于，单个 Anthropic [`AIMessage`](https://reference.langchain.com/javascript/langchain-core/messages/AIMessage) 的内容可以是单个字符串，也可以是**内容块列表**。例如，当 Anthropic 模型[调用工具](/oss/javascript/langchain/tools)时，工具调用是消息内容的一部分（同时也暴露在标准化的 `AIMessage.tool_calls` 字段中）：
...
Anthropic 支持[缓存部分提示](https://platform.claude.com/docs/en/build-with-claude/prompt-caching)，以降低需要长上下文的用例的成本。您可以缓存工具以及整个消息和单个块。

包含一个或多个块或工具定义且带有 `"cache_control": { "type": "ephemeral" }` 字段的初始请求将自动缓存该部分提示。此初始缓存步骤会产生额外费用，但后续请求将以降低的费率计费。缓存的生命周期为 5 分钟，但每次命中缓存时都会刷新。要进行更长时间的缓存，请在 `cache_control` 字段中指定 `"ttl": "1h"`。

存在一个最小可缓存提示长度，该长度根据模型而异。有关更多信息，请参阅[提示缓存详细信息](https://platform.claude.com/docs/en/build-with-claude/prompt-caching#cache-limitations)。

以下是一个缓存包含 LangChain [概念文档](/oss/javascript/concepts/) 的系统消息部分的示例：
...
我们可以看到，在从 Anthropic 返回的原始使用情况字段中有一个名为 `cache_creation_input_tokens` 的新字段。

如果我们再次使用相同的消息，我们可以看到长文本的输入 token 是从缓存中读取的：
...
您还可以通过在工具定义中设置相同的 `"cache_control": { "type": "ephemeral" }` 来缓存工具。这目前需要您以 [Anthropic 的原始工具格式](https://platform.claude.com/docs/en/agents-and-tools/tool-use/overview) 绑定工具。以下是一个示例：
...
有关提示缓存工作原理的更多信息，请参阅 [Anthropic 文档](https://platform.claude.com/docs/en/build-with-claude/prompt-caching#how-prompt-caching-works)。

## 自定义客户端

Anthropic 模型[可能托管在 Google Vertex 等云服务上](https://platform.claude.com/docs/en/build-with-claude/claude-on-vertex-ai)，这些服务依赖于与主要 Anthropic 客户端接口相同的底层客户端。您可以通过提供一个返回已初始化 Anthropic 客户端实例的 `createClient` 方法来访问这些服务。以下是一个示例：
...
在此示例中，我们传递[搜索结果](https://platform.claude.com/docs/en/build-with-claude/search-results)作为消息内容的一部分。这允许 Claude 在其响应中引用来自您自己的检索系统的特定段落或片段。

当您希望 Claude 引用来自特定知识集的信息，但您希望直接提供自己预取/缓存的内容，而不是让模型自动搜索或检索时，此方法很有帮助。
...
您还可以使用工具提供搜索结果，模型可以在其响应中引用这些结果。这非常适合 RAG（或[检索增强生成](https://en.wikipedia.org/wiki/Retrieval-augmented_generation)）工作流，其中 Claude 可以决定何时何地从何处检索信息。当以[搜索结果](https://platform.claude.com/docs/en/build-with-claude/search-results)的形式返回此信息时，它使 Claude 能够根据从工具返回的材料创建引用。

以下是如何创建一个以 Anthropic 引用 API 期望的格式返回搜索结果的工具：
...
了解有关 [LangChain 中 RAG 工作原理](https://js.langchain.com/docs/concepts/rag/) 的更多信息

[了解有关工具调用的更多信息](/oss/javascript/langchain/tools)

### 与文本分割器一起使用

Anthropic 还允许您使用[自定义文档](https://platform.claude.com/docs/en/build-with-claude/citations#custom-content-documents)类型指定您自己的拆分。LangChain 文本分割器可用于为此目的生成有意义的拆分。请参阅下面的示例，其中我们拆分了 LangChain.js README（一个 markdown 文档）并将其作为上下文传递给 Claude：
...
有关所有 `ChatAnthropic` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-anthropic/ChatAnthropic)。

---

<div className="source-links">
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/javascript/integrations/chat/anthropic.mdx) 或[提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
