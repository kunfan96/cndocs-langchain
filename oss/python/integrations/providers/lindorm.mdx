...
Lindorm 是阿里云的云原生多模数据库，它支持对宽表、时序、文本、对象、流和空间数据等多种类型数据进行统一访问和集成处理。它兼容 SQL、HBase/Cassandra/S3、TSDB、HDFS、Solr 和 Kafka 等多种标准接口，并与第三方生态系统工具无缝集成。这使其适用于日志、监控、计费、广告、社交网络、出行和风控等场景。Lindorm 也是支持阿里巴巴核心业务的数据库之一。
...
要使用部署在 Lindorm AI 服务中的嵌入模型，请导入 LindormAIEmbeddings。
