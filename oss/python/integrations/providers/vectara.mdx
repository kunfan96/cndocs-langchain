...
[Vectara](https://vectara.com/) 是一个值得信赖的 AI 助手和代理平台，专注于为关键任务应用提供企业级就绪能力。
Vectara 无服务器 RAG 即服务在易于使用的 API 背后提供了 RAG 的所有组件，包括：
...
本笔记本展示了如何使用基本检索功能，当仅将 Vectara 用作向量存储（不进行摘要）时，包括：`similarity_search` 和 `similarity_search_with_score`，以及使用 LangChain 的 `as_retriever` 功能。
...
3. 接下来，您需要创建 API 密钥来访问语料库。在语料库视图中点击 **"Access Control"** 选项卡，然后点击 **"Create API Key"** 按钮。为您的密钥命名，并选择您希望密钥是仅查询还是查询+索引。点击 "Create"，您现在就拥有一个有效的 API 密钥。请保密此密钥。
...
请注意，我们使用的是 add_files 接口，它不需要任何本地处理或分块 - Vectara 接收文件内容并执行所有必要的预处理、分块和将文件嵌入到其知识存储中。
...
- 我们启用摘要，指定我们希望 LLM 选择前 7 个匹配的块并用英语回复
...
有关 Vectara 作为向量存储的更多详细信息，请[转到此笔记本](../vectorstores/vectara.ipynb)。
...
Vectara 提供智能查询重写选项，通过自动从自然语言查询生成元数据过滤表达式来提高搜索精度。此功能分析用户查询，提取相关的元数据过滤器，并重新表述查询以专注于核心信息需求。
