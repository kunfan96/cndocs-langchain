---
title: 单元测试
description: 使用假聊天模型和内存持久化测试智能体逻辑，无需调用 API。
---
...
[`fakeModel`](https://reference.langchain.com/javascript/langchain/index/fakeModel) 是一个构建器风格的假聊天模型，允许您编写精确的响应（文本、工具调用、错误）并断言模型接收的内容。它扩展自 [`BaseChatModel`](https://reference.langchain.com/javascript/langchain-core/language_models/chat_models/BaseChatModel)，因此可以在任何需要真实模型的地方使用。
...
const model = fakeModel()
  .respond(new Error("rate limit exceeded"))  // 第 1 轮：抛出错误  // [!code highlight]
...
`.respond()` 也接受一个函数，该函数根据输入消息计算响应。函数接收完整的消息数组，并返回一个 [`BaseMessage`](https://reference.langchain.com/javascript/langchain-core/messages/BaseMessage) 或一个 `Error`：
...
了解如何使用真实模型提供商 API 测试您的智能体，请参阅[集成测试](/oss/javascript/langchain/test/integration-testing)。
