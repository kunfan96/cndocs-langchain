...
有关 SAP HANA 向量存储的设置详细信息，请参阅 [向量存储：SAP HANA](/oss/javascript/integrations/vectorstores/sap_hanavector) 指南。
...
为了能够以良好的性能进行自查询，我们为 HANA 中的向量存储表创建了额外的元数据字段：
...
现在是重头戏：这是如何为 HANA 向量存储构建 SelfQueryRetriever：
