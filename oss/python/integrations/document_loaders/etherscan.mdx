---
title: "Etherscan 集成"
description: "使用 LangChain Python 与 Etherscan 文档加载器集成。"
---

>[Etherscan](https://docs.etherscan.io/) 是领先的区块链浏览器、搜索、API 和分析平台，适用于以太坊（一个去中心化智能合约平台）。

## 概述

`Etherscan` 加载器使用 `Etherscan API` 来加载以太坊主网上特定账户下的交易历史。

您需要一个 `Etherscan api key` 才能继续。免费的 api key 每秒有 5 次调用配额。
...
* 检索以太坊主网上特定账户下的以太坊余额（以 wei 为单位）

如果账户没有相应的交易，加载器将返回一个包含一个文档的列表。文档内容为空。

您可以向加载器传递不同的过滤器以访问上述不同功能：
...
所有与交易历史相关的功能都受限于 Etherscan 的限制，最多 1000 条历史记录。您可以使用以下参数查找所需的交易历史：
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/document_loaders/etherscan.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
