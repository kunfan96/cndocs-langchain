...
<Warning>
  你可能已经注意到我们使用了 `ends` 来指定 `nodeA`
  可以导航到哪些节点。这对于图渲染是必要的，并告诉 LangGraph `nodeA` 可以导航到
  `nodeB` 和 `nodeC`。
</Warning>
...
const State = new StateSchema({
  // 注意：我们在这里定义了一个归约器
  foo: new ReducedValue(z.string().default(""), { // [!code highlight]
    reducer: (x, y) => x + y,
  }),
});
...
<Warning>
  ...
  中的消息列表**必须**包含一个
  `ToolMessage`。这对于生成的消息历史记录有效是必要的（LLM
  提供商要求带有工具调用的 AI 消息后必须跟有工具结果消息）。
</Warning>
...
<div className="source-links">
  ...
  <Callout icon="edit">
    [在 GitHub
    上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/langgraph/use-graph-api.mdx)
