---
title: "Redis 缓存与 LangChain 集成"
description: "使用 LangChain Python 集成 Redis 缓存。"
---

本 Notebook 演示如何使用 langchain-redis 包中的 `RedisCache` 和 `RedisSemanticCache` 类为 LLM 响应实现缓存。

## 设置

首先，安装所需依赖并确保有 Redis 实例正在运行。

```python
pip install -U langchain-core langchain-redis langchain-openai redis
```

使用 Docker 启动 Redis 服务器：

```
docker run -d -p 6379:6379 redis:latest
```

或根据您操作系统的说明在本地安装并运行 Redis。
...
本 Notebook 演示了如何使用 langchain-redis 包中的 `RedisCache` 和 `RedisSemanticCache`。这些缓存机制通过减少冗余 API 调用并利用语义相似性实现智能缓存，可以显著提高基于 LLM 的应用性能。基于 Redis 的实现为分布式系统中的缓存提供了一种快速、可扩展且灵活的解决方案。
