...
本库支持访问多种 Google 模型，包括 Gemini 系列模型及其 Nano Banana 图像生成模型。你可以通过 Google 的 [Google AI](https://ai.google.dev/) API（有时也称为生成式 AI API 或 AI Studio API）或通过 Google Cloud Platform 的 [Vertex AI](https://cloud.google.com/vertex-ai) 服务来访问这些模型。

这将帮助你开始使用 `ChatGoogle` [聊天模型](/oss/javascript/langchain/models)。有关所有 `ChatGoogle` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-google/index/ChatGoogle)。
...
<Tip>
  ...
  有关迁移详情，请参阅[旧版包](/oss/javascript/integrations/providers/google#legacy-packages)。
</Tip>
...
要通过 Google AI Studio（有时称为生成式 AI API）使用该模型，你需要一个 API 密钥。你可以从 [Google AI Studio](https://aistudio.google.com/app/apikey) 获取一个。
...
Vertex AI 还支持[快速模式](https://cloud.google.com/vertex-ai/docs/start/introduction-unified-platform#express-mode)，允许你使用 API 密钥进行身份验证。你可以从 [Google Cloud 控制台](https://console.cloud.google.com/vertex-ai/studio/api-key)获取 Vertex AI API 密钥。
...
使用 [Vertex AI 快速模式](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/start/express-mode/overview)时，你还需要在实例化模型时将平台类型指定为 `gcp`。
...
对于 Google Cloud 上的生产环境，建议使用[应用默认凭据 (ADC)](https://cloud.google.com/docs/authentication/provide-credentials-adc)。这在 Node.js 环境中受支持。

如果你在本地机器上运行，可以通过安装 [Google Cloud SDK](https://cloud.google.com/sdk) 并运行以下命令来设置 ADC：
...
如果你在 Web 环境中运行或希望直接提供凭据，可以使用 `GOOGLE_CLOUD_CREDENTIALS` 环境变量。这应包含你的服务账号密钥文件的**内容**（而不是路径）。
...
如果你想获得模型调用的自动追踪，也可以通过取消注释以下内容来设置你的 [LangSmith](/langsmith/home) API 密钥：
...
{/* TODO: 添加关于 enableEnhancedCivicAnswers 的部分 */}
...
`ChatGoogle` 支持标准的 LangChain [工具调用](/oss/javascript/langchain/tools) 以及 Gemini 特有的“专用工具”（如代码执行和接地）。
...
{/* TODO: 添加关于 mediaResolution 的部分 */}
{/* TODO: 添加关于视频剪辑间隔和帧率的部分 */}
...
`speechConfig` 可以是一个[完整的 Gemini 语音配置对象](https://ai.google.dev/api/generate-content#SpeechConfig)，但在大多数情况下，你只需要提供一个包含预构建[语音名称](https://ai.google.dev/gemini-api/docs/speech-generation#voices)的字符串。
