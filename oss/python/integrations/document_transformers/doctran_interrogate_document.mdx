---
title: "Doctran 文档询问集成"
description: "使用 LangChain Python 与 Doctran 文档询问文档转换器集成。"
---

向量存储知识库中使用的文档通常以叙述或对话格式存储。然而，大多数用户查询都是问题格式。如果我们**在向量化文档之前将其转换为问答格式**，就可以增加检索到相关文档的可能性，并减少检索到不相关文档的可能性。

我们可以使用 [Doctran](https://github.com/psychic-api/doctran) 库来实现这一点，该库利用 OpenAI 的函数调用功能来“询问”文档。

有关基于原始文档与询问文档的各种查询的向量相似度分数基准测试，请参阅[此笔记本](https://github.com/psychic-api/doctran/blob/main/benchmark.ipynb)。
...
询问文档后，结果将作为新文档返回，其中元数据中提供了问题和答案。
