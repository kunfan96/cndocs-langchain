---
title: "Langfair 集成"
description: "使用 LangChain Python 与 Langfair 集成。"
---

LangFair 是一个全面的 Python 库，专为对大型语言模型（LLM）用例进行偏差和公平性评估而设计。LangFair [仓库](https://github.com/cvs-health/langfair) 包含一个全面的框架，用于[选择偏差和公平性指标](https://github.com/cvs-health/langfair/tree/main#-choosing-bias-and-fairness-metrics-for-an-llm-use-case)，以及[演示笔记本](https://github.com/cvs-health/langfair/tree/main/examples)和一本讨论 LLM 偏差和公平性风险、评估指标及最佳实践的[技术手册](https://arxiv.org/abs/2407.10853)。

请探索我们的[文档站点](https://cvs-health.github.io/langfair/)，获取有关使用 LangFair 的详细说明。

## ⚡ 快速入门指南
### （可选）创建用于使用 LangFair 的虚拟环境
...
### 使用示例
以下代码示例说明了如何使用 LangFair 评估文本生成和摘要用例中的偏差和公平性风险。以下示例假设用户已从其用例中定义了一个提示列表 `prompts`。

##### 生成 LLM 响应
要生成响应，我们可以使用 LangFair 的 `ResponseGenerator` 类。首先，我们必须创建一个 `langchain` LLM 对象。下面我们使用 `ChatVertexAI`，但**可以使用 [LangChain 的任何 LLM 类](https://js.langchain.com/docs/integrations/chat/) 代替**。请注意，`InMemoryRateLimiter` 用于避免速率限制错误。
```python
...
```
我们可以使用 `ResponseGenerator.generate_responses` 为每个提示生成 25 个响应，这是毒性评估的惯例。
```python
...
duplicated_prompts = generations["data"]["prompt"] # 以便提示与响应对应
```

##### 计算毒性指标
毒性指标可以使用 `ToxicityMetrics` 计算。请注意，使用 `torch.device` 是可选的，如果 GPU 可用，应使用它以加速毒性计算。
```python
# import torch # 如果 GPU 可用则取消注释
# device = torch.device("cuda") # 如果 GPU 可用则取消注释
from langfair.metrics.toxicity import ToxicityMetrics
tm = ToxicityMetrics(
    # device=device, # 如果 GPU 可用则取消注释,
)
...
auto_object = AutoEval(
    prompts=prompts,
    langchain_llm=llm,
    # toxicity_device=device # 如果 GPU 可用则取消注释
)
