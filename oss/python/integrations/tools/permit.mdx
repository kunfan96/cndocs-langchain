...
Permit 是一个访问控制平台，它使用 RBAC、ABAC 和 ReBAC 等各种模型提供细粒度、实时的权限管理。它使组织能够在其应用程序中实施动态策略，确保只有授权用户才能访问特定资源。

## 概述

本包提供两个 LangChain 工具，用于使用 Permit 进行 JWT 验证和权限检查：
...
确保您的 PDP（策略决策点）正在 PERMIT_PDP_URL 运行。
...
我们可以通过首先将工具绑定到[工具调用模型](https://docs.permit.io/)，然后调用它，从而在链中使用我们的工具：

<ChatModelTabs customVarName="llm" />

### 附加演示脚本

要查看完全可运行的演示，请查看此[仓库](https://github.com/permitio/langchain-permit)中的 `/langchain_permit/examples/demo_scripts` 文件夹。您将找到：
...
只需运行 `python demo_jwt_validation.py` 或 `python demo_permissions_check.py`（设置好环境变量后）即可查看这些工具的实际运行效果。
