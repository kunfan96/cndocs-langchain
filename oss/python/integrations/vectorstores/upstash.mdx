---
title: "Upstash 向量集成"
description: "使用 LangChain Python 集成 Upstash 向量存储。"
---

> [Upstash Vector](https://upstash.com/docs/vector/overall/whatisvector) 是一个专为向量嵌入而设计的无服务器向量数据库。
>
> LangChain 的向量集成是对 [upstash-vector](https://github.com/upstash/vector-py) 包的封装。
>
> Python 包在底层使用了 [vector rest api](https://upstash.com/docs/vector/api/get-started)。

## 安装

在 [upstash 控制台](https://console.upstash.com/vector) 创建一个免费的向量数据库，并选择所需的维度和距离度量。
...
- 将 Upstash Vector 的 `Index` 实例传递给构造函数

此外，还需要一个 [`Embeddings`](https://reference.langchain.com/python/langchain-core/embeddings/embeddings/Embeddings) 实例将文本转换为嵌入向量。这里以 `OpenAIEmbeddings` 为例
...
另一种创建 `UpstashVectorStore` 的方式是[通过选择模型来创建 Upstash Vector 索引](https://upstash.com/docs/vector/features/embeddingmodels#using-a-model)并传入 `embedding=True`。在此配置下，文档或查询将以文本形式发送到 Upstash 并在那里进行嵌入。

```python
store = UpstashVectorStore(embedding=True)
```

如果您有兴趣尝试此方式，可以如上所述更新 `store` 的初始化，然后运行后续教程。

## 加载文档

加载一个示例文本文件并将其分割成块，以便转换为向量嵌入。
...
向量存储使用嵌入对象对文本块进行嵌入，并批量插入到数据库中。此操作返回已插入向量的 id 数组。
...
> 查询请求中返回的分数是一个 0 到 1 之间的归一化值，其中 1 表示最高相似度，0 表示最低相似度，与所使用的相似度函数无关。更多信息请参阅 [文档](https://upstash.com/docs/vector/overall/features#vector-similarity-functions)。
...
命名空间可用于区分不同类型的文档。这可以减少搜索空间，从而提高查询效率。未提供命名空间时，将使用默认命名空间。
...
元数据可用于过滤查询结果。您可以参阅 [文档](https://upstash.com/docs/vector/features/filtering) 了解更复杂的过滤方式。
...
您可以使用 info 函数获取数据库的相关信息，例如距离度量维度。

> 当发生插入操作时，数据库会进行索引。在此过程中，新向量无法被查询。`pendingVectorCount` 表示当前正在进行索引的向量数量。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/vectorstores/upstash.mdx) 或[提交 Issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
