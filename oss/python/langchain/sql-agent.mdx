...
在本教程中，你将学习如何使用 LangChain [代理](/oss/python/langchain/agents) 构建一个能够回答 SQL 数据库问题的代理。

从整体上看，该代理将：

<Steps>
<Step title="从数据库中获取可用的表和模式" />
<Step title="判断哪些表与问题相关" />
<Step title="获取相关表的模式" />
<Step title="根据问题和模式信息生成查询语句" />
<Step title="使用 LLM 对查询语句进行常见错误检查" />
<Step title="执行查询并返回结果" />
<Step title="修正数据库引擎反馈的错误，直到查询成功" />
<Step title="根据结果组织回答" />
</Steps>

<Warning>
构建 SQL 数据库的问答系统需要执行模型生成的 SQL 查询，这本身存在一定风险。请确保数据库连接权限始终尽可能限制在代理所需的最小范围内。这可以降低（但不能消除）构建模型驱动系统所带来的风险。
</Warning>
...
- 用于读取 SQL 数据库的[工具](/oss/python/langchain/tools)
- LangChain [代理](/oss/python/langchain/agents)
- [人工审核](/oss/python/langchain/human-in-the-loop)流程
...
配置 [LangSmith](https://smith.langchain.com) 以检查链或代理内部发生的情况，然后设置以下环境变量：
...
本教程将使用 [SQLite 数据库](https://www.sqlitetutorial.net/sqlite-sample-database/)。SQLite 是一种轻量级数据库，便于搭建和使用。我们将加载 `chinook` 数据库，这是一个代表数字媒体商店的示例数据库。

为方便起见，我们已将数据库（`Chinook.db`）托管在公开的 GCS 存储桶中。
...
我们将使用 `langchain_community` 包中提供的 SQL 数据库封装器与数据库交互。该封装器提供了一个简单的接口来执行 SQL 查询并获取结果：
...
## 3. 添加数据库交互工具
使用 `langchain_community` 包中的 `SQLDatabase` 封装器与数据库交互，该封装器提供了一个简单的接口来执行 SQL 查询并获取结果：
...
使用 [`create_agent`](https://reference.langchain.com/python/langchain/agents/factory/create_agent) 以最少代码构建一个 [ReAct 代理](https://arxiv.org/pdf/2210.03629)。代理将解析请求并生成 SQL 命令，由工具负责执行。若命令出错，错误信息将返回给模型，模型可结合原始请求和新错误信息生成新命令，如此循环直到 LLM 成功生成命令或达到最大迭代次数。这种向模型提供反馈（此处为错误信息）的模式非常强大。

使用描述性的系统提示初始化代理，以定制其行为：
...
代理正确地编写了查询、检查了查询并执行以得出最终回答。

<Note>
    你可以在 [LangSmith 追踪](https://smith.langchain.com/public/cd2ce887-388a-4bb1-a29d-48208ce50d15/r) 中查看上述运行的所有细节，包括执行步骤、调用的工具、LLM 看到的提示词等。
</Note>

### （可选）使用 Studio

[Studio](/langsmith/studio) 提供"客户端"循环以及记忆功能，使你可以将其作为聊天界面运行并查询数据库。你可以提问如"告诉我数据库的架构"或"显示前 5 名客户的发票"。你将看到生成的 SQL 命令及其输出结果。启动方法如下。
<Accordion title="在 Studio 中运行代理">
...
在运行目录中，需要一个内容如下的 `langgraph.json` 文件：
...
在执行代理的 SQL 查询之前进行检查，可以有效防止意外操作或低效查询。

LangChain 代理内置支持[人工审核中间件](/oss/python/langchain/human-in-the-loop)，为代理工具调用添加监督。我们来配置代理，在调用 `sql_db_query` 工具时暂停等待人工审核：
...
```
<Note>
我们为代理添加了[检查点](/oss/python/langchain/short-term-memory)，以便暂停和恢复执行。有关此功能及可用中间件配置的详细说明，请参阅[人工审核指南](/oss/python/langchain/human-in-the-loop)。
</Note>

运行代理后，在执行 `sql_db_query` 工具之前，代理将暂停等待审核：
```python
question = "Which genre on average has the longest tracks?"
...
```
我们可以使用 [Command](/oss/python/langgraph/use-graph-api#combine-control-flow-and-state-updates-with-command) 恢复执行（此处为接受查询）：
```python
...
详情请参阅[人工审核指南](/oss/python/langchain/human-in-the-loop)。



## 下一步

如需更深入的定制，请参阅[本教程](/oss/python/langgraph/sql-agent)，了解如何直接使用 LangGraph 原语实现 SQL 代理。
