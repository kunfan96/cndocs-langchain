...
description: "使用 LangChain Python 与 ChatPredictionGuard 聊天模型集成。"
---

>[Prediction Guard](https://predictionguard.com) 是一个安全、可扩展的 GenAI 平台，可保护敏感数据、防止常见 AI 故障，并在经济实惠的硬件上运行。

## 概述

### 集成详情

此集成利用 Prediction Guard API，该 API 包含各种安全保护和安全功能。
...
Prediction Guard 提供了一个工具调用 API，允许您描述工具及其参数，从而使模型能够返回一个包含要调用的工具和该工具输入的 JSON 对象。工具调用对于构建使用工具的链和代理，以及更普遍地从模型获取结构化输出非常有用。

### ChatPredictionGuard.bind_tools()

使用 `ChatPredictionGuard.bind_tools()`，您可以将 Pydantic 类、字典模式和 LangChain 工具作为工具传递给模型，然后模型会重新格式化这些工具以供使用。
...
请注意，AIMessage 具有 [`tool_calls`](https://reference.langchain.com/python/langchain/messages/#langchain.messages.AIMessage.tool_calls) 属性。这包含一个标准化的 `ToolCall` 格式，该格式与模型提供商无关。
...
使用 Prediction Guard，您可以使用我们的输入检查之一来保护模型输入免受 PII 或提示注入的影响。有关更多信息，请参阅 [Prediction Guard 文档](https://docs.predictionguard.com/docs/process-llm-input/)。
...
使用 Prediction Guard，您可以使用事实性检查模型输出以防止幻觉和错误信息，并使用毒性检查以防止有毒响应（例如脏话、仇恨言论）。有关更多信息，请参阅 [Prediction Guard 文档](https://docs.predictionguard.com/docs/validating-llm-output)。
