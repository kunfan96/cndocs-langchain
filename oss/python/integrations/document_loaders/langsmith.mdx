...
description: "使用 LangChain Python 与 LangSmithLoader 文档加载器集成。"
---

本指南提供了开始使用 `LangSmithLoader` [文档加载器](/oss/python/integrations/document_loaders)的快速概览。有关所有 `LangSmithLoader` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/python/langchain-core/document_loaders/langsmith/LangSmithLoader)。
...
如果您想获得自动化的顶级跟踪，也可以开启 LangSmith 跟踪：
...
在此示例中，我们将克隆并加载一个公共 LangSmith 数据集。克隆会在我们的个人 LangSmith 账户上创建此数据集的副本。您只能加载拥有个人副本的数据集。
