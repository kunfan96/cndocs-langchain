...
[PremAI](https://premai.io/) 是一个一体化平台，简化了由生成式 AI 驱动的健壮、生产就绪应用程序的创建。通过简化开发流程，PremAI 让您能够专注于增强用户体验并推动应用程序的整体增长。您可以[快速开始使用 Prem AI 平台](https://docs.premai.io/quick-start)。
...
在继续之前，请确保您已在 PremAI 上创建了账户并已创建了一个项目。如果没有，请参阅[快速入门](https://docs.premai.io/introduction)指南以开始使用 PremAI 平台。创建您的第一个项目并获取您的 API 密钥。
...
导入所需模块后，让我们设置客户端。目前假设我们的 `project_id` 是 `8`。但请确保使用您的项目 ID，否则会抛出错误。

要将 langchain 与 prem 一起使用，您无需传递任何模型名称或为聊天客户端设置任何参数。默认情况下，它将使用 [LaunchPad](https://docs.premai.io/get-started/launchpad) 中使用的模型名称和参数。
...
第一种方法将给我们一个静态结果。而第二种方法将逐个流式传输令牌。以下是生成类似聊天的补全的方式。
...
上面看起来很有趣，对吧？我将我的默认 launchpad 系统提示设置为：`Always sound like a pirate`。您也可以根据需要覆盖默认系统提示。以下是操作方法。
...
Prem 仓库允许用户上传文档（.txt、.pdf 等）并将这些仓库连接到 LLM。您可以将 Prem 仓库视为原生 RAG，其中每个仓库都可以被视为一个向量数据库。您可以连接多个仓库。更多信息，请参阅 [Prem 仓库](https://docs.premai.io/get-started/repositories)。
...
首先，我们通过定义一些仓库 ID 来开始定义我们的仓库。确保 ID 是有效的仓库 ID。更多信息，请参阅[如何获取仓库 ID](https://docs.premai.io/get-started/repositories)。

> 请注意：类似于 `model_name`，当您调用参数 `repositories` 时，您可能会覆盖在 launchpad 中连接的仓库。
...
使用 **Prem**，编写和管理提示可以变得非常简单。[launchpad](https://docs.premai.io/get-started/launchpad) 中的 **_Templates_** 选项卡可帮助您编写所需的任意数量的提示，并在 SDK 中使用它们，使您的应用程序使用这些提示运行。更多信息，请参阅 [Prem 提示模板](https://docs.premai.io/get-started/prem-templates)。

要将 Prem 模板与 LangChain 原生使用，您需要向 `HumanMessage` 传递一个 id。此 id 应该是您的提示模板的变量名称。[`HumanMessage`](https://reference.langchain.com/python/langchain-core/messages/human/HumanMessage) 中的 `content` 应该是该变量的值。
...
将此 `human_messages` 传递给 ChatPremAI 客户端。请注意：不要忘记传递额外的 `template_id` 以使用 Prem 模板调用生成。如果您不了解 `template_id`，可以在[我们的文档](https://docs.premai.io/get-started/prem-templates)中了解更多。以下是一个示例：
...
类似于上面，如果您想覆盖系统提示和生成参数，您需要添加以下内容：

```python
import sys

# 出于某些实验原因，如果您想覆盖系统提示，那么您
# 也可以在此处传递。但是，不建议覆盖已部署模型的系统提示。
...
为了传递工具并让 LLM 选择它需要调用的工具，我们需要传递一个工具模式。工具模式是函数定义以及关于函数做什么、函数的每个参数是什么等的适当文档字符串。下面是一些简单的算术函数及其模式。
...
# 现在定义函数，其中参数的模式将是 OperationInput
...
我们将上面显示的此消息附加到 LLM，这作为上下文并使 LLM 知道它调用了哪些函数。
...
2. 在我们的第二次调用中，我们将解析 LLM 决定的这组工具并运行它们（在我们的例子中，将是我们定义的函数，使用 LLM 提取的参数），并将此结果传递给 LLM。
