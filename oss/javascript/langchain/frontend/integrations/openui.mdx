...
| **令牌中间的抖动** | `useStableText` — 仅在完整语句边界（`name = Expr(…)`) 而非每个令牌时才允许 Renderer 更新 |
...
OpenUI 的 `Button` 组件支持 `continue_conversation` 动作类型。当用户点击后续按钮时，`Renderer` 会触发 `onAction`，上面的 `AIMessageView` 会将按钮的标签作为下一条用户消息提交，这与在输入框中输入的代码路径完全相同。

通过系统提示词中的 `additionalRules` 为每个报告添加“进一步探索”部分：
