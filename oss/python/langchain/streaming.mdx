...
* <Icon icon="binary" size={16} /> [**流式传输 LLM 令牌**](#llm-tokens)—在语言模型生成令牌时进行流式传输。
* <Icon icon="bulb" size={16} /> [**流式传输思考/推理令牌**](#streaming-thinking-/-reasoning-tokens)—在模型推理生成时呈现推理过程。
...
<Note>
如果您的代理包含多个 LLM，请参阅下面关于[从子代理流式传输](#streaming-from-sub-agents)的部分。
</Note>
...
<Note>
如果已完成的消息在代理的[状态](/oss/python/langchain/agents#memory)中跟踪，您可以使用 `stream_mode=["messages", "updates"]`，如[流式传输工具调用](#streaming-tool-calls)部分所示，在流式传输期间访问已完成的消息。
</Note>
...
<Tip>
部署到 LangSmith 时，在任何不希望其输出流式传输到客户端的模型上设置 `streaming=False`。这在部署前在您的图代码中配置。
</Note>
