...
description: "使用 LangChain Python 集成 Mediawiki dump 文档加载器。"
---

>[MediaWiki XML 转储](https://www.mediawiki.org/wiki/Manual:Importing_XML_dumps) 包含 wiki 的内容（所有修订版本的 wiki 页面），但不包含站点相关数据。XML 转储不会创建 wiki 数据库的完整备份，转储文件不包含用户账户、图片、编辑日志等内容。

本节介绍如何将 MediaWiki XML 转储文件加载为可供下游使用的文档格式。

它使用来自 `mediawiki-utilities` 的 `mwxml` 进行转储，并使用来自 `earwig` 的 `mwparserfromhell` 解析 MediaWiki wikicode。

转储文件可以通过 dumpBackup.php 或 Wiki 的 Special:Statistics 页面获取。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/document_loaders/mediawikidump.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
