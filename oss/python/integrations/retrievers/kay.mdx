...
>[Kai Data API](https://www.kay.ai/) 专为 RAG 🕵️ 构建。我们正在整理世界上最大的数据集作为高质量嵌入，以便您的 AI 代理可以即时检索上下文。最新模型、快速检索和零基础设施。

此笔记本向您展示如何检索 [Kay](https://kay.ai/) 支持的数据集。您目前可以搜索 `SEC 文件` 和 `美国公司的新闻稿`。访问 [kay.ai](https://kay.ai) 获取最新数据发布。如有任何问题，请加入我们的 [discord](https://discord.gg/hAnE4e5T6M) 或 [在 Twitter 上联系我们](https://twitter.com/vishalrohra_)。
...
您还需要一个 API 密钥：您可以免费在 [https://kay.ai](https://kay.ai/) 获取一个。获得 API 密钥后，必须将其设置为环境变量 `KAY_API_KEY`。

`KayAiRetriever` 有一个静态的 `.create()` 工厂方法，接受以下参数：

* `dataset_id: string` 必需 -- Kay 数据集 ID。这是关于特定实体（如公司、人或地点）的数据集合。例如，尝试 `"company"`
* `data_type: List[string]` 可选 -- 这是数据集中基于其来源或格式的类别，例如“公司”数据集中的“SEC 文件”、“新闻稿”或“报告”。例如，在“公司”数据集下尝试 ["10-K", "10-Q", "PressRelease"]。如果留空，Kay 将检索所有类型中最相关的上下文。
* `num_contexts: int` 可选，默认为 6 -- 每次调用 `get_relevant_documents()` 时检索的文档块数量。
