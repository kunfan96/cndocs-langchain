...
本示例展示了如何使用 `LangChain` 来使用 `watsonx.ai` 工具包。
...
要访问 IBM watsonx.ai 工具包，您需要创建一个 IBM watsonx.ai 帐户，获取 API 密钥，并安装 `langchain-ibm` 集成包。
...
<Note>
    可用工具列表可能因使用的是 IBM watsonx.ai for IBM Cloud 还是 IBM watsonx.ai 软件而异。
</Note>
...
在此示例中，工具具有一个配置模式，其中包含用于设置返回最大结果数的 `maxResults` 参数。
...
在此示例中，工具具有一个输入模式，其中包含一个必需参数和一个可选参数。
...
要正确地将输入传递给 `invoke()`，您需要创建一个 `invoke_input` 字典，其中必需参数作为键及其值。
