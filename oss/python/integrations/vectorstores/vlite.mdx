...
VLite 是一个简单且极速的向量数据库，允许您使用嵌入向量以语义方式存储和检索数据。VLite 使用 numpy 构建，是一个轻量级、功能齐全的数据库，用于在您的项目中实现 RAG（检索增强生成）、相似性搜索和嵌入向量。
...
VLite 会为您处理文本的分块和嵌入，您可以通过预先分块文本和/或将这些块嵌入到 VLite 数据库中来更改这些参数。
...
您可以分别使用 `add_texts` 和 `add_documents` 方法将文本或文档添加到 VLite 向量数据库。
...
VLite 还支持最大边际相关性（MMR）搜索，该搜索针对查询的相似性和检索文档之间的多样性进行优化。
