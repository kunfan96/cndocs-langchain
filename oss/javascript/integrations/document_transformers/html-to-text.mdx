...
使用 HtmlToTextTransformer 从文档中剥离 HTML 标签可以生成内容更丰富的块，从而使检索更有效。

## 设置

您需要安装 [`html-to-text`](https://www.npmjs.com/package/html-to-text) npm 包：
...
以下示例抓取 Hacker News 主题，根据 HTML 标签进行拆分以基于标签的语义信息对块进行分组，然后从各个块中提取内容：
...
您可以向转换器传递 [`html-to-text` 包接受的任何参数](https://www.npmjs.com/package/html-to-text)来自定义其工作方式。
