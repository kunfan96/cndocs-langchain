...
在本教程中，我们将使用 LangGraph 构建一个能够回答有关 SQL 数据库问题的自定义代理。

LangChain 提供了内置的 [代理](/oss/javascript/langchain/agents) 实现，这些实现使用 [LangGraph](/oss/javascript/langgraph/overview) 原语实现。如果需要更深度的定制，可以在 LangGraph 中直接实现代理。本指南演示了 SQL 代理的示例实现。您可以在 [这里](/oss/javascript/langchain/sql-agent) 找到使用更高级别的 LangChain 抽象构建 SQL 代理的教程。

<Warning>
构建 SQL 数据库的问答系统需要执行模型生成的 SQL 查询。这样做存在固有风险。请确保您的数据库连接权限始终针对您的代理需求尽可能狭窄。这将减轻但不能消除构建模型驱动系统的风险。
</Warning>

[预构建的代理](/oss/javascript/langchain/sql-agent) 让我们能够快速入门，但我们依靠系统提示来约束其行为——例如，我们指示代理始终以“列出表”工具开始，并在执行查询之前始终运行查询检查器工具。

我们可以通过自定义代理在 LangGraph 中强制执行更高程度的控制。在这里，我们实现了一个简单的 ReAct 代理设置，具有用于特定工具调用的专用节点。我们将使用与预构建代理相同的 [状态]。
...
- [人机交互 (Human-in-the-loop)](/oss/javascript/langgraph/interrupts) 流程
...
### LangSmith
设置 [LangSmith](https://smith.langchain.com) 以检查您的链或代理内部发生的情况。然后设置以下环境变量：
...
选择支持 [工具调用](/oss/javascript/integrations/providers/overview) 的模型：
...
为了方便起见，我们将数据库 (`Chinook.db`) 托管在一个公共 GCS 存储桶上。
...
我们将使用 `@langchain/classic/sql_db` 模块中可用的便捷 SQL 数据库包装器与数据库进行交互。该包装器提供了一个简单的接口来执行 SQL 查询并获取结果：
...
现在，我们可以使用 [Graph API](/oss/javascript/langgraph/graph-api) 将这些步骤组装成一个工作流。我们在查询生成步骤定义一个 [条件边](/oss/javascript/langgraph/graph-api#conditional-edges)，如果生成了查询，则路由到查询检查器，如果没有工具调用（即 LLM 已经对查询给出了响应），则结束。
...
2Balls to the Wall221U. Dirkschneider, W. Hoffmann, H. Frank, P. Baltes, S. Kaufmann, G. Hoffmann34256255104240.99
...
在执行代理的 SQL 查询之前进行检查，以防止任何意外操作或低效率，这可能是明智之举。

在这里，我们利用 LangGraph 的 [人机交互](/oss/javascript/langgraph/interrupts) 功能，在执行 SQL 查询之前暂停运行并等待人工审查。使用 LangGraph 的 [持久层](/oss/javascript/langgraph/persistence)，我们可以无限期地暂停运行（或者至少在持久层处于活动状态时）。

让我们将 `sql_db_query` 工具包装在一个接收人工输入的节点中。我们可以使用 [interrupt](/oss/javascript/langgraph/interrupts) 函数来实现这一点。下面，我们允许输入批准工具调用、编辑其参数或提供用户反馈。
...
<Note>
上述实现遵循更广泛的 [人机交互](/oss/javascript/langgraph/interrupts) 指南中的 [工具中断示例](/oss/javascript/langgraph/interrupts#configuring-interrupts)。有关详细信息和替代方案，请参阅该指南。
</Note>

现在让我们重新组装我们的图。我们将用人工审查替换程序化检查。请注意，我们现在包含一个 [检查点器 (checkpointer)](/oss/javascript/langgraph/persistence); 这是暂停和恢复运行所必需的。
...
有关详细信息，请参阅 [人机交互指南](/oss/javascript/langgraph/interrupts)。
