---
title: "Brave 搜索集成"
description: "使用 LangChain Python 集成 Brave 搜索工具。"
---

本笔记本介绍如何使用 Brave 搜索工具。
前往 [Brave 网站](https://brave.com/search/api/) 注册免费账户并获取 API 密钥。
...
# 或者，如果你想从环境变量 BRAVE_SEARCH_API_KEY 获取 API 密钥，并保持 search_kwargs 为空
# tool = BraveSearch()

# 或者，如果你想只提供 API 密钥，并保持 search_kwargs 为空
# tool = BraveSearch.from_api_key(api_key=api_key)

# 或者，如果你想只提供 search_kwargs，并从 BRAVE_SEARCH_API_KEY 环境变量读取 API 密钥
