...
> [Linkup](https://www.linkup.so/) 提供了一个 API，用于将大语言模型连接到网络和 Linkup Premium Partner 来源。

这将帮助你开始使用 LinkupSearchRetriever [检索器](/oss/python/langchain/retrieval/)。
...
要使用 Linkup 提供商，你需要一个有效的 API 密钥，可以通过[注册 Linkup](https://app.linkup.so/sign-up) 找到。然后你可以将其设置为 `LINKUP_API_KEY` 环境变量。对于下面的链示例，你还需要将 OpenAI API 密钥设置为 `OPENAI_API_KEY` 环境变量，你也可以在这里进行设置：
...
如果你想从单个查询中获得自动跟踪，也可以通过取消下面的注释来设置你的 [LangSmith](/langsmith/home) API 密钥：
