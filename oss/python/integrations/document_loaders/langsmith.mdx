...
description: "使用 LangChain Python 与 LangSmithLoader 文档加载器进行集成。"
---

本指南提供了开始使用 `LangSmithLoader` [文档加载器](/oss/python/integrations/document_loaders) 的快速概述。有关所有 `LangSmithLoader` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/python/langchain-core/document_loaders/langsmith/LangSmithLoader)。
...
如果您想获得自动化的最佳追踪，也可以开启 LangSmith 追踪：
...
在本示例中，我们将克隆并加载一个公共 LangSmith 数据集。克隆会在您的个人 LangSmith 账户上创建此数据集的副本。您只能加载您拥有个人副本的数据集。
