...
>[Elasticsearch](https://www.elastic.co/elasticsearch/) 是一个分布式的 RESTful 搜索和分析引擎。它提供了一个分布式的、支持多租户的全文搜索引擎，具有 HTTP Web 接口和无模式 JSON 文档支持。它支持关键词搜索、向量搜索、混合搜索和复杂过滤。

`ElasticsearchRetriever` 是一个通用封装器，可通过 [Query DSL](https://www.elastic.co/guide/en/elasticsearch/reference/current/query-dsl.html) 灵活访问所有 `Elasticsearch` 特性。大多数场景下，其他类（`ElasticsearchStore`、`ElasticsearchEmbeddings` 等）就足够用了，但如果不够用，可以使用 `ElasticsearchRetriever`。

本指南将帮助你快速上手 Elasticsearch [检索器](/oss/python/langchain/retrieval)。有关 `ElasticsearchRetriever` 所有特性和配置的详细文档，请访问 [API 参考](https://python.langchain.com/api_reference/elasticsearch/retrievers/langchain_elasticsearch.retrievers.ElasticsearchRetriever.html)。
...
- Elastic Cloud：[Elastic Cloud](https://cloud.elastic.co/) 是托管的 Elasticsearch 服务。注册[免费试用](https://www.elastic.co/cloud/cloud-trial-overview)。
  要连接到不需要登录凭证的 Elasticsearch 实例（以启用安全的方式启动 Docker 实例），请将 Elasticsearch URL 和索引名称以及嵌入对象传递给构造函数。

- 本地安装 Elasticsearch：在本地运行 Elasticsearch 来快速上手。最简单的方式是使用官方 Elasticsearch Docker 镜像。更多信息请参阅 [Elasticsearch Docker 文档](https://www.elastic.co/guide/en/elasticsearch/reference/current/docker.html)。
...
该检索器在 `langchain-elasticsearch` 包中。为了演示，我们还将安装 `langchain-community` 以生成文本[嵌入](/oss/python/integrations/text_embedding)。
...
此处定义与 Elasticsearch 的连接。本例使用本地运行的实例。你也可以在 [Elastic Cloud](https://cloud.elastic.co/) 创建账号并开始[免费试用](https://www.elastic.co/cloud/cloud-trial-overview)。
...
对于向量搜索，本例使用随机嵌入仅作演示。在实际使用中，请选择 LangChain 中可用的[嵌入](/oss/python/integrations/text_embedding)类之一。
...
通常，用户在 Elasticsearch 索引中已有数据时才使用 `ElasticsearchRetriever`。这里我们索引一些示例文本文档。如果你已经使用 `ElasticsearchStore.from_documents` 创建了索引，也同样适用。
...
使用[互惠排名融合](https://www.elastic.co/guide/en/elasticsearch/reference/current/rrf.html)（RRF）将向量搜索与 BM25 搜索结合，合并结果集。
...
注意，查询匹配的结果排在最前面。通过过滤的其他文档也在结果集中，但它们的得分相同。

### 自定义文档映射器

可以自定义将 Elasticsearch 结果（hit）映射为 LangChain 文档的函数。
