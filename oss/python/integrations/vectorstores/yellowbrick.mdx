...
[Yellowbrick](https://yellowbrick.com/yellowbrick-data-warehouse/) 是一个弹性、大规模并行处理 (MPP) SQL 数据库，可在云端和本地运行，使用 Kubernetes 实现扩展、弹性和云可移植性。Yellowbrick 旨在解决最大、最复杂、业务关键型的数据仓库用例。Yellowbrick 提供的规模化效率也使其能够作为高性能、可扩展的向量数据库，使用 SQL 存储和搜索向量。

## 将 Yellowbrick 用作 ChatGpt 的向量存储

本教程演示如何创建一个由 ChatGpt 支持的简单聊天机器人，该机器人使用 Yellowbrick 作为向量存储来支持检索增强生成 (RAG)。您需要：

1. 一个 [Yellowbrick 沙盒](https://cloudlabs.yellowbrick.com/) 账户
2. 一个来自 [OpenAI](https://platform.openai.com/) 的 API 密钥

本教程分为五个部分。首先，我们将使用 langchain 创建一个基线聊天机器人，以便在没有向量存储的情况下与 ChatGpt 交互。其次，我们将在 Yellowbrick 中创建一个嵌入表，该表将代表向量存储。第三，我们将加载一系列文档（Yellowbrick 手册的管理章节）。第四，我们将创建这些文档的向量表示并将其存储在 Yellowbrick 表中。最后，我们将向改进后的聊天机器人发送相同的查询以查看结果。
...
# 建立与 Yellowbrick 的连接参数。如果您已注册沙盒，请在此处填写欢迎邮件中的信息：
...
## 第 1 部分：创建一个由 ChatGpt 支持但没有向量存储的基线聊天机器人

我们将使用 langchain 来查询 ChatGPT。由于没有向量存储，ChatGPT 将没有上下文来回答问题。
...
llm = ChatOpenAI(
    model_name="gpt-3.5-turbo",  # 如果您有权访问 GPT-4，请修改 model_name
    temperature=0,
    max_tokens=256,
)
...
要将您的文档嵌入加载到 Yellowbrick 中，您应该创建自己的表来存储它们。请注意，包含该表的 Yellowbrick 数据库必须是 UTF-8 编码的。

在 UTF-8 数据库中创建一个具有以下模式的表，提供您选择的表名：

```python
# 建立与 Yellowbrick 数据库的连接
try:
...
遍历文档，将它们分成可处理的块，创建嵌入并插入到 Yellowbrick 表中。这大约需要 5 分钟。
...
separator = "\n## "  # 此分隔符假设仓库中的 Markdown 文档大多使用 ### 作为逻辑主标题
...
Yellowbrick 还支持使用局部敏感哈希 (Locality-Sensitive Hashing) 方法进行索引。这是一种近似最近邻搜索技术，允许以准确性为代价来权衡相似性搜索时间。该索引引入了两个新的可调参数：

- 超平面的数量，作为参数提供给 `create_lsh_index(num_hyperplanes)`。文档越多，需要的超平面就越多。LSH 是一种降维形式。原始嵌入被转换为低维向量，其中分量的数量与超平面的数量相同。
- 汉明距离 (Hamming distance)，一个表示搜索范围的整数。较小的汉明距离会导致更快的检索，但准确性较低。

以下是如何在我们加载到 Yellowbrick 的嵌入上创建索引的方法。我们还将重新运行之前的聊天会话，但这次检索将使用索引。请注意，对于如此少量的文档，您不会看到索引在性能方面的好处。
...
可以修改此代码以询问不同的问题。您也可以将自己的文档加载到向量存储中。langchain 模块非常灵活，可以解析各种文件（包括 HTML、PDF 等）。

您还可以修改此代码以使用 Huggingface 嵌入模型和 Meta 的 Llama 2 LLM，以获得完全私有的聊天机器人体验。
