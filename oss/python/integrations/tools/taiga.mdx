...
- **`create_entity_tool`**：在 Taiga 中创建用户故事、任务和议题。
- **`search_entities_tool`**：在 Taiga 中搜索用户故事、任务和议题。
- **`get_entity_by_ref_tool`**：通过引用获取用户故事、任务或议题。
- **`update_entity_by_ref_tool`**：通过引用更新用户故事、任务或议题。
- **`add_comment_by_ref_tool`**：为用户故事、任务或议题添加评论。
- **`add_attachment_by_ref_tool`**：为用户故事、任务或议题添加附件。
...
此集成要求您将 `TAIGA_URL`、`TAIGA_API_URL`、`TAIGA_USERNAME`、`TAIGA_PASSWORD` 和 `OPENAI_API_KEY` 设置为环境变量，以向 Taiga 进行身份验证。
...
以下示例展示了如何在 `langchain_taiga` 中实例化 Taiga 工具。请根据您的具体用法进行调整。
...
如果您有一个模型生成的 `ToolCall`，请按照下面所示的格式将其传递给 `tool.invoke()`。
...
以下是一个完整的示例，展示了如何在链或代理中与 LLM 集成 `create_entity_tool` 和 `search_entities_tool` 工具。此示例假设您有一个函数（如 [`create_agent`](https://reference.langchain.com/python/langchain/agents/factory/create_agent)），该函数设置了一个 LangChain 风格的代理，能够在适当时调用工具。
...
example_query = "请在 slug 项目中创建一个新的用户故事，主题为 'subject'：'slug'"

# 5. 以流式模式执行代理（或根据您的代码结构执行）
