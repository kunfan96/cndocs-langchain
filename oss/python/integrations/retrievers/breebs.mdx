...
任何人都可以基于存储在 Google Drive 文件夹中的 PDF 创建一个 Breeb，即知识胶囊。
任何 LLM/聊天机器人都可以使用 Breeb 来提高其专业知识、减少幻觉并访问来源。
在幕后，Breebs 实现了多种检索增强生成 (RAG) 模型，以在每次迭代中无缝提供有用的上下文。
...
社区已经创建了数十个 Breebs，并且可以免费使用。它们涵盖了广泛的专业知识领域，从有机化学到神话，以及关于诱惑和去中心化金融的技巧。

## 创建新的 Breeb

要生成新的 Breeb，只需将 PDF 文件编译到一个公开共享的 Google Drive 文件夹中，然后在 [BREEBS 网站](https://www.breebs.com/) 上点击“Create Breeb”按钮来启动创建过程。目前最多可以包含 120 个文件，总字符数限制为 1500 万个。
