...
本文将帮助您开始使用 `TavilySearchAPIRetriever` [检索](/oss/javascript/langchain/retrieval)。有关所有 `TavilySearchAPIRetriever` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/javascript/langchain-community/retrievers/tavily_search_api/TavilySearchAPIRetriever)。
...
您需要将 `TAVILY_API_KEY` 环境变量设置为您的 Tavily API 密钥，或者将其作为 `apiKey` 传递给构造函数。您可以通过[在其网站上注册](https://tavily.com/)来获取密钥。
...
有关所有 `TavilySearchAPIRetriever` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/javascript/langchain-community/retrievers/tavily_search_api/TavilySearchAPIRetriever)。
