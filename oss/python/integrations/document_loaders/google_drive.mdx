...
本笔记本介绍如何从 `Google Drive` 加载文档。目前，仅支持 `Google Docs`。
...
首次使用 GoogleDriveLoader 时，浏览器中会显示同意屏幕以进行用户身份验证。身份验证后，`token.json` 将在提供的或默认路径下自动创建。此外，如果该路径下已存在 `token.json`，则不会提示您进行身份验证。

`GoogleDriveLoader` 可以从 Google Docs 文档 ID 列表或文件夹 ID 加载。您可以从 URL 获取您的文件夹和文档 ID：
...
当您传递 `folder_id` 时，默认情况下会加载所有类型为文档、表格和 PDF 的文件。您可以通过传递 `file_types` 参数来修改此行为。
...
处理 Google Docs 和 Google Sheets 以外的文件时，向 `GoogleDriveLoader` 传递可选的文件加载器可能会有所帮助。如果您传递了文件加载器，该文件加载器将用于处理没有 Google Docs 或 Google Sheets MIME 类型的文档。以下是使用文件加载器从 Google Drive 加载 Excel 文档的示例。
...
您也可以使用以下模式处理包含混合文件和 Google Docs/Sheets 的文件夹：
...
为了与容器兼容，身份验证使用环境变量 `GOOGLE_ACCOUNT_FILE` 指向凭证文件（用于用户或服务）。
...
可以加载由 Google Drive Loader 摄入的每个文件的授权身份，以及每个文档的元数据。
...
所有 Google 文件在元数据中都有一个 'description' 字段。此字段可用于存储文档摘要或其他索引标签（参见方法 `lazy_update_description_with_summary()`）。

如果您使用 `mode="snippet"`，则仅使用描述作为正文。否则，`metadata['summary']` 包含该字段。

有时，可以使用特定的过滤器从文件名中提取信息，以选择具有特定标准的文件。您可以使用过滤器。

有时，会返回许多文档。没有必要同时将所有文档保存在内存中。您可以使用方法的延迟版本，一次获取一个文档。最好使用复杂的查询代替递归搜索。对于每个文件夹，如果激活 `recursive=True`，则必须应用查询。
