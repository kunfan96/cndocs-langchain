---
title: "Steam 工具包集成"
description: "使用 LangChain Python 集成 Steam 工具包。"
---

>[Steam（维基百科）](https://en.wikipedia.org/wiki/Steam_(service)) 是由 `Valve Corporation` 开发的一项数字游戏发行服务和商店。它能自动为 Valve 的游戏提供更新，并扩展到第三方游戏的发行。`Steam` 提供多种功能，包括带有 Valve 反作弊措施的游戏服务器匹配、社交网络及游戏流媒体服务。
...
本指南演示如何使用 Steam API 与 LangChain 配合，根据您当前的 Steam 游戏库获取游戏推荐，或收集您指定的 Steam 游戏信息。
...
要使用此工具包，请准备好您的 OpenAI API Key、Steam API Key（从[此处](https://steamcommunity.com/dev/apikey)获取）以及您自己的 SteamID。获取 Steam API Key 后，可在下方将其设置为环境变量。工具包将读取 "STEAM_KEY" API Key 环境变量进行身份验证，请务必设置好。您还需要设置 "OPENAI_API_KEY" 和 "STEAM_ID"。
...
初始化 LLM、SteamWebAPIWrapper、SteamToolkit，以及最重要的 LangChain 智能体来处理您的查询！
