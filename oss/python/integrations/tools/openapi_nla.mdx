...
description: "使用 LangChain Python 集成自然语言 API 工具包。"
---

`自然语言 API` 工具包（`NLAToolkits`）允许 LangChain Agent 高效地规划并组合跨端点的调用。

本 notebook 演示了 `Speak`、`Klarna` 和 `Spoonacluar` API 的组合示例。
...
某些端点可能需要通过访问令牌等方式进行用户身份验证。下面展示如何通过 `Requests` 包装器对象传入身份验证信息。

由于每个 NLATool 都为其封装的 API 提供了简洁的自然语言接口，顶层对话 Agent 能更轻松地整合各个端点以满足用户请求。

**添加 Spoonacular 端点。**

1. 访问 [Spoonacular API 控制台](https://spoonacular.com/food-api/console#Profile) 并注册免费账户。
2. 点击 `Profile` 并复制您的 API 密钥到下方。
