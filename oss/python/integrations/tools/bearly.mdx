...
description: "使用 LangChain Python 集成 Bearly 代码解释器工具。"
---

> Bearly 代码解释器允许远程执行代码。这使其成为代理的代码沙盒的完美选择，可用于安全实现诸如代码解释器之类的功能。
...
```text
在沙盒环境中评估 Python 代码。环境在每次执行时重置。您必须每次都发送整个脚本并打印输出。脚本应为可评估的纯 Python 代码。它应为 Python 格式，而非 Markdown。代码不应被反引号包裹。所有 Python 包（包括 requests、matplotlib、scipy、numpy、pandas 等）均可用。如果您有任何文件输出，请将它们写入相对于执行路径的 "output/" 目录。只能从目录、stdout 和 stdin 读取输出。不要使用诸如 plot.show() 之类的东西，因为它不会起作用，而是将它们写入 `output/`，并将返回文件的链接。使用 print() 输出任何内容和结果，以便您可以捕获输出。
...
{'stdout': '40594.518\n', 'stderr': '', 'fileLinks': [], 'exitCode': 0}如果最新的 GDP 数字增长 50%，2030 年的 GDP 将约为 40,594.518 十亿美元。

> 链条结束。
```

```text
'如果最新的 GDP 数字增长 50%，2030 年的 GDP 将约为 40,594.518 十亿美元。'
```

```python
# 图表输出
agent.invoke("创建一个美观且标注清晰的 GDP 随时间增长的图表")
```
...
正在调用：`bearly_interpreter`，参数为 `{'python_code': "\nimport pandas as pd\nimport matplotlib.pyplot as plt\n\n# 加载数据\ndf = pd.read_csv('US_GDP.csv')\n\n# 将 'DATE' 列转换为日期时间格式\ndf['DATE'] = pd.to_datetime(df['DATE'])\n\n# 绘制数据\nplt.figure(figsize=(10,6))\nplt.plot(df['DATE'], df['GDP'], label='US GDP')\nplt.xlabel('Year')\nplt.ylabel('GDP (in billions)')\nplt.title('US GDP Over Time')\nplt.legend()\nplt.grid(True)\nplt.savefig('output/US_GDP.png')\n"}`
...
![美国 GDP 随时间增长](https://bearly-cubby.c559ae877a0a39985f534614a037d899.r2.cloudflarestorage.com/prod/bearly-cubby/temp/interpreter/2023_10/089daf37e9e343ba5ff21afaaa78b967c3466a550b3b11bd5c710c052b559e97/sxhM8gop2AYP88n5uHCsOJ6yTYNQm-HimZ70DcwQ4VI.png?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=c058d02de50a3cf0bb7e21c8e2d062c5%2F20231010%2F%2Fs3%2Faws4_request&X-Amz-Date=20231010T000000Z&X-Amz-Expires=604800&X-Amz-SignedHeaders=host&X-Amz-Signature=104dc0d4a4b71eeea1030dda1830059920cb0f354fa00197b439eb8565bf141a)

x 轴代表年份，y 轴代表以十亿美元计的 GDP。折线图显示了美国 GDP 随时间的增长。
