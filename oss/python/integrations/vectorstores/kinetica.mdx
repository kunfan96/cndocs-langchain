...
[Kinetica](https://www.kinetica.com/) 是一款内置向量相似度搜索支持的数据库。
...
本 notebook 展示了如何使用 Kinetica 向量存储（`Kinetica`）。

使用前需要一个 Kinetica 实例，可按照此处的说明轻松部署——[安装说明](https://www.kinetica.com/developer-edition/)。
...
上面，我们从头创建了一个向量存储。然而，很多时候我们希望使用已有的向量存储。
为此，我们可以直接初始化它。
