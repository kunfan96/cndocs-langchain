---
title: "AnalyticDB 集成"
description: "使用 LangChain JavaScript 与 AnalyticDB 向量存储集成。"
---

[AnalyticDB for PostgreSQL](https://www.alibabacloud.com/help/en/analyticdb-for-postgresql/latest/product-introduction-overview) 是一项大规模并行处理（MPP）数据仓库服务，旨在在线分析海量数据。

`AnalyticDB for PostgreSQL` 基于开源 `Greenplum Database` 项目开发，并由 `阿里云` 进行了深度扩展增强。AnalyticDB for PostgreSQL 兼容 ANSI SQL 2003 语法以及 PostgreSQL 和 Oracle 数据库生态系统。AnalyticDB for PostgreSQL 还支持行存储和列存储。AnalyticDB for PostgreSQL 能够以高性能离线处理 PB 级数据，并支持高并发在线查询。

本笔记本展示了如何使用与 `AnalyticDB` 向量数据库相关的功能。

要运行，您应该拥有一个正在运行的 [AnalyticDB](https://www.alibabacloud.com/help/en/analyticdb-for-postgresql/latest/product-introduction-overview) 实例：
