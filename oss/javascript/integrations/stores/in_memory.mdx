...
本指南将帮助您开始使用 [InMemoryStore](/oss/javascript/integrations/stores)。有关所有 `InMemoryStore` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/javascript/langchain-core/stores/InMemoryStore)。

`InMemoryStore` 允许为存储中的值分配一个泛型类型。我们将分配 `BaseMessage` 类型作为我们值的类型，以保持聊天历史存储的主题。
...
如果您想获取所有键，可以调用 `yieldKeys` 方法。或者，您可以传递一个键前缀，以仅获取匹配该前缀的键。
...
有关所有 `InMemoryStore` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/javascript/langchain-core/stores/InMemoryStore)
