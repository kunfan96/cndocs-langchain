---
title: "PDFMinerLoader 集成"
description: "使用 LangChain Python 与 PDFMinerLoader 文档加载器集成。"
---

本指南提供了快速入门 PDFMiner [文档加载器](/oss/python/integrations/document_loaders) 的概述。有关所有 `PDFMinerLoader` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/python/langchain-community/document_loaders/pdf/PDFMinerLoader)。
...
使用 PDFMinerLoader 不需要凭据。

如果您想获得模型调用的自动化一流跟踪，也可以通过取消注释下方代码来设置您的 [LangSmith](/langsmith/home) API 密钥：
...
加载 PDF 文件时，可以以两种不同方式对其进行分割：

- 按页面
- 作为单个文本流

默认情况下，PDFMinerLoader 会按页面分割 PDF。

### 按页面提取 PDF。每个页面提取为一个 langchain 文档对象
...
这可以简单地是 \n 或 \f 以清晰指示页面更改，或 \<!-- PAGE BREAK --> 以便在 Markdown 查看器中无缝注入而不会产生视觉效果。
...
这对于将文本语义分块为各个部分很有帮助，因为输出的 html 内容可以通过 `BeautifulSoup` 解析，以获取有关字体大小、页码、PDF 页眉/页脚等的更结构化和丰富的信息。
...
有关所有 `PDFMinerLoader` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/python/langchain-community/document_loaders/pdf/PDFMinerLoader)
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/document_loaders/pdfminer.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
<Callout icon="terminal-2">
    [使用这些文档](/use-these-docs) 通过 MCP 将其连接到 Claude、VSCode 等，以获取实时答案。
</Callout>
</div>