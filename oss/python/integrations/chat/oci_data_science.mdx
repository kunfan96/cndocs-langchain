...
description: "使用 LangChain Python 与 ChatOCIModelDeployment 聊天模型集成。"
---

这将帮助您开始使用 OCIModelDeployment [聊天模型](/oss/python/langchain/models)。有关所有 `ChatOCIModelDeployment` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/python/langchain-community/chat-models)。

[OCI Data Science](https://docs.oracle.com/en-us/iaas/data-science/using/home.htm) 是一个完全托管且无服务器的平台，供数据科学团队在 Oracle Cloud Infrastructure 中构建、训练和管理机器学习模型。您可以使用 [AI Quick Actions](https://blogs.oracle.com/ai-and-datascience/post/ai-quick-actions-in-oci-data-science) 轻松地在 [OCI Data Science Model Deployment Service](https://docs.oracle.com/en-us/iaas/data-science/using/model-dep-about.htm) 上部署 LLM。您可以选择使用 vLLM 或 TGI 等流行的推理框架部署模型。默认情况下，模型部署端点模拟 OpenAI API 协议。
...
| :---: | :---: | :---: |  :---: | :---: | :---: | :---: | :---: | :---: |
| 取决于部署模型 | 取决于部署模型 | 取决于部署模型 | 取决于部署模型 | 取决于部署模型 | ✅ | ✅ | ✅ | ✅ |

一些模型功能，包括工具调用、结构化输出和多模态输入，取决于部署的模型。
...
请确保拥有访问 OCI Data Science Model Deployment 端点所需的[策略](https://docs.oracle.com/en-us/iaas/data-science/using/model-dep-policies-auth.htm#model_dep_policies_auth__predict-endpoint)。
...
* 当您需要部署模型的通用入口点时，使用 `ChatOCIModelDeployment`。您可以在实例化此类时通过 `model_kwargs` 传递模型参数。这提供了灵活性和配置的便利性，无需依赖特定于框架的细节。
...
* 使用特定于框架的类，如 `ChatOCIModelDeploymentVLLM`：当您使用特定框架（例如 `vLLM`）并需要通过构造函数直接传递模型参数时，此方法适用，简化了设置过程。
