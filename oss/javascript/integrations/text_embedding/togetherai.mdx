...
这将帮助你开始使用 LangChain 的 TogetherAIEmbeddings [嵌入模型](/oss/javascript/integrations/text_embedding)。有关 `TogetherAIEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_embeddings_togetherai.TogetherAIEmbeddings.html)。
...
要访问 TogetherAI 嵌入模型，你需要创建一个 TogetherAI 账户，获取 API 密钥，并安装 `@langchain/community` 集成包。

### 凭证

你可以在[这里](https://api.together.xyz/)注册 Together 账户并创建 API 密钥。完成后，设置 `TOGETHER_AI_API_KEY` 环境变量：
...
嵌入模型通常用于检索增强生成 (RAG) 流程，既作为索引数据的一部分，也用于后续的检索。有关更详细的说明，请参阅 [**学习**标签页](/oss/javascript/learn/)下的 RAG 教程。

下面，了解如何使用我们上面初始化的 `embeddings` 对象来索引和检索数据。在这个例子中，我们将使用演示用的 [`MemoryVectorStore`](/oss/javascript/integrations/vectorstores/memory) 来索引和检索示例文档。
...
你可以使用 `embedDocuments` 嵌入多个文本以进行索引。此方法使用的内部机制可能（但不一定）与嵌入查询不同：
...
有关所有 TogetherAIEmbeddings 功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_embeddings_togetherai.TogetherAIEmbeddings.html)。
