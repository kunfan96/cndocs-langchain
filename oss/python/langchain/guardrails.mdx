...
LangChain提供了内置防护栏（例如，[PII检测](#pii-detection)、[人在回路](#human-in-the-loop)）和一个灵活的中间件系统，用于使用任一方法构建自定义防护栏。
...
<Tip>
  有关实现审批工作流的完整详细信息，请参阅[人在回路文档](/oss/python/langchain/human-in-the-loop)。
</Tip>
