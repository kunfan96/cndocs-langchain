...
通过此集成，您可以使用 Jina 嵌入模型为您的文本数据获取嵌入。以下是嵌入模型的[链接](https://jina.ai/embeddings)。

首先，您需要在 Jina 网站上注册并获取 [Jina 的 API 令牌](https://jina.ai/embeddings)。您可以从 API 演练场的下拉菜单中复制模型名称。
