...
我们将 `deepagents` 视为["agent harness"](/oss/python/concepts/products#agent-harnesses-like-the-deep-agents-sdk)。它与其他 agent 框架具有相同的核心工具调用循环，但具有内置工具和能力。
...
[`FilesystemMiddleware`](/oss/python/deepagents/middleware) 在工具结果超过 token 阈值时自动将其驱逐到文件系统，防止上下文窗口饱和。
