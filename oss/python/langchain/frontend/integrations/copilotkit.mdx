...
[CopilotKit](https://www.copilotkit.ai/) 提供了一个完整的 React 聊天运行时，当您希望代理返回**结构化 UI 有效载荷**而不仅仅是纯文本时，它与 LangGraph 配合得特别好。在此模式中，您的 LangGraph 部署同时提供图 API 和自定义 CopilotKit 端点，而前端则将助理消息解析为动态 React 组件。

当您希望满足以下条件时，这种方法非常有用：
...
在 Python 中，等效的工作发生在中间件中：规范化 CopilotKit 上下文，并将 `useAgentContext(...)` 中的 `output_schema` 转发到模型的结构化输出配置中。
