---
title: "RankLLM 重排序集成"
description: "使用 LangChain Python 集成 RankLLM 重排序文档转换器。"
---

**[RankLLM](https://github.com/castorini/rank_llm)** 是一个**灵活的重排序框架**，支持**列表式、成对式和逐点式排序模型**。它包含 **RankVicuna、RankZephyr、MonoT5、DuoT5、LiT5 和 FirstMistral**，并集成了 **FastChat、vLLM、SGLang 和 TensorRT-LLM** 以实现高效推理。RankLLM 针对**检索和排序任务**进行了优化，既支持**开源 LLM**，也支持 **RankGPT 和 RankGemini** 等专有重排序器。它支持**批量推理、首 token 重排序，以及通过 BM25 和 SPLADE 进行检索**。

> **注意：** 如果使用内置检索器，RankLLM 需要安装 **Pyserini、JDK 21、PyTorch 和 Faiss** 才能使用检索功能。
...
RankZephyr 采用列表式重排序以提升检索质量，但需要至少 24GB 显存才能高效运行。
