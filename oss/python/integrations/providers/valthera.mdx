...
> [Valthera](https://github.com/valthera/valthera) 是一个开源框架，赋能 LLM 代理驱动有意义、具备上下文感知的用户互动。它实时评估用户的动机和能力，确保仅在用户最易接受时触发通知和操作。
>
> **langchain-valthera** 将 Valthera 与 LangChain 集成，使开发者能够构建更智能、基于行为的互动系统，提供个性化交互。
...
from mocks import hubspot, posthog, snowflake  # 将这些替换为你的实际连接器实现
from langchain_valthera.tools import ValtheraTool
...
print("✅ ValtheraTool 已成功初始化用于 LangChain 集成！")
```

langchain-valthera 集成允许你评估用户行为并决定最佳的互动方案，确保在你的 LangChain 应用程序中交互既及时又相关。
