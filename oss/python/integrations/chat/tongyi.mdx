...
---
通义千问是由阿里巴巴达摩院开发的大型语言模型。它能够通过自然语言理解和语义分析来理解用户意图，基于用户以自然语言输入的内容。它为用户提供不同领域和任务的服务与帮助。通过提供清晰详细的指令，您可以获得更符合您期望的结果。

在本笔记本中，我们将主要介绍如何在 `langchain/chat_models` 包中使用 langchain 与 [通义](https://www.aliyun.com/product/dashscope) 进行 `Chat` 对应。
