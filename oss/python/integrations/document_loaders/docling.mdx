---
title: "Docling 集成"
description: "使用 LangChain Python 与 Docling 文档加载器集成。"
---

[Docling](https://github.com/DS4SD/docling) 将 PDF、DOCX、PPTX、HTML 和其他格式解析为丰富的统一表示，包括文档布局、表格等，使其为生成式 AI 工作流（如 RAG）做好准备。
...
- `ExportType.DOC_CHUNKS`（默认）：如果您希望将每个输入文档分块，然后将每个单独的块捕获为下游的独立 LangChain 文档，或
- `ExportType.MARKDOWN`：如果您希望将每个输入文档捕获为独立的 LangChain 文档
...
> 为获得最佳转换速度，请尽可能使用 GPU 加速；例如，如果在 Colab 上运行，请使用启用 GPU 的运行时。
...
- `file_path`：源作为单个字符串（URL 或本地文件）或其可迭代对象
...
> 注意：在这种情况下，可以忽略显示 `"Token indices sequence length is longer than the specified maximum sequence length..."` 的消息——更多详情请参见此 [docling-core GitHub issue](https://github.com/DS4SD/docling-core/issues/119#issuecomment-2577418826)。
...
> - 以下示例管道使用 HuggingFace 的推理 API；为增加 LLM 配额，可以通过环境变量 `HF_TOKEN` 提供令牌。
> - 此管道的依赖项可以如下安装（`--no-warn-conflicts` 适用于 Colab 的预填充 Python 环境；对于更严格的使用，可以随意移除）：
...
请注意，源包含丰富的 grounding 信息，包括段落标题（即章节）、页面和精确的边界框。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/document_loaders/docling.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
