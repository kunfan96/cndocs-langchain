...
本指南演示如何使用 [Deep Agents](/oss/javascript/deepagents) 从零开始构建一个内容写作代理。

你构建的代理将：

1. 从 `AGENTS.md` 和技能文件夹加载语音和工作流规则
1. 将网络研究任务委托给具有 `web_search` 功能的专用子代理
1. 根据加载的技能起草博客或社交内容
...
添加 `tsx` 以运行 `content_writer.ts`。`--input-type=module` 标志仅适用于 `--eval`、`--print` 或标准输入，不适用于脚本文件路径。
...
在项目根目录创建 `AGENTS.md`。
当你稍后创建代理并指定此文件作为[记忆](/oss/javascript/deepagents/long-term-memory)参数的一部分时，它会被加载到系统提示中，以便品牌语音和研究期望适用于每次运行。
...
创建一个 `skills/` 目录。每个技能是一个包含 `SKILL.md` 文件的文件夹，该文件包含 YAML frontmatter（`name`、`description`）和技能说明。

创建 `skills/blog-post/SKILL.md` 并将以下文本复制到其中，其中包含有关创建长篇文章、优化 SEO 内容和生成封面图像的信息。
...
当你稍后创建代理并指定技能文件夹时，这些技能文件夹中 `SKILLS.md` 文件的 frontmatter 会被加载到系统提示中，以便代理在任务匹配技能描述时可以使用该技能。
...
使用 [createDeepAgent](https://reference.langchain.com/javascript/deepagents/agent/createDeepAgent) 创建深度代理时，传入记忆路径、技能目录、图像工具、内联子代理定义和一个以示例目录为根的 [FilesystemBackend](/oss/javascript/deepagents/backends)，以便 `./AGENTS.md` 和 `./skills/` 等路径能正确解析。
...
- 在 `subagents.yaml` 中添加子代理，并在 `load_subagents` 中注册工具
- 阅读[子代理](/oss/javascript/deepagents/subagents)、[技能](/oss/javascript/deepagents/skills)和[自定义](/oss/javascript/deepagents/customization)以进行更深入的配置

---

<div className="source-links">
<Callout icon="terminal-2">
    [将这些文档](/use-these-docs)连接到 Claude、VSCode 等，通过 MCP 获取实时答案。
</Callout>
<Callout icon="edit">
