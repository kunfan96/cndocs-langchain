...
description: "使用 LangChain Python 集成 TavilySearchAPI 检索器。"
---

>[Tavily 的 Search API](https://tavily.com) 是一款专为 AI 智能体（LLM）构建的搜索引擎，能够快速提供实时、准确、真实的搜索结果。

我们可以将其用作[检索器](/oss/python/langchain/retrieval)。本文将展示该集成特有的功能。完成后，您可以探索[相关用例页面](/oss/python/langchain/rag)，了解如何在更大的链中使用此向量存储。
...
如果您希望对单个查询进行自动追踪，可以取消注释以下内容并设置您的 [LangSmith](/langsmith/home) API 密钥：
...
<Callout icon="edit">
    [在 GitHub 上编辑此页](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/retrievers/tavily.mdx) 或 [提交 issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
