---
title: "Cohere 重排序集成"
description: "使用 LangChain JavaScript 与 Cohere 重排序文档压缩器集成。"
---

重排序文档可以极大地改进任何 RAG 应用程序和文档检索系统。

从高层次来看，重排序 API 是一种语言模型，它分析文档并根据其与给定查询的相关性重新排序。
...
在这里，我们可以看到 `.rerank()` 方法仅返回文档的索引（与输入文档的索引匹配）及其相关性分数。
...
从结果中，我们可以看到它返回了前 3 个文档，并为每个文档分配了 `relevanceScore`。

正如预期的那样，具有最高 `relevanceScore` 的文档是引用华盛顿特区的文档，分数为 `98.7%`！

### 与 `CohereClient` 一起使用

如果您在 Azure、AWS Bedrock 或独立实例上使用 Cohere，可以使用 `CohereClient` 创建一个带有您端点的 `CohereRerank` 实例。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/javascript/integrations/document_compressors/cohere_rerank.mdx) 或[提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
