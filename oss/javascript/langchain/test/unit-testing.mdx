---
title: 单元测试
description: 使用假聊天模型和内存持久化测试智能体逻辑，无需 API 调用。
---

单元测试独立地执行智能体中细小、确定性的部分。通过用内存假模型（即 fixture）替换真实 LLM，您可以编写确切的响应（文本、工具调用和错误），从而使测试快速、免费且可重复，无需 API 密钥。

## 使用 `fakeModel` 模拟聊天模型

[`fakeModel`](https://reference.langchain.com/javascript/langchain/index/fakeModel) 是一个构建器风格的假聊天模型，它允许您编写确切的响应（文本、工具调用、错误）并断言模型接收到的内容。它扩展自 [`BaseChatModel`](https://reference.langchain.com/javascript/langchain-core/language_models/chat_models/BaseChatModel)，因此可以在任何需要真实模型的地方使用。
...
`.respond()` 通过传递带有 `tool_calls` 的 [`AIMessage`](https://reference.langchain.com/javascript/langchain-core/messages/AIMessage) 来支持工具调用：
...
<Tip>
    `.respond()` 和 `.respondWithTools()` 可以按任意顺序自由混合使用。这对于测试智能体循环特别有用，其中模型在工具调用和文本响应之间交替。
</Tip>
...
`.respond()` 也接受一个函数，该函数根据输入消息计算响应。该函数接收完整的消息数组，并返回 [`BaseMessage`](https://reference.langchain.com/javascript/langchain-core/messages/BaseMessage) 或 `Error`：
...
<Note>
    每个函数都是一个单独的队列条目，只消耗一次。要为多个回合重用相同的动态逻辑，请队列化多个 `respond` 函数调用。
</Note>
...
传递给 `.withStructuredOutput()` 的模式将被忽略。模型始终返回使用 `.structuredResponse()` 配置的值。这使测试专注于应用程序逻辑而非解析。
...
<Accordion title="完整示例：使用 vitest 测试工具调用智能体">
...
了解如何使用真实模型提供程序 API 测试您的智能体，请参阅[集成测试](/oss/javascript/langchain/test/integration-testing)。
