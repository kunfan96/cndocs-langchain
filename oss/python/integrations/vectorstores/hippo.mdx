---
title: "Hippo 集成"
description: "使用 LangChain Python 与 Hippo 向量存储集成。"
---

>[Transwarp Hippo](https://www.transwarp.cn/en/subproduct/hippo) 是一款企业级云原生分布式向量数据库，支持海量向量数据集的存储、检索和管理。它高效解决了向量相似性搜索和高密度向量聚类等问题。`Hippo` 具有高可用性、高性能和易于扩展的特点。它拥有多种功能，例如多种向量搜索索引、数据分区与分片、数据持久化、增量数据摄入、向量标量字段过滤以及混合查询。它能有效满足企业对海量向量数据的高实时性搜索需求。
...
这里，我们使用 LangChain 的 CharacterTextSplitter 进行分割。分隔符是句号。分割后，文本段不超过 1000 个字符，重复字符数为 0。
