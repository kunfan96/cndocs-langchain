...
description: "使用 LangChain Python 与 turbopuffer 向量存储集成。"
---

>[turbopuffer](https://turbopuffer.com) 是一个快速、经济高效的向量数据库，用于搜索和检索。

本指南展示如何将 `TurbopufferVectorStore` 与 LangChain 一起使用。
...
如果您想获得模型调用的自动跟踪，也可以通过取消注释以下内容来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
创建一个 turbopuffer 客户端和命名空间，然后初始化向量存储：
...
有关支持的过滤运算符的完整列表，请参阅 [turbopuffer 过滤文档](https://turbopuffer.com/docs/reference/query#filter-parameters)。
