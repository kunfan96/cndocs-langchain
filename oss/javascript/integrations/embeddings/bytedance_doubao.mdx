...
description: "使用 LangChain JavaScript 集成 ByteDanceDoubaoEmbeddings 嵌入模型。"
---

这将帮助你开始使用 LangChain 的 ByteDanceDoubao [嵌入模型](/oss/javascript/integrations/embeddings)。有关 `ByteDanceDoubaoEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-community/embeddings/bytedance_doubao/ByteDanceDoubaoEmbeddings)。
...
你需要注册一个 [ARK API 密钥](https://console.volcengine.com/ark/region:ark+cn-beijing/apiKey) 并将其设置为名为 `ARK_API_KEY` 的环境变量。然后，你应该为嵌入模型[创建一个接入点](https://console.volcengine.com/ark/region:ark+cn-beijing/endpoint)，并使用接入点的名称作为 `model`。
...
在底层，向量存储和检索器实现分别调用 `embeddings.embedDocument(...)` 和 `embeddings.embedQuery(...)` 来为 `fromDocuments` 和检索器的 `invoke` 操作中使用的文本创建嵌入。
...
你可以使用 `embedDocuments` 嵌入多个文本用于索引。此方法使用的内部机制可能（但不一定）与嵌入查询不同：
...
有关所有 `ByteDanceDoubaoEmbeddings` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-community/embeddings/bytedance_doubao/ByteDanceDoubaoEmbeddings)。
