...
>[DocArray](https://github.com/docarray/docarray) 是一款多功能的开源工具，用于管理多模态数据。它允许你按自己的方式组织数据，并提供灵活性，可通过多种文档索引后端进行存储和搜索。更棒的是——你可以利用 `DocArray` 文档索引创建 `DocArrayRetriever`，构建出色的 LangChain 应用！

本 notebook 分为两个部分。[第一部分](#document-index-backends) 介绍了所有五种受支持的文档索引后端，并提供了每种后端的设置和索引指南，以及如何构建 `DocArrayRetriever` 来查找相关文档。
在[第二部分](#movie-retrieval-using-hnswdocumentindex)中，我们将选取其中一种后端，通过一个基本示例演示其用法。
...
在开始构建索引之前，重要的是先定义文档的模式（schema）。这决定了文档包含哪些字段以及每个字段保存什么类型的数据。

在本演示中，我们将创建一个包含 'title'（字符串）、'title_embedding'（numpy 数组）、'year'（整数）和 'color'（字符串）的简单模式。
...
`InMemoryExactNNIndex` 将所有文档存储在内存中。对于小型数据集，这是一个很好的起点，无需启动数据库服务器。

了解更多：[docs.docarray.org/user_guide/storing/index_in_memory/](https://docs.docarray.org/user_guide/storing/index_in_memory/)
...
`HnswDocumentIndex` 是一种轻量级文档索引实现，完全在本地运行，最适合中小型数据集。它将向量存储在 [hnswlib](https://github.com/nmslib/hnswlib) 磁盘上，并将其他所有数据存储在 [SQLite](https://www.sqlite.org/index.html) 中。

了解更多：[docs.docarray.org/user_guide/storing/index_hnswlib/](https://docs.docarray.org/user_guide/storing/index_hnswlib/)
...
`WeaviateDocumentIndex` 是基于 [Weaviate](https://weaviate.io/) 向量数据库构建的文档索引。

了解更多：[docs.docarray.org/user_guide/storing/index_weaviate/](https://docs.docarray.org/user_guide/storing/index_weaviate/)
...
`ElasticDocIndex` 是基于 [ElasticSearch](https://github.com/elastic/elasticsearch) 构建的文档索引。

了解更多：[这里](https://docs.docarray.org/user_guide/storing/index_elastic/)
...
`QdrantDocumentIndex` 是基于 [Qdrant](https://qdrant.tech/) 向量数据库构建的文档索引。

了解更多：[这里](https://docs.docarray.org/user_guide/storing/index_qdrant/)
