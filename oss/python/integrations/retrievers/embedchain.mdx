...
>
>它可作为[开源包](https://github.com/embedchain/embedchain)和[托管平台解决方案](https://app.embedchain.ai/)使用。

此笔记本展示了如何使用基于 `Embedchain` 的检索器。
...
* `yaml_path: string` 可选 -- YAML 配置文件的路径。如果未提供，则使用默认配置。您可以浏览[文档](https://docs.embedchain.ai/)以探索各种自定义选项。
...
# 或者，如果您想自定义，请传递 yaml 配置路径
# retriever = EmbedchainRetiever.create(yaml_path="config.yaml")
```

# 添加数据

在 embedchain 中，您可以添加尽可能多的支持数据类型。您可以浏览我们的[文档](https://docs.embedchain.ai/)以查看支持的数据类型。

Embedchain 会自动推断数据的类型。您可以添加字符串、URL 或本地文件路径。
