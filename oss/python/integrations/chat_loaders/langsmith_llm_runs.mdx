...
本笔记本演示如何直接从 LangSmith 的 LLM 运行中加载数据，并在该数据上微调模型。
该过程很简单，包含 3 个步骤。
...
确保您已安装 langchain >= 0.0.311，并使用您的 LangSmith API 密钥配置了环境。
...
第一步是选择要微调的运行。一个常见情况是选择那些在跟踪中收到积极用户反馈的 LLM 运行。您可以在 [LangSmith Cookbook](https://github.com/langchain-ai/langsmith-cookbook/blob/main/exploratory-data-analysis/exporting-llm-runs-and-feedback/llm_run_etl.ipynb) 和 [文档](https://docs.smith.langchain.com/tracing/use-cases/export-runs/local) 中找到相关示例。
