---
title: "Etherscan 集成"
description: "使用 LangChain Python 集成 Etherscan 文档加载器。"
---

>[Etherscan](https://docs.etherscan.io/) 是领先的区块链浏览器、搜索、API 和分析平台，专为以太坊——一个去中心化智能合约平台提供服务。

## 概述

`Etherscan` 加载器使用 `Etherscan API` 加载 `Ethereum 主网` 上特定账户的交易历史记录。

您需要一个 `Etherscan API 密钥` 才能继续操作。免费 API 密钥每秒有 5 次调用的配额。
...
* 检索以太坊主网上特定账户的以太坊余额（单位：wei）

如果账户没有相应的交易，加载器将返回一个包含单个文档的列表，该文档的内容为 ''。

您可以向加载器传递不同的过滤器来访问上述不同的功能：
...
* "erc721_transaction"
* "erc1155_transaction"
过滤器默认值为 normal_transaction

如有任何问题，您可以访问 [Etherscan API 文档](https://etherscan.io/tx/0x0ffa32c787b1398f44303f731cb06678e086e4f82ce07cebf75e99bb7c079c77) 或通过 [i@inevitable.tech](mailto:i@inevitable.tech) 联系。

所有与交易历史相关的函数均受 Etherscan 限制，最多返回 1000 条历史记录。您可以使用以下参数来查找所需的交易历史：

* offset：默认为 20。每次显示 20 条交易记录
* page：默认为 1。用于控制分页。
* start_block：默认为 0。交易历史从第 0 个区块开始。
* end_block：默认为 99999999。交易历史截止至第 99999999 个区块
