...
>[Outlines](https://github.com/dottxt-ai/outlines) 是一个用于约束语言生成的 Python 库。它为各种语言模型提供了统一的接口，并允许使用正则表达式匹配、类型约束、JSON 模式和上下文无关语法等技术进行结构化生成。
...
此集成允许您将 Outlines 模型与 LangChain 一起使用，提供 LLM 和聊天模型接口。
...
1. **正则表达式匹配**：
   ...
   ```
   这将确保生成的文本匹配指定的正则表达式模式（在本例中为有效的 IP 地址）。
