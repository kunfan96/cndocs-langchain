...
> [CrateDB] 是一个分布式且可扩展的 SQL 数据库，用于存储和分析海量数据，即使在复杂查询下也能实现近实时处理。它兼容 PostgreSQL，基于 Lucene 构建，并继承自 Elasticsearch。


## 安装与设置

### 设置 CrateDB
有两种快速开始使用 CrateDB 的方式。或者，选择其他 [CrateDB 安装选项]。

#### 在本地机器上启动 CrateDB
示例：使用 Docker 或 Podman 运行一个禁用安全功能的单节点 CrateDB 实例。不建议在生产环境中使用。
...
## 文档
有关 CrateDB 包装器的更详细演练，请参阅 [使用 LangChain 与 CrateDB]。另请参阅 [CrateDB 的所有功能] 以了解 CrateDB 提供的其他功能。
...
### 向量存储
使用 CrateDB 围绕 `FLOAT_VECTOR` 和 `KNN_MATCH` 的向量存储功能进行相似性搜索和其他用途。另请参阅 [CrateDBVectorStore 教程]。
...
### 文档加载器
使用文档加载器 `CrateDBLoader` 从 CrateDB 数据库表加载文档，该加载器基于 SQLAlchemy。另请参阅 [CrateDBLoader 教程]。
...
### 完整缓存
标准/完整缓存可避免在提供的提示与已遇到的提示完全相同时调用 LLM。
...
语义缓存允许用户基于用户输入与先前缓存输入之间的语义相似性来检索缓存的提示。它还可以避免在不需要时调用 LLM。
