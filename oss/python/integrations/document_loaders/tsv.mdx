---
title: "TSV 集成"
description: "使用 LangChain Python 集成 TSV 文档加载器。"
---

>[制表符分隔值（TSV）](https://en.wikipedia.org/wiki/Tab-separated_values) 文件是一种用于存储表格数据的简单文本格式。记录之间以换行符分隔，记录内的值之间以制表符分隔。

## `UnstructuredTSVLoader`

您也可以使用 `UnstructuredTSVLoader` 加载表格数据。使用 `UnstructuredTSVLoader` 的优势之一是，在 `"elements"` 模式下，元数据中将包含表格的 HTML 表示。
