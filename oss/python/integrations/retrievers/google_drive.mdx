...
`token.json` 的位置使用相同的目录（或使用参数 `token_path`）。请注意，`token.json` 会在您首次使用检索器时自动创建。

`GoogleDriveRetriever` 可以通过一些请求检索选定的文件。

默认情况下，如果您使用 `folder_id`，则该文件夹内的所有文件都可以检索为 `Document`。
...
| `gdrive-query-in-folder`                 | 在 `folder_id` 中搜索 `query`（如果 `_recursive=true` 则包含子文件夹）|
...
每个 Google Drive 在元数据中都有一个 `description` 字段（参见 *文件的详细信息*）。
