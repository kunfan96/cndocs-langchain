...
FMP（Financial Modeling Prep）LangChain 集成提供了一种无缝的方式，通过自然语言查询访问金融市场数据。此集成提供两个主要组件：
...
# 基本查询
# fmt: off
result = tool.invoke({"query": "苹果公司当前的股价是多少？"})
# fmt: on
...
# 创建代理
prompt = "你是一个有用的助手。根据提供的上下文回答用户的问题。"
...
advanced_tool = FMPDataTool(
    ...
    temperature=0.2,  # 调整温度以获得更集中/更分散的响应
)
...
您可以像使用其他工具一样，通过创建所需模型的链来链式调用该工具。
