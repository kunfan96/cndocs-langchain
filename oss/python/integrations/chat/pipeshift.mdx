...
这将帮助你开始使用 Pipeshift [聊天模型](/oss/python/langchain/models/)。有关所有 `ChatPipeshift` 功能和配置的详细文档，请访问 [API 参考](https://dashboard.pipeshift.com/docs)。
...
要访问 Pipeshift 模型，你需要在 Pipeshift 上创建一个账户，获取 API 密钥，并安装 `langchain-pipeshift` 集成包。
...
如果你想自动跟踪模型调用，也可以取消注释以下内容来设置你的 [LangSmith](/langsmith/home) API 密钥：
...
有关所有 `ChatPipeshift` 功能和配置的详细文档，请访问 API 参考：[dashboard.pipeshift.com/docs](https://dashboard.pipeshift.com/docs)
