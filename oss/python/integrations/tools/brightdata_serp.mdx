...
|原生异步|返回工件|返回数据|定价|
|:-:|:-:|:--|:-:|
|❌|❌|标题、URL、摘要、位置及其他搜索结果数据|需要 Bright Data 账户|
...
这里我们展示如何实例化 BrightDataSERP 工具的一个实例。此工具允许您使用 Bright Data 的 SERP API 执行搜索引擎查询，支持多种自定义选项，包括地理定位、语言偏好、设备类型模拟和特定搜索类型。

该工具在实例化时接受各种参数：

- `bright_data_api_key`（必需，str）：用于身份验证的 Bright Data API 密钥。
- `zone`（可选，str）：用于 SERP API 的 Bright Data 区域名称。默认为 "serp"。您可以在 [Bright Data 控制面板](https://brightdata.com/cp/zones) 中配置自定义区域。
...
Bright Data 使用 "区域" 来管理不同的 API 配置。您可以在初始化时设置区域，也可以在每次请求时覆盖它。
