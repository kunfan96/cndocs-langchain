---
title: "Valthera 集成"
description: "使用 LangChain Python 与 Valthera 工具集成。"
---

使 AI 代理能够在用户最有可能响应时与他们互动。

## 概述

Valthera 是一个开源框架，使 LLM 代理能够以更有意义的方式与用户互动。它基于 BJ Fogg 的行为模型 (B=MAT) 构建，并利用来自多个来源（如 HubSpot、PostHog 和 Snowflake）的数据，在触发操作前评估用户的**动机**和**能力**。
...
class MockHubSpotConnector(BaseConnector):
    """
    模拟从 HubSpot 检索数据。提供潜在客户评分、生命周期阶段和营销指标等信息。
    """
...
在本节中，我们将实例化核心组件。首先，我们创建一个数据聚合器来组合来自自定义连接器的数据。然后，我们为动机和能力配置评分指标。
...
# 现在可以通过调用 data_aggregator.get_user_context(user_id) 获取统一的用户数据
```
...
接下来，我们设置推理引擎和触发器生成器，然后通过实例化 Valthera 工具将所有组件组合在一起。最后，我们执行代理工作流以处理输入消息。
...
try:
    api_key = get_openai_api_key()  # 重用该函数以保持一致性
...
- **推理引擎：** `ReasoningEngine` 评估决策规则以确定适当的操作（触发、改进动机、改进能力或推迟）。
