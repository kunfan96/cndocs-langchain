...
[AgentQL](https://www.agentql.com/) 工具提供网页交互功能，可使用 [AgentQL 查询](https://docs.agentql.com/agentql-query)或自然语言提示从任意网页中提取结构化数据。AgentQL 支持多种语言和网页，且不会随时间推移和页面变化而失效。

## 概述

AgentQL 提供以下三种工具：

- **`ExtractWebDataTool`** 通过 URL 从网页中提取 JSON 格式的结构化数据，可使用 [AgentQL 查询](https://docs.agentql.com/agentql-query/query-intro)或自然语言描述来指定所需数据。

以下两种工具还可打包为 `AgentQLBrowserToolkit`，必须与 `Playwright` 浏览器或通过 Chrome DevTools Protocol（CDP）连接的远程浏览器实例配合使用：

- **`ExtractWebDataBrowserTool`** 使用 [AgentQL 查询](https://docs.agentql.com/agentql-query/query-intro)或自然语言描述，从浏览器当前活动网页中提取 JSON 格式的结构化数据。
...
要使用 AgentQL 工具，您需要从 [AgentQL 开发者门户](https://dev.agentql.com/)获取自己的 API key，并设置 AgentQL 环境变量。
...
- `api_key`：您在 [dev.agentql.com](https://dev.agentql.com) 获取的 AgentQL API key。**`可选`。**
...
- `mode`：`"standard"` 使用深度数据分析，而 `"fast"` 以一定的分析深度换取速度，适用于大多数场景。[在此指南中了解更多关于模式的信息。](https://docs.agentql.com/accuracy/standard-mode) **默认值为 `"fast"`。**
- `is_screenshot_enabled`：提取数据前是否截图。截图以 Base64 字符串形式返回在 'metadata' 中。**默认值为 `False`。**

`ExtractWebDataTool` 基于 AgentQL 的 REST API 实现，您可以在 [API 参考文档](https://docs.agentql.com/rest-api/api-reference)中查看更多参数详情。
...
`ExtractWebDataBrowserTool` 基于 AgentQL SDK 实现。您可以在 AgentQL 的 [API 参考文档](https://docs.agentql.com/python-sdk/api-references/agentql-page#querydata)中找到更多参数和函数的详情。
...
`GetWebElementBrowserTool` 基于 AgentQL SDK 实现。您可以在 AgentQL 的 [API 参考文档](https://docs.agentql.com/python-sdk/api-references/agentql-page#queryelements)中找到更多参数和函数的详情。`
...
- `url`：要提取数据的网页 URL。
- `query`：要执行的 AgentQL 查询。若需要提取精确结构化的数据，请使用 AgentQL 查询。在[文档](https://docs.agentql.com/agentql-query)中了解如何编写 AgentQL 查询，或在 [AgentQL Playground](https://dev.agentql.com/playground) 中测试。
- `prompt`: 要从页面提取的数据的自然语言描述。AgentQL 将根据您的提示推断数据结构。若希望用自由格式语言描述数据而无需定义特定结构，请使用 `prompt`。

**注意：** 使用 AgentQL 时必须定义 `query` 或 `prompt` 之一。
...
在提取数据之前，必须先使用 LangChain 的 [Playwright](https://python.langchain.com/docs/integrations/tools/playwright/) 工具导航到目标网页。
...
您可以通过将 AgentQL 工具绑定到[支持工具调用的模型](/oss/python/langchain/tools/)，然后调用它来在链中使用：
...
您可以将 AgentQL 工具与 AI Agent 配合使用，使用 `AgentQLBrowserToolkit`。该工具包包含 `ExtractDataBrowserTool` 和 `GetWebElementBrowserTool`。以下示例展示了将 AgentQL 工具包与 Playwright 工具结合使用的 Agent 浏览器操作。
