...
description: "使用 LangChain Python 集成 Ctranslate2 LLM。"
---

**CTranslate2** 是一个用于 Transformer 模型高效推理的 C++ 和 Python 库。

该项目实现了一个自定义运行时，应用了多种性能优化技术，例如权重量化、层融合、批次重排序等，以在 CPU 和 GPU 上加速 Transformer 模型推理并降低内存占用。

完整的功能列表和支持的模型包含在[项目仓库](https://opennmt.net/CTranslate2/guides/transformers.html)中。入门请查阅官方[快速入门指南](https://opennmt.net/CTranslate2/quickstart.html)。
...
要在 CTranslate2 中使用 Hugging Face 模型，首先需要使用 `ct2-transformers-converter` 命令将其转换为 CTranslate2 格式。该命令需要预训练模型名称和转换后模型目录的路径。
