---
title: "Libsql 集成"
description: "使用 LangChain JavaScript 与 Libsql 向量存储集成。"
---

[Turso](https://turso.tech) 是一个基于 [libSQL](https://docs.turso.tech/libsql) 构建的、兼容 SQLite 的数据库，libSQL 是 SQLite 的开放贡献分支。向量相似性搜索作为原生数据类型内置于 Turso 和 libSQL 中，使您能够直接在数据库中存储和查询向量。

LangChain.js 支持使用本地 libSQL 或远程 Turso 数据库作为向量存储，并提供了一个简单的 API 与其交互。

本指南提供了快速入门 `LibSQLVectorStore` [向量存储](/oss/javascript/integrations/vectorstores)的概览。
...
要使用 libSQL 向量存储，您需要创建一个 Turso 帐户或设置一个本地 SQLite 数据库，并安装 `@langchain/community` 集成包。

本指南还将使用 OpenAI 嵌入，这需要您安装 `@langchain/openai` 集成包。如果您愿意，也可以使用其他支持的嵌入模型。

使用 libSQL 向量存储时，您可以使用本地 SQLite，也可以使用托管的 Turso 数据库。
...
```
现在是时候创建数据库了。您可以在本地创建一个，也可以使用托管的 Turso 数据库。
...
CREATE TABLE IF NOT EXISTS TABLE_NAME (
    ...
    content TEXT,
    metadata TEXT,
    EMBEDDING_COLUMN F32_BLOB(1536) -- 1536 维的 f32 向量，用于 OpenAI
);
```
...
```
请确保将 `TABLE_NAME` 和 `EMBEDDING_COLUMN` 替换为您在上一步中使用的值。

## 实例化

要初始化一个新的 `LibSQL` 向量存储，您需要在远程工作时提供数据库 URL 和身份验证令牌，或者通过传递本地 SQLite 的文件名。
