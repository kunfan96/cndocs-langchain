---
title: "百度千帆集成"
description: "使用 LangChain Python 与百度千帆大语言模型集成。"
---
...
在本笔记本中，我们将介绍如何主要使用 langchain 中的 `Completion`（对应 `langchain/llms` 包）与 [千帆](https://cloud.baidu.com/doc/WENXINWORKSHOP/index.html) 进行集成：

## API 初始化

要使用基于百度千帆的大语言模型服务，您必须初始化以下参数：
...
- 1. （可选，如果模型已包含在默认模型中，则跳过此步）在千帆控制台中部署您的模型，获取您自己的定制部署端点。
