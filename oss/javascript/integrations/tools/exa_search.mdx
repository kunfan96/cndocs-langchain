...
description: "使用 LangChain JavaScript 集成 ExaSearchResults 工具。"
---

Exa（前身为 Metaphor Search）是一款专为大语言模型（LLM）设计的搜索引擎。使用自然语言查询搜索互联网上的文档，然后从所需文档中检索清理后的 HTML 内容。

与基于关键词的搜索（如 Google）不同，Exa 的神经搜索能力使其能够语义理解查询并返回相关文档。例如，我们可以搜索 `"fascinating article about cats"` 并比较 Google 和 Exa 的搜索结果。Google 基于关键词“fascinating”返回 SEO 优化的列表文章。而 Exa 则直接有效。
...
首先，获取一个 Exa API 密钥并将其添加为环境变量。通过[注册 Exa](https://dashboard.exa.ai/login) 每月可获得 1000 次免费搜索。
...
以下展示如何实例化一个 `ExaSearchResults` 工具实例：
...
我们可以创建使用 `ExaRetriever` 和 `createRetrieverTool` 的 LangChain 工具。使用这些工具，我们可以构建一个简单的搜索代理，能够回答关于任何主题的问题。

我们将使用 LangGraph 来创建代理。确保你已安装 `@langchain/langgraph`：
...
有关所有 `ExaSearchResults` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/javascript/langchain-exa/ExaSearchResults)
