---
title: "Neo4j 向量索引集成"
description: "使用 LangChain Python 与 Neo4j 向量索引向量存储进行集成。"
---

>[Neo4j](https://neo4j.com/) 是一个开源图数据库，集成了向量相似性搜索支持。
...
我们还可以使用 `from_existing_graph` 方法从现有图初始化向量存储。此方法从数据库中提取相关文本信息，并计算文本嵌入并将其存储回数据库。
...
Neo4j 还支持关系向量索引，其中嵌入存储为关系属性并被索引。关系向量索引无法通过 LangChain 填充，但您可以将其连接到现有的关系向量索引。
...
您还可以通过使用自定义 Cypher 片段来从图中获取其他信息，从而自定义响应。
在底层，最终的 Cypher 语句是这样构造的：
...
这是一个将除 `embedding` 外的所有节点属性作为字典传递给 `text` 列的示例，
...
Neo4j 集成了向量和关键词索引，这允许您使用混合搜索方法。
...
本节介绍如何在索引上执行带来源的问答。这是通过使用 `RetrievalQAWithSourcesChain` 来完成的，该链从索引中查找文档。
