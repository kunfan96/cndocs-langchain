...
description: "使用 LangChain Python 与 ElasticsearchEmbeddingsCache 存储集成。"
---
...
与其他 `ByteStores` 一样，您可以使用 `ElasticsearchEmbeddingsCache` 实例进行[文档摄取中的持久缓存](/oss/python/integrations/embeddings#caching)以用于 RAG。

但是，默认情况下，缓存的向量不可搜索。开发人员可以自定义 Elasticsearch 文档的构建，以添加索引向量字段。
...
当重写映射和文档构建时，请仅进行添加性修改，保持基础映射完整。
