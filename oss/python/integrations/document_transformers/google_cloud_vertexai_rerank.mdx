---
title: "Google Cloud Vertex AI 重排序器集成"
description: "使用 LangChain Python 与 Google Cloud Vertex AI 重排序器文档转换器集成。"
---

> [Vertex Search Ranking API](https://cloud.google.com/generative-ai-app-builder/docs/ranking) 是 [Vertex AI Agent Builder](https://cloud.google.com/generative-ai-app-builder/docs/builder-apis) 中的独立 API 之一。它接收文档列表，并根据文档与查询的相关性对这些文档进行重新排序。与仅查看文档和查询语义相似性的嵌入相比，排名 API 可以提供文档回答给定查询的精确分数。排名 API 可用于在检索初始候选文档集后提高搜索结果的质量。

>排名 API 是无状态的，因此在调用 API 之前无需索引文档。您只需传入查询和文档即可。这使得该 API 非常适合对来自任何文档检索器的文档进行重新排序。

>更多信息，请参阅[排名和重新排序文档](https://cloud.google.com/generative-ai-app-builder/docs/ranking)。
...
在此示例中，我们将使用 [Google Wiki 页面](https://en.wikipedia.org/wiki/Google) 来演示 Vertex Ranking API 的工作原理。

我们使用标准的 `加载 -> 拆分 -> 嵌入数据` 管道。

嵌入使用 [Vertex Embeddings API](https://cloud.google.com/vertex-ai/generative-ai/docs/embeddings/get-text-embeddings#supported_models) 模型 - `textembedding-gecko@003` 创建。
...
print(f"您的 {len(data)} 个文档已被拆分为 {len(splits)} 个块")

if vectordb is not None:  # 如果已存在，则删除现有的 vectordb
...
# 使用 VertexAIRanker 作为重排序器创建 ContextualCompressionRetriever
...
排名 API 用于提高排名质量，并确定一个分数，该分数表示每条记录与查询的相关性。

您可以看到未排序文档和已排序文档之间的差异。排名 API 将语义上最相关的文档移动到 LLM 上下文窗口的顶部，从而帮助其形成更好的推理答案。
...
让我们检查几个重新排序的文档。我们观察到检索器仍然返回相关的 LangChain 类型 [文档](https://reference.langchain.com/python/langchain-core/documents/base/Document)，但作为元数据字段的一部分，我们还接收来自排名 API 的 `relevance_score`。
...
此示例展示了一个完整的 RAG 链，其中包含一个简单的提示模板，说明如何使用 Vertex Ranking API 执行重新排序。
...
reranker = VertexAIRank(
    ...
    location_id=RANKING_LOCATION_ID,
    ranking_config="default_ranking_config",
    title_field="source",  # 来自现有文档的元数据字段键
    top_n=5,
)
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/document_transformers/google_cloud_vertexai_rerank.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
