---
title: "FireCrawl 集成"
description: "使用 LangChain Python 与 FireCrawl 文档加载器集成。"
---

[FireCrawl](https://firecrawl.dev/?ref=langchain) 爬取并将任何网站转换为 LLM 就绪的数据。它爬取所有可访问的子页面，并为每个页面提供干净的 Markdown 和元数据。无需站点地图。
...
您需要获取自己的 API 密钥。参见 [firecrawl.dev](https://firecrawl.dev)
...
- `crawl`：爬取 URL 及所有可访问的子页面，并为每个页面返回 Markdown。
- `map`：映射 URL 并返回语义相关页面的列表。
...
您还可以向加载器传递 `params`。这是一个传递给爬虫的选项字典。有关更多信息，请参阅 [FireCrawl API 文档](https://github.com/mendableai/firecrawl-py)。
...
您还可以向加载器传递 `params`。这是一个传递给加载器的选项字典。有关更多信息，请参阅 [FireCrawl API 文档](https://github.com/mendableai/firecrawl-py)。

---

## API 参考

有关所有 `FireCrawlLoader` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/python/langchain-community/document_loaders/firecrawl/FireCrawlLoader)

---

<div className="source-links">
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/document_loaders/firecrawl.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
