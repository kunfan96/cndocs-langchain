...
GitHub 加载器需要 [ignore npm 包](https://www.npmjs.com/package/ignore) 作为对等依赖项。安装方法如下：
...
要忽略特定文件，可以将 `ignorePaths` 数组传递给构造函数：
...
您可能希望针对与 `github.com` 不同的 GitHub 实例，例如，如果您的公司有 GitHub Enterprise 实例。
为此，您需要两个额外的参数：

- `baseUrl` - 您的 GitHub 实例的基本 URL，因此 githubUrl 匹配 `<baseUrl>/<owner>/<repo>/...`
...
如果您的仓库有子模块，您必须决定加载器是否应跟随它们。您可以使用布尔值 `processSubmodules` 参数进行控制。默认情况下，子模块不会被处理。
...
在需要以内存高效的方式处理大型仓库的情况下。您可以使用 `loadAsStream` 方法异步流式传输整个 GitHub 仓库的文档。
