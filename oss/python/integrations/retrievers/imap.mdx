...
本指南将帮助你快速上手 IMAP [检索器](/oss/python/integrations/retrievers)。`ImapRetriever` 可以将 IMAP 服务器中的电子邮件作为 LangChain `Document` 对象进行搜索和检索。
...
`ImapRetriever` 在 `langchain-imap` 包中：

```bash
pip install -U langchain-imap
```

如需使用 docling 进行完整文档处理（DOCX、PPTX 等）（尚未测试）：
...
出于测试目的，可以使用 GreenMail 设置本地 IMAP 服务器：
...
要使用 `ImapRetriever`，需要使用 `ImapConfig` 配置 IMAP 服务器详情：
...
- **auth_method**：认证方式（默认："login"）
- **ssl_mode**：SSL 模式——"ssl"（默认）、"starttls" 或 "plain"
- **verify_cert**：对于自签名证书设置为 `False`（不建议用于生产环境）
- **k**：要检索的文档数量
...
- `"full_content"`：使用 docling 从办公文档中进行完整提取（需要 `[docling]` 扩展）
...
与其他检索器一样，`ImapRetriever` 可以通过链集成到 LLM 应用中。以下是一个完整示例，使用 LLM 生成 IMAP 查询并根据电子邮件内容回答问题：
