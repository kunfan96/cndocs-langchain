...
>[MediaWiki XML 转储](https://www.mediawiki.org/wiki/Manual:Importing_XML_dumps) 包含 Wiki 的内容（包含所有修订版本的 Wiki 页面），但不包含与站点相关的数据。XML 转储不会创建 Wiki 数据库的完整备份，转储不包含用户帐户、图像、编辑日志等。

本文档介绍如何将 MediaWiki XML 转储文件加载到文档格式中，以便在下游使用。

它使用 `mediawiki-utilities` 中的 `mwxml` 进行转储，并使用 `earwig` 中的 `mwparserfromhell` 来解析 MediaWiki 维基代码。
