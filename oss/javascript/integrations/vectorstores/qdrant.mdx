...
description: "使用 LangChain JavaScript 与 QdrantVectorStore 集成。"
---
...
[Qdrant](https://qdrant.tech/) 是一个向量相似性搜索引擎。它提供了一个生产就绪的服务，具有便捷的 API，用于存储、搜索和管理点——带有附加有效负载的向量。

本指南提供了开始使用 Qdrant [向量存储](/oss/javascript/integrations/vectorstores) 的快速概述。有关所有 `QdrantVectorStore` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-qdrant/QdrantVectorStore)。
...
安装所需依赖项后，按照 [Qdrant 设置说明](https://qdrant.tech/documentation/quickstart/) 在您的计算机上使用 Docker 运行 Qdrant 实例。记下容器运行的 URL。
...
有关 Qdrant 过滤语法的更多信息，请参见 [此页面](https://qdrant.tech/documentation/concepts/filtering/)。请注意，所有值必须以 `metadata.` 为前缀。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/javascript/integrations/vectorstores/qdrant.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
