...
> [Vertex Search Ranking API](https://cloud.google.com/generative-ai-app-builder/docs/ranking) 是 [Vertex AI Agent Builder](https://cloud.google.com/generative-ai-app-builder/docs/builder-apis) 中的独立 API 之一。它接收一组文档，并根据文档与查询的相关性对其进行重新排序。与仅关注文档与查询语义相似度的嵌入方法相比，排名 API 能够为每个文档与给定查询的匹配程度给出精确的评分。排名 API 可在检索初始候选文档集合后，用于提升搜索结果的质量。

> 排名 API 是无状态的，因此在调用 API 之前无需对文档建立索引。您只需传入查询和文档即可。这使得该 API 非常适合对来自任意文档检索器的结果进行重新排序。

> 更多信息，请参阅 [文档排名与重排序](https://cloud.google.com/generative-ai-app-builder/docs/ranking)。
...
在本示例中，我们将使用 [Google 维基百科页面](https://en.wikipedia.org/wiki/Google)演示 Vertex Ranking API 的工作原理。

我们采用标准的 `加载 -> 分割 -> 嵌入数据` 流程。
...
让我们使用相同的查询分别调用 `basic_retriever` 和 `retriever_with_reranker`，并对比检索到的文档。

Ranking API 接收 `basic_retriever` 的输出，并将其传递给 Ranking API。

Ranking API 用于提升排名质量，并为每条记录与查询的相关性给出评分。

您可以观察到未排名文档与排名后文档之间的差异。Ranking API 将语义最相关的文档移至 LLM 上下文窗口的顶部，从而帮助模型基于推理给出更好的回答。
...
让我们检查几个重新排序后的文档。可以观察到，检索器返回的仍是 LangChain 标准 [documents](https://python.langchain.com/api_reference/core/documents/langchain_core.documents.base.Document.html) 类型，但在元数据字段中还包含了来自 Ranking API 的 `relevance_score`。
...
以下示例展示了一个完整的 RAG 链，通过简单的提示词模板演示如何使用 Vertex Ranking API 进行重新排序。
