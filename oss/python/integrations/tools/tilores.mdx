---
title: "Tilores 集成"
description: "使用 LangChain Python 集成 Tilores 工具。"
---

本文档介绍如何开始使用 [Tilores](/oss/python/integrations/providers/tilores) 工具。
如需更复杂的示例，请参阅我们的[客户洞察聊天机器人示例](https://github.com/tilotech/identity-rag-customer-insights-chatbot)。
...
要访问 Tilores，您需要[创建并配置一个实例](https://app.tilores.io)。如果您希望先试用 Tilores，可以使用[只读演示凭据](https://github.com/tilotech/identity-rag-customer-insights-chatbot?tab=readme-ov-file#1-configure-customer-data-access)。
...
`tilores_search` 工具的参数取决于 Tilores 中[配置的模式](https://docs.tilotech.io/tilores/schema/)。以下示例将使用演示实例（生成数据）的模式。

### [直接使用参数调用](/oss/python/langchain/tools)

以下示例搜索柏林一位名叫 Sophie Müller 的人。Tilores 数据中包含多位同名人士，并返回其已知的电子邮件地址和电话号码。
...
如果我们想了解第一个实体中各记录之间的关联，可以使用 `edge_tool`。注意 Tilores 实体解析引擎会自动识别这些记录之间的关系。更多详情请参阅[边文档](https://docs.tilotech.io/tilores/rules/#edges)。
...
我们可以先将工具绑定到[支持工具调用的模型](/oss/python/langchain/tools/)，然后在链中使用该工具：
...
有关 Tilores 所有功能和配置的详细文档，请访问官方文档：[docs.tilotech.io/tilores/](https://docs.tilotech.io/tilores/)
