...
本指南提供了开始使用 WRITER [工具](/oss/python/langchain/tools)的概述。有关所有 WRITER 功能和配置的详细文档，请访问 [WRITER 文档](https://dev.writer.com/home)。
...
函数是最常见的工具类型，它允许 LLM 调用外部 API、从数据库获取数据，以及通常执行您想要进行的任何外部操作。访问 WRITER 的 [工具调用文档](https://dev.writer.com/home/tool-calling) 获取更多信息。

#### 图

`Graph` 工具使用 WRITER 的知识图谱，这是一个基于图的检索增强生成（RAG）系统。使用此工具时，开发人员提供一个图 ID 来引用其特定的知识图谱。然后，模型使用此图来查找相关信息，并生成对提示中问题的准确答案。这允许模型在对话期间访问和利用自定义知识库。更多详情，请参阅 WRITER 的 [知识图谱 API 文档](https://dev.writer.com/home/knowledge-graph)。

#### 翻译

翻译工具允许您在与 Palmyra 模型对话时翻译文本。虽然 Palmyra X 模型可以执行翻译任务，但它们并未针对这些任务进行优化，如果没有正确的提示，可能表现不佳。更多信息，请参阅 WRITER 的 [翻译 API 文档](https://dev.writer.com/home/translation-tool#translate-text-in-a-chat)。

#### 网页搜索

网页搜索工具允许您在与 Palmyra 模型对话时搜索网络以获取当前信息。虽然 Palmyra 模型拥有广泛的知识，但它们可能无法访问最新的信息或实时数据。网页搜索工具使您的 AI 助手能够从网络上查找最新的信息、新闻和事实。更多信息，请参阅 WRITER 的 [网页搜索 API 文档](https://dev.writer.com/home/web-search-tool#web-search-in-a-chat)。

## 设置

注册 [WRITER AI Studio](https://app.writer.com/aistudio/signup?utm_campaign=devrel) 以生成 API 密钥（您可以按照此 [快速入门](https://dev.writer.com/home/quickstart)）。然后，设置 `WRITER_API_KEY` 环境变量：
...
翻译工具允许您在与 Palmyra 模型对话时翻译文本。虽然 Palmyra X 模型可以执行翻译任务，但它们并未针对这些任务进行优化，如果没有正确的提示，可能表现不佳。
...
网页搜索工具允许您在与 Palmyra 模型对话时搜索网络以获取当前信息。虽然 Palmyra 模型拥有广泛的知识，但它们可能无法访问最新的信息或实时数据。网页搜索工具使您的 AI 助手能够从网络上查找最新的信息、新闻和事实。

要使用网页搜索工具，导入并初始化内置的 `WebSearchTool`：
...
def get_supercopa_trophies_count(club_name: str) -> int | None:
    ...
    Args:
        club_name: 您想调查其超级杯奖杯信息的俱乐部

    Returns:
        超级杯奖杯数量，如果有关请求俱乐部的信息不存在则返回 None
    """
...
get_product_info = {
    ...
    "function": {
        ...
        "description": "通过产品 ID 获取产品信息",
        "parameters": {
            "type": "object",
            "properties": {
                "product_id": {
                    "type": "number",
                    "description": "要检索其信息的产品的唯一标识符",
                }
            },
            "required": ["product_id"],
        },
...
**重要提示**：WRITER 一次只允许绑定一个 WRITER 工具（translation、graph、web_search、llm、image、vision）。不能同时绑定多个 WRITER 工具。但是，可以将多个自定义函数工具与一个 WRITER 工具一起绑定。
...
print(search_response.content)  # 输出：基于网页搜索的当前 AI 发展
```
...
WRITER 图工具的工作方式与其他工具不同；使用时，WRITER 服务器会自动处理调用知识图谱并使用 RAG 生成响应。由于这种自动化的服务器端处理，您无法独立调用 `GraphTool` 或将其用作 LangChain 链的一部分。您必须直接将 `GraphTool` 与 `ChatWriter` 实例一起使用，如上面的示例所示。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/tools/writer.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>