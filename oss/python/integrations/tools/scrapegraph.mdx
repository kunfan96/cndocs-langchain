...
本指南提供了 ScrapeGraph [工具](/oss/python/integrations/tools/) 快速入门概述。有关所有 ScrapeGraph 功能和配置的详细文档，请参阅 [API 参考](https://python.langchain.com/docs/integrations/tools/scrapegraph)。
...
使用这些工具需要 ScrapeGraph AI API 密钥。请在 [scrapegraphai.com](https://scrapegraphai.com) 获取。
...
设置 [LangSmith](https://smith.langchain.com/) 也很有帮助（但非必需），以获得最佳可观测性：
...
### [直接带参数调用](/oss/python/langchain/tools)

让我们逐个尝试每个工具：

### SmartCrawler 工具

SmartCrawlerTool 允许您从网站爬取多个页面并提取结构化数据，支持深度控制、页面限制和域名限制等高级爬取选项。
...
有关所有 ScrapeGraph 功能和配置的详细文档，请参阅 [LangChain API 参考](https://python.langchain.com/docs/integrations/tools/scrapegraph)。
