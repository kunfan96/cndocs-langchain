...
在评估器-优化器工作流中，一次 LLM 调用创建响应，另一次调用评估该响应。如果评估器或[人在回路中](/oss/python/langgraph/interrupts)确定响应需要改进，则会提供反馈并重新创建响应。此循环持续进行，直到生成可接受的响应。
...
<Note>
要开始使用代理，请参阅[快速入门](/oss/python/langchain/quickstart)或阅读更多关于它们在 LangChain 中[如何工作](/oss/python/langchain/agents)的信息。
</Note>
