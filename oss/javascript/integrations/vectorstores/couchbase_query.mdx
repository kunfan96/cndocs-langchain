...
description: "使用 LangChain JavaScript 与 Couchbase 查询向量存储集成。"
---

`CouchbaseQueryVectorStore` 是 Couchbase 中向量搜索的首选实现。它使用 [查询服务](https://docs.couchbase.com/server/current/learn/services-and-indexes/services/query-service.html) (SQL++) 和 [索引服务](https://docs.couchbase.com/server/current/learn/services-and-indexes/services/index-service.html) 进行向量相似性搜索，而不是搜索服务。这提供了一种更强大、更直接的方法，通过使用带有向量函数的 SQL++ 查询进行向量操作。

有关 Couchbase 向量搜索能力的更多信息，请参阅官方文档：[选择正确的向量索引](https://docs.couchbase.com/server/current/vector-index/use-vector-indexes.html)。
...
- **查询和索引服务**：使用 Couchbase 的查询服务（SQL++）而非搜索服务
...
### 超大规模索引
一种专门的向量索引，使用 Couchbase 的向量索引能力针对向量操作进行了优化：
...
- `IndexType.HYPERSCALE` - 专门的向量索引，用于优化向量搜索性能
- `IndexType.COMPOSITE` - 通用索引，可包含向量和标量字段
...
2. **选择索引类型**：
   ...
   - 对于结合向量相似性与标量字段过滤的混合查询，使用**复合索引**
3. **调优参数**：根据数据量和性能要求调整 `indexScanNprobes` 和 `indexTrainlist`
...
#### 训练数据不足
如果您看到与训练数据不足相关的错误，可能需要：
- 增加 `indexTrainlist` 参数（默认建议：每个质心约 50 个向量）
- 确保您的集合中有足够的带有向量嵌入的文档
- 对于少于 100 万个向量的集合，使用 `number_of_vectors / 1000` 作为质心数
...
### 使用 `CouchbaseQueryVectorStore` 之前需要创建索引吗？

不需要，与基于搜索的 `CouchbaseSearchVectorStore` 不同，基于查询的实现可以在没有预先创建的索引的情况下工作。但是，创建适当的向量索引（超大规模或复合）将显著提高查询性能。

### 何时应该使用超大规模索引与复合索引？

- 当您主要执行向量相似性搜索且对其他字段的过滤最少时，使用**超大规模索引**
- 当您经常在同一个查询中结合向量相似性与标量字段过滤时，使用**复合索引**
...
可以，两者都可以处理相同的文档结构。但是，它们使用不同的服务（搜索与查询）并且有不同的索引要求。
