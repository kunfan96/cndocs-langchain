...
Deep Agents Deploy 将您的智能体配置部署为 [LangSmith 部署](/langsmith/deployment)：一个水平可扩展的服务器，包含 30 多个端点，包括 MCP、A2A、Agent Protocol、Human in the Loop和记忆 API。基于开放标准构建：
...
<ResponseField name="model" type="string">
  `provider:model`
  格式的模型标识符。请参阅[支持的模型](/oss/javascript/deepagents/models#supported-models)。
</ResponseField>
...
<Note>`name` 字段是整个配置文件中唯一必需的值。其他所有内容都有默认值。</Note>
...
<ResponseField name="scope" type="string" default="thread">
  沙箱生命周期范围。`"thread"` 为每个对话创建一个沙箱。`"assistant"`
  在同一助手的所有对话之间共享一个沙箱。
</ResponseField>
...
<ResponseField
  name="subtitle"
  type="string"
  default="Your deep agent, deployed."
>
...
- [**Human in the Loop**](/oss/javascript/deepagents/human-in-the-loop)：敏感操作的审批门
...
<div className="source-links">
  ...
  <Callout icon="edit">
    [在 GitHub
    上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/deepagents/deploy.mdx)
