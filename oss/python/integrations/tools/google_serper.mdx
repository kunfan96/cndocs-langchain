---
title: "Google Serper 集成"
description: "使用 LangChain Python 集成 Google Serper 工具。"
---

本笔记本介绍如何使用 `Google Serper` 组件在网络上进行搜索。首先，您需要在 [serper.dev](https://serper.dev) 注册免费账户并获取您的 API 密钥。
...
要创建使用 Google Serper 工具的 Agent，请安装 LangGraph

```python
pip install -qU langgraph langchain-openai
```

并使用 [`create_agent`](https://reference.langchain.com/python/langchain/agents/factory/create_agent) 功能初始化一个 ReAct Agent。您还需要设置您的 OPENAI_API_KEY（访问 [platform.openai.com](https://platform.openai.com)）以访问 OpenAI 的聊天模型。
...
如果您还希望以结构化方式获取包含元数据的结果，可以使用封装器的 `results` 方法。
...
要查看所有支持的过滤器，只需前往 [Google 搜索](https://google.com)，搜索某个内容，点击"工具"，添加日期过滤器，然后查看 URL 中的 "tbs=" 参数。

## 搜索 Google 地点

我们也可以使用此封装器查询 Google 地点。例如：
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/tools/google_serper.mdx) 或[提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
