...
要开始使用，您需要一个拥有活跃 Bigtable 实例和表的 Google Cloud 项目。
...
该集成位于 `langchain-google-bigtable` 包中。以下命令还会安装 `langchain-google-vertexai` 以用于嵌入缓存示例。
...
设置您的 Google Cloud 项目，以便在此笔记本中使用其资源。

如果您不知道项目 ID，可以运行 `gcloud config list` 或查看支持页面：[查找项目 ID](https://support.google.com/googleapi/answer/7014113)。
...
### BigtableEngine
`BigtableEngine` 对象处理存储的执行上下文，特别是异步操作。建议初始化一个引擎并在多个存储中重复使用，以获得更好的性能。
...
该存储支持同步（`mset`、`mget`）和异步（`amset`、`amget`）方法。本指南使用异步版本。
...
### 获取
使用 `amget` 检索值。如果找不到某个键，则该键返回 `None`。
