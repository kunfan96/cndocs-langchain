...
我们可以将其用作一个[检索器](/oss/python/langchain/retrieval)。它将展示此集成特有的功能。在了解之后，探索[相关用例页面](/oss/python/langchain/rag)以学习如何将此向量存储用作更大链的一部分可能会很有用。
...
如果您想从单个查询中获取自动跟踪，也可以通过取消注释下方内容来设置您的 [LangSmith](/langsmith/home) API 密钥：
...
有关所有 `TavilySearchAPIRetriever` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/python/langchain-community/retrievers/tavily_search_api/TavilySearchAPIRetriever)。

---

<div className="source-links">
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/retrievers/tavily.mdx) 或[提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
