...
本指南提供了快速入门 Stripe 代理工具包的概览。

您可以在 [Stripe 的发布公告](https://stripe.dev/blog/adding-payments-to-your-agentic-workflows) 或项目的 [PyPI 页面](https://pypi.org/project/stripe-agent-toolkit/) 上阅读更多关于 `StripeAgentToolkit` 的信息。
...
这个外部管理的包托管在 `stripe-agent-toolkit` 项目之外，由 Stripe 团队管理。

您可以使用 `pip` 安装它，以及用于以下示例的 langgraph：
...
除了安装包之外，您还需要使用您的 Stripe 账户密钥来配置集成，该密钥可在您的 [Stripe 控制面板](https://dashboard.stripe.com/account/apikeys) 中找到。
