...
description: "使用 LangChain JavaScript 与 LocalFileStore 存储进行集成。"
---
...
这将帮助您开始使用 [`LocalFileStore`](/oss/javascript/integrations/stores)。有关所有 `LocalFileStore` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/javascript/langchain/storage/file_system/LocalFileStore)。
...
<Warning>
**此文件存储可以修改所提供目录及任何子文件夹中的任何文本文件。**

请确保在初始化存储时指定的路径不包含其他文件。
...
如果您想获取所有键，可以调用 `yieldKeys` 方法。或者，您可以传递一个键前缀，只获取匹配该前缀的键。
...
有关所有 `LocalFileStore` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/javascript/langchain/storage/file_system/LocalFileStore)
