...
本笔记本介绍如何使用自托管的 `SearxNG` 搜索 API 来搜索网络。
...
对于某些引擎，如果直接有可用的 `answer`，封装器将打印该答案而非完整的搜索结果列表。如果您希望获取所有结果，可以使用封装器的 `results` 方法。
...
SearxNG 支持 [135 个搜索引擎](https://docs.searxng.org/user/configured_engines.html)。您还可以使用任意命名参数来自定义 Searx 封装器，这些参数将传递给 Searx 搜索 API。在以下示例中，我们将更有趣地使用 Searx 搜索 API 的自定义搜索参数。
...
在此示例中，我们将使用 `categories` 参数查找学术论文，并通过 `time_range` 限制结果范围（并非所有引擎都支持时间范围选项）。

我们还希望以包含元数据的结构化方式获取结果。为此，我们将使用封装器的 `results` 方法。
...
在此示例中，我们在 `it` 类别下查询 `large language models`，然后筛选来自 github 的结果。
...
我们也可以直接查询来自 `github` 和其他代码托管平台的结果。
