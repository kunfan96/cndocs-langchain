...
>[TruLens](https://trulens.org) 是一个[开源](https://github.com/truera/trulens)包，为基于大语言模型（LLM）的应用程序提供检测和评估工具。
...
# 默认情况下，这将检查主应用输入和主应用输出之间的语言匹配。

# 整体问题与答案之间的问答相关性。
qa_relevance = Feedback(openai.relevance).on_input_output()
# 默认情况下，这将评估主应用输入和主应用输出的反馈。
...
为评估 LLM 设置好反馈函数后，您可以使用 TruChain 包装您的应用程序，以获得详细的跟踪、日志记录和 LLM 应用程序评估。
...
# 注意：此处指定的任何 `feedbacks` 都会在链被使用时进行评估和记录。
...
这样做将帮助您一目了然地了解 LLM 应用程序的性能。当您迭代 LLM 应用程序的新版本时，您可以比较它们在所有不同质量指标上的表现。您还可以查看记录级别的评估，并探索每条记录的链元数据。
