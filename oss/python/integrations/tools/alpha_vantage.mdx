...
在[他们的网站](https://www.alphavantage.co/support/#api-key)生成 `ALPHAVANTAGE_API_KEY`。
...
`_get_time_series_daily` 方法返回指定全球股票的日期、每日开盘价、每日最高价、每日最低价、每日收盘价和每日成交量，涵盖最近 100 个数据点。
...
`_get_top_gainers_losers` 方法返回美国市场中前 20 名涨幅最大、跌幅最大和最活跃的股票。
