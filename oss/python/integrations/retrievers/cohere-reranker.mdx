---
title: "Cohere 重排序集成"
description: "使用 LangChain Python 与 Cohere 重排序检索器集成。"
---

>[Cohere](https://cohere.ai/about) 是一家加拿大初创公司，提供自然语言处理模型，帮助公司改善人机交互。

本笔记本展示如何在检索器中使用 [Cohere 的重排序端点](https://docs.cohere.com/docs/reranking)。
...
让我们从初始化一个简单的向量存储检索器开始，并存储 2023 年国情咨文演讲（分块）。我们可以将检索器设置为检索大量（20 篇）文档。
...
现在让我们用 `ContextualCompressionRetriever` 封装我们的基础检索器。我们将添加一个 `CohereRerank`，使用 Cohere 重排序端点对返回的结果进行重排序。请注意，在 CohereRerank 中指定模型名称是必须的！
