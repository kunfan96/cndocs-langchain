...
>[Airbyte](https://github.com/airbytehq/airbyte) 是一个用于 ELT 管道的数据集成平台，可将 API、数据库和文件中的数据加载到数据仓库和数据湖中。它拥有面向数据仓库和数据库的最大规模的 ELT 连接器目录。

本文介绍如何将 Airbyte 中的任何源加载到 LangChain 文档中。
...
`AirbyteLoader` 的一个强大功能是能够从上游源加载大型文档。处理大型数据集时，默认的 `.load()` 行为可能速度较慢且占用大量内存。为避免此问题，您可以使用 `.lazy_load()` 方法以更节省内存的方式加载文档。
...
大部分配置将在 `config` 中，您可以在 [Airbyte 文档](https://docs.airbyte.com/integrations/) 中每个源的“配置字段参考”部分找到具体的配置选项。
