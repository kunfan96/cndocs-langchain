...
您可以在 [OpenAI Platform](https://platform.openai.com) 文档中找到有关 OpenAI 最新模型、成本、上下文窗口和支持输入类型的信息。

<Tip>
    **API 参考**

    有关所有功能和配置选项的详细文档，请访问 [`ChatOpenAI`](https://reference.langchain.com/python/langchain-openai/chat_models/base/ChatOpenAI) API 参考。
</Tip>

<Warning>
    **API 范围**

    `ChatOpenAI` 仅针对 [官方 OpenAI API 规范](https://github.com/openai/openai-openapi)。来自第三方提供商的非标准响应字段（例如 `reasoning_content`、`reasoning`、`reasoning_details`）**不会被提取或保留**。如果您使用的是扩展了聊天补全或响应格式的提供商，例如 [OpenRouter](https://openrouter.ai/)、[LiteLLM](https://litellm.ai/)、[vLLM](https://docs.vllm.ai/) 或 [DeepSeek](https://api-docs.deepseek.com/)，请改用提供商特定的包。有关详细信息，请参见 [聊天补全 API 兼容性](/oss/python/integrations/chat#chat-completions-api)。
</Warning>
...
有关可用模型参数的完整列表，请参见 [`ChatOpenAI`](https://reference.langchain.com/python/langchain-openai/chat_models/base/ChatOpenAI) API 参考。

<Note>
    **令牌参数弃用**

    OpenAI 在 2024 年 9 月弃用了 `max_tokens`，转而使用 `max_completion_tokens`。虽然 `max_tokens` 仍受支持以实现向后兼容，但它会在内部自动转换为 `max_completion_tokens`。
</Note>
...
OpenAI 的聊天补全 API 默认不流式传输令牌使用情况统计信息（请参阅 [OpenAI API 参考中的流式选项](https://platform.openai.com/docs/api-reference/completions/create#completions-create-stream_options)）。
...
<Info>
    **Azure OpenAI v1 API 支持**
    ...
    自 `langchain-openai>=1.0.1` 起，`ChatOpenAI` 可以直接使用新的 [v1 API](https://learn.microsoft.com/en-us/azure/ai-foundry/openai/api-version-lifecycle?tabs=python#next-generation-api-1) 与 Azure OpenAI 端点一起使用。这提供了一种统一的方式来使用 OpenAI 模型，无论托管在 OpenAI 还是 Azure 上。
...
v1 API 增加了对 [Microsoft Entra ID](https://learn.microsoft.com/en-us/azure/ai-foundry/openai/how-to/managed-identity)（原 Azure AD）身份验证的原生支持，并具有自动令牌刷新功能。将令牌提供程序可调用对象传递给 `api_key` 参数：
...
令牌提供程序是一个可调用对象，可自动检索和刷新身份验证令牌，无需手动管理令牌过期。
...
您还可以在使用异步函数时将令牌提供程序可调用对象传递给 `api_key` 参数。您必须从 `azure.identity.aio` 导入 DefaultAzureCredential：
...
<Note>
当使用异步可调用对象作为 API 密钥时，必须使用异步方法（`ainvoke`、`astream` 等）。同步方法将引发错误。
</Note>
...
OpenAI 有一个 [工具调用](https://platform.openai.com/docs/guides/function-calling)（我们在这里交替使用“工具调用”和“函数调用”）API，它允许您描述工具及其参数，并让模型返回一个 JSON 对象，其中包含要调用的工具和该工具的输入。工具调用对于构建使用工具的链和代理以及从模型获取结构化输出非常有用。
...
<Note>
    如果 `strict=True`，工具定义也将被验证，并且只接受 JSON 模式的子集。关键的是，模式不能有可选参数（那些具有默认值的参数）。

    请阅读 [完整文档](https://developers.openai.com/api/docs/guides/structured-outputs#supported-schemas) 了解支持的模式类型。
</Note>
...
请注意，AIMessage 具有 `tool_calls` 属性。这包含在标准化的 ToolCall 格式中，该格式与模型提供商无关。
...
<Accordion title="上下文无关语法">

OpenAI 支持为自定义工具输入指定 [上下文无关语法](https://platform.openai.com/docs/guides/function-calling#context-free-grammars)，格式为 `lark` 或 `regex`。有关详细信息，请参阅 [OpenAI 文档](https://platform.openai.com/docs/guides/function-calling#context-free-grammars)。`format` 参数可以传递给 `@custom_tool`，如下所示：
...
OpenAI 支持原生的 [结构化输出功能](https://platform.openai.com/docs/guides/structured-outputs)，这保证其响应符合给定的模式。

您可以在单个模型调用中访问此功能，也可以通过指定 LangChain [代理](/oss/python/langchain/agents) 的 [响应格式](/oss/python/langchain/structured-output) 来访问。请参见下面的示例。
...
使用 [`ProviderStrategy`](/oss/python/langchain/agents#providerstrategy) 指定 `response_format`，以在生成最终响应时使用 OpenAI 的结构化输出功能。
...
<Tip>
**您可以使用 `response.text` 仅将响应的文本内容恢复为字符串。例如，要流式传输响应文本：**
...
要触发图像生成，请将 `{"type": "image_generation"}` 作为另一个工具传递给模型。
...
与 [网络搜索](#web-search) 一样，响应将包含带有引文的内容块：
...
OpenAI 支持 [工具搜索](https://developers.openai.com/api/docs/guides/tools-tool-search/) 功能，允许模型根据需要搜索工具并将其加载到其上下文中。OpenAI 会将检索到的工具定义注入到活动上下文的末尾，以保留其 [缓存](#prompt-caching)。

要启用工具搜索，请使用 `@tool(extras={"defer_loading": True})` 标记工具，并将 OpenAI 的搜索工具添加到可用工具中。请参见下面的示例。

<Accordion title="服务器端工具搜索">

OpenAI 可以搜索可用工具并在同一响应中返回加载的工具（如果合适，还会返回工具调用）：
```python
...
要完全控制底层工具搜索过程，您可以在搜索工具定义中指定 `"execution": "client"`。如果模型选择搜索工具，它将在其响应中包含一个 `tool_search_call` 块。然后，您可以提供一个包含工具定义的 `tool_search_output` 块。

以下示例展示了如何使用 [自定义中间件](/oss/python/langchain/middleware/custom) 来编排此过程。该示例实现了一个定义搜索逻辑的可调用对象。中间件然后包括：
...
[`ChatOpenAI`](https://reference.langchain.com/python/langchain-openai/chat_models/base/ChatOpenAI) 支持 `"computer-use-preview"` 模型，这是一个专为内置计算机使用工具设计的专用模型。要启用，请将 [计算机使用工具](https://platform.openai.com/docs/guides/tools-computer-use) 作为另一个工具传递。

目前，计算机使用的工具输出存在于消息的 `content` 字段中。要回复计算机使用工具调用，请构造一个 [`ToolMessage`](https://reference.langchain.com/python/langchain-core/messages/tool/ToolMessage)，其 `additional_kwargs` 中包含 `{"type": "computer_call_output"}`。消息的内容将是屏幕截图。下面，我们演示一个简单的示例。
...
接下来，我们使用这些属性构造一个 [`ToolMessage`](https://reference.langchain.com/python/langchain-core/messages/tool/ToolMessage)：

1. 它具有与计算机调用的 `call_id` 匹配的 `tool_call_id`。
...
除了传递整个序列，我们还可以使用 [`previous_response_id`](#passing-previous_response_id)：
...
响应 API 支持管理 [对话状态](https://platform.openai.com/docs/guides/conversation-state?api-mode=responses)。

#### 手动管理状态

您可以手动管理状态，或使用 [LangGraph](/oss/python/langgraph/quickstart) 管理状态，就像其他聊天模型一样：
...
<Tip>
**您可以使用 [LangGraph](https://langchain-ai.github.io/langgraph/) 在各种后端（包括内存和 Postgres）中为您管理对话线程。请参阅 [本教程](/oss/python/langgraph/quickstart) 以开始使用。**

</Tip>

#### 传递 `previous_response_id`

使用响应 API 时，LangChain 消息将在其元数据中包含一个 `"id"` 字段。将此 ID 传递给后续调用将继续对话。请注意，从计费角度来看，这与手动传递消息 [等效](https://platform.openai.com/docs/guides/conversation-state?api-mode=responses#openai-apis-for-conversation-state)。
...
如果我们设置 `use_previous_response_id=True`，输入消息（直到最近的响应）将从请求有效负载中删除，并且 `previous_response_id` 将使用最近响应的 ID 设置。
...
响应 API 支持自动 [服务器端上下文压缩](https://developers.openai.com/api/docs/guides/compaction)。当对话大小达到令牌阈值时，这会减少对话大小，从而支持长时间运行的交互：
...
```
启用后，`AIMessage` 响应可能在内容中包含类型为 `"compaction"` 的块。这些应保留在对话历史中，并可以以 [常规方式](/oss/python/langchain/short-term-memory) 附加到消息序列中。最近 `compaction` 项之前的消息可以保留，也可以丢弃以提高延迟。

### 推理输出

一些 OpenAI 模型会生成单独的文本内容来说明其推理过程。有关详细信息，请参阅 OpenAI 的 [推理文档](https://platform.openai.com/docs/guides/reasoning?api-mode=responses)。

OpenAI 可以返回模型推理的摘要（尽管它不公开原始推理令牌）。要配置 [`ChatOpenAI`](https://reference.langchain.com/python/langchain-openai/chat_models/base/ChatOpenAI) 返回此摘要，请指定 `reasoning` 参数。如果设置了此参数，[`ChatOpenAI`](https://reference.langchain.com/python/langchain-openai/chat_models/base/ChatOpenAI) 将自动路由到响应 API。
...
<Tip>
    **故障排除：推理模型的空响应**
    ...
    如果您从 `gpt-5-nano` 等推理模型获得空响应，这可能是由于限制性令牌限制所致。模型使用令牌进行内部推理，可能没有剩余令牌用于最终输出。
...
OpenAI 有支持多模态输入的模型。您可以将图像、PDF 或音频传递给这些模型。有关如何在 LangChain 中执行此操作的更多信息，请访问 [多模态输入](/oss/python/langchain/messages#multimodal) 文档。

您可以在 [OpenAI 的文档](https://platform.openai.com/docs/models) 中查看支持不同模态的模型列表。

对于所有模态，LangChain 都支持其跨提供商标准以及 OpenAI 的原生内容块格式。

要将多模态数据传递给 [`ChatOpenAI`](https://reference.langchain.com/python/langchain-openai/chat_models/base/ChatOpenAI)，请创建一个包含数据的 [内容块](/oss/python/langchain/messages/)，并将其合并到消息中，例如：
...
请参阅 [多模态消息使用指南](/oss/python/langchain/messages#multimodal) 中的示例。
...
阅读有关 [OpenAI 多模态消息的文件名](/oss/python/langchain/messages#multimodal) 的更多信息。

请参阅 [PDF 文档使用指南](/oss/python/langchain/messages#multimodal) 中的示例。
...
请参阅 [音频使用指南](/oss/python/langchain/messages#multimodal) 中的示例。
...
一些 OpenAI 模型（例如其 `gpt-4o` 和 `gpt-4o-mini` 系列）支持 [预测输出](https://platform.openai.com/docs/guides/latency-optimization#use-predicted-outputs)，这允许您提前传入 LLM 预期输出的已知部分以减少延迟。这对于编辑文本或代码等情况非常有用，因为模型输出中只有一小部分会发生变化。
...
<Note>
预测作为额外令牌计费，并可能增加您的使用量和成本，以换取降低的延迟。
</Note>
...
...格式将是传入 `model_kwargs['audio']['format']` 的格式。

我们还可以在 OpenAI `expires_at` 到达之前，将此包含音频数据的消息作为消息历史的一部分传回模型。

<Note>
**输出音频存储在 `AIMessage.additional_kwargs` 的 `audio` 键下，但输入内容块在 `HumanMessage.content` 列表中使用 `input_audio` 类型和键进行键入。**
...
OpenAI 的 [提示缓存](https://platform.openai.com/docs/guides/prompt-caching) 功能会自动缓存超过 1024 个令牌的提示，以降低成本并提高响应时间。此功能适用于所有最新模型（`gpt-4o` 及更新版本）。
...
OpenAI 提供多种 [服务层级](https://platform.openai.com/docs/guides/flex-processing)。“灵活”层级为请求提供更便宜的价格，但代价是响应可能需要更长时间，并且资源可能并不总是可用。这种方法最适合非关键任务，包括模型测试、数据增强或可以异步运行的作业。
...
请注意，这是一个测试版功能，仅适用于部分模型。有关更多详细信息，请参阅 OpenAI [文档](https://platform.openai.com/docs/guides/flex-processing)。
