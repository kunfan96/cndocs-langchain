...
您可以使用[中间件](/oss/javascript/langchain/middleware)在战略点拦截执行来实现防护栏——在代理开始前、完成后，或围绕模型和工具调用。
...
LangChain提供了内置防护栏（例如，[PII检测](#pii-detection)、[Human in the Loop](#human-in-the-loop)）以及一个灵活的中间件系统，用于使用任一方法构建自定义防护栏。
...
PII检测中间件对于以下情况很有帮助：具有合规要求的医疗保健和金融应用程序、需要清理日志的客户服务代理，以及通常任何处理敏感用户数据的应用程序。
...
Human in the Loop中间件对于以下情况很有帮助：金融交易和转账、删除或修改生产数据、向外部方发送通信，以及任何具有重大业务影响的操作。
...
<Tip>
  有关实施审批工作流的完整详情，请参阅[Human in the
...
- [Human in the Loop](/oss/javascript/langchain/human-in-the-loop) - 为敏感操作添加人工审查
