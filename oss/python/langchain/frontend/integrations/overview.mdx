...
<CardGroup cols={2}>
  ...
  <Card title="AI Elements" icon="package" href="/oss/python/langchain/frontend/integrations/ai-elements">
    用于 AI 聊天的可组合 shadcn/ui 组件。直接使用 `Conversation`、`Message`、`Tool` 和 `Reasoning` 并将它们直接连接到 `stream.messages`。
  </Card>
...
每个库都适合略有不同的集成模型。选择取决于您要构建的 UI 类型：
...
所有四个库都能很好地与 LangChain 代理配合使用，后三个库还直接连接到 [`useStream`](https://reference.langchain.com/javascript/langchain-react/index/useStream)。当您想要更丰富的运行时层和可以与 [LangGraph](/oss/python/langgraph/overview) 部署并存的专用端点时，CopilotKit 特别有用。
