---
title: "Grobid 集成"
description: "使用 LangChain Python 与 Grobid 文档加载器集成。"
---

GROBID 是一个用于提取、解析和重构原始文档的机器学习库。

它被设计并期望用于解析学术论文，在这方面表现尤为出色。请注意：如果提供给 Grobid 的文章是大型文档（例如学位论文）且超过一定数量的元素，它们可能无法被处理。

此加载器使用 Grobid 将 PDF 解析为 `Documents`，这些文档保留了与文本部分相关的元数据。

---
最佳方法是通过 Docker 安装 Grobid，请参阅 [grobid.readthedocs.io/en/latest/Grobid-docker/](https://grobid.readthedocs.io/en/latest/Grobid-docker/)。
有关其他说明，请参阅 [Grobid 提供程序页面](/oss/python/integrations/providers/grobid)。

一旦 Grobid 启动并运行，您就可以按照以下描述进行交互。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/document_loaders/grobid.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>