...
<Tip>
了解如何[将你的 agents 部署到 LangSmith](/oss/javascript/langchain/deploy) 以获得生产就绪的托管，包括内置的可观测性、身份验证和扩展。
</Tip>
...
当与通过 [`createAgent`](https://reference.langchain.com/javascript/functions/langchain.index.createAgent.html) 创建的 agents 或使用 [`StateGraph`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.StateGraph.html) 创建的 graphs 一起使用时，`useStream` hook 支持完整的类型推断。将 `typeof agent` 或 `typeof graph` 作为类型参数传递以自动推断工具调用类型。

### 使用 `createAgent`

使用 [`createAgent`](https://reference.langchain.com/javascript/functions/langchain.index.createAgent.html) 时，工具调用类型会自动从你注册到 agent 的工具中推断：
...
对于自定义 [`StateGraph`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.StateGraph.html) 应用，状态类型从 graph 的 annotation 推断：
...
当 agent 需要人工批准工具执行时处理中断。在[如何处理中断](/oss/javascript/langgraph/interrupts#pause-using-interrupt)指南中了解更多。
...
- [Agent Chat UI](/oss/javascript/langchain/ui) — LangGraph agents 的预构建聊天界面
- [Human-in-the-loop](/oss/javascript/langchain/human-in-the-loop) — 配置人工审核中断
- [Multi-agent 系统](/oss/javascript/langchain/multi-agent) — 使用多个 LLMs 构建 agents
