...
description: "使用 LangChain Python 集成 ChatFriendli 聊天模型。"
---

> [Friendli](https://friendli.ai/) 通过可扩展、高效的部署选项，增强 AI 应用性能并优化成本节约，专为高需求 AI 工作负载量身定制。

本教程将指导您使用 LangChain 集成 `ChatFriendli` 以构建聊天应用。`ChatFriendli` 提供了一种灵活的方式来生成对话式 AI 响应，支持同步和异步调用。
...
您可以初始化一个 Friendli 聊天模型，并选择要使用的模型。默认模型是 `mixtral-8x7b-instruct-v0-1`。您可以在 [docs.friendli.ai](https://docs.periflow.ai/guides/serverless_endpoints/pricing#text-generation-models) 查看可用模型。
...
您也可以使用 `invoke`、`batch`、`generate` 和 `stream` 功能。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/chat/friendli.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>