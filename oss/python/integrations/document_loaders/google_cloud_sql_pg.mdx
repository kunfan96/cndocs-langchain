...
> [Cloud SQL for PostgreSQL](https://cloud.google.com/sql/docs/postgres) 是一项完全托管的数据库服务，可帮助您在 Google Cloud Platform 上设置、维护、管理和管理 PostgreSQL 关系数据库。利用 Cloud SQL for PostgreSQL 的 LangChain 集成，扩展您的数据库应用程序以构建 AI 驱动的体验。
...
建立 PostgreSQL 作为文档加载器的要求和参数之一是 `PostgresEngine` 对象。`PostgresEngine` 配置了到您的 Cloud SQL for PostgreSQL 数据库的连接池，从而实现应用程序的成功连接并遵循行业最佳实践。
...
默认情况下，将使用 [IAM 数据库身份验证](https://cloud.google.com/sql/docs/postgres/iam-authentication) 作为数据库身份验证方法。此库使用来自环境的 [应用默认凭据 (ADC)](https://cloud.google.com/docs/authentication/application-default-credentials) 所属的 IAM 主体。

或者，也可以使用 [内置数据库身份验证](https://cloud.google.com/sql/docs/postgres/users)（使用用户名和密码访问 Cloud SQL 数据库）。只需向 `PostgresEngine.from_instance()` 提供可选的 `user` 和 `password` 参数：
...
加载器返回一个文档列表，每行一个文档，页面内容采用指定的字符串格式，例如文本（空格分隔的连接）、JSON、YAML、CSV 等。JSON 和 YAML 格式包含标题，而文本和 CSV 不包含字段标题。
