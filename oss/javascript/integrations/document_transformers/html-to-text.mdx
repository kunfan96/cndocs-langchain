...
description: "使用 LangChain JavaScript 与 Html-to-text 文档转换器集成。"
---

在摄取 HTML 文档以供后续检索时，我们通常只关心网页的实际内容，而非其语义结构。使用 HtmlToTextTransformer 从文档中剥离 HTML 标签，可以生成内容更丰富的块，从而使检索更加有效。
...
```
虽然转换器本身不需要，但以下使用示例需要 [`cheerio`](https://www.npmjs.com/package/cheerio) 来进行网页抓取：
...
以下示例抓取一个 Hacker News 帖子，根据 HTML 标签进行分割，以便根据标签的语义信息对块进行分组，然后从各个块中提取内容：
...
你可以向转换器传递 [`html-to-text` 包接受的任何[参数](https://www.npmjs.com/package/html-to-text)来自定义其工作方式。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/javascript/integrations/document_transformers/html-to-text.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>