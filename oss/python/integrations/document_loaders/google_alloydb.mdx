...
description: "使用 LangChain Python 与 Google AlloyDB for PostgreSQL 文档加载器集成。"
---
...
将 AlloyDB 设置为向量存储的要求和参数之一是 `AlloyDBEngine` 对象。`AlloyDBEngine` 配置到 AlloyDB 数据库的连接池，使您的应用程序能够成功连接，并遵循行业最佳实践。
...
或者，也可以使用使用用户名和密码访问 AlloyDB 数据库的[内置数据库身份验证](https://cloud.google.com/alloydb/docs/database-users/about)。只需向 `AlloyDBEngine.from_instance()` 提供可选的 `user` 和 `password` 参数：
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/document_loaders/google_alloydb.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>