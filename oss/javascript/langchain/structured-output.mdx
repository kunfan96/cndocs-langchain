...
<Tip>
本页介绍使用 `createAgent` 的代理的结构化输出。若要直接在模型上使用结构化输出（不通过代理），请参阅 [模型 - 结构化输出](/oss/javascript/langchain/models#structured-output)。
</Tip>
...
控制代理返回结构化数据的方式。您可以提供 Zod 模式、任何 [Standard Schema](https://standardschema.dev/) 兼容的模式或 JSON Schema 对象。默认情况下，代理使用工具调用策略，其中输出是通过额外的工具调用创建的。某些模型支持原生结构化输出，在这种情况下，代理将使用该策略。
...
<Tip>
    如果使用 `langchain>=1.1`，对原生结构化输出功能的支持会从模型的[配置文件数据](/oss/javascript/langchain/models#model-profiles)动态读取。如果数据不可用，请使用其他条件或手动指定：
    ```typescript
...
<Note>
    如果提供商原生支持您选择的模型的结构化输出，那么编写 `responseFormat: contactInfoSchema` 与 `responseFormat: providerStrategy(contactInfoSchema)` 在功能上是等效的。
