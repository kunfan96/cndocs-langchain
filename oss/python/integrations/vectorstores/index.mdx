...
title: "向量存储集成"
sidebarTitle: "向量存储"
description: "使用 LangChain Python 与向量存储进行集成。"
---
...
添加 [`Document`](https://reference.langchain.com/python/langchain-core/documents/base/Document) 对象（包含 `page_content` 和可选元数据），如下所示：
...
使用 `similarity_search` 发出语义查询，该查询返回最接近的嵌入文档：
...
Elasticsearch 将在 `http://localhost:9200` 处可用。`elastic` 用户的密码和 API 密钥存储在 `elastic-start-local` 文件夹的 `.env` 文件中。
...
| 向量存储 | 按 ID 删除 | 过滤 | 按向量搜索 | 带分数搜索 | 异步 | 通过标准测试 | 多租户 | 在添加文档中包含 ID |
|------------|-------------|-----------|-----------------|------------------|--------|---------------------|---------------|-------------------|
