---
title: 图执行
description: 可视化多步骤图流水线，包含每个节点的状态和流式内容
---

LangGraph 代理并非黑盒。每个图都由**命名节点**组成，这些节点按顺序或并行执行：分类、研究、分析、合成。图执行卡片通过为每个节点渲染一个卡片，使其流水线可见，显示其状态，实时流式传输其内容，并跟踪整个工作流的完成情况。用户可以准确看到代理正在做什么、当前处于哪个步骤以及每个步骤的输出。
...
LangGraph 图定义了一系列节点，每个节点负责特定任务。例如，一个研究流水线可能包含：
...
每个节点将其输出写入图状态中的特定键。通过将这些节点名称和状态键映射到 UI 组件，您可以创建整个流水线的可视化表示。
...
当代理流式传输时，每条消息都附带元数据，标识生成它的图节点。使用 `getMessagesMetadata` 提取 `langgraph_node` 值，并将令牌路由到正确的卡片：
...
顶部的水平进度条为用户提供整个流水线的鸟瞰图。每个步骤都是一个带标签的段，随着节点完成而填充：
...
模式是：对于实时更新显示流式内容，一旦节点完成，则回退到已提交的状态值。
...
并非所有图都有固定的节点集。某些流水线会根据输入添加或跳过节点。通过检查哪些状态键实际具有值来处理此问题：
...
<Info>
如果您的图具有条件分支（例如，对于简单的事实查询跳过“研究”），则跳过的节点将永远不会出现在流式内容或状态值中。您的流水线进度条应通过淡化或隐藏跳过的步骤来反映这一点。
</Info>
...
- **显示预计时间**。如果您有每个节点耗时的历史数据，请显示时间估计以设定用户期望。
- **添加全局进度指示器**。在流水线视图顶部，用整体进度条（例如，“步骤 2/4”）补充每个节点的卡片。
- **按节点处理错误**。如果某个节点失败，请在其卡片中显示错误，而不会折叠整个流水线。其他节点仍可能成功完成。
