...
> [Firestore](https://cloud.google.com/firestore) 是一个无服务器的面向文档的数据库，可扩展以满足任何需求。扩展您的数据库应用，利用 Firestore 的 LangChain 集成构建 AI 驱动的体验。
...
1. `source` - Query、CollectionGroup、DocumentReference 的实例，或指向 Firestore 集合的单个 `\` 分隔路径。
