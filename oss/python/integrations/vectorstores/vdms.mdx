---
title: "英特尔视觉数据管理系统 (VDMS) 集成"
description: "使用 LangChain Python 与英特尔视觉数据管理系统 (VDMS) 向量存储进行集成。"
---

本笔记本介绍如何开始使用 VDMS 作为向量存储。

>英特尔的 [视觉数据管理系统 (VDMS)](https://github.com/IntelLabs/vdms) 是一种用于高效访问大型“视觉”数据的存储解决方案，旨在通过搜索存储为图的视觉元数据来实现云规模，并支持机器友好的视觉数据增强以实现更快访问。VDMS 根据 MIT 许可证授权。有关 `VDMS` 的更多信息，请访问 [VDMS 维基](https://github.com/IntelLabs/vdms/wiki)。
...
为简单起见，本笔记本将在本地主机上使用端口 55555 部署 VDMS 服务器。
...
如果多次提供相同的 ID，`add_documents` 不会检查 ID 是否唯一。因此，请使用 `upsert` 在添加前删除现有的 ID 条目。
...
之前，我们根据其 `id` 删除文档。这里，由于未提供 ID，所有文档都将被删除。
...
* [使用 VDMS 的多模态 RAG](https://github.com/langchain-ai/langchain/blob/v0.3/cookbook/multi_modal_RAG_vdms.ipynb)
* [使用 VDMS 的视觉 RAG](https://github.com/langchain-ai/langchain/blob/v0.3/cookbook/visual_RAG_vdms.ipynb)
...
在这里，我们使用 Faiss IndexHNSWFlat 索引和 L2 作为相似性搜索的距离度量将文档添加到 VDMS。我们搜索与查询相关的三个文档 (`k=3`)，并返回分数以及文档。
...
我们使用 Faiss IndexIVFFlat 索引和 IP 作为相似性搜索的距离度量将文档添加到 VDMS。我们搜索与查询相关的三个文档 (`k=3`)，并返回分数以及文档。
...
在本节中，我们使用过滤器识别近邻组 (FLINNG) 索引和 IP 作为相似性搜索的距离度量将文档添加到 VDMS。我们搜索与查询相关的三个文档 (`k=3`)，并返回分数以及文档。
...
***注意：*** `id` 作为附加元数据生成为整数，而 `langchain_id`（内部 ID）是每个条目的唯一字符串。
