---
title: "Llmrails 集成"
description: "使用 LangChain Python 与 Llmrails 向量存储进行集成。"
---
...
此笔记本展示了如何使用与 `LLMRails` 的 LangChain 集成相关的功能。
请注意，与本类别中的许多其他集成不同，LLMRails 提供了一个端到端的托管服务，用于检索增强生成，其中包括：

1. 一种从文档文件中提取文本并将其分块为句子的方法。
2. 其自己的嵌入模型和向量存储 - 每个文本段被编码为向量嵌入并存储在 LLMRails 内部向量存储中。
3. 一个查询服务，自动将查询编码为嵌入，并检索最相关的文本段（包括支持[混合搜索](https://docs.llmrails.com/datastores/search)）。
...
1. 如果您还没有 LLMRails 帐户，请[注册](https://console.llmrails.com/signup)。
2. 接下来，您需要创建 API 密钥以访问 API。在语料库视图中单击 **"API Keys"** 选项卡，然后单击 **"Create API Key"** 按钮。为您的密钥命名。单击 "Create key"，您现在就拥有了一个活动的 API 密钥。请对此密钥保密。
...
1. 在创建 LLMRails 向量存储对象时将其作为参数提供：
...
要向您的数据存储添加文本，首先您必须转到 [Datastores](https://console.llmrails.com/datastores) 页面并创建一个。单击创建数据存储按钮，为您的数据存储选择一个名称和嵌入模型。然后从新创建的数据存储设置中获取您的数据存储 ID。
