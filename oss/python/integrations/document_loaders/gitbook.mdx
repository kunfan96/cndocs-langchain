---
title: "Gitbook 集成"
description: "使用 LangChain Python 集成 Gitbook 文档加载器。"
---

>[GitBook](https://docs.gitbook.com/) 是一个现代化的文档平台，团队可以在此记录从产品到内部知识库和 API 的一切内容。

本 notebook 展示了如何从任意 `GitBook` 中拉取页面数据。
...
若要实现此功能，需要使用根路径（本例中为 `https://docs.gitbook.com`）初始化 GitbookLoader，并将 `load_all_paths` 设置为 `True`。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/document_loaders/gitbook.mdx) 或 [提交 Issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
