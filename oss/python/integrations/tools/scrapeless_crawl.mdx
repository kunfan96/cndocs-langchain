---
title: "Scrapeless 爬取集成"
description: "使用 LangChain Python 与 Scrapeless 爬取工具集成。"
---

[**Scrapeless**](https://www.scrapeless.com/) 提供灵活且功能丰富的数据采集服务，具有广泛的参数自定义和多格式导出支持。这些功能使 LangChain 能够更有效地集成和利用外部数据。核心功能模块包括：

**DeepSerp**

- **Google 搜索**：支持全面提取 Google SERP 数据，涵盖所有结果类型。
  - 支持选择本地化 Google 域名（例如 `google.com`、`google.ad`）以检索特定区域的搜索结果。
  - 支持分页以检索第一页之后的结果。
...
- **Google 趋势**：从 Google 检索关键词趋势数据，包括随时间变化的受欢迎程度、区域兴趣和相关搜索。
  ...
  - 允许通过特定 Google 属性（网络、YouTube、新闻、购物）进行过滤，以进行特定来源的趋势分析。
...
ScrapelessCrawlerScrapeTool 允许您使用 Scrapeless 的 Crawler Scrape API 从一个或多个网站抓取内容。您可以提取主要内容、控制格式、标头、等待时间和输出类型。
...
- `formats`（可选，List[str]）：定义抓取输出的格式。默认为 `['markdown']`。选项包括：
  ...
  - `'screenshot'`
  - `'html'`
- `only_main_content`（可选，bool）：是否仅返回主要页面内容，排除页眉、导航、页脚等。默认为 True。
...
- `headers`（可选，Dict[str, str]）：随请求发送的自定义标头（例如用于 cookie 或用户代理）。默认为 None。
- `wait_for`（可选，int）：抓取前等待的时间（毫秒）。用于让页面有时间完全加载。默认为 `0`。
- `timeout`（可选，int）：请求超时时间（毫秒）。默认为 `30000`。

### ScrapelessCrawlerCrawlTool

ScrapelessCrawlerCrawlTool 允许您使用 Scrapeless 的 Crawler Crawl API 从基础 URL 开始爬取网站。它支持高级 URL 过滤、爬取深度控制、内容抓取选项、标头自定义等。
...
- `include_paths`（可选，List[str]）：URL 路径名正则表达式模式，用于包含匹配的 URL。仅匹配这些模式的 URL 会被包含。例如，设置 `["blog/.*"]` 将仅包含 `/blog/` 路径下的 URL。默认为 None。
...
- `ignore_sitemap`（可选，bool）：是否在爬取期间忽略网站站点地图。默认为 False。
- `ignore_query_params`（可选，bool）：是否忽略查询参数差异以避免重新抓取相似 URL。默认为 False。
- `deduplicate_similar_urls`（可选，bool）：是否对相似 URL 进行去重。默认为 True。
- `regex_on_full_url`（可选，bool）：是否对完整 URL 应用正则表达式匹配，而不仅仅是路径。默认为 True。
...
- `headers`（可选，Dict[str, str]）：随请求发送的自定义 HTTP 标头，例如 cookie 或用户代理字符串。默认为 None。
