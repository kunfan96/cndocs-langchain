...
description: "使用 LangChain JavaScript 与 ChatCloudflareWorkersAI 聊天模型集成。"
---

[Workers AI](https://developers.cloudflare.com/workers-ai/) 允许你从自己的代码中，在 Cloudflare 网络上运行机器学习模型。

这将帮助你开始使用 `ChatCloudflareWorkersAI` [聊天模型](/oss/javascript/langchain/models)。有关所有 `ChatCloudflareWorkersAI` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-cloudflare/ChatCloudflareWorkersAI)。
...
要访问 Cloudflare Workers AI 模型，你需要创建一个 Cloudflare 帐户，获取 API 密钥，并安装 `@langchain/cloudflare` 集成包。

### 凭证

前往[此页面](https://developers.cloudflare.com/workers-ai/)注册 Cloudflare 并生成 API 密钥。完成此操作后，记下你的 `CLOUDFLARE_ACCOUNT_ID` 和 `CLOUDFLARE_API_TOKEN`。

目前尚不支持在 Cloudflare Worker 中传递绑定。
