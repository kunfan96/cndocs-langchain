---
title: "LLMRails 集成"
description: "使用 LangChain Python 与 LLMRails 向量存储集成。"
---

>[LLMRails](https://www.llmrails.com/) 是一个用于构建生成式 AI 应用的 API 平台。它提供了一个易于使用的 API，用于文档索引和查询，该服务由 LLMRails 管理，并针对性能和准确性进行了优化。
...
本笔记本展示了如何使用与 `LLMRails` 和 LangChain 集成相关的功能。
请注意，与该类别中的许多其他集成不同，LLMRails 提供了一个端到端的托管检索增强生成服务，其中包括：

1. 一种从文档文件中提取文本并将其分块为句子的方法。
2. 其自己的嵌入模型和向量存储 - 每个文本段都被编码为向量嵌入并存储在 LLMRails 内部向量存储中。
3. 一个查询服务，可自动将查询编码为嵌入，并检索最相关的文本段（包括支持[混合搜索](https://docs.llmrails.com/datastores/search)）。

所有这些功能都在此 LangChain 集成中得到支持。

# 设置

你需要一个 LLMRails 帐户才能将 LLMRails 与 LangChain 一起使用。要开始，请按照以下步骤操作：

1. 如果你还没有 LLMRails 帐户，请[注册](https://console.llmrails.com/signup)一个。
2. 接下来，你需要创建 API 密钥以访问 API。在语料库视图中点击 **"API Keys"** 选项卡，然后点击 **"Create API Key"** 按钮。为你的密钥命名。点击 "Create key"，你现在就拥有了一个有效的 API 密钥。请保密此密钥。

要将 LangChain 与 LLMRails 一起使用，你需要此值：api_key。
你可以通过两种方式将这些值提供给 LangChain：

1. 在你的环境中包含这两个变量：`LLM_RAILS_API_KEY`、`LLM_RAILS_DATASTORE_ID`。

> 例如，你可以使用 os.environ 和 getpass 如下设置这些变量：
...
要将文本添加到你的数据存储，首先你需要转到 [Datastores](https://console.llmrails.com/datastores) 页面并创建一个。点击 Create Datastore 按钮，为你的数据存储选择一个名称和嵌入模型。然后从新创建的数据存储设置中获取你的数据存储 ID。
...
有时我们可能想要执行搜索，但同时获得相关性分数以了解特定结果的好坏。
...
LLMRails 与所有其他 LangChain 向量存储一样，最常被用作 LangChain 检索器：
