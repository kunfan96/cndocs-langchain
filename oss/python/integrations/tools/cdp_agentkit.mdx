...
description: "使用 LangChain Python 集成 Cdp agentkit 工具包。"
---

`CDP Agentkit` 工具包包含使 LLM 代理能够与 [Coinbase 开发者平台](https://docs.cdp.coinbase.com/) 交互的工具。该工具包提供了对 CDP SDK 的封装，允许代理执行链上操作，如转账、交易和智能合约交互。
...
9. **register_basename** - 为钱包注册一个 basename

我们鼓励您添加自己的工具，可以使用 CDP 和 web2 API，以创建一个满足您需求的代理。
...
要使用此工具包，您必须首先设置以下环境变量以访问 [CDP API](https://docs.cdp.coinbase.com/mpc-wallet/docs/quickstart) 来创建钱包并进行链上交互。您可以在 [CDP Portal](https://cdp.coinbase.com/) 上免费注册获取 API 密钥：
