...
>[LLMLingua](https://github.com/microsoft/LLMLingua) 利用一个紧凑、训练良好的语言模型（例如 GPT2-small、LLaMA-7B）来识别和移除提示中的非必要令牌。这种方法能够实现大语言模型（LLMs）的高效推理，在性能损失极小的情况下实现高达 20 倍的压缩。
...
现在让我们用 `ContextualCompressionRetriever` 包装我们的基础检索器，使用 `LLMLinguaCompressor` 作为压缩器。
