---
title: "LanceDB 集成"
description: "使用 LangChain Python 集成 LanceDB 向量存储。"
---

>[LanceDB](https://lancedb.com/) 是一款基于 Lance 数据格式构建的开源向量搜索数据库，具备持久存储能力，大大简化了嵌入的检索、过滤和管理。完全开源。

本 notebook 展示了如何使用基于 Lance 数据格式的 `LanceDB` 向量数据库相关功能。
...
您也可以在 `from_documents()` 类方法中传入 `region`、`api_key`、`uri`
...
此外，您还可以将表加载为 DataFrame 或保存为 CSV 文件进行探索：
