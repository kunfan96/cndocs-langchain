...
>[Azure Cosmos DB for Apache Gremlin](https://learn.microsoft.com/en-us/azure/cosmos-db/gremlin/introduction) 是一项图数据库服务，可用于存储包含数十亿顶点和边的大型图。您可以以毫秒级延迟查询图，并轻松演进图结构。
>
...
本笔记本展示了如何使用 LLM 为图数据库提供自然语言接口，您可以使用 `Gremlin` 查询语言进行查询。
...
您需要一个 Azure CosmosDB 图数据库实例。一个选择是在 Azure 中创建一个[免费的 CosmosDB 图数据库实例](https://learn.microsoft.com/en-us/azure/cosmos-db/free-tier)。

创建您的 Cosmos DB 帐户和图时，请使用 `/type` 作为分区键。
...
对于 Cosmos，使用自然 ID 是有意义的，因为它们在图资源管理器中可见。
...
```python
# 底层的 python-gremlin 在笔记本中运行时存在问题
# 以下行是解决该问题的变通方法
nest_asyncio.apply()
