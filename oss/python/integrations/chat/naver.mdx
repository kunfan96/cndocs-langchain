---
title: "ChatClovaX 集成"
description: "使用 LangChain Python 与 ChatClovaX 聊天模型集成。"
---

本指南提供了快速入门概览，介绍如何通过 CLOVA Studio 使用 Naver 的 HyperCLOVA X [聊天模型](https://python.langchain.com/docs/concepts/chat_models)。有关所有 `ChatClovaX` 功能和配置的详细文档，请参阅 [API 参考](https://guide.ncloud-docs.com/docs/clovastudio-dev-langchain)。

[CLOVA Studio](http://clovastudio.ncloud.com/) 提供多种聊天模型。您可以在 CLOVA Studio 指南[文档](https://guide.ncloud-docs.com/docs/clovastudio-model)中找到有关最新模型的信息，包括其成本、上下文窗口和支持的输入类型。
...
3. 为要使用的模型创建 CLOVA Studio 测试应用或服务应用（参见 [CLOVA Studio 测试应用设置](https://guide.ncloud-docs.com/docs/clovastudio-playground-testapp)。）
4. 发布测试或服务 API 密钥（参见 [Naver API 密钥文档](https://api.ncloud-docs.com/docs/ai-naver-clovastudio-summary#API%ED%82%A4)。）
...
除了下面的 `invoke`，`ChatClovaX` 还支持批处理、流式传输及其异步功能。
...
CLOVA Studio 支持工具调用（也称为"[函数调用](https://api.ncloud-docs.com/docs/clovastudio-chatcompletionsv3-fc)"），允许您描述工具及其参数，并让模型返回一个 JSON 对象，其中包含要调用的工具及其输入。这对于构建使用工具的链和代理，以及更普遍地从模型获取结构化输出非常有用。

**注意**：您应将 `max_tokens` 设置为大于 1024，以使用 CLOVA Studio 中的工具调用功能。

### ChatClovaX.bind_tools()

通过 `ChatClovaX.bind_tools`，我们可以轻松地将 Pydantic 类、字典模式、LangChain 工具甚至函数作为工具传递给模型。在底层，这些被转换为 OpenAI 兼容的工具模式，如下所示：
...
请注意，`AIMessage` 有一个 [`tool_calls`](https://reference.langchain.com/python/langchain/messages/#langchain.messages.AIMessage.tool_calls) 属性。它包含标准化的 `ToolCall` 格式，与模型提供者无关。
...
**注意**：结构化输出需要禁用思考模式。将 `thinking.effort` 设置为 `none`。
...
对于支持的模型，当启用[思考](https://api.ncloud-docs.com/docs/clovastudio-chatcompletionsv3-thinking)功能（默认启用）时，它将输出导致最终答案的逐步推理过程。
