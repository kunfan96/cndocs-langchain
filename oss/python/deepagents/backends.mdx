...
**建议的保护措施：**
1. 启用[人机协作 (HITL) 中间件](/oss/python/deepagents/human-in-the-loop)来审查敏感操作。
...
**工作原理：**
- 将文件存储在运行时提供的 LangGraph [`BaseStore`](https://reference.langchain.com/python/langgraph/store/#langgraph.store.base.BaseStore) 中，实现跨线程的持久存储。

**最适合：**
- 当你已经使用配置好的 LangGraph store 运行时（例如，Redis、Postgres 或 [`BaseStore`](https://reference.langchain.com/python/langgraph/store/#langgraph.store.base.BaseStore) 背后的云实现）。
