---
title: "ManticoreSearch VectorStore 集成"
description: "使用 LangChain Python 与 ManticoreSearch VectorStore 集成。"
---

[ManticoreSearch](https://manticoresearch.com/) 是一个开源搜索引擎，提供快速、可扩展且用户友好的功能。它起源于 [Sphinx Search](http://sphinxsearch.com/) 的一个分支，现已发展并融入了现代搜索引擎的特性和改进。ManticoreSearch 以其强大的性能和易于集成到各种应用程序中的特点而脱颖而出。

ManticoreSearch 最近引入了[向量搜索功能](https://manual.manticoresearch.com/dev/Searching/KNN)，从搜索引擎版本 6.2 开始，并且仅在安装了 [manticore-columnar-lib](https://github.com/manticoresoftware/columnar) 包的情况下可用。这一功能是一个重大的进步，允许执行基于向量相似性的搜索。

目前，向量搜索功能仅在搜索引擎的开发（dev）版本中可用。因此，必须使用开发版的 [manticoresearch-dev](https://pypi.org/project/manticoresearch-dev/) Python 客户端来有效利用此功能。
