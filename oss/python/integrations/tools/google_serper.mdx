...
description: "使用 LangChain Python 与 Google Serper 工具集成。"
---
...
要创建一个使用 Google Serper 工具的代理，请安装 LangGraph
...
如果您还想以结构化方式获取结果，包括元数据。为此，我们将使用包装器的 `results` 方法。
...
要获取所有支持的过滤器，只需访问 [Google Search](https://google.com)，搜索某些内容，点击“工具”，添加您的日期过滤器，并检查 URL 中的“tbs=”。

## 搜索 Google 地点

我们还可以使用此包装器查询 Google 地点。例如：
