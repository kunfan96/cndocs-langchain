---
title: "MyScale 集成"
description: "使用 LangChain Python 与 MyScale 向量存储集成。"
---
...
1. 环境变量
    ...
    `MyScaleSettings` 下的每个属性都可以使用前缀 `MYSCALE_` 进行设置，并且不区分大小写。
...
您可以直接访问 MyScale 的 SQL WHERE 子句。您可以按照标准 SQL 编写 `WHERE` 子句。
...
您可以使用 `.drop()` 方法删除整个表，或使用 `.delete()` 方法部分删除数据。
