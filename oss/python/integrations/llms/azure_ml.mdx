---
title: "Azure ML 集成"
description: "使用 LangChain Python 集成 Azure ML LLM。"
---

[Azure ML](https://azure.microsoft.com/en-us/products/machine-learning/) 是一个用于构建、训练和部署机器学习模型的平台。用户可以在模型目录中探索可部署的模型类型，该目录提供来自不同提供商的基础模型和通用模型。

本 notebook 介绍如何使用托管在 `Azure ML Online Endpoint` 上的 LLM。
...
您需要[在 Azure ML 上部署模型](https://learn.microsoft.com/en-us/azure/machine-learning/how-to-use-foundation-models?view=azureml-api-2#deploying-foundation-models-to-endpoints-for-inferencing)或[部署到 Azure AI Foundry（原 Azure AI Studio）](https://learn.microsoft.com/en-us/azure/ai-studio/how-to/deploy-models-open)，并获取以下参数：

* `endpoint_url`：端点提供的 REST 端点 URL。
* `endpoint_api_type`：将模型部署到**专用端点**（托管基础设施）时，使用 `endpoint_type='dedicated'`；使用**按需付费**方式（模型即服务）部署时，使用 `endpoint_type='serverless'`。
* `endpoint_api_key`：端点提供的 API 密钥。
* `deployment_name`：（可选）使用端点的模型部署名称。

## 内容格式化器

`content_formatter` 参数是一个处理器类，用于将 AzureML 端点的请求和响应转换为所需格式。由于模型目录中的模型种类繁多，各模型处理数据的方式可能不同，因此提供了 `ContentFormatterBase` 类，允许用户按需转换数据。以下是已提供的内容格式化器：

* `GPT2ContentFormatter`：格式化 GPT2 的请求和响应数据
* `DollyContentFormatter`：格式化 Dolly-v2 的请求和响应数据
* `HFContentFormatter`：格式化文本生成类 Hugging Face 模型的请求和响应数据
* `CustomOpenAIContentFormatter`：格式化遵循 OpenAI API 兼容方案的模型（如 LLaMa2）的请求和响应数据

*注意：`OSSContentFormatter` 正在被弃用，由 `GPT2ContentFormatter` 替代。两者逻辑相同，但 `GPT2ContentFormatter` 是更合适的名称。由于更改向后兼容，您仍可继续使用 `OSSContentFormatter`。*

## 示例

### 示例：使用实时端点进行 LlaMa 2 文本补全
