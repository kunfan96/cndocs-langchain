...
本指南提供了 PGVector [向量存储](/oss/javascript/integrations/vectorstores) 入门的快速概述。有关所有 `PGVectorStore` 功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_vectorstores_pgvector.PGVectorStore.html)。
...
要使用 PGVector 向量存储，您需要设置启用了 [`pgvector`](https://github.com/pgvector/pgvector) 扩展的 Postgres 实例。您还需要安装 `@langchain/community` 集成包，并将其作为对等依赖项安装 [`pg`](https://www.npmjs.com/package/pg) 包。
...
根据您设置实例的方式，有多种连接到 Postgres 的方法。这是一个使用 `pgvector` 团队提供的预构建 Docker 镜像的本地设置示例。
...
要实例化向量存储，请调用 `.initialize()` 静态方法。这将自动检查是否存在由传入的 `config` 中的 `tableName` 给出的表。如果不存在，它将创建具有所需列的表。

<Warning>
**安全**：用户生成的数据（如用户名）不应用作表名和列名的输入。
**这可能会导致 SQL 注入！**
</Warning>
...
您可以通过创建连接池，然后直接通过构造函数创建新的 `PGVectorStore` 实例来重用连接。

请注意，在使用构造函数之前，您应该至少调用一次 `.initialize()` 以正确设置数据库和表。
...
默认情况下，该扩展执行顺序扫描搜索，召回率为 100%。您可能会考虑为近似最近邻 (ANN) 搜索创建 HNSW 索引，以加快 `similaritySearchVectorWithScore` 的执行时间。要在向量列上创建 HNSW 索引，请使用 `createHnswIndex()` 方法。

方法参数包括：

- `dimensions`：定义向量数据类型的维度数，最多 2000。例如，对于 OpenAI 的 text-embedding-ada-002 和 Amazon 的 amazon.titan-embed-text-v1 模型，使用 1536。

- `m?`：每层最大连接数（默认为 16）。较小的值可以提高索引构建时间，而较高的值可以加快搜索查询。

- `efConstruction?`：用于构建图的动态候选列表的大小（默认为 64）。较高的值可能会提高索引质量，但会增加索引构建时间。

- `distanceFunction?`：您要使用的距离函数名称，将根据 distanceStrategy 自动选择。

有关更多信息，请参阅 [Pgvector GitHub 存储库](https://github.com/pgvector/pgvector?tab=readme-ov-file#hnsw) 和 [HNSW 论文：Malkov Yu A. 和 Yashunin D. A.。2020。使用分层导航小世界图的高效且鲁棒的近似最近邻搜索](https://arxiv.org/pdf/1603.09320)
...
有关所有 `PGVectorStore` 功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_vectorstores_pgvector.PGVectorStore.html)。
