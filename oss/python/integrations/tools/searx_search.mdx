---
title: "SearxNG 搜索集成"
description: "使用 LangChain Python 与 SearxNG 搜索工具集成。"
---

本笔记本介绍如何使用自托管的 `SearxNG` 搜索 API 进行网络搜索。

您可以[查看此链接](https://docs.searxng.org/dev/search_api.html)了解更多关于 `Searx API` 参数的信息。
...
对于某些引擎，如果直接提供 `answer`，包装器将打印答案而不是完整的搜索结果列表。如果您想获取所有结果，可以使用包装器的 `results` 方法。
...
在此示例中，我们将使用 `engines` 参数查询 Wikipedia。
...
在此示例中，我们将使用 `categories` 参数查找科学论文，并将结果限制在 `time_range`（并非所有引擎都支持时间范围选项）。
...
在此示例中，我们查询 `it` 类别下的 `large language models`。然后我们过滤来自 github 的结果。
