...
> 本笔记本展示了如何使用 **PineconeRerank** 进行两阶段向量检索重排序，该功能使用 Pinecone 托管的重排序 API，如 `langchain_pinecone/libs/pinecone/rerank.py` 中所示。
...
如果您的文档是字典或具有自定义字段，请使用 `rank_fields` 指定要进行排名的字段。
...
您可以将特定于模型的参数（例如 `truncate`）直接传递给 `.rerank()`。

如何处理超过模型支持长度的输入。接受的值：END 或 NONE。
...
- `.compress_documents(documents, query)` (返回带有 `relevance_score` 元数据的 [`Document`](https://reference.langchain.com/python/langchain-core/documents/base/Document) 对象)
