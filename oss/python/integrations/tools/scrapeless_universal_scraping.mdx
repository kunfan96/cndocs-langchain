...
此处展示如何实例化 Scrapeless 通用抓取工具的实例。此工具允许您使用具有 JavaScript 渲染能力的无头浏览器抓取任何网站，支持可自定义的输出类型和特定地理位置的代理支持。
...
- `js_wait_until`（可选，字符串）：定义何时认为 JavaScript 渲染的页面已准备就绪。默认为 `'domcontentloaded'`。选项包括：
  - `load`：等待页面完全加载。
...
- `response_type`（可选，字符串）：定义响应的格式。默认为 `'html'`。选项包括：
  ...
  - `png`：返回 PNG 截图。
  - `jpeg`：返回 JPEG 截图。
- `response_image_full_page`（可选，布尔值）：使用截图输出（png 或 jpeg）时是否捕获并返回整页图像。默认为 False。
- `selector`（可选，字符串）：用于限定页面内抓取范围的特定 CSS 选择器。默认为 `None`。
- `proxy_country`（可选，字符串）：用于特定地理位置代理访问的两位国家代码（例如 `'us'`、`'gb'`、`'de'`、`'jp'`）。默认为 `'ANY'`。
