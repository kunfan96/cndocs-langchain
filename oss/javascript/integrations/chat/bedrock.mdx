...
sidebarTitle: "聊天"
description: "使用 LangChain JavaScript 集成 BedrockChat 聊天模型。"
---

[Amazon Bedrock](https://aws.amazon.com/bedrock/) 是一项完全托管的服务，通过单一 API 提供来自 AI21 Labs、Anthropic、Cohere、Meta、Stability AI 和 Amazon 等领先 AI 公司的高性能基础模型（FMs）选择，以及构建安全、隐私和负责任 AI 的生成式 AI 应用程序所需的广泛功能。

这将帮助您开始使用 `BedrockChat` [聊天模型](/oss/javascript/langchain/models)。有关所有 `BedrockChat` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/javascript/langchain-community/chat_models/bedrock/BedrockChat)。

<Tip>
更新的 [`ChatBedrockConverse` 聊天模型现已通过专用的 `@langchain/aws`](/oss/javascript/integrations/chat/bedrock_converse) 集成包提供。使用此包可以与更多模型一起使用[工具调用](/oss/javascript/langchain/tools)。
</Tip>
...
前往 [AWS 文档](https://docs.aws.amazon.com/bedrock/latest/userguide/getting-started.html) 注册 AWS 并设置您的凭证。您还需要为您的账户启用模型访问权限，可以通过[遵循这些说明](https://docs.aws.amazon.com/bedrock/latest/userguide/model-access.html)来完成。
...
LangChain 的 `BedrockChat` 集成位于 `@langchain/community` 包中。您还需要安装几个官方 AWS 包作为对等依赖项：
...
使用 Bedrock 模型进行工具调用的方式与[其他模型](/oss/javascript/langchain/tools)类似，但请注意并非所有 Bedrock 模型都支持工具调用。有关更多信息，请参阅 [AWS 模型文档](https://docs.aws.amazon.com/bedrock/latest/APIReference/welcome.html)。

---

## API 参考

有关所有 `BedrockChat` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/javascript/langchain-community/chat_models/bedrock/BedrockChat)。
