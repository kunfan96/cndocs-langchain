---
title: "TiDB 向量集成"
description: "使用 LangChain Python 与 TiDB 向量存储集成。"
---

> [TiDB Cloud](https://www.pingcap.com/tidb-serverless) 是一个全面的数据库即服务（DBaaS）解决方案，提供专用和无服务器选项。TiDB Serverless 现在将内置向量搜索集成到 MySQL 生态中。通过此增强功能，您可以使用 TiDB Serverless 无缝开发 AI 应用程序，无需新的数据库或额外的技术栈。创建一个免费的 TiDB Serverless 集群，并在 [pingcap.com/ai](https://pingcap.com/ai) 开始使用向量搜索功能。

本指南提供了关于使用 TiDB 向量功能的详细指南，展示了其特性和实际应用。
...
下面的代码片段在 TiDB 中创建一个名为 `TABLE_NAME` 的表，针对向量搜索进行了优化。成功执行此代码后，您将能够直接在 TiDB 数据库中查看和访问 `TABLE_NAME` 表。
...
此外，可以使用 similarity_search_with_relevance_scores 方法来获取相关性分数，其中分数越高表示相似性越大。
...
使用元数据过滤器执行搜索，以检索与应用过滤器匹配的特定数量的最近邻结果。
...
在 LangChain 中，检索器是一个接口，用于根据非结构化查询检索文档，提供比向量存储更广泛的功能。下面的代码演示了如何将 TiDB Vector 用作检索器。
...
让我们看一个高级用例 - 一位旅行代理正在为希望拥有特定设施（如干净的休息室和素食选择）的机场的客户制作定制旅行报告。该过程涉及：

- 在机场评论中进行语义搜索，以提取符合这些设施的机场代码。
- 随后的 SQL 查询将这些代码与路线信息连接起来，详细说明与客户偏好相符的航空公司和目的地。
...
或者，我们可以通过使用单个 SQL 查询来简化流程，一步完成搜索。
