---
title: "SQLToolkit 集成"
description: "使用 LangChain JavaScript 与 SQLToolkit 工具集成。"
---

这将帮助您开始使用 `SqlToolkit` [工具包](/oss/javascript/langchain/tools#prebuilt-tools)。如需更多信息，您也可以查看 [Python SQL 工具包文档](https://python.langchain.com/docs/integrations/toolkits/sql_database/)。
...
| `info-sql`        | 此工具的输入是一个逗号分隔的表列表，输出是这些表的架构和示例行。请务必先调用 list-tables-sql 以确保表实际存在！示例输入："table1, table2, table3"。          |
| `list-tables-sql` | 输入是一个空字符串，输出是数据库中表的逗号分隔列表。                                                                                                                                                     |
| `query-checker`   | 使用此工具在执行查询之前再次检查您的查询是否正确。在使用 query-sql 执行查询之前，请务必使用此工具！                                                                                                 |

该工具包适用于在 SQL 数据库上提问、执行查询、验证查询等。

## 设置

此示例使用 Chinook 数据库，这是一个可用于 SQL Server、Oracle、MySQL 等的示例数据库。要设置它，请遵循[这些说明](https://database.guide/2-sample-databases-sqlite/)，将 `.db` 文件放在您的代码所在目录中。

如果您希望从单个工具的运行中获取自动跟踪，也可以通过取消注释以下内容来设置您的 [LangSmith](/langsmith/home) API 密钥：
...
此工具包位于 `langchain` 包中。您还需要安装 `typeorm` 同级依赖项。
