...
Vectara 在底层使用其自己的嵌入，因此您无需自己提供任何嵌入或调用其他服务来获取嵌入。
...
const store = await VectaraStore.fromTexts(
  ...
  [{ foo: "bar" }, { foo: "baz" }],
  // 这不会产生任何效果。为清晰起见，请提供一个 FakeEmbeddings 实例。
...
- 创建一个 [免费的 Vectara 帐户](https://vectara.com/integrations/langchain)。
...
请注意，您可以提供多个语料库 ID（用逗号分隔）以一次查询多个语料库。例如：`VECTARA_CORPUS_ID=3,8,9,43`。
...
请注意，`lambda` 是一个与 Vectara 的混合搜索能力相关的参数，它在神经搜索和布尔/精确匹配之间提供了一种权衡，如 [Vectara 的词汇匹配文档](https://docs.vectara.com/docs/api-reference/search-apis/lexical-matching) 中所述。我们建议将 0.025 作为默认值，同时为高级用户提供了在需要时自定义此值的方法。
