---
title: "Vsdx 集成"
description: "使用 LangChain Python 与 Vsdx 文档加载器集成。"
---

> [Visio 文件](https://fr.wikipedia.org/wiki/Microsoft_Visio)（扩展名为 .vsdx）与 Microsoft Visio 相关联，这是一款图表创建软件。它存储有关图表的结构、布局和图形元素的信息。这种格式便于在商业、工程和计算机科学等领域创建和共享可视化内容。

一个 Visio 文件可以包含多个页面。其中一些页面可能作为其他页面的背景，并且这可能跨越多个图层。此**加载器**从每个页面及其关联页面中提取文本内容，从而能够从每个页面提取所有可见文本，类似于 OCR 算法的操作。

**警告**：仅扩展名为 **.vsdx** 的 Visio 文件与此加载器兼容。扩展名为 .vd、... 等的文件不兼容，因为它们无法转换为压缩的 XML。
