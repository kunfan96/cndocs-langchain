...
> [Annoy](https://github.com/spotify/annoy)（`Approximate Nearest Neighbors Oh Yeah`，近似最近邻）是一个带有 Python 绑定的 C++ 库，用于搜索空间中与给定查询点相近的点。它还能创建大型只读的基于文件的数据结构并映射到内存中，以便多个进程共享同一份数据。

使用此集成需要通过 `pip install -qU langchain-community` 安装 `langchain-community`。

本 notebook 展示了如何使用 `Annoy` 向量数据库的相关功能。

```{note}
注意：Annoy 是只读的——一旦索引构建完成，就无法再添加新的嵌入向量！
如果您希望逐步向向量存储中添加新条目，请选择其他替代方案！
```
