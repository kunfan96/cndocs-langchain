...
description: "使用 LangChain Python 集成 LinkupSearchTool 工具。"
---

> [Linkup](https://www.linkup.so/) 提供 API，可将 LLM 连接到互联网以及 Linkup 高级合作伙伴数据源。

本指南为使用 LinkupSearchTool [工具](/oss/python/langchain/tools) 提供快速入门概述。如需了解 LinkupSearchTool 所有功能和配置的详细文档，请访问 [API 参考](https://python.langchain.com/api_reference/linkup/tools/linkup_langchain.search_tool.LinkupSearchTool.html)。
...
要使用 Linkup 提供商，您需要一个有效的 API 密钥，可以通过[此处](https://app.linkup.so/sign-up)注册获取。要运行以下示例，您还需要一个 OpenAI API 密钥。
...
设置 [LangSmith](https://smith.langchain.com/) 也很有帮助（但不是必须的），可获得一流的可观测性：
...
我们可以将工具在链中使用，首先将其绑定到[支持工具调用的模型](/oss/python/langchain/tools/)，然后进行调用：
...
有关所有 `LinkupSearchTool` 功能和配置的详细文档，请访问 [API 参考](https://python.langchain.com/api_reference/linkup/tools/linkup_langchain.search_tool.LinkupSearchTool.html)。

---

<div className="source-links">
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/tools/linkup_search.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
