...
DataStax [Astra DB](https://astra.datastax.com/register) 是一个基于 [Apache Cassandra](https://cassandra.apache.org/_/index.html) 构建的无服务器向量数据库，并通过易于使用的 JSON API 便捷地提供服务。
...
Astra DB 支持 `cosine`（默认）、`dot_product` 和 `euclidean` 相似性搜索；这在首次创建向量存储时作为 `CreateCollectionOptions` 的一部分进行定义：
