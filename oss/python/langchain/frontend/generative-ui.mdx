...
生成式用户界面让 AI 能够从自然语言提示生成完整的用户界面。AI 的输出**就是**用户界面本身，而不是在聊天气泡中渲染文本响应：表单、卡片、仪表板等等。开发者定义可用的组件（即“目录”），AI 将它们组合成一个有效的用户界面树。
...
1. **定义目录**：声明 AI 可以使用的组件，并指定类型化的属性
2. **提示 AI**：用自然语言描述你想要的用户界面
3. **AI 生成规范**：一个描述组件树的 JSON 文档
4. **安全渲染**：json-render 的 `Renderer` 使用你的组件渲染该规范

目录充当护栏：AI 只能使用你定义的组件，并且属性必须匹配你的模式。输出始终是可预测且安全的。

## 定义组件目录

目录描述了 AI 被允许使用的每个组件。每个组件都有一个用于其属性的 Zod 模式，以及一个 AI 读取以了解何时使用它的描述：
...
<Tip>
保持目录专注。仅包含 AI 在用例中需要的组件。
较小的目录比大而全的方法能产生更好的结果。
</Tip>

## 构建组件注册表

注册表将每个目录组件映射到其实际的渲染实现。
使用 `defineRegistry` 在目录属性和你的组件函数之间获得类型安全的绑定：
...
代理使用结构化输出来返回 json-render 规范。使用你的代理的助手 ID 设置 `useStream`，然后从 AI 消息的 `tool_calls` 中提取规范：
...
在流式传输期间，规范是增量构建的。元素一个接一个地到达，并且最初可能缺少 `type` 或 `props`。仅过滤出完整的元素，并将 `loading={true}` 传递给 `Renderer`，这会告诉它静默跳过尚未到达的子元素。UI 按组件逐个构建：

```tsx
/*
 * 过滤流式传输的规范，仅包含具有有效类型/属性的元素，
 * 从而在 AI 响应构建时实现渐进式渲染。将
...
- **使用描述性的组件描述**：AI 使用这些描述来了解何时使用每个组件。清晰的描述能带来更好的用户界面生成。
- **渲染前验证**：在传递给 Renderer 之前，始终检查元素是否具有有效的 `type` 和非空的 `props`，因为流式传输会提供部分数据。
- **为流式传输设计**：在流式传输期间传递 `loading={true}`，以便 Renderer 优雅地处理尚未到达的子元素。用户可以看到 UI 实时构建，而不是等待完整响应。
- **使用设计令牌进行样式设置**：使用 CSS 自定义属性，以便渲染的组件自动适应浅色和深色主题。
