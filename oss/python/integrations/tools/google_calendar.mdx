...
> [Google 日历](https://workspace.google.com/intl/en-419/products/calendar/) 是 Google Workspace 的一款产品，允许用户组织他们的日程和活动。它是一个基于云的日历，允许用户创建、编辑和删除活动。它还允许用户与他人共享他们的日历。
...
### [使用参数直接调用](/oss/python/langchain/tools#create-tools)

您可以通过以字典格式传递所需参数来直接调用工具。以下是使用 `CalendarCreateEvent` 工具创建新活动的示例。
