---
title: "Annoy 集成"
description: "使用 LangChain Python 与 Annoy 向量存储集成。"
---

> [Annoy](https://github.com/spotify/annoy) (`Approximate Nearest Neighbors Oh Yeah`) 是一个 C++ 库，带有 Python 绑定，用于搜索空间中接近给定查询点的点。它还创建大型的只读基于文件的数据结构，这些结构被映射到内存中，以便多个进程可以共享相同的数据。
...
```{note}
注意：Annoy 是只读的 - 一旦索引构建完成，你就无法再添加任何嵌入！
如果你想逐步向 VectorStore 添加新条目，最好选择其他替代方案！
```
