---
title: 如何在同一线程上运行多个代理
sidebarTitle: 在同一线程上运行多个代理
---
在 LangSmith 部署中，线程并未与特定代理显式关联。
这意味着您可以在同一线程上运行多个代理，从而允许不同的代理从初始代理的进度继续执行。

在此示例中，我们将创建两个代理，然后在同一线程上调用它们。
您将看到第二个代理会使用第一个代理在线程中生成的[检查点](/oss/python/langgraph/persistence#checkpoints)信息作为上下文进行响应。
...
现在，我们可以在默认助手上运行它，并看到这第二个助手知道初始问题，并能回答“那你呢？”这个问题：
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/langsmith/same-thread.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>