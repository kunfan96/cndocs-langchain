...
这将帮助您开始使用 `ChatCohere` [聊天模型](/oss/javascript/langchain/models)。有关所有 `ChatCohere` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/javascript/langchain-cohere/ChatCohere)。
...
您可以 [注册 Cohere 帐户并创建 API 密钥](https://dashboard.cohere.com/welcome/register)。
...
该 API 还允许其他非静态文档的连接。
一个例子是他们的 `web-search` 连接器，它允许您传递一个查询，API 将搜索网络以查找相关文档。
下面的示例演示了如何使用此功能。
...
- 执行了搜索查询，将结果数据存储在 `searchQueries` 和 `searchResults` 字段中。在 `searchQueries` 字段中，我们看到他们为了获得更好的结果而重新表述了我们的查询。
- 根据搜索查询生成了三个文档。
- 生成了一个引用列表
- 基于上述操作和内容生成了最终响应。
...
有关所有 `ChatCohere` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/javascript/langchain-cohere/ChatCohere)。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/javascript/integrations/chat/cohere.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>