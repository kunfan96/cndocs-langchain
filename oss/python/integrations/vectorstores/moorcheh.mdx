---
title: "Moorcheh 集成"
description: "使用最大化信息二值化 (MIB) 和信息论评分 (ITS) 实现的闪电般快速的语义搜索引擎和向量存储"
---

# Moorcheh

[Moorcheh](https://www.moorcheh.ai/) 是一个闪电般快速的语义搜索引擎和向量存储。Moorcheh 不使用简单的距离度量（如 L2 或余弦相似度），而是使用最大化信息二值化 (MIB) 和信息论评分 (ITS) 来检索准确的文档块。

以下教程将指导您如何使用 Moorcheh 和 LangChain 上传和存储文本文档及向量嵌入，并为所有查询检索相关块。
...
一旦您的命名空间已创建并且您已将文档上传到其中，您就可以直接通过向量存储查询文档。设置您希望用于回答查询的查询和 LLM。有关支持的 LLM 的更多信息，请访问我们的 [GitHub 页面](https://github.com/moorcheh-ai/moorcheh-python-sdk)。
