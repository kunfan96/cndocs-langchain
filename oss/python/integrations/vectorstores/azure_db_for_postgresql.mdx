...
[Azure Database for PostgreSQL - 灵活服务器](https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/service-overview) 是一个基于开源 Postgres 数据库引擎的关系型数据库服务。它是一个完全托管的数据库即服务，能够处理关键任务工作负载，提供可预测的性能、安全性、高可用性和动态可扩展性。

本指南将向您展示如何利用此集成的[向量数据库](https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/how-to-use-pgvector)将文档存储在集合中，创建索引，并使用近似最近邻算法（如余弦距离、L2（欧几里得距离）和 IP（内积））执行向量搜索查询，以定位与查询向量接近的文档。

## 向量支持

Azure Database for PostgreSQL - 灵活服务器使您能够在 PostgreSQL 中高效地存储和查询数百万个向量嵌入。同时，将您的 AI 用例从概念验证扩展到生产环境：

* 提供熟悉的 SQL 接口来查询向量嵌入和关系型数据。
* 通过 [DiskANN 索引算法](https://aka.ms/pg-diskann-docs) 增强 `pgvector`，实现对超过 1 亿个向量的更快、更精确的相似性搜索。
* 通过将关系型元数据、向量嵌入和时序数据集成到单个数据库中，简化操作。
* 利用强大的 PostgreSQL 生态系统和 Azure 云的力量，提供企业级功能，包括复制和高可用性。
...
Entra 身份验证允许您使用 Entra 身份来验证对 PostgreSQL 服务器的访问。这消除了为数据库用户管理单独用户名和密码的需要，并允许您利用与其他 Azure 服务相同的安全机制。

本指南已设置为使用任一身份验证方法。您可以在笔记本中稍后配置是否使用 Entra 身份验证。
...
您需要 Azure Database for PostgreSQL 的[连接详细信息](https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/quickstart-create-server-portal#get-the-connection-information)，并将它们作为环境变量添加以运行此笔记本。

如果您想使用 Microsoft Entra 身份验证，请将 `USE_ENTRA_AUTH` 标志设置为 `True`。如果使用 Entra 身份验证，您只需提供主机和数据库名称。如果使用密码身份验证，您还需要设置用户名和密码。
...
以下部分演示如何设置 LangChain 以使用 Microsoft Entra 身份验证。LangChain Azure Postgres 包中的 `AzurePGConnectionPool` 类使用 `azure.identity` 库中的 `DefaultAzureCredential` 为 Azure Database for PostgreSQL 服务获取令牌。

该连接可以传递给 `AzurePGVectorStore` LangChain 向量存储的 `connection` 参数。

#### 登录 Azure

要登录 Azure，请确保您已安装 [Azure CLI](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli)。您需要在终端中运行以下命令：
...
未指定嵌入类型，默认为 'vector'。
未指定嵌入维度，默认为 1536。
未指定嵌入索引，默认为带有 'vector_cosine_ops' 操作类的 'DiskANN'。
```
...
vector_store = AzurePGVectorStore(
    ...
    embedding_dimension=768,                # 设置为与模型输出匹配
...
[DiskANN](https://aka.ms/pg-diskann-blog) 是一种可扩展的近似最近邻搜索算法，可在任何规模下实现高效的向量搜索。它提供高召回率、高每秒查询数和低查询延迟，即使对于十亿点数据集也是如此。这些特性使其成为处理大量数据的强大工具。
