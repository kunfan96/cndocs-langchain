...
为了能够以良好的性能进行自查询，我们在 HANA 中为向量存储表创建了额外的元数据字段：
...
现在进入主要部分：以下是为 HANA 向量存储构建 SelfQueryRetriever 的方法：
...
const metadataFieldInfo: AttributeInfo[] = [
  { name: "name", description: "人的名字", type: "string" },
  { name: "is_active", description: "人是否活跃", type: "boolean" },
  { name: "id", description: "人的 ID", type: "integer" },
  { name: "height", description: "人的身高", type: "float" },
];

const contentDescription = "一个人的集合";
