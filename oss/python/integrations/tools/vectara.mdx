---
title: "Vectara 集成"
description: "使用 LangChain Python 与 Vectara 工具集成。"
---

[Vectara](https://vectara.com/) 是一个值得信赖的 AI 助手和代理平台，专注于企业级关键任务应用的就绪性。更多[详情](../providers/vectara.ipynb)。

[Vectara](https://vectara.com/) 提供了多种可与 LangChain 一起使用的工具。

- **VectaraSearch**：用于对您的语料库进行语义搜索
- **VectaraRAG**：用于使用 RAG 生成摘要
- **VectaraIngest**：用于将文档摄取到您的语料库中
- **VectaraAddFiles**：用于上传文件

## 设置

要使用 `Vectara 工具`，您首先需要安装合作伙伴包。
...
1.  如果您还没有帐户，请[注册](https://www.vectara.com/integrations/langchain)一个免费的 Vectara 试用版。
2.  在您的帐户中，您可以创建一个或多个语料库。每个语料库代表一个区域，用于存储从输入文档摄取的文本数据。要创建语料库，请使用 **“Create Corpus”** 按钮。然后，为您的语料库提供名称和描述。您可以选择定义过滤属性并应用一些高级选项。如果单击您创建的语料库，您可以在顶部看到其名称和语料库 ID。
3.  接下来，您需要创建 API 密钥以访问语料库。在语料库视图中单击 **“Access Control”** 选项卡，然后单击 **“Create API Key”** 按钮。为您的密钥命名，并选择您希望密钥是仅查询还是查询+索引。单击“Create”，您现在就拥有一个活动的 API 密钥。请对此密钥保密。
...
请注意，我们使用的是 `VectaraAddFiles` 工具，它不需要任何本地处理或分块 - Vectara 接收文件内容并执行所有必要的预处理、分块和嵌入，将其存入其知识存储。

在此示例中，它使用的是 .txt 文件，但同样适用于许多其他[文件类型](https://docs.vectara.com/docs/api-reference/indexing-apis/file-upload/file-upload-filetypes)。
...
- 我们启用摘要，指定我们希望 LLM 选择前 7 个匹配的块并以英语响应

使用此配置，让我们创建一个 LangChain 工具 `VectaraRAG` 对象，该对象封装了完整的 Vectara RAG 流程：
...
`VectaraSearch` 工具可以用作检索器。

在这种情况下，它的行为就像任何其他 LangChain 检索器一样。此模式的主要用途是语义搜索，在这种情况下我们禁用摘要：
...
您可以将 Vectara 工具与其他 LangChain 组件链式连接。该示例展示了如何：

- 设置 ChatOpenAI 模型以进行额外处理
- 为特定摘要需求创建自定义提示模板
- 使用 LangChain 的 Runnable 接口将多个组件链式连接
- 处理并格式化来自 Vectara 的 JSON 响应
...
下面的代码演示了如何将 Vectara 工具与 LangChain 一起使用来创建代理。
...
`VectaraIngest` 工具允许您直接将文本内容摄取到您的 Vectara 语料库中。当您有文本内容想要添加到语料库而无需先创建文件时，这非常有用。
...
有关详细信息，请查看 Vectara [工具](https://github.com/vectara/langchain-vectara/blob/main/libs/vectara/langchain_vectara/tools.py)的实现。

---

<div className="source-links">
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/tools/vectara.mdx) 或[提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
