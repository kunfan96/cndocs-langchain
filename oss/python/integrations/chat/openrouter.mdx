...
本文将帮助您快速上手 OpenRouter [聊天模型](/oss/python/langchain/models)。OpenRouter 是一个统一 API，通过单一端点提供来自多个提供商（OpenAI、Anthropic、Google、Meta 等）的模型访问。

<Tip>
    **API 参考**

    有关所有功能和配置选项的详细文档，请参阅 [`ChatOpenRouter`](https://reference.langchain.com/python/integrations/langchain_openrouter) API 参考。
</Tip>

完整的可用模型列表，请访问 [OpenRouter 模型页面](https://openrouter.ai/models)。
...
要通过 OpenRouter 访问模型，您需要创建 [OpenRouter 账户](https://openrouter.ai/)、获取 API 密钥，并安装 `langchain-openrouter` 集成包。
...
`ChatOpenRouter` 通过 [`with_structured_output`](/oss/python/langchain/models#structured-output) 方法支持结构化输出。提供两种方法：`function_calling`（默认）和 `json_schema`。
...
使用 [`ProviderStrategy`](/oss/python/langchain/agents#providerstrategy) 指定 `response_format`，在生成 Agent 最终响应时启用结构化输出。
...
对于支持推理的模型（如 `anthropic/claude-sonnet-4.5`、`deepseek/deepseek-r1`），可以通过 `reasoning` 参数启用推理 token。详情请参阅 [OpenRouter 推理文档](https://openrouter.ai/docs/guides/best-practices/reasoning-tokens)：
...
更多关于内容块的信息，请参阅[标准内容块](/oss/python/langchain/messages#standard-content-blocks)指南。
...
<Note>
    effort 到 budget 的映射取决于具体模型。例如，Google Gemini 模型将 effort 映射到内部的 `thinkingLevel`，而非确切的 token 预算。详情请参阅 [OpenRouter 推理文档](https://openrouter.ai/docs/guides/best-practices/reasoning-tokens)。
</Note>
...
OpenRouter 支持接受多模态输入的模型的[多模态输入](/oss/python/langchain/messages#multimodal)。可用的模态取决于您选择的模型——请查看 [OpenRouter 模型页面](https://openrouter.ai/models)了解详情。
...
<Note>
    并非所有模型都支持所有模态。请查看 [OpenRouter 模型页面](https://openrouter.ai/models)了解各模型的具体支持情况。
</Note>

### 图像输入

使用 [`HumanMessage`](https://reference.langchain.com/python/langchain-core/messages/human/HumanMessage) 的列表内容格式，将图像与文本一起传入。
...
完整的选项列表请参阅 [OpenRouter 提供商路由文档](https://openrouter.ai/docs/guides/routing/provider-selection)。
...
有关 `ChatOpenRouter` 所有特性和配置的详细文档，请参阅 [`ChatOpenRouter`](https://reference.langchain.com/python/integrations/langchain_openrouter) API 参考。

更多关于 OpenRouter 平台、模型和功能的信息，请参阅 [OpenRouter 文档](https://openrouter.ai/docs)。
