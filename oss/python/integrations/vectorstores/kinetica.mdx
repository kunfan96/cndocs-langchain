...
description: "使用 LangChain Python 与 Kinetica VectorStore API 向量存储进行集成。"
---
...
这需要一个 Kinetica 实例，可以使用此处给出的说明轻松设置 - [安装说明](https://www.kinetica.com/developer-edition/)。
...
Kinetica 模块将尝试使用集合名称创建一个表。
确保集合名称是唯一的，并且用户具有创建表的权限。
...
上面，我们从头创建了一个向量存储。然而，很多时候我们想使用现有的向量存储。
为此，我们可以直接初始化它。
...
如果您有一个现有集合，可以通过执行 `from_documents` 并设置 `pre_delete_collection` = True 来覆盖它。
