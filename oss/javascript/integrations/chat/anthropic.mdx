...
[Anthropic](https://www.anthropic.com/) 是一家 AI 安全和研究公司。他们是 Claude 的创建者。
...
Anthropic 模型与大多数其他模型之间需要注意的一个主要区别是，单个 Anthropic [`AIMessage`](https://reference.langchain.com/javascript/langchain-core/messages/AIMessage) 的内容可以是单个字符串或**内容块列表**。例如，当 Anthropic 模型[调用工具](/oss/javascript/langchain/tools)时，工具调用是消息内容的一部分（并且也暴露在标准化的 `AIMessage.tool_calls` 字段中）：
...
Anthropic 支持[缓存部分提示](https://platform.claude.com/docs/en/build-with-claude/prompt-caching)，以降低需要长上下文的用例成本。您可以缓存工具以及整个消息和单个块。

包含一个或多个块或工具定义的初始请求，如果带有 `"cache_control": { "type": "ephemeral" }` 字段，将自动缓存提示的那部分。此初始缓存步骤将产生额外费用，但后续请求将按折扣费率计费。缓存的生命周期为 5 分钟，但每次命中缓存时都会刷新。

目前还有一个最小可缓存提示长度，根据模型而异。您可以[在此处](https://platform.claude.com/docs/en/build-with-claude/prompt-caching#structuring-your-prompt)查看此信息。

目前这需要您使用 beta 标头初始化模型。以下是缓存包含 LangChain [概念文档](/oss/javascript/concepts/)的系统消息部分的示例：
...
我们可以看到，Anthropic 返回的原始 usage 字段中有一个名为 `cache_creation_input_tokens` 的新字段。
...
您还可以通过在工具定义中设置相同的 `"cache_control": { "type": "ephemeral" }` 来缓存工具。目前这需要您以 [Anthropic 的原始工具格式](https://platform.claude.com/docs/en/agents-and-tools/tool-use/overview)绑定工具。这是一个例子：
...
Anthropic 模型[可能托管在 Google Vertex 等云服务上](https://platform.claude.com/docs/en/build-with-claude/claude-on-vertex-ai)，这些服务依赖于具有与主要 Anthropic 客户端相同接口的不同底层客户端。您可以通过提供返回 Anthropic 客户端初始化实例的 `createClient` 方法来访问这些服务。这是一个例子：
...
Anthropic 支持[引用](https://platform.claude.com/docs/en/build-with-claude/citations)功能，该功能允许 Claude 根据用户提供的源材料为其答案附加上下文。此源材料可以作为[文档内容块](https://platform.claude.com/docs/en/build-with-claude/citations#document-types)（描述完整文档）提供，也可以作为[搜索结果](https://platform.claude.com/docs/en/build-with-claude/search-results)（描述从检索系统返回的相关段落或片段）提供。当查询中包含 `"citations": { "enabled": true }` 时，Claude 可能会在其响应中生成对所提供材料的直接引用。

### 文档示例

在此示例中，我们传递一个[纯文本文档](https://platform.claude.com/docs/en/build-with-claude/citations#plain-text-documents)。在后台，Claude 自动将输入文本[分块](https://platform.claude.com/docs/en/build-with-claude/citations#plain-text-documents)为句子，这些句子用于生成引用。
...
在此示例中，我们将[搜索结果](https://platform.claude.com/docs/en/build-with-claude/search-results)作为消息内容的一部分传递。这允许 Claude 在其响应中引用您自己的检索系统中的特定段落或片段。

当您希望 Claude 引用特定知识集中的信息，但又希望直接带来您自己的预提取/缓存内容，而不是让模型自动搜索或检索它们时，此方法很有用。
...
您还可以使用工具提供模型可以在其响应中引用的搜索结果。这非常适合 RAG（或 [检索增强生成](https://en.wikipedia.org/wiki/Retrieval-augmented_generation)）工作流，其中 Claude 可以决定何时何地检索信息。当以[搜索结果](https://platform.claude.com/docs/en/build-with-claude/search-results)形式返回此信息时，它使 Claude 能够从工具返回的材料中创建引用。

以下是如何创建一个返回 Anthropic 引用 API 预期格式的搜索结果的工具：
...
[在此处](https://js.langchain.com/docs/concepts/rag/)了解有关 RAG 如何在 LangChain 中工作的更多信息

[了解有关工具调用的更多信息](/oss/javascript/langchain/tools)

### 与文本分割器一起使用

Anthropic 还允许您使用[自定义文档](https://platform.claude.com/docs/en/build-with-claude/citations#custom-content-documents)类型指定自己的分割。LangChain 文本分割器可用于为此目的生成有意义的分割。请参阅以下示例，我们分割 LangChain.js README（一个 markdown 文档）并将其作为上下文传递给 Claude：
...
<Info>
    **上下文管理从 `@langchain/anthropic@0.3.29` 开始支持**
</Info>
