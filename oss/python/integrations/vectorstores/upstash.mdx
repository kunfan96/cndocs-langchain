---
title: "Upstash 向量集成"
description: "使用 LangChain Python 与 Upstash 向量存储进行集成。"
---

> [Upstash Vector](https://upstash.com/docs/vector/overall/whatisvector) 是一个无服务器向量数据库，专为处理向量嵌入而设计。
>
> 向量 LangChain 集成是 [upstash-vector](https://github.com/upstash/vector-py) 包的包装器。
>
...
此外，需要一个 [`Embeddings`](https://reference.langchain.com/python/langchain-core/embeddings/embeddings/Embeddings) 实例来将给定文本转换为嵌入。这里我们以 `OpenAIEmbeddings` 为例
...
创建 `UpstashVectorStore` 的另一种方法是[通过选择模型创建 Upstash Vector 索引](https://upstash.com/docs/vector/features/embeddingmodels#using-a-model)并传递 `embedding=True`。在此配置中，文档或查询将作为文本发送到 Upstash 并在那里进行嵌入。

```python
store = UpstashVectorStore(embedding=True)
```

如果您有兴趣尝试这种方法，可以像上面那样更新 `store` 的初始化并运行教程的其余部分。

## 加载文档

加载示例文本文件并将其拆分为可转换为向量嵌入的块。
...
向量存储使用嵌入对象嵌入文本块，并将其批量插入数据库。这将返回插入向量的 ID 数组。
...
命名空间可用于分隔不同类型的文档。这可以提高查询效率，因为搜索空间会减小。如果未提供命名空间，则使用默认命名空间。
...
元数据可用于过滤查询结果。您可以参考[文档](https://upstash.com/docs/vector/features/filtering)查看更多复杂的过滤方式。
