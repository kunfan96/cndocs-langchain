...
[FireCrawl](https://firecrawl.dev/?ref=langchain) 可以爬取并将任何网站转换为 LLM 就绪的数据。它会爬取所有可访问的子页面，并为每个页面提供干净的 Markdown 和元数据。无需站点地图。
...
- `crawl`：爬取 URL 及所有可访问的子页面，并返回每个页面的 Markdown。
...
您还可以向加载器传递 `params`。这是一个传递给爬虫的选项字典。更多信息请参阅 [FireCrawl API 文档](https://github.com/mendableai/firecrawl-py)。
...
您还可以向加载器传递 `params`。这是一个传递给加载器的选项字典。更多信息请参阅 [FireCrawl API 文档](https://github.com/mendableai/firecrawl-py)。

---

## API 参考

有关所有 `FireCrawlLoader` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/python/langchain-community/document_loaders/firecrawl/FireCrawlLoader)
