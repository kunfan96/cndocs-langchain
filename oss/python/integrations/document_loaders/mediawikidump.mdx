...
>[MediaWiki XML 转储](https://www.mediawiki.org/wiki/Manual:Importing_XML_dumps) 包含一个维基的内容（包含所有修订版本的维基页面），但不包含与站点相关的数据。XML 转储不会创建维基数据库的完整备份，转储不包含用户账户、图片、编辑日志等。

本文介绍如何将 MediaWiki XML 转储文件加载到我们可以用于下游处理的文档格式中。

它使用 `mediawiki-utilities` 中的 `mwxml` 进行转储，并使用 `earwig` 中的 `mwparserfromhell` 解析 MediaWiki 维基代码。

转储文件可以通过 dumpBackup.php 或在维基的 Special:Statistics 页面获取。

```python
# mediawiki-utilities 在未合并的分支中支持 XML schema 0.11
pip install -qU git+https://github.com/mediawiki-utilities/python-mwtypes@updates_schema_0.11
# mediawiki-utilities mwxml 存在一个 bug，修复 PR 待处理
