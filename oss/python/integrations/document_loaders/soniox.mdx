...
示例：如何使用 `SonioxDocumentLoader` 转录音频文件并使用 LLM 生成摘要。
...
提供特定领域的 [上下文](https://soniox.com/docs/stt/concepts/context) 以提高转录准确性。上下文有助于模型理解您的领域、识别重要术语并应用自定义词汇表。
...
| `polling_interval_seconds`     | `float`                      | 否   | `1.0`                          | 状态轮询间隔时间（秒）                             |
| `timeout_seconds`              | `float`                      | 否   | `300.0`（5 分钟）              | 等待转录完成的最大时间                             |
...
| `client_reference_id`            | `str`               | 供您记录的自定义参考 ID                               |
...
浏览 [API 文档](https://soniox.com/docs/stt/api-reference/transcriptions/create_transcription) 获取支持的完整选项列表。
...
- `start_ms`：开始时间（毫秒）
- `end_ms`：结束时间（毫秒）
- `speaker`：说话人 ID（如果启用了分离），例如 `"1"`、`"2"` 等
- `language`：检测到的语言（如果启用了识别），例如 `"en"`、`"fr"` 等
- `translation_status`：翻译状态（`"original"`、`"translated"` 或 `"none"`）

了解更多关于 [Soniox API 参考](https://soniox.com/docs/stt/api-reference/transcriptions/get_transcription_transcript) 的信息。
