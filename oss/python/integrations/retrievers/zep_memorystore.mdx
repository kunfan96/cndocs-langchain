...
```python
# 提供你的 Zep API 密钥。注意这是可选的。参见 https://docs.getzep.com/deployment/auth
AUTHENTICATE = False
...
```python
# 预加载一些消息到记忆中。默认消息窗口是 12 条消息。我们希望超出这个数量以演示自动摘要功能。
test_history = [
...
time.sleep(
    10
)  # 等待消息被嵌入和摘要。速度取决于 OpenAI API 延迟和你的速率限制。
```
...
更多信息请参阅：[docs.getzep.com/sdk/search_query/](https://docs.getzep.com/sdk/search_query/)
...
MMR 会重新排序结果，以确保你填充到提示中的摘要既相关，又为 LLM 提供了额外的信息。
