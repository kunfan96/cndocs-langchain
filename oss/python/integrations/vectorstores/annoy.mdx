---
title: "Annoy 集成"
description: "使用 LangChain Python 与 Annoy 向量存储进行集成。"
---

> [Annoy](https://github.com/spotify/annoy) (`Approximate Nearest Neighbors Oh Yeah`) 是一个 C++ 库，带有 Python 绑定，用于在空间中搜索与给定查询点接近的点。它还会创建大型只读基于文件的数据结构，这些结构被映射到内存中，以便多个进程可以共享相同的数据。
...
```{note}
注意：Annoy 是只读的 - 一旦索引构建完成，您无法再添加任何嵌入！
如果您想逐步向 VectorStore 添加新条目，那么最好选择替代方案！
```
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/vectorstores/annoy.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
