...
LangChain 提供了内置的[代理](/oss/javascript/langchain/agents)实现，这些实现使用 [LangGraph](/oss/javascript/langgraph/overview) 原语构建。如果需要更深层次的定制，可以直接在 LangGraph 中实现代理。本指南演示了一个 SQL 代理的示例实现。有关实践入门，请参阅[使用更高级的 LangChain 抽象构建 SQL 代理](/oss/javascript/langchain/sql-agent)。

<Warning>
构建 SQL 数据库的问答系统需要执行模型生成的 SQL 查询。这样做存在固有风险。请确保您的数据库连接权限始终尽可能严格地限定在代理所需范围内。这将减轻（但不能消除）构建模型驱动系统的风险。
</Warning>

[预构建代理](/oss/javascript/langchain/sql-agent)让我们可以快速上手，但我们依赖系统提示来约束其行为——例如，我们指示代理始终从“列出表”工具开始，并在执行查询之前始终运行查询检查器工具。

我们可以通过自定义代理在 LangGraph 中实施更高程度的控制。在这里，我们实现了一个简单的 ReAct 代理设置，为特定的工具调用设置了专用节点。我们将使用与预构建代理相同的[状态]。
...
- [人在回路中](/oss/javascript/langgraph/interrupts)流程
...
我们将使用 `@langchain/classic/sql_db` 模块中提供的便捷 SQL 数据库包装器与数据库交互。该包装器提供了一个简单的接口来执行 SQL 查询和获取结果：
...
const getSchemaTool = tool(
  ...
  {
    name: "sql_db_schema",
    description: "此工具的输入是以逗号分隔的表列表，输出是这些表的结构和示例行。请务必先调用 sql_db_list_tables 以确保表确实存在！示例输入：table1, table2, table3",
    schema: z.object({
...
const queryTool = tool(
  ...
  {
    name: "sql_db_query",
    description: "此工具的输入是一个详细且正确的 SQL 查询，输出是来自数据库的结果。如果查询不正确，将返回错误消息。如果返回错误，请重写查询，检查查询，然后重试。",
    schema: z.object({
...
将这些步骤放在专用节点中使我们能够 (1) 在需要时强制进行工具调用，以及 (2) 自定义与每个步骤关联的提示。
...
我们现在可以使用 [图 API](/oss/javascript/langgraph/graph-api) 将这些步骤组装成一个工作流。我们在查询生成步骤定义了一个[条件边](/oss/javascript/langgraph/graph-api#conditional-edges)，如果生成了查询，则路由到查询检查器，或者如果没有工具调用存在（即 LLM 已经对查询给出了响应），则结束。
...
<Tip>
有关上述运行的详细信息，请参阅 [LangSmith 跟踪](https://smith.langchain.com/public/a6a96896-686a-4040-b9b5-28d701453d6f/r)。
</Tip>


## 6. 实现人在回路中审查

在执行代理的 SQL 查询之前进行检查是谨慎的做法，以避免任何意外操作或低效。

在这里，我们利用 LangGraph 的[人在回路中](/oss/javascript/langgraph/interrupts)功能，在执行 SQL 查询之前暂停运行并等待人工审查。使用 LangGraph 的[持久化层](/oss/javascript/langgraph/persistence)，我们可以无限期地暂停运行（或至少在持久化层存活期间）。

让我们将 `sql_db_query` 工具包装在一个接收人工输入的节点中。我们可以使用 [interrupt](/oss/javascript/langgraph/interrupts) 函数来实现这一点。下面，我们允许输入以批准工具调用、编辑其参数或提供用户反馈。
...
<Note>
上述实现遵循了更广泛的[人在回路中](/oss/javascript/langgraph/interrupts)指南中的[工具中断示例](/oss/javascript/langgraph/interrupts#interrupts-in-tools)。有关详细信息和替代方案，请参阅该指南。
</Note>

现在让我们重新组装我们的图。我们将用人工审查替换程序化检查。请注意，我们现在包含了一个[检查点](/oss/javascript/langgraph/persistence)；这是暂停和恢复运行所必需的。
...
```
我们可以使用 [Command](/oss/javascript/langgraph/use-graph-api#combine-control-flow-and-state-updates-with-command) 来接受或编辑工具调用：
...
有关详细信息，请参阅[人在回路中指南](/oss/javascript/langgraph/interrupts)。
