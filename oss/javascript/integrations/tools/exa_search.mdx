...
Exa（以前称为 Metaphor Search）是一个完全为 LLM 设计的搜索引擎。使用自然语言查询在互联网上搜索文档，然后从所需的文档中检索清理过的 HTML 内容。

与基于关键字的搜索（Google）不同，Exa 的神经搜索功能使其能够从语义上理解查询并返回相关文档。例如，我们可以搜索 `"fascinating article about cats"` 并比较 Google 和 Exa 的搜索结果。Google 给出的是基于关键字“fascinating”的 SEO 优化的列表。Exa 则能直接给出结果。
...
首先，获取一个 Exa API 密钥并将其添加为环境变量。[在此处](https://dashboard.exa.ai/login)注册可获得每月 1000 次免费搜索。

```typescript
process.env.EXASEARCH_API_KEY="your-api-key"
```

设置 LangSmith 进行一流的可观测性也很有帮助（但不是必须的）：
...
在这里，我们展示如何实例化 `ExaSearchResults` 工具的实例：
...
### [直接带参数调用](/oss/javascript/langchain/tools)
...
我们还可以使用模型生成的 `ToolCall` 调用该工具，在这种情况下将返回 [`ToolMessage`](https://reference.langchain.com/javascript/langchain-core/messages/ToolMessage)：
...
我们可以通过先将工具绑定到[工具调用模型](/oss/javascript/langchain/tools)然后调用它来在链中使用我们的工具：
...
我们将使用 LangGraph 创建代理。确保您已安装 `@langchain/langgraph`：
...
有关所有 `ExaSearchResults` 功能和配置的详细文档，请前往 [API 参考](https://api.js.langchain.com/classes/langchain_exa.ExaSearchResults.html)
