...
>[Kai Data API](https://www.kay.ai/) 专为 RAG 构建 🕵️ 我们正在策划全球最大的数据集作为高质量嵌入，以便您的 AI 代理可以即时检索上下文。最新模型、快速检索和零基础设施。

本笔记本向您展示如何检索 [Kay](https://kay.ai/) 支持的数据集。您目前可以搜索 `SEC Filings` 和 `美国公司的新闻稿`。访问 [kay.ai](https://kay.ai) 获取最新数据发布。如有任何问题，请加入我们的 [discord](https://discord.gg/hAnE4e5T6M) 或 [在推特上联系我们](https://twitter.com/vishalrohra_)。
...
* `dataset_id: string` 必需 -- Kay 数据集 ID。这是关于特定实体（如公司、人员或地点）的数据集合。例如，尝试 `"company"`
* `data_type: List[string]` 可选 -- 这是基于数据集来源或格式的类别，例如“company”数据集中的“SEC Filings”、“Press Releases”或“Reports”。例如，在“company”数据集下尝试 ["10-K", "10-Q", "PressRelease"]。如果留空，Kay 将跨所有类型检索最相关的上下文。
