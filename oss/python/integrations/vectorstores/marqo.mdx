---
title: "Marqo 集成"
description: "使用 LangChain Python 集成 Marqo 向量存储。"
---

本 notebook 展示了如何使用与 Marqo 向量存储相关的功能。

>[Marqo](https://www.marqo.ai/) 是一个开源向量搜索引擎。Marqo 允许您存储和查询文本、图像等多模态数据。Marqo 使用大量开源模型为您创建向量，您也可以提供自己微调的模型，Marqo 将负责加载和推理。

使用本集成需要通过 `pip install -qU langchain-community` 安装 `langchain-community`。

要使用我们的 Docker 镜像运行此 notebook，请先执行以下命令获取 Marqo：
...
+ 如果您有另一个应用程序中的图像和文本对数据库，可以直接在 LangChain 中通过 Marqo 向量存储使用它。请注意，引入自定义多模态索引会禁用 `add_texts` 方法。

+ 如果您有文本文档数据库，可以将其引入 LangChain 框架，并通过 `add_texts` 添加更多文本。

通过在搜索方法中向 `page_content_builder` 回调传入自定义函数，可以自定义返回的文档。
...
我们还提供 Marqo 的加权查询功能，这是一种组合复杂语义搜索的强大方式。
