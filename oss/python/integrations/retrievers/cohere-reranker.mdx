---
title: "Cohere 重排序器集成"
description: "使用 LangChain Python 与 Cohere 重排序器检索器集成。"
---
...
现在让我们用 `ContextualCompressionRetriever` 包装我们的基础检索器。我们将添加一个 `CohereRerank`，它使用 Cohere 重排序端点来对返回的结果进行重排序。
请注意，在 CohereRerank 中必须指定模型名称！
