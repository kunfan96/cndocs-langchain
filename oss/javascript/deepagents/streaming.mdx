...
Deep Agents 建立在 LangGraph 的流式基础设施之上，对子代理流提供一流支持。当深度代理将工作委托给子代理时，您可以独立地从每个子代理流式传输更新——实时跟踪进度、LLM 令牌和工具调用。
...
Deep Agents 使用 LangGraph 的子图流式传输来呈现来自子代理执行的事件。要接收子代理事件，请在流式传输时启用 `stream_subgraphs`。
...
当启用 `subgraphs` 时，每个流式事件都包含一个**命名空间**，用于标识是哪个代理产生的。命名空间是节点名称和任务 ID 的路径，代表代理层次结构。
...
在子代理工具内使用 `config.writer` 发出自定义进度事件：
...
let lastSource = "";
let midLine = false; // 当我们写入了令牌但没有尾随换行符时为 true
...
- [子代理](/oss/javascript/deepagents/subagents)—使用 Deep Agents 配置和使用子代理
- [前端流式传输](/oss/javascript/deepagents/streaming/frontend)—使用 `useStream` 为 Deep Agents 构建 React UI
