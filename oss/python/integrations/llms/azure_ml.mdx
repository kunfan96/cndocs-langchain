...
你必须[在 Azure ML 上部署模型](https://learn.microsoft.com/en-us/azure/machine-learning/how-to-use-foundation-models?view=azureml-api-2#deploying-foundation-models-to-endpoints-for-inferencing)或[部署到 Azure AI Foundry（原 Azure AI Studio）](https://learn.microsoft.com/en-us/azure/ai-studio/how-to/deploy-models-open)并获取以下参数：

* `endpoint_url`：端点提供的 REST 端点 URL。
* `endpoint_api_type`：将模型部署到**专用端点**（托管的基础设施）时使用 `endpoint_type='dedicated'`。使用**按需付费**产品（模型即服务）部署模型时使用 `endpoint_type='serverless'`。
