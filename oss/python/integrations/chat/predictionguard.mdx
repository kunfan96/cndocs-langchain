...
>[Prediction Guard](https://predictionguard.com) 是一个安全、可扩展的生成式 AI 平台，可保护敏感数据、防止常见的 AI 故障，并可在经济实惠的硬件上运行。

## 概述

### 集成详情

此集成使用 Prediction Guard API，该 API 包含各种安全防护和安全功能。
...
要访问 Prediction Guard 模型，[请联系 Prediction Guard](https://predictionguard.com/get-started) 获取 API 密钥并开始使用。
...
Prediction Guard 有一个工具调用 API，允许您描述工具及其参数，这使得模型能够返回一个包含要调用的工具及其输入的 JSON 对象。工具调用对于构建使用工具的链和代理，以及更普遍地从模型获取结构化输出非常有用。

### ChatPredictionGuard.bind_tools()

使用 `ChatPredictionGuard.bind_tools()`，您可以将 Pydantic 类、字典模式和 LangChain 工具作为工具传递给模型，然后这些工具会被重新格式化以供模型使用。
...
请注意，AIMessage 有一个 [`tool_calls`](https://reference.langchain.com/python/langchain/messages/#langchain.messages.AIMessage.tool_calls) 属性。它包含标准化的 `ToolCall` 格式，与模型提供商无关。
...
使用 Prediction Guard，您可以使用我们的输入检查之一来保护模型输入免受 PII 或提示注入的影响。更多信息请参阅 [Prediction Guard 文档](https://docs.predictionguard.com/docs/process-llm-input/)。
...
使用 Prediction Guard，您可以使用事实性检查来验证模型输出，以防止幻觉和错误信息，并使用毒性检查来防止有毒响应（例如脏话、仇恨言论）。更多信息请参阅 [Prediction Guard 文档](https://docs.predictionguard.com/docs/validating-llm-output)。
