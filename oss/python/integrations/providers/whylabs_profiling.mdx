...
>[WhyLabs](https://docs.whylabs.ai/docs/) 是一个可观测性平台，旨在监控数据管道和机器学习应用，以检测数据质量退化、数据漂移和模型性能下降。该平台建立在名为 `whylogs` 的开源包之上，使数据科学家和工程师能够：
>
>- 快速设置：使用轻量级开源库 whylogs，开始为任何数据集生成统计概况。
>- 将数据集概况上传到 WhyLabs 平台，以集中且可定制地监控/告警数据集特征以及模型输入、输出和性能。
>- 无缝集成：可与任何数据管道、机器学习基础设施或框架互操作。为您的现有数据流生成实时洞察。在此处了解更多关于我们的集成。
>- 扩展至 TB 级：处理您的大规模数据，同时保持较低的计算需求。与批处理或流式数据管道集成。
...
> *注意*：回调支持直接将这些变量传递给回调，当未直接传递认证信息时，它将默认使用环境变量。直接传递认证信息允许将概况写入 WhyLabs 中的多个项目或组织。
