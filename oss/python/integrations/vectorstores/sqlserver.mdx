...
Azure SQL 是一项兼具可扩展性、安全性和高可用性的强大服务，提供现代数据库解决方案的所有优势。它利用先进的查询优化器和企业级功能，在执行传统 SQL 查询的同时进行向量相似度搜索，增强数据分析和决策能力。
...
代码位于集成包中：[langchain-sqlserver](https:\github.com\langchain-ai\langchain-azure\tree\main\libs\sqlserver)。
...
运行本笔记本无需凭据，只需确保已下载 `langchain-sqlserver` 包。
如果希望获得一流的模型调用自动追踪功能，也可以通过取消以下注释来设置你的 [LangSmith](/langsmith/home) API 密钥：
...
在 Azure 门户的数据库设置中查找你的 Azure SQL DB 连接字符串。
...
本示例使用 Azure OpenAI 生成嵌入，但你也可以使用 LangChain 提供的其他嵌入模型。
...
向量存储支持一组可应用于文档元数据字段的过滤器。此功能使开发人员和数据分析师能够精细化查询，确保搜索结果与需求准确匹配。通过基于特定元数据属性的过滤，用户可以缩小搜索范围，只关注最相关的数据子集。
...
有关可在 Azure SQL 向量存储上执行的各种搜索的完整列表，请参阅 [API 参考](https://python.langchain.com/api_reference/sqlserver/index.html)。
...
以下示例演示如何将文件从 Azure Blob Storage 容器加载到 SQL 向量存储（文档拆分为块后）。
[Azure Blob Storage](https://learn.microsoft.com/en-us/azure/storage/blobs/storage-blobs-introduction) 是 Microsoft 面向云端的对象存储解决方案，专为存储海量非结构化数据而优化。
...
问答功能允许用户就故事、人物和事件提出具体问题，并获得简洁、富含上下文的答案。这不仅加深了对书籍的理解，还让用户感觉自己置身于这个神奇的世界中。

## 转换为检索器进行查询

LangChain 向量存储通过高效的相似度搜索简化了复杂问答系统的构建，可根据用户查询找到最相关的前 10 篇文档。**retriever** 由 **vector\_store** 创建，问答链使用 **create\_stuff\_documents\_chain** 函数构建。使用 **ChatPromptTemplate** 类创建提示模板，确保响应结构化且富含上下文。在问答应用中，向用户展示答案的来源文档往往非常重要。LangChain 内置的 **create\_retrieval\_chain** 会将检索到的来源文档以"context"键传递到输出中：

了解更多关于 LangChain RAG 教程及上述术语，请点击[此处](/oss/python/langchain/rag)。
...
有关 SQLServer 向量存储功能和配置的详细文档，请参阅 API 参考：[https://python.langchain.com/api\_reference/sqlserver/index.html](https:\python.langchain.com\api_reference\sqlserver\index.html)
