...
此笔记本展示了如何在检索器中使用 [watsonx 的重排序端点](https://cloud.ibm.com/apidocs/watsonx-ai#text-rerank)。
...
出于实验目的，也请安装 `faiss` 或 `faiss-cpu` 包：
...
让我们首先初始化一个简单的向量存储检索器，并存储 2023 年国情咨文演讲（分块）。我们可以设置检索器以检索大量（20）文档。

初始化 `WatsonxEmbeddings`。有关更多详细信息，请参见 [WatsonxEmbeddings](/oss/python/integrations/embeddings/ibm_watsonx)。

**注意**：

- 为了提供 API 调用的上下文，您必须添加 `project_id` 或 `space_id`。有关更多信息，请参见 [文档](https://www.ibm.com/docs/en/watsonx-as-a-service?topic=projects)。
- 根据您配置的服务实例所在区域，使用 [特定区域的认证 URL](https://ibm.github.io/watsonx-ai-python-sdk/setup_cloud.html#authentication) 之一。
...
初始化 `ChatWatsonx`。有关更多详细信息，请参见 [ChatWatsonx](/oss/python/integrations/chat/ibm_watsonx)。
