...
LangGraph 提供两种不同的 API 来构建智能体工作流：**图 API** 和**函数式 API**。这两种 API 共享相同的底层运行时，并且可以在同一应用程序中结合使用，但它们是为不同的用例和开发偏好而设计的。
...
当您需要并行运行多个操作然后合并结果时，图 API 可以自然地处理这一点。
...
[函数式 API](/oss/python/langgraph/functional-api) 使用命令式方法，将 LangGraph 功能集成到标准过程式代码中。
...
您可以在同一应用程序中结合使用这两种 API。当系统的不同部分有不同需求时，这非常有用。
...
当您希望以最小的更改将 LangGraph 功能添加到现有代码、拥有简单的线性工作流或需要快速原型开发能力时，请选择**函数式 API**。

两种 API 都提供相同的核心 LangGraph 功能（持久化、流式处理、Human in the Loop、记忆），但以不同的范式封装它们，以适应不同的开发风格和用例。
