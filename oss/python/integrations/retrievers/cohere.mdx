...
本笔记本介绍如何开始使用 `Cohere RAG` 检索器。这使你能够利用通过各种连接器搜索文档或提供自己文档的能力。
...
```python
_pretty_print(await rag.ainvoke("What is cohere ai?"))  # 异步版本
```
...
请注意，连接器和文档不能同时使用。如果你选择在 `invoke` 方法中提供文档，它们将优先使用，连接器对于该特定请求将不会被使用，如上面的代码片段所示！
