---
title: "YDB 集成"
description: "使用 LangChain Python 与 YDB 向量存储进行集成。"
---

> [YDB](https://ydb.tech/) 是一个多功能的开源分布式 SQL 数据库，它结合了高可用性、可扩展性、强一致性和 ACID 事务。它同时支持事务性（OLTP）、分析性（OLAP）和流式工作负载。
...
本笔记本没有凭据，只需确保您已按上述方式安装软件包。

如果您希望获得最佳的模型调用自动化跟踪，也可以通过取消注释下方内容来设置您的 [LangSmith](/langsmith/home) API 密钥：
...
您可以使用 `add_documents` 函数向向量存储添加项。
...
您可以使用 `delete` 函数通过 ID 从向量存储中删除项。
...
以下是如何将向量存储转换为检索器，然后使用简单查询和筛选器调用检索器。
