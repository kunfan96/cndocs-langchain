---
title: "Log10 集成"
description: "使用 LangChain Python 与 Log10 集成。"
---
...
Log10 是一个[开源](https://github.com/log10-io/log10)的无代理 LLM 数据管理和应用开发平台，允许您记录、调试和标记您的 LangChain 调用。

## 快速开始

1. 在 [log10.io](https://log10.io) 创建您的免费账户。
2. 分别从“设置”和“组织”选项卡中获取您的 `LOG10_TOKEN` 和 `LOG10_ORG_ID`，并将它们添加为环境变量。
3. 同时将 `LOG10_URL=https://log10.io` 和您常用的 LLM API 密钥（例如 `OPENAI_API_KEY` 或 `ANTHROPIC_API_KEY`）添加到您的环境变量中。
...
您也可以混合使用直接的 OpenAI 调用和 LangChain LLM 调用：
