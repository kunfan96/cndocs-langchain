...
description: "使用 LangChain JavaScript 集成 MistralAIEmbeddings 嵌入模型。"
---

本指南将帮助你开始使用 LangChain 的 MistralAIEmbeddings [嵌入模型](/oss/javascript/integrations/embeddings)。有关 `MistralAIEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-mistralai/MistralAIEmbeddings)。

## 概述

### 集成详情

| 类 | 包 | 本地 | [Python 支持](https://python.langchain.com/docs/integrations/embeddings/mistralai/) | 下载量 | 版本 |
| :--- | :--- | :---: | :---: |  :---: | :---: |
...
要访问 MistralAI 嵌入模型，你需要创建一个 MistralAI 账户，获取 API 密钥，并安装 `@langchain/mistralai` 集成包。

### 凭证

前往 [console.mistral.ai](https://console.mistral.ai/) 注册 `MistralAI` 并生成 API 密钥。完成后，设置 `MISTRAL_API_KEY` 环境变量：

```bash
export MISTRAL_API_KEY="your-api-key"
```

如果你希望自动跟踪模型调用，也可以取消注释以下行来设置你的 [LangSmith](/langsmith/home) API 密钥：
...
嵌入模型通常用于检索增强生成（RAG）流程中，既作为索引数据的一部分，也用于后续检索。更多详细说明，请参阅 [**学习** 标签页](/oss/javascript/learn/) 下的 RAG 教程。
...
你可以直接调用这些方法来获取你自己的用例所需的嵌入。

### 嵌入单个文本

你可以使用 `embedQuery` 嵌入用于搜索的查询。这会生成特定于查询的向量表示：
...
你可以使用 `embedDocuments` 嵌入多个文本用于索引。此方法内部使用的机制可能（但不一定）与嵌入查询不同：
...
有关所有 `MistralAIEmbeddings` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-mistralai/MistralAIEmbeddings)。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/javascript/integrations/embeddings/mistralai.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>