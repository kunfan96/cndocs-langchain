...
使用 `BigtableSaver.add_documents(<documents>)` 保存 LangChain 文档。要初始化 `BigtableSaver` 类，您需要提供 2 件事：
...
默认情况下，[`Document`](https://reference.langchain.com/python/langchain-core/documents/base/Document) 对象上的 `metadata` 映射将包含一个键 `rowkey`，其值为行的 rowkey 值。要向该映射添加更多项目，请使用 metadata_mapping。
...
如果 Bigtable 中有一个包含 JSON 字符串的列，您希望将其添加到输出文档元数据中，可以向 BigtableLoader 添加以下参数。请注意，`metadata_as_json_encoding` 的默认值是 UTF-8。
