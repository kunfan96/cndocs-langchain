...
* **以下之一**：
  ...
  4. **`autoExtractOntology: true`**（不建议用于生产环境——参见说明）

`graphUri` 与本体

* **`graphUri`**：
  您的 SAP HANA Cloud 实例中包含实例数据（有时超过 10 万个三元组）的命名图。
  如果未提供 `graphUri`，或提供 `""` 或 `"DEFAULT"`，则使用默认图。
* **本体**：一个精简的模式（通常约 50-100 个三元组），描述类、属性、定义域、值域、标签、注释和子类关系。本体指导 SPARQL 生成和结果解释。

### 使用 **DEFAULT** 图创建图实例

有关默认图的更多信息，请参阅 [默认图与命名图](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-knowledge-graph-guide/default-graph-and-named-graphs)。
...
使用自定义 `CONSTRUCT` 查询来选择性地提取模式三元组。
...
> **注意**：不建议在生产环境中使用自动提取——它通常会省略重要的三元组，如 `rdfs:label`、`rdfs:comment` 和 `rdfs:subClassOf`。
