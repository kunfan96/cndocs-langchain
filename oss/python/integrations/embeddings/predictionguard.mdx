...
本集成展示了如何将 Prediction Guard 嵌入集成与 LangChain 一起使用。此集成支持文本和图像，可以单独使用，也可以作为匹配对一起使用。
...
首先，安装 Prediction Guard 和 LangChain 包。然后，设置所需的环境变量并设置包导入。
...
Prediction Guard 嵌入生成支持文本和图像。此集成包含跨各种函数的支持。
...
向量存储和检索器实现调用 `embeddings.embed_documents(...)` 和 `embeddings.embed_query(...)` 来为 `from_texts` 和检索 `invoke` 操作中使用的文本创建嵌入。
