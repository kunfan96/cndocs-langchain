---
title: "Neo4j 向量索引集成"
description: "使用 LangChain Python 集成 Neo4j 向量索引向量存储。"
---

>[Neo4j](https://neo4j.com/) 是一个内置向量相似度搜索支持的开源图数据库。
...
上面我们从头创建了一个向量存储。然而，通常我们希望使用现有的向量存储。为此，我们可以直接初始化它。
...
我们也可以使用 `from_existing_graph` 方法从现有图中初始化向量存储。此方法从数据库中提取相关文本信息，并将文本嵌入计算后存储回数据库。
...
Neo4j 还支持关系向量索引，其中嵌入向量存储为关系属性并建立索引。关系向量索引无法通过 LangChain 填充，但您可以连接到现有的关系向量索引。
...
Neo4j 向量存储还支持通过结合并行运行时和精确最近邻搜索来进行元数据过滤。
...
您也可以通过使用自定义 Cypher 代码片段来自定义响应，该片段可从图中获取其他信息。
...
以下是将除 `embedding` 之外的所有节点属性作为字典传递给 `text` 列的示例：
...
您也可以向检索查询传递 Cypher 参数。参数可用于附加过滤、遍历等操作...
...
Neo4j 同时集成了向量索引和关键词索引，允许您使用混合搜索方法
...
本节介绍如何使用 `RetrievalQAWithSourcesChain` 对索引进行带来源的问答。
