...
当单个 agent 拥有太多[工具](/oss/javascript/langchain/tools)并对使用哪个做出糟糕的决策时，当任务需要具有大量上下文的专业知识（长提示词和特定领域的工具）时，或者当您需要强制执行顺序约束仅在满足某些条件后才解锁功能时，多 agent 模式特别有价值。

<Tip>
多 agent 设计的核心是**[上下文工程](/oss/javascript/langchain/context-engineering)**——决定每个 agent 看到什么信息。您系统的质量取决于确保每个 agent 能够访问其任务所需的正确数据。
</Tip>
...
| 模式 | 工作原理 |
|--------------|--------------|
| [**子 agent**](/oss/javascript/langchain/multi-agent/subagents) | 主 agent 将子 agent 作为工具进行协调。所有路由都通过主 agent，由主 agent 决定何时以及如何调用每个子 agent。 |
| [**交接**](/oss/javascript/langchain/multi-agent/handoffs) | 行为根据状态动态变化。工具调用更新状态变量以触发路由或配置更改，切换 agent 或调整当前 agent 的工具和提示词。 |
| [**技能**](/oss/javascript/langchain/multi-agent/skills) | 按需加载专业化的提示词和知识。单个 agent 保持控制，同时根据需要从技能中加载上下文。 |
| [**路由器**](/oss/javascript/langchain/multi-agent/router) | 路由步骤对输入进行分类并将其定向到一个或多个专业化 agent。结果被综合成一个组合响应。 |
| [**自定义工作流**](/oss/javascript/langchain/multi-agent/custom-workflow) | 使用 [LangGraph](/oss/javascript/langgraph/overview) 构建定制执行流程，混合确定性逻辑和 agent 行为。将其他模式作为节点嵌入到您的工作流中。 |
...
|---------|:-----------------------:|:---------------:|:----------:|:-----------------------:|
| [**子 agent**](/oss/javascript/langchain/multi-agent/subagents) | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐ |
...
- **处理的 token**：所有调用中的总[上下文窗口](/oss/javascript/langchain/context-engineering)使用量。更多 token = 更高的处理成本和潜在的上下文限制。
...
| 模式 | 模型调用 | 最佳选择 |
|---------|:-----------:|:--------:|
| [**子 agent**](/oss/javascript/langchain/multi-agent/subagents) | 4 | |
...
| 优化目标 | [子 agent](/oss/javascript/langchain/multi-agent/subagents) | [交接](/oss/javascript/langchain/multi-agent/handoffs) | [技能](/oss/javascript/langchain/multi-agent/skills) | [路由器](/oss/javascript/langchain/multi-agent/router) |
|--------------|:---------:|:--------:|:------:|:------:|
| 单次请求 | | ✅ | ✅ | ✅ |
| 重复请求 | | ✅ | ✅ | |
| 并行执行 | ✅ | | | ✅ |
