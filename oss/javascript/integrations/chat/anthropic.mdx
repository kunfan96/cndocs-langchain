...
[Anthropic](https://www.anthropic.com/) 是一家人工智能安全与研究公司。他们是 Claude 的创造者。

这将帮助你开始使用 `ChatAnthropic` [聊天模型](/oss/javascript/langchain/models)。有关所有 `ChatAnthropic` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/javascript/langchain-anthropic/ChatAnthropic)。
...
需要注意 Anthropic 模型与大多数其他模型之间的一个关键区别：单个 Anthropic [`AIMessage`](https://reference.langchain.com/javascript/langchain-core/messages/AIMessage) 的内容可以是单个字符串，也可以是**内容块列表**。例如，当 Anthropic 模型[调用工具](/oss/javascript/langchain/tools)时，工具调用是消息内容的一部分（同时也暴露在标准化的 `AIMessage.tool_calls` 字段中）：
...
Anthropic 支持[缓存提示的某些部分](https://platform.claude.com/docs/en/build-with-claude/prompt-caching)，以降低需要长上下文的用例的成本。你可以缓存工具以及整个消息或单个块。

包含一个或多个块或工具定义且带有 `"cache_control": { "type": "ephemeral" }` 字段的初始请求将自动缓存该部分提示。此初始缓存步骤会产生额外费用，但后续请求将以降低的费率计费。缓存的生命周期为 5 分钟，但每次命中缓存时都会刷新。对于更长的缓存时间，请在 `cache_control` 字段中指定 `"ttl": "1h"`。

存在一个最小可缓存提示长度，根据模型而异。有关更多信息，请参阅[提示缓存详情](https://platform.claude.com/docs/en/build-with-claude/prompt-caching#cache-limitations)。

以下是缓存包含 LangChain [概念文档](/oss/javascript/concepts/)的系统消息部分的示例：
...
<Note>
**此处的信息指的是接收模型文本输出并尝试将其解析为更结构化表示形式的解析器。**

越来越多的模型支持函数（或工具）调用，这可以自动处理此问题。
建议使用函数/工具调用而不是输出解析。
参见 [LangChain 工具文档](/oss/javascript/langchain/tools)。

</Note>

负责接收模型的输出并将其转换为更适合下游任务的格式。
当你使用 LLM 生成结构化数据，或规范化聊天模型和 LLM 的输出时，这非常有用。

输出解析器必须实现两个主要方法：

- “获取格式指令”：一个返回字符串的方法，其中包含有关如何格式化语言模型输出的说明。
- “解析”：一个接收字符串（假定为语言模型的响应）并将其解析为某种结构的方法。

以及一个可选方法：

- “带提示解析”：一个接收字符串（假定为语言模型的响应）和提示（假定为生成该响应的提示）并将其解析为某种结构的方法。提供提示主要是为了在输出解析器希望以某种方式重试或修复输出时，需要从中获取信息。
...
**输入类型**：预期的输入类型。大多数输出解析器适用于字符串和消息，但有些（如 OpenAI Functions）需要具有特定参数的消息。
...
我们可以看到，在 Anthropic 返回的原始使用量字段中有一个名为 `cache_creation_input_tokens` 的新字段。
...
你也可以通过在工具定义中设置相同的 `"cache_control": { "type": "ephemeral" }` 来缓存工具。这目前要求你以 [Anthropic 的原始工具格式](https://platform.claude.com/docs/en/agents-and-tools/tool-use/overview)绑定工具。以下是一个示例：
...
Anthropic 模型[可能托管在 Google Vertex 等云服务上](https://platform.claude.com/docs/en/build-with-claude/claude-on-vertex-ai)，这些服务依赖于与主要 Anthropic 客户端具有相同接口的不同底层客户端。你可以通过提供一个返回已初始化 Anthropic 客户端实例的 `createClient` 方法来访问这些服务。以下是一个示例：
...
在此示例中，我们将[搜索结果](https://platform.claude.com/docs/en/build-with-claude/search-results)作为消息内容的一部分传递。这允许 Claude 在其响应中引用来自你自己的检索系统的特定段落或片段。

当你希望 Claude 引用来自特定知识集的信息，但希望直接带来你自己的预取/缓存内容，而不是让模型自动搜索或检索时，这种方法很有帮助。
...
你也可以使用工具提供搜索结果，模型可以在其响应中引用这些结果。这非常适合 RAG（或[检索增强生成](https://en.wikipedia.org/wiki/Retrieval-augmented_generation)）工作流，Claude 可以决定何时何地检索信息。当以[搜索结果](https://platform.claude.com/docs/en/build-with-claude/search-results)形式返回此信息时，它使 Claude 能够从工具返回的材料中创建引用。

以下是如何创建一个返回 Anthropic 引用 API 期望格式的搜索结果的工具：
...
了解有关 [RAG 在 LangChain 中如何工作](https://js.langchain.com/docs/concepts/rag/)的更多信息

[了解有关工具调用的更多信息](/oss/javascript/langchain/tools)

### 与文本分割器一起使用

Anthropic 还允许你使用[自定义文档](https://platform.claude.com/docs/en/build-with-claude/citations#custom-content-documents)类型指定自己的分割。LangChain 文本分割器可用于为此目的生成有意义的分割。参见下面的示例，我们分割 LangChain.js README（一个 markdown 文档）并将其作为上下文传递给 Claude：
