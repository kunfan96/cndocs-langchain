...
Anchor 是一个用于 AI 智能体浏览器自动化的平台，它解决了为缺乏 API 或 API 覆盖有限的 Web 应用程序自动化工作流程的挑战。它简化了基于浏览器的自动化的创建、部署和管理，将复杂的 Web 交互转化为简单的 API 端点。

本指南提供了开始使用 Anchor 浏览器工具的快速概览。有关 Anchor 浏览器的更多信息，请访问 [Anchorbrowser.io](https://anchorbrowser.io?utm=langchain) 或 [Anchor 浏览器文档](https://docs.anchorbrowser.io?utm=langchain)。

## 概览

### 集成详情

用于 LangChain 的 Anchor 浏览器包是 [langchain-anchorbrowser](https://pypi.org/project/langchain-anchorbrowser)，当前最新版本为 ![PyPI - Version](https://img.shields.io/pypi/v/langchain-anchorbrowser?style=flat-square&label=%20)。
...
| `AnchorWebTaskToolKit` | langchain-anchorbrowser | 使用 AI 执行智能网页任务（简单与高级模式） | 见下文 |

`langchain-anchorbrowser` 中允许的参数仅分别是 Anchor 浏览器 API 参考中列出参数的子集：[获取网页内容](https://docs.anchorbrowser.io/sdk-reference/tools/get-webpage-content?utm=langchain)、[网页截图](https://docs.anchorbrowser.io/sdk-reference/tools/screenshot-webpage?utm=langchain) 和 [执行网页任务](https://docs.anchorbrowser.io/sdk-reference/ai-tools/perform-web-task?utm=langchain)。

**信息：** Anchor 目前为 langchain 实现了 `SimpleAnchorWebTaskTool` 和 `AdvancedAnchorWebTaskTool` 工具，并配合 `browser_use` 智能体使用。
...
使用您的 Anchor 浏览器凭证。根据需要在 Anchor 浏览器 [API 密钥页面](https://app.anchorbrowser.io/api-keys?utm=langchain) 获取它们。
...
我们也可以使用模型生成的 ToolCall 来调用工具，这种情况下将返回一个 ToolMessage：
...
我们可以通过首先将工具绑定到[工具调用模型](/oss/python/langchain/tools/)，然后调用它，从而在链中使用我们的工具：
