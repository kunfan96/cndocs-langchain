...
默认情况下，结果以逗号分隔的键值对字符串形式返回。您也可以通过设置 `output_format="list"` 将搜索结果作为列表返回，或通过设置 `output_format="json"` 作为 JSON 字符串返回。
...
您也可以直接向 `DuckDuckGoSearchResults` 传递自定义的 `DuckDuckGoSearchAPIWrapper`，以对搜索结果进行更多控制。
