...
基于字符的分割是最简单的文本分割方法。它使用指定的字符序列（默认为 `"\n\n"`）来分割文本，块长度以字符数衡量。
...
您可以选择以下方法：
- `.splitText` — 返回纯字符串块。
- `.createDocuments` — 返回 LangChain [Document](https://reference.langchain.com/javascript/langchain-core/documents/Document) 对象，当需要为下游任务保留元数据时非常有用。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/integrations/splitters/character_text_splitter.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
