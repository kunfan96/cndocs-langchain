...
LangFair 是一个全面的 Python 库，专为对大语言模型（LLM）使用场景进行偏见和公平性评估而设计。LangFair [仓库](https://github.com/cvs-health/langfair)包含一套完整的框架，用于[选择偏见和公平性指标](https://github.com/cvs-health/langfair/tree/main#-choosing-bias-and-fairness-metrics-for-an-llm-use-case)，以及[演示 notebook](https://github.com/cvs-health/langfair/tree/main/examples) 和讨论 LLM 偏见与公平性风险、评估指标和最佳实践的[技术手册](https://arxiv.org/abs/2407.10853)。

请访问我们的[文档站点](https://cvs-health.github.io/langfair/)，获取 LangFair 使用的详细说明。

## ⚡ 快速入门指南
### （可选）为使用 LangFair 创建虚拟环境
我们建议在安装 LangFair 之前使用 venv 创建一个新的虚拟环境。请按照[此处](https://docs.python.org/3/library/venv.html)的说明操作。
...
### 使用示例
以下代码示例演示如何使用 LangFair 评估文本生成和摘要使用场景中的偏见与公平性风险。以下示例假设用户已为其使用场景定义了一个提示词列表 `prompts`。

##### 生成 LLM 响应
要生成响应，我们可以使用 LangFair 的 `ResponseGenerator` 类。首先，我们必须创建一个 `langchain` LLM 对象。以下使用 `ChatVertexAI`，但**也可以使用 [LangChain 的任何 LLM 类](https://js.langchain.com/docs/integrations/chat/)**。注意，`InMemoryRateLimiter` 用于避免速率限制错误。
```python
...
##### 计算毒性指标
毒性指标可使用 `ToxicityMetrics` 计算。注意，`torch.device` 是可选的，如果有 GPU 可用，应使用它来加速毒性计算。
```python
...
##### 计算刻板印象指标
刻板印象指标可使用 `StereotypeMetrics` 计算。
```python
...
反事实指标可使用 `CounterfactualMetrics` 轻松计算。
```python
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/providers/langfair.mdx) 或 [提交 issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
