...
>[you.com API](https://api.you.com) 是一套旨在帮助开发者将大语言模型的输出基于最新、最准确、最相关信息进行落地的工具集，这些信息可能未包含在其训练数据集中。

## 设置

检索器位于 `langchain-community` 包中。

你还需要设置你的 you.com API 密钥。
...
```python
# .results 返回解析后的结果，每个代码片段都在一个 Document 中
response = utility.results(query="What is the weather in NY")

# .results 应该为每个 `snippet` 返回一个 Document
print(len(response))
