...
确保您的 SAP HANA 实例正在运行。从环境变量加载凭据并使用您首选的 HANA 客户端创建连接。
...
在 [什么是 SAP HANA?](https://www.sap.com/products/data-cloud/hana/what-is-sap-hana.html) 中了解有关 SAP HANA 的更多信息。

## 初始化

要初始化 `HanaDB` 向量存储，您需要数据库连接和嵌入实例。SAP HANA Cloud 向量引擎支持外部和内部嵌入。
...
或者，您可以使用其原生 `VECTOR_EMBEDDING()` 函数直接在 SAP HANA 中计算嵌入。如果您的 TypeScript 环境中有可用的内部嵌入支持，请类似地初始化并将其传递给 `HanaDB`。有关内部嵌入的更多信息，请参阅 [SAP HANA VECTOR_EMBEDDING 函数](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-vector-engine-guide/vector-embedding-function-vector)。
...
一旦您有了连接和嵌入实例，通过将它们连同用于存储向量的表名传递给 `HanaDB` 来创建向量存储：
...
一旦您创建了向量存储，我们可以通过添加和删除不同的项目与其进行交互。

### 向向量存储添加项目

我们可以使用 `addDocuments` 函数向我们的向量存储添加项目。
...
执行带有元数据搜索过滤的最大边际相关性 (MMR) 可以按如下方式完成：
...
const docsRetriever = await retriever.invoke("foobar", {
  filter: { quality: "good" },
});
...
向量索引可以显着加快向量的 top-k 最近邻查询。用户可以使用 `createHnswIndex` 函数创建分层导航小世界 (HNSW) 向量索引。
...
- [如何：使用 RAG 进行问答](https://js.langchain.com/docs/how_to/#qa-with-rag)
...
由于 HANA 的 dbapi 驱动程序默认将向量列输出为 Buffer 对象，我们将创建一个辅助函数将函数转换为数字列表。
...
rowVal.forEach((r) => {
  ...
  console.log(
    `VEC_VECTOR: ${(embedding.length, embedding.slice(0, 3).concat(["..."]).concat(embedding.slice(-3)))}`,
  ); // The vector
});
```
...
- 一个类型为 `NCLOB` 或 `NVARCHAR` 的列，用于嵌入的文本/上下文
- 一个类型为 `NCLOB` 或 `NVARCHAR` 的列，用于元数据
- 一个类型为 `REAL_VECTOR` 或 `HALF_VECTOR` 的列，用于嵌入向量

表可以包含额外的列。当新文档插入表中时，这些额外的列必须允许 NULL 值。
...
rowOwnTable.forEach((r) => {
  ...
  console.log(
    `MY_VECTOR: ${(embedding.length, embedding.slice(0, 3).concat(["..."]).concat(embedding.slice(-3)))}`,
  ); // The vector
});
```
...
为了允许灵活的元数据值，所有元数据默认作为 JSON 存储在元数据列中。如果已知一些使用的元数据键和值类型，可以通过创建以键名为列名的目标表并将它们通过 `specificMetadataColumns` 列表传递给 HanaDB 构造函数，将它们存储在额外的列中。匹配这些值的元数据键在插入期间被复制到特殊列中。过滤器对于 `specificMetadataColumns` 列表中的键使用特殊列而不是元数据 JSON 列。
...
特殊列对 langchain 接口的其余部分完全透明。一切都像以前一样工作，只是性能更高。

```typescript
const advancedFilter = { CUSTOMTEXT: { $like: "%value%" } };
const foundPerformantDocs = await dbPerformant.similaritySearch(
  "What's up?",
  2,
  advancedFilter,
);
...
将加载的文档块添加到表中。对于此示例，我们要删除表中可能存在的以前运行的任何先前内容。
...
`最大边际相关性` 针对与查询的相似性以及所选文档之间的多样性进行了优化。将从数据库中检索前 20 个 (fetch_k) 项目。然后 MMR 算法将找到最佳的 2 个 (k) 匹配项。
...
- **默认参数**：在 `createHnswIndex` 函数中，如果用户没有为 `m`、`efConstruction` 或 `efSearch` 等参数提供自定义值，则会自动使用默认值（例如 `m=64`、`efConstruction=128`、`efSearch=200`）。这些值确保在不需要用户干预的情况下以合理的性能创建索引。
...
<div className="source-links">
  <Callout icon="edit">
    [Edit this page on
    GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss/javascript/integrations/vectorstores/sap_hanavector.mdx)
