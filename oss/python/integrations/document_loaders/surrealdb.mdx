...
>**SurrealDB 的主要特点包括：**
>
>* **减少开发时间：** SurrealDB 通过移除大多数服务器端组件的需求来简化您的数据库和 API 技术栈，使您能够更快、更便宜地构建安全、高性能的应用程序。
>* **实时协作 API 后端服务：** SurrealDB 既作为数据库，也作为 API 后端服务，实现实时协作。
...
* `filter_criteria`：用于构建 `WHERE` 子句以从表中过滤结果的字典。
