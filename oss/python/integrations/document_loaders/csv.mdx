---
title: "CSV 集成"
description: "使用 LangChain Python 集成 CSV 文档加载器。"
---

>[逗号分隔值（CSV）](https://en.wikipedia.org/wiki/Comma-separated_values)文件是一种使用逗号分隔值的定界文本文件。文件的每一行是一条数据记录，每条记录由一个或多个字段组成，字段之间用逗号分隔。

以每行对应一个文档的方式加载 [csv](https://en.wikipedia.org/wiki/Comma-separated_values) 数据。
...
请参阅 [csv 模块](https://docs.python.org/3/library/csv.html)文档，了解支持哪些 csv 参数。
...
使用 `source_column` 参数为每行生成的文档指定来源。若未指定，则将 `file_path` 作为从 CSV 文件中创建的所有文档的来源。

当使用从 CSV 文件加载的文档构建基于来源回答问题的链时，此功能非常实用。
...
您也可以使用 `UnstructuredCSVLoader` 加载表格。使用 `UnstructuredCSVLoader` 的一个优势是，如果在 `"elements"` 模式下使用，表格的 HTML 表示将在元数据中可用。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/document_loaders/csv.mdx) 或[提交 Issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
