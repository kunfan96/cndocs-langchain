---
title: "自然语言 API 工具包集成"
description: "使用 LangChain Python 与自然语言 API 工具包工具集成。"
---

`Natural Language API` 工具包 (`NLAToolkits`) 允许 LangChain 代理高效地规划并组合跨端点的调用。

本笔记本演示了 `Speak`、`Klarna` 和 `Spoonacluar` API 的示例组合。
...
Question: 您必须回答的输入问题
Thought: 您应该始终思考要做什么
Action: 要采取的操作，应为 [{tool_names}] 之一
Action Input: 指示 AI Action 代表的内容。
Observation: 代理的响应
... (此 Thought/Action/Action Input/Observation 可重复 N 次)
Thought: 我现在知道最终答案了。用户无法看到我的任何观察、API 响应、链接或工具。
Final Answer: 对原始输入问题的最终答案，包含适量的细节

在回复最终答案时，请记住您回复的对象无法看到您的任何 Thought/Action/Action Input/Observations，因此如果其中有任何相关信息，您需要在回复中明确包含它。"""
```
...
```python
mrkl.invoke(
    {"input": "我有一个意大利语班级的年终派对，需要为此购买一些意大利服装"}
)
```
...
某些端点可能需要通过访问令牌等进行用户身份验证。这里我们展示如何通过 `Requests` 包装器对象传入身份验证信息。

由于每个 NLATool 都为其包装的 API 提供了一个简洁的自然语言接口，因此顶层对话代理可以更轻松地整合每个端点以满足用户的请求。

**添加 Spoonacular 端点。**

1. 访问 [Spoonacular API 控制台](https://spoonacular.com/food-api/console#Profile) 并创建一个免费账户。
2. 点击 `Profile` 并复制您的 API 密钥。
...
# 使查询更复杂！
user_input = (
    "我正在学习意大利语，我的语言班正在举办年终派对... "
    " 你能帮我找一套意大利服装来穿，并"
    " 找一个合适的食谱来准备，这样我就可以用意大利语为班级做演示吗？"
)
```
...
natural_language_api_tools[1].run(
    "告诉 LangChain 听众用意大利语说 'enjoy the meal'，谢谢！"
)
```
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/tools/openapi_nla.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
