...
详细说明请参阅：[启用三元组存储](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-knowledge-graph-guide/enable-triple-store/)

要将 SAP HANA 知识图谱引擎与 LangChain 一起使用，请安装 `langchain-hana` 包：

```python
pip install langchain_hana
```

首先，创建一个到您的 SAP HANA Cloud 实例的连接。
...
* **以下之一**：
  ...
  4. **`auto_extract_ontology=True`**（不建议用于生产环境——参见说明）
