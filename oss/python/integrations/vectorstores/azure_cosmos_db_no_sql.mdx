...
description: "使用 LangChain Python 与 Azure Cosmos DB NoSQL 向量存储集成。"
---
...
Azure Cosmos DB 是驱动 OpenAI ChatGPT 服务的数据库。它提供个位数毫秒的响应时间、自动且即时的可扩展性，以及在任何规模下保证的速度。

Azure Cosmos DB for NoSQL 现在提供预览版的向量索引和搜索功能。此功能旨在处理高维向量，支持在任何规模下进行高效且准确的向量搜索。您现在可以直接将向量与数据一起存储在文档中。这意味着数据库中的每个文档不仅可以包含传统的无模式数据，还可以包含作为文档其他属性的高维向量。这种数据和向量的同置存储允许进行高效的索引和搜索，因为向量与其所代表的数据存储在相同的逻辑单元中。这简化了数据管理、AI 应用程序架构以及基于向量的操作效率。
...
<Note>
自 `langchain-azure-cosmosdb` v1.0 起，所有 Azure Cosmos DB NoSQL 集成（向量存储、语义缓存、聊天消息历史记录以及 LangGraph 检查点、缓存和存储）均从独立的 [`langchain-azure-cosmosdb`](https://pypi.org/project/langchain-azure-cosmosdb/) 包发布，并从 `langchain_azure_cosmosdb` 导入。支持同步和异步客户端，以及 Microsoft Entra ID（`DefaultAzureCredential`）身份验证。
</Note>
...
# 将文档及其嵌入插入到 AzureCosmosDBNoSql 中
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/vectorstores/azure_cosmos_db_no_sql.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>