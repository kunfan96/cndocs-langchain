...
description: 开始在 LangChain 中使用旧版 Gemini LLM。
---

<Warning>
**您当前所在的页面介绍的是将 Google 模型用作文本补全模型的方法。许多热门 Google 模型是[对话补全模型](/oss/python/langchain/models)。**

您可能正在寻找[此页面](/oss/python/integrations/chat/google_generative_ai/)。
</Warning>

本指南介绍如何在 LangChain 中使用 [Google Generative AI](https://developers.generativeai.google/) 模型。注意：这与 Google Cloud Vertex AI [集成](/oss/python/integrations/llms/google_vertex_ai)是分开的。

## 设置

要使用 Google Generative AI，您必须安装 `langchain-google-genai` Python 包并生成 API 密钥。[了解更多详情](https://developers.generativeai.google/)。
...
Gemini 模型具有默认安全设置，可以被覆盖。如果您的模型产生了大量"安全警告"，可以尝试调整模型的 `safety_settings` 属性。例如，要关闭对危险内容的安全屏蔽，可以按如下方式构建 LLM：
...
关于可用类别和阈值的枚举，请参阅 Google 的[安全设置类型](https://ai.google.dev/api/python/google/generativeai/types/SafetySettingDict)。

---

<div className="source-links">
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/llms/google_generative_ai.mdx) 或 [提交 Issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
