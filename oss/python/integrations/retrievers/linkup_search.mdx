...
> [Linkup](https://www.linkup.so/) 提供了一个 API，用于将 LLM 连接到网络和 Linkup Premium Partner 来源。

这将帮助您开始使用 LinkupSearchRetriever [检索器](/oss/python/langchain/retrieval/)。

### 集成详情

| 检索器 | 来源 | 包 |
| :--- | :--- | :---: |
`LinkupSearchRetriever` | 网络和合作伙伴来源 | langchain-linkup |

## 设置

要使用 Linkup 提供商，您需要一个有效的 API 密钥，您可以通过[注册 Linkup](https://app.linkup.so/sign-up) 找到它。然后，您可以将其设置为 `LINKUP_API_KEY` 环境变量。对于下面的链示例，您还需要将 OpenAI API 密钥设置为 `OPENAI_API_KEY` 环境变量，您也可以在此处进行设置：
...
retriever = LinkupSearchRetriever(
    ...
    linkup_api_key=None,  # API 密钥可以在此处传递或设置为 LINKUP_API_KEY 环境变量
)
```
