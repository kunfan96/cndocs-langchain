---
title: "PDFLoader 集成"
description: "使用 LangChain JavaScript 集成 PDFLoader 文档加载器。"
---
...
本笔记本提供了快速入门 `PDFLoader` [文档加载器](/oss/javascript/integrations/document_loaders) 的概览。有关所有 `PDFLoader` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/javascript/langchain-community/document_loaders/fs/pdf/PDFLoader)。
...
| 类 | 包 | 兼容性 | 本地 | PY 支持 |
| :--- | :--- | :---: | :---: |  :---: |
| [`PDFLoader`](https://reference.langchain.com/javascript/langchain-community/document_loaders/fs/pdf/PDFLoader) | [`@langchain/community`](https://www.npmjs.com/package/@langchain/community) | 仅限 Node | ✅ | 🟠 (见下方说明) |

## 设置

要访问 `PDFLoader` 文档加载器，您需要安装 `@langchain/community` 集成包以及 `pdf-parse` 包。
...
默认情况下，我们使用 `pdf-parse` 捆绑的 `pdfjs` 构建版本，该版本与大多数环境兼容，包括 Node.js 和现代浏览器。如果您想使用更新版本的 `pdfjs-dist` 或者想使用自定义构建的 `pdfjs-dist`，可以通过提供一个自定义的 `pdfjs` 函数来实现，该函数返回一个解析为 `PDFJS` 对象的 Promise。

在以下示例中，我们使用 `pdfjs-dist` 的 "legacy"（参见 [pdfjs 文档](https://github.com/mozilla/pdf.js/wiki/Frequently-Asked-Questions#which-browsersenvironments-are-supported)）构建版本，该版本包含默认构建中未包含的多个 polyfill。
...
PDF 文件种类繁多，这使得读取它们具有挑战性。加载器默认解析单个文本元素并用空格将它们连接在一起，但如果您看到过多的空格，这可能不是期望的行为。在这种情况下，您可以像这样用空字符串覆盖分隔符：
...
有关所有 PDFLoader 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/javascript/langchain-community/document_loaders/fs/pdf/PDFLoader)。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/javascript/integrations/document_loaders/file_loaders/pdf.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>