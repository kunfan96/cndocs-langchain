---
title: "Langfuse 集成"
description: "使用 LangChain Python 与 Langfuse 集成。"
---
...
[Langfuse 追踪](https://langfuse.com/docs/tracing)通过 LangChain 回调机制（[Python](https://python.langchain.com/docs/how_to/#callbacks)、[JS](https://js.langchain.com/docs/how_to/#callbacks)）与 LangChain 集成。因此，Langfuse SDK 会为你的 LangChain 应用的每次运行自动创建一个嵌套追踪。这使你能够记录、分析和调试你的 LangChain 应用。

你可以通过 (1) 构造函数参数或 (2) 环境变量来配置集成。通过在 [cloud.langfuse.com](https://cloud.langfuse.com) 注册或[自托管 Langfuse](https://langfuse.com/self-hosting) 来获取你的 Langfuse 凭证。
...
# 将 Langfuse 处理器添加为回调（传统方式和 LCEL）
...
本节演示 [Langfuse](https://langfuse.com/docs) 如何帮助调试、分析和迭代你的 LangGraph 应用，使用 [LangChain 集成](https://langfuse.com/docs/integrations/langchain/tracing)。

### 初始化 langfuse

**注意：** 你需要至少运行 Python 3.11（[GitHub Issue](https://github.com/langfuse/langfuse/issues/1926)）。

使用你在 Langfuse UI 项目设置中的 [API 密钥](https://langfuse.com/faq/all/where-are-langfuse-api-keys)初始化 Langfuse 客户端，并将其添加到你的环境中。
...
# 从 https://cloud.langfuse.com 获取你的项目密钥
...
class State(TypedDict):
    # 消息的类型是 "list"。注解中的 `add_messages` 函数定义了如何更新此状态键
...
# 聊天机器人节点函数接受当前 State 作为输入，并返回更新后的消息列表。这是所有 LangGraph 节点函数的基本模式。
...
# 设置一个结束点。这指示图 "任何时候运行此节点，你都可以退出。"
graph_builder.set_finish_point("chatbot")

# 要能够运行我们的图，需要在图构建器上调用 "compile()"。这会创建一个 "CompiledGraph"，我们可以在其状态上调用 invoke。
graph = graph_builder.compile()
```

#### 将 langfuse 作为回调添加到调用中

现在，我们将添加 [LangChain 的 Langfuse 回调处理器](https://langfuse.com/docs/integrations/langchain/tracing)来追踪我们应用的步骤：`config={"callbacks": [langfuse_handler]}`


```python
from langfuse.langchain import CallbackHandler

# 初始化用于 LangChain 的 Langfuse CallbackHandler（追踪）
langfuse_handler = CallbackHandler()
...
- 要了解如何评估你的 LangGraph 应用的性能，请查看 [LangGraph 评估指南](https://langfuse.com/docs/integrations/langchain/example-langgraph-agents)。
