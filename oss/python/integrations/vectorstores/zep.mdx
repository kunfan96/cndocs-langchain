---
title: "Zep 集成"
description: "使用 LangChain Python 与 Zep 向量存储集成。"
---

> 从对话历史中召回、理解和提取数据。为个性化 AI 体验提供动力。

> [Zep](https://www.getzep.com) 是 AI 助手应用程序的长期记忆服务。
> 通过 Zep，您可以让 AI 助手具备回忆过去对话的能力，无论时间多久，
> 同时还能减少幻觉、降低延迟和成本。
...
在以下示例中，我们使用 Zep 的自动嵌入功能，该功能使用低延迟嵌入模型在 Zep 服务器上自动嵌入文档。

## 注意

- 这些示例使用 Zep 的异步接口。通过删除方法名称前的 `a` 前缀来调用同步接口。
- 如果您传入 [`Embeddings`](https://reference.langchain.com/python/langchain-core/embeddings/embeddings/Embeddings) 实例，Zep 将使用它来嵌入文档，而不是自动嵌入。
...
# 实例化 VectorStore。由于 Zep 中尚不存在该集合，
# 它将被创建并填充我们传入的文档。
...
使用元数据过滤器缩小结果范围。首先加载另一本书："Adventures of Sherlock Holmes"
