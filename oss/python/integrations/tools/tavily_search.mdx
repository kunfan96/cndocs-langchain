---
title: "Tavily 搜索集成"
description: "使用 LangChain Python 与 Tavily 搜索工具集成。"
---
...
这里我们展示如何实例化 Tavily 搜索工具的实例。该工具接受各种参数以自定义搜索。实例化后，我们使用一个简单的查询调用该工具。此工具允许您使用 Tavily 的搜索 API 端点完成搜索查询。

实例化
该工具在实例化期间接受各种参数：

- max_results (可选, int): 返回的最大搜索结果数。默认为 5。
- topic (可选, str): 搜索的类别。可以是 "general"、"news" 或 "finance"。默认为 "general"。
- include_answer (可选, bool): 在结果中包含对原始查询的答案。默认为 False。
- include_raw_content (可选, bool): 包含每个搜索结果的已清理和解析的 HTML。默认为 False。
- include_images (可选, bool): 在响应中包含查询相关的图像列表。默认为 False。
- include_image_descriptions (可选, bool): 包含每个图像的描述性文本。默认为 False。
- search_depth (可选, str): 搜索深度，可以是 "basic" 或 "advanced"。默认为 "basic"。
- time_range (可选, str): 从当前日期回溯的时间范围，用于过滤结果 - "day"、"week"、"month" 或 "year"。默认为 None。
- include_domains (可选, List[str]): 要专门包含的域列表。默认为 None。
- exclude_domains (可选, List[str]): 要专门排除的域列表。默认为 None。
...
- `query` (必需): 自然语言搜索查询
- 以下参数也可以在调用期间设置：`include_images`、`search_depth`、`time_range`、`include_domains`、`exclude_domains`、`include_images`
- 出于可靠性和性能原因，某些影响响应大小的参数无法在调用期间修改：`include_answer` 和 `include_raw_content`。这些限制可防止意外的上下文窗口问题并确保结果一致。

注意：可选参数可供代理动态设置，如果您在实例化时设置了一个参数，然后使用不同的值调用工具，工具将使用您在调用期间传递的值。
...
我们可以通过将工具绑定到代理，直接将工具与代理执行器一起使用。这使代理能够动态设置 Tavily 搜索工具的可用参数。

在下面的示例中，当我们要求代理查找“哪个国家主办了 2024 年欧洲杯？仅包含维基百科来源。”时，代理将动态设置参数并调用 Tavily 搜索工具：使用 `{'query': 'Euro 2024 host nation', 'include_domains': ['wikipedia.org']}` 调用 `tavily_search`
...
有关 Tavily 搜索 API 功能和配置的详细文档，请访问 API 参考：[docs.tavily.com/documentation/api-reference/endpoint/search](https://docs.tavily.com/documentation/api-reference/endpoint/search)
