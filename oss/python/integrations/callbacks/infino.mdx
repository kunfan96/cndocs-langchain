---
title: "Infino 集成"
description: "使用 LangChain Python 集成 Infino 回调。"
---

>[Infino](https://github.com/infinohq/infino) 是专为日志、指标和追踪设计的可扩展遥测存储。Infino 可作为独立的可观测性解决方案，也可作为可观测性技术栈中的存储层。

本示例展示了如何通过 `LangChain` 和 [Infino](https://github.com/infinohq/infino) 调用 OpenAI 和 ChatOpenAI 模型时，追踪以下内容：

* 提示输入
* 来自 `ChatGPT` 或其他 `LangChain` 模型的响应
* 延迟
* 错误
* 消耗的 Token 数量
...
现在使用 matplotlib 创建延迟、错误和 Token 消耗的图表。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/callbacks/infino.mdx) 或 [提交 Issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
