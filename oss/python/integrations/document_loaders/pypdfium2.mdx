...
description: "使用 LangChain Python 与 Pypdfium2loader 文档加载器集成。"
---

本指南提供了快速入门 `PyPDF` [文档加载器](/oss/python/integrations/document_loaders) 的概述。有关所有 `DocumentLoader` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/python/langchain-community/document_loaders/pdf/PyPDFium2Loader)。
...
附加元数据特定于每个解析器。这些信息可能很有帮助（例如用于对 PDF 进行分类）。
...
有关所有 `PyPDFium2Loader` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/python/langchain-community/document_loaders/pdf/PyPDFium2Loader)
