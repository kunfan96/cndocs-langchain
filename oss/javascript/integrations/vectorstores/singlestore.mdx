...
使用以下部分将 SingleStore 连接到 LangChain.js。
...
`SingleStoreVectorStore` 管理一个连接池。建议在终止应用程序之前调用 `await store.end();`，以确保所有连接都已正确关闭并防止任何可能的资源泄漏。
...
通过利用 [ANN 向量索引](https://docs.singlestore.com/cloud/reference/sql-reference/vector-functions/vector-indexing/)，使用 SingleStore DB 8.5 或更高版本来增强您的搜索效率。
