...
请前往 [API 参考文档](https://python.langchain.com/api_reference/community/chat_models/langchain_community.chat_models.cohere.ChatCohere.html) 查看所有属性和方法的详细说明。

## 安装配置

该集成位于 `langchain-cohere` 包中，可通过以下命令安装：

```bash
pip install -U langchain-cohere
```

还需要获取 [Cohere API 密钥](https://cohere.com/) 并设置 `COHERE_API_KEY` 环境变量：
...
同时建议（非必须）配置 [LangSmith](https://smith.langchain.com/) 以获得最佳的可观测性体验：
