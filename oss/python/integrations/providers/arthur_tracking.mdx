---
title: "Arthur 集成"
description: "使用 LangChain Python 与 Arthur 集成。"
---

>[Arthur](https://arthur.ai) 是一个模型监控和可观测性平台。

以下指南展示了如何使用 Arthur 回调处理器运行已注册的聊天大语言模型，以自动将模型推理记录到 Arthur。

如果您目前尚未将模型接入 Arthur，请访问我们的[生成式文本模型接入指南](https://docs.arthur.ai/user-guide/walkthroughs/model-onboarding/generative_text_onboarding.html)。有关如何使用 `Arthur SDK` 的更多信息，请访问我们的[文档](https://docs.arthur.ai/)。
...
使用 Arthur 回调处理器创建 LangChain 大语言模型
...
使用此 `run` 函数运行聊天大语言模型，会将聊天历史保存在一个持续的列表中，以便对话可以引用早期消息，并将每个响应记录到 Arthur 平台。您可以在[模型仪表板页面](https://app.arthur.ai/)上查看此模型推理的历史记录。
