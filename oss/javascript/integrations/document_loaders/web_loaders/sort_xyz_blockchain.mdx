---
title: "区块链数据集成"
description: "使用 LangChain JavaScript 与区块链数据文档加载器进行集成。"
---

此示例展示了如何通过 sort.xyz SQL API 加载区块链数据，包括合约地址的 NFT 元数据和交易记录。

您需要一个免费的 Sort API 密钥，请访问 sort.xyz 获取。
...
/**
 * 参见 https://docs.sort.xyz/docs/api-keys 获取您的免费 Sort API 密钥。
 * 参见 https://docs.sort.xyz 了解可用查询的更多信息。
 * 参见 https://docs.sort.xyz/reference 了解 Sort REST API 的更多信息。
 */
...
export const run = async () => {
  // 初始化 OpenAI 模型。使用 /examples 中 .env 的 OPENAI_API_KEY
  ...
  /*
  从以太坊区块链加载 NFT 元数据。提示：要按特定 ID 加载，请参见下面的 SQL 查询示例。
  */
  ...
  const nftPrompt =
    "使用以下 JSON 文档中的属性，用 4 句话描述该角色的故事。";
  ...
  */
  ...
  const latestPrompt =
    "仅用每篇文档 4 句话描述以下 JSON 文档。尽可能包含详细信息。";
  ...
  /*
    使用原始 SQL 和 NFT 索引加载特定 NFT 的元数据。参见 https://docs.sort.xyz 了解如何构建 SQL。
  */
  ...
  const sqlPrompt =
    "使用以下 JSON 文档中的属性，在新咖啡店的广告中描述该角色。";
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/javascript/integrations/document_loaders/web_loaders/sort_xyz_blockchain.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
