...
文档重排序可以极大地改进任何 RAG 应用程序和文档检索系统。

从宏观上看，重排序 API 是一个语言模型，它分析文档并根据其与给定查询的相关性对其进行重新排序。

Cohere 提供了用于文档重排序的 API。在此示例中，我们将展示如何使用它。
...
正如预期的那样，具有最高 `relevanceScore` 的文档是引用华盛顿特区的文档，得分为 `98.7%`！

### 与 `CohereClient` 一起使用

如果您在 Azure、AWS Bedrock 或独立实例上使用 Cohere，可以使用 `CohereClient` 使用您的端点创建 `CohereRerank` 实例。
