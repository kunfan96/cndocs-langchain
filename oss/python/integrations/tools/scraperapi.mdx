...
* **可选（镜像 ScraperAPI 查询参数）：**
  ...
  * `premium`: `bool` – 使用高级代理
  * `render`: `bool` – 在返回 HTML 前运行 JS
  * `keep_headers`: `bool` – 包含响应头

有关完整的修饰符列表，请参阅 [ScraperAPI 请求自定义文档](https://docs.scraperapi.com/python/making-requests/customizing-requests)。
...
通过 `/structured/amazon/search` 获取结构化的商品结果。
...
以下是在 AI 代理中使用这些工具的示例。`ScraperAPITool` 赋予 AI 浏览任何网站、总结文章以及点击链接在页面间导航的能力。
