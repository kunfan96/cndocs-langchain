...
Azure Cosmos DB for NoSQL 现在提供预览版的向量索引和搜索功能。该功能旨在处理高维向量，能够在任意规模下实现高效、精确的向量搜索。您现在可以将向量直接与数据一起存储在文档中。这意味着数据库中的每个文档不仅可以包含传统的无模式数据，还可以将高维向量作为文档的其他属性。数据与向量的这种共存方式允许高效索引和搜索，因为向量与其所代表的数据存储在同一逻辑单元中。这简化了数据管理、AI 应用程序架构以及基于向量的操作效率。
