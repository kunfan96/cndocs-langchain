---
title: "Hippo 集成"
description: "使用 LangChain Python 集成 Hippo 向量存储。"
---

>[Transwarp Hippo](https://www.transwarp.cn/en/subproduct/hippo) 是一款企业级云原生分布式向量数据库，支持对海量向量数据集进行存储、检索和管理。它能有效解决向量相似度检索和高密度向量聚类等问题。`Hippo` 具备高可用性、高性能和易扩展性等特点，并拥有多种向量搜索索引、数据分区与分片、数据持久化、增量数据摄取、向量标量字段过滤及混合查询等丰富功能，能够有效满足企业对海量向量数据高实时检索的需求。

## 入门指南

此处唯一的前提条件是来自 OpenAI 官网的 API 密钥。请确保您已经启动了一个 Hippo 实例。
...
此处，我们使用 LangChain 的 CharacterTextSplitter 进行分段，分隔符为句号。分段后，文本段不超过 1000 个字符，重复字符数为 0。
