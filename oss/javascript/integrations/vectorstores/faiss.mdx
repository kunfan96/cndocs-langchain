...
LangChain.js 支持将 Faiss 用作本地运行的向量存储，可以保存到文件。它还提供了从 [LangChain Python 实现](https://python.langchain.com/docs/integrations/vectorstores/faiss#saving-and-loading) 读取已保存文件的能力。

本指南提供了快速入门 Faiss [向量存储](/oss/javascript/integrations/vectorstores) 的概览。有关所有 `FaissStore` 功能和配置的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-community/vectorstores/faiss/FaissStore)。
