...
> [Cloud SQL](https://cloud.google.com/sql) 是一项完全托管的关系型数据库服务，提供高性能、无缝集成和出色的可扩展性。它支持 [MySQL](https://cloud.google.com/sql/mysql)、[PostgreSQL](https://cloud.google.com/sql/postgresql) 和 [SQL Server](https://cloud.google.com/sql/sqlserver) 数据库引擎。利用 Cloud SQL 的 LangChain 集成，扩展您的数据库应用以构建 AI 驱动的体验。
...
确认可以在本笔记本的运行时环境中访问数据库后，填写以下值并在运行示例脚本之前运行该单元格。

```python
# @markdown 请填写您的 Google Cloud 区域和 Cloud SQL 实例名称。
REGION = "us-central1"  # @param {type:"string"}
INSTANCE = "test-instance"  # @param {type:"string"}

# @markdown 请指定一个用于演示的数据库和表。
...
向 Google Cloud 进行身份验证，以登录到此笔记本的 IAM 用户身份访问您的 Google Cloud 项目。
...
在从 MySQL 表保存或加载文档之前，我们需要先配置一个到 Cloud SQL 数据库的连接池。`MySQLEngine` 会配置一个到您的 Cloud SQL 数据库的连接池，使您的应用程序能够成功连接，并遵循行业最佳实践。
...
4. `database`：要连接到的 Cloud SQL 实例上的数据库名称。
...
通过 `MySQLEngine.init_document_table(<table_name>)` 初始化一个具有默认架构的表。表列：
...
使用 `MySQLDocumentSaver.add_documents(<documents>)` 保存 LangChain 文档。要初始化 `MySQLDocumentSaver` 类，您需要提供 2 项内容：
...
从 SQL 查询生成的视图可能具有与默认表不同的架构。在这种情况下，MySQLLoader 的行为与从具有非默认架构的表加载相同。请参阅[加载具有自定义文档页面内容和元数据的文档](#load-documents-with-customized-document-page-content-and-metadata)部分。
...
首先，我们准备一个具有非默认架构的示例表，并填充一些任意数据。
...
例如，在这里，`content_columns` 中列的值将连接成一个空格分隔的字符串，作为加载文档的 `page_content`，而加载文档的 `metadata` 将仅包含 `metadata_columns` 中指定列的键值对。
...
为了将 LangChain 文档保存到具有自定义元数据字段的表中。我们需要首先通过 `MySQLEngine.init_document_table()` 创建这样一个表，并指定我们希望它具有的 `metadata_columns` 列表。在此示例中，创建的表将具有以下表列：
...
我们也可以通过 `MySQLDocumentSaver.delete(<documents>)` 从具有自定义元数据列的表中删除文档。删除标准是：
