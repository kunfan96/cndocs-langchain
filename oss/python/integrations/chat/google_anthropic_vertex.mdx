...
description: "使用 LangChain Python 与 ChatAnthropicVertex 聊天模型集成。"
---

<Warning>
    **Claude 3 Haiku 弃用通知：** 从 2026 年 8 月 23 日起，Anthropic 将终止对 Claude 3 Haiku 的支持。请迁移到 Claude Haiku 4.5 (`claude-haiku-4-5@20251001`)，该模型已在 Vertex AI 上正式发布。详情请参阅 [Anthropic 的模型弃用政策](https://docs.anthropic.com/en/docs/resources/model-deprecations)。
</Warning>

> [Anthropic Claude](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-claude) 模型在 Vertex AI 上提供完全托管和无服务器的模型 API。要在 Vertex AI 上使用 Claude 模型，请直接向 Vertex AI API 端点发送请求。由于 Anthropic Claude 模型使用托管 API，因此无需配置或管理基础设施。
...
<Note>
    指定正确的 [Claude 模型版本](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-claude#claude-opus)。尽可能使用包含日期后缀的模型标识符（例如 `claude-haiku-4-5@20251001`）。
</Note>

```python
# TODO : 将下面替换为你的项目 ID 和区域
project = "<project_id>"
location = "<region>"
