...
将 Cloud SQL 设置为向量存储的要求和参数之一是 `PostgresEngine` 对象。`PostgresEngine` 配置了到您的 Cloud SQL 数据库的连接池，使您的应用程序能够成功连接，并遵循行业最佳实践。
...
默认情况下，[IAM 数据库身份验证](https://cloud.google.com/sql/docs/postgres/iam-authentication#iam-db-auth)将用作数据库身份验证方法。此库使用来自环境的[应用默认凭据 (ADC)](https://cloud.google.com/docs/authentication/application-default-credentials) 所属的 IAM 主体。
...
或者，也可以使用[内置数据库身份验证](https://cloud.google.com/sql/docs/postgres/built-in-authentication)（使用用户名和密码访问 Cloud SQL 数据库）。只需向 `PostgresEngine.from_instance()` 提供可选的 `user` 和 `password` 参数：
