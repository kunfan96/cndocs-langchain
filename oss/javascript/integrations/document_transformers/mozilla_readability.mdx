...
在摄取 HTML 文档以供后续检索时，我们通常只对网页的实际内容感兴趣，而非其语义结构。
使用 MozillaReadabilityTransformer 从文档中剥离 HTML 标签，可以生成内容更丰富的块，从而使检索更有效。

## 设置

你需要安装 [`@mozilla/readability`](https://www.npmjs.com/package/@mozilla/readability) 和 [`jsdom`](https://www.npmjs.com/package/jsdom) npm 包：

```bash npm
npm install @mozilla/readability jsdom
```
虽然转换器本身不需要，但下面的使用示例需要 [`cheerio`](https://www.npmjs.com/package/cheerio) 进行网页抓取：
...
以下示例抓取一个 Hacker News 帖子，根据 HTML 标签进行分割，以根据标签的语义信息对块进行分组，然后从各个块中提取内容：
...
你可以向转换器传递 [`@mozilla/readability` 包接受的任何[参数](https://www.npmjs.com/package/@mozilla/readability)，以自定义其工作方式。
