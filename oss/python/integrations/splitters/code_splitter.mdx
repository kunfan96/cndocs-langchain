...
[RecursiveCharacterTextSplitter](https://reference.langchain.com/python/langchain-text-splitters/character/RecursiveCharacterTextSplitter) 包含预置的分隔符列表，这些列表对于[分割特定编程语言的文本](/oss/python/integrations/splitters/)非常有用。
...
要实例化一个针对特定语言定制的分割器，请将枚举中的一个值传递给
```python
