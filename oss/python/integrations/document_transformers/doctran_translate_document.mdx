...
description: "使用 LangChain Python 与 Doctran 语言翻译文档转换器集成。"
---

通过嵌入向量比较文档的好处在于可以跨语言工作。"Harrison says hello" 和 "Harrison dice hola" 在向量空间中会占据相似的位置，因为它们在语义上具有相同的含义。

然而，在向量化之前使用大语言模型（LLM）**将文档翻译成其他语言**仍然很有用。当用户可能使用不同语言查询知识库，或者对于特定语言没有最先进的嵌入模型时，这尤其有帮助。

我们可以使用 [Doctran](https://github.com/psychic-api/doctran) 库来实现这一点，该库利用 OpenAI 的函数调用功能在语言之间翻译文档。
...
翻译文档后，结果将作为新文档返回，其 page_content 已翻译成目标语言
...
翻译文档后，结果将作为新文档返回，其 page_content 已翻译成目标语言。当文档被分成多个部分时，异步版本将提高性能。它还将确保以正确的顺序返回输出。
