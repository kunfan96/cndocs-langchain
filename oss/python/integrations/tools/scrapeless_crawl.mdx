---
title: "Scrapeless 爬虫集成"
description: "使用 LangChain Python 集成 Scrapeless 爬虫工具。"
---

[**Scrapeless**](https://www.scrapeless.com/) 提供灵活且功能丰富的数据获取服务，支持广泛的参数自定义和多格式导出。这些能力使 LangChain 能够更有效地集成和利用外部数据。其核心功能模块包括：
...
- **Google 趋势**：从 Google 获取关键词趋势数据，包括随时间变化的热度、地区兴趣和相关搜索。
  - 支持多关键词比较。
  - 支持多种数据类型：`interest_over_time`、`interest_by_region`、`related_queries` 和 `related_topics`。
  - 允许按特定 Google 属性（网络、YouTube、新闻、购物）进行过滤，以进行特定来源的趋势分析。
...
ScrapelessCrawlerScrapeTool 允许您使用 Scrapeless 的 Crawler Scrape API 从一个或多个网站抓取内容。您可以提取主要正文、控制格式、请求头、等待时间和输出类型。

该工具接受以下参数：

- `urls`（必需，List[str]）：您要抓取的一个或多个网站 URL。
- `formats`（可选，List[str]）：定义抓取输出的格式。默认为 `['markdown']`。选项包括：
  ...
  - `'screenshot'`
  - `'html'`
- `only_main_content`（可选，bool）：是否仅返回主要页面内容，排除页头、导航栏、页脚等。默认为 True。
...
- `headers`（可选，Dict[str, str]）：随请求发送的自定义请求头（例如用于 cookies 或 user-agent）。默认为 None。
- `wait_for`（可选，int）：抓取前等待的时间（毫秒）。对于让页面完全加载很有用。默认为 `0`。
- `timeout`（可选，int）：请求超时时间（毫秒）。默认为 `30000`。

### ScrapelessCrawlerCrawlTool

ScrapelessCrawlerCrawlTool 允许您使用 Scrapeless 的 Crawler Crawl API 从一个基础 URL 开始爬取网站。它支持高级 URL 过滤、爬取深度控制、内容抓取选项、请求头自定义等。
...
- `include_paths`（可选，List[str]）：用于在爬取中包含匹配 URL 的 URL 路径名正则表达式模式。只有匹配这些模式的 URL 才会被包含。例如，设置 `["blog/.*"]` 将仅包含 `/blog/` 路径下的 URL。默认为 None。
- `exclude_paths`（可选，List[str]）：用于在爬取中排除匹配 URL 的 URL 路径名正则表达式模式。例如，设置 `["blog/.*"]` 将排除 `/blog/` 路径下的 URL。默认为 None。
- `max_depth`（可选，int）：相对于基础 URL 的最大爬取深度，通过 URL 路径中的斜杠数量来衡量。默认为 `10`。
- `max_discovery_depth`（可选，int）：基于发现顺序的最大爬取深度。根页面和站点地图页面的深度为 `0`。例如，设置为 `1` 并忽略站点地图将仅爬取输入的 URL 及其直接链接。默认为 None。
...
- `allow_backward_links`（可选，bool）：是否允许爬取 URL 层级结构之外的反向链接。默认为 False。
...
- `formats`（可选，List[str]）：抓取内容的格式。默认为 ["markdown"]。选项包括：
  ...
  - `'screenshot'`
  - `'html'`
- `only_main_content`（可选，bool）：是否仅返回主要正文，排除页头、导航栏、页脚等。默认为 True。
...
- `wait_for`（可选，int）：抓取内容前等待的时间（毫秒），允许页面完全加载。默认为 `0`。
- `timeout`（可选，int）：请求超时时间（毫秒）。默认为 `30000`。
...
for chunk in agent.stream(
    {
        "messages": [
            (
                "human",
                "使用 scrapeless crawler crawl 工具爬取网站 https://example.com 并将 markdown 内容作为字符串输出。",
            )
        ]
    },
    stream_mode="values",
...
网站 https://example.com 的爬取已完成。以下是从该网站提取的 markdown 内容：
...
您可以在 [IANA 示例域名页面](https://www.iana.org/domains/example) 找到更多信息。
```
...
for chunk in agent.stream(
    {
        "messages": [
            (
                "human",
                "使用 scrapeless crawler scrape 工具获取网站 https://example.com 的内容，并将 html 内容作为字符串输出。",
            )
        ]
    },
    stream_mode="values",
...
网站 "https://example.com" 的 HTML 内容如下：
...
- [Scrapeless 文档](https://docs.scrapeless.com/en/crawl/quickstart/introduction/)
- [Scrapeless API 参考](https://apidocs.scrapeless.com/api-17509003)
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/tools/scrapeless_crawl.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>