...
description: "使用 LangChain Python 与 LinkupSearchTool 工具集成。"
---

> [Linkup](https://www.linkup.so/) 提供了一个 API，用于将 LLM 连接到网络和 Linkup Premium Partner 来源。

本指南提供了快速入门 `LinkupSearchTool` [工具](/oss/python/langchain/tools) 的概述。
...
要使用 Linkup 提供商，您需要一个有效的 API 密钥，您可以通过[注册](https://app.linkup.so/sign-up)找到它。要运行以下示例，您还需要一个 OpenAI API 密钥。
...
这里我们展示如何实例化 LinkupSearchTool 的实例：
