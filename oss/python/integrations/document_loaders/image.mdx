---
title: "图像集成"
description: "使用 LangChain Python 集成图像文档加载器。"
---

本文介绍如何将图像加载为可供 LangChain 其他模块使用的文档格式。

它使用 [Unstructured](https://unstructured.io/) 处理多种图像格式，例如 `.jpg` 和 `.png`。请参阅[本指南](/oss/python/integrations/providers/unstructured/)了解如何在本地设置 Unstructured，包括所需的系统依赖项。
...
在底层，Unstructured 会为不同的文本块创建不同的"元素"。默认情况下，我们会将这些元素合并在一起，但您可以通过指定 `mode="elements"` 来保留这种分隔。
