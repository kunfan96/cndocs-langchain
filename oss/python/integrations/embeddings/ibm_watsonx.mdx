...
description: "使用 LangChain Python 与 WatsonxEmbeddings 嵌入模型集成。"
---
...
此单元格定义了使用 watsonx 嵌入所需的 WML 凭证。
...
LangChain IBM 集成包含在 `langchain-ibm` 包中：
...
- 根据您预配服务实例的区域，使用其中一个[特定区域的认证 URL](https://ibm.github.io/watsonx-ai-python-sdk/setup_cloud.html#authentication)。
...
下面，了解如何使用我们上面初始化的 `embeddings` 对象来索引和检索数据。在本示例中，我们将在 `InMemoryVectorStore` 中索引和检索一个示例文档。
...
有关所有 `WatsonxEmbeddings` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/python/integrations/langchain_ibm/WatsonxEmbeddings/)。
