---
title: 子代理流式处理
description: 显示具有流式内容、进度跟踪和可折叠卡片的专家子代理
---
...
提交消息时，启用子图流式处理并设置适当的递归限制。深度代理工作流通常涉及多层嵌套子图，因此较高的递归限制可防止过早终止：
...
function SynthesisIndicator({
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg bg-purple-50 px-4 py-2 text-sm text-purple-700">
      ...
      正在综合来自 {subagents.length} 个子代理
      {subagents.length !== 1 ? "的" : ""}结果...
    </div>
  );
}
```
...
- **代码审查管道**，不同的代理处理安全审查、风格检查、性能分析和文档审查
...
- **显示任务描述**。`toolCall.args.description` 字段告诉用户每个子代理被要求做什么。始终突出显示此信息。
