...
[Databricks 向量搜索](https://docs.databricks.com/en/generative-ai/vector-search.html) 是一个无服务器的相似性搜索引擎，允许您将数据的向量表示（包括元数据）存储在向量数据库中。通过向量搜索，您可以从 Unity Catalog 管理的 Delta 表创建自动更新的向量搜索索引，并使用简单的 API 进行查询，以返回最相似的向量。
...
如果您已经拥有端点和索引，可以跳过本节，直接转到“实例化”部分。
