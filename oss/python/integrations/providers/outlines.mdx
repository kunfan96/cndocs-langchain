---
title: "Outlines 集成"
description: "使用 LangChain Python 与 Outlines 集成。"
---

>[Outlines](https://github.com/dottxt-ai/outlines) 是一个用于受约束的语言生成的 Python 库。它为各种语言模型提供了统一的接口，并允许使用正则表达式匹配、类型约束、JSON 模式和上下文无关文法等技术进行结构化生成。
...
此集成允许您在 LangChain 中使用 Outlines 模型，提供 LLM 和聊天模型接口。
...
1. **正则表达式匹配**：
   ...
   ```
   这将确保生成的文本与指定的正则表达式模式匹配（在本例中，是一个有效的 IP 地址）。
...
3. **JSON 模式**：
   ...
   ```
   这确保生成的输出遵循指定的 JSON 模式或 Pydantic 模型。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/providers/outlines.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>