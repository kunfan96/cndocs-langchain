---
title: "ChatTongyi 集成"
description: "使用 LangChain Python 与 ChatTongyi 聊天模型集成。"
---
通义千问是阿里巴巴达摩院开发的大型语言模型。它能够通过自然语言理解和语义分析，基于用户的自然语言输入来理解用户意图。它在不同领域和任务中为用户提供服务和帮助。通过提供清晰详细的指令，您可以获得更符合预期的结果。
在本笔记本中，我们将介绍如何使用 langchain 与 [通义](https://www.aliyun.com/product/dashscope)，主要使用对应于 langchain 中 `langchain/chat_models` 包的 `Chat` 模型。
...
通义千问视觉模型（qwen-vl-plus/qwen-vl-max）是能够处理图像的模型。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/chat/tongyi.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>