...
本笔记本提供了使用 `RecursiveUrlLoader` [文档加载器](/oss/javascript/integrations/document_loaders/) 的快速入门概述。有关所有 `RecursiveUrlLoader` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-community/document_loaders/web/recursive_url/RecursiveUrlLoader)。
...
例如，让我们看看 LangChain.js 介绍文档。

这有许多有趣的子页面，我们可能希望加载、拆分并稍后批量检索。
...
要访问 `RecursiveUrlLoader` 文档加载器，您需要安装 `@langchain/community` 集成和 [`jsdom`](https://www.npmjs.com/package/jsdom) 包。
