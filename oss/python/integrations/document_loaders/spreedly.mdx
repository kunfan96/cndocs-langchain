...
>[Spreedly](https://docs.spreedly.com/) 是一项服务，允许您安全地存储信用卡，并使用它们与任意数量的支付网关和第三方 API 进行交易。它通过同时提供卡片令牌化/保管库服务以及网关和接收器集成服务来实现这一点。由 Spreedly 令牌化的支付方式存储在 `Spreedly` 中，允许您独立存储一张卡片，然后根据业务需求将该卡片传递给不同的端点。

本笔记本介绍如何从 [Spreedly REST API](https://docs.spreedly.com/reference/api/v1/) 加载数据到可被 LangChain 摄取的格式，以及用于向量化的示例用法。
...
此文档加载器目前不支持分页，也不支持访问需要额外参数的更复杂对象。它还需要一个 `resource` 选项，该选项定义了您想要加载的对象。
