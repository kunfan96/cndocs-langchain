...
要从向量存储中删除文档，请使用在初始化 VectorStore 时与 `row_id` 列中的值对应的 ID。
...
要从向量存储中删除文档，您可以利用文档本身。VectorStore 初始化期间提供的 content 列和 metadata 列将用于查找与文档对应的行。任何匹配的行随后将被删除。
