...
description: "使用 LangChain JavaScript 与 VectorStoreToolkit 工具进行集成。"
---

这将帮助您开始使用 `VectorStoreToolkit` [工具包](/oss/javascript/langchain/tools#prebuilt-tools)。有关所有 `VectorStoreToolkit` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-core/vectorstores/VectorStore)。

`VectorStoreToolkit` 是一个工具包，它接收一个向量存储，并将其转换为一个工具，然后可以被调用、传递给 LLM、代理等。

## 设置

如果您想从单个工具的运行中获取自动跟踪，也可以通过取消注释下方代码来设置您的 [LangSmith](/langsmith/home) API 密钥：
...
有关所有 `VectorStoreToolkit` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-core/vectorstores/VectorStore)。
