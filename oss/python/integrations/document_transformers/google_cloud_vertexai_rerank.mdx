...
description: "使用 LangChain Python 集成 Google Cloud Vertex AI 重排序器文档转换器。"
---

> [Vertex Search Ranking API](https://cloud.google.com/generative-ai-app-builder/docs/ranking) 是 [Vertex AI Agent Builder](https://cloud.google.com/generative-ai-app-builder/docs/builder-apis) 中的独立 API 之一。它接收文档列表，并根据文档与查询的相关性对这些文档进行重新排序。与仅查看文档和查询语义相似性的嵌入不同，排名 API 可以为文档回答给定查询的匹配程度提供精确分数。排名 API 可用于在检索初始候选文档集后提高搜索结果的质量。

>排名 API 是无状态的，因此无需在调用 API 之前索引文档。您只需传入查询和文档即可。这使得该 API 非常适合对来自任何文档检索器的文档进行重新排序。

>更多信息，请参阅[对文档进行排名和重新排序](https://cloud.google.com/generative-ai-app-builder/docs/ranking)。
...
让我们用相同的查询分别查询 `basic_retriever` 和 `retriever_with_reranker`，并比较检索到的文档。

Ranking API 接收来自 `basic_retriever` 的输入，并将其传递给 Ranking API。

Ranking API 用于提高排名质量，并确定一个分数，该分数表示每条记录与查询的相关性。

您可以看到未排序文档和已排序文档之间的差异。Ranking API 将语义最相关的文档移动到 LLM 上下文窗口的顶部，从而帮助其形成更好的推理答案。
...
让我们检查几个重新排序后的文档。我们观察到检索器仍然返回相关的 LangChain 类型 [文档](https://reference.langchain.com/python/langchain-core/documents/base/Document)，但作为元数据字段的一部分，我们也从 Ranking API 接收到了 `relevance_score`。
...
这展示了一个完整的 RAG 链示例，其中包含一个简单的提示模板，说明如何使用 Vertex Ranking API 执行重新排序。
