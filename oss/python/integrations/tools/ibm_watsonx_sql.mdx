...
本示例展示如何使用 `langchain-ibm` `watsonx.ai` SQL 数据库工具包，该工具包使用 Flight 服务。

<Warning>
构建 SQL 数据库的问答系统需要执行模型生成的 SQL 查询，这存在固有的安全风险。请确保您的数据库连接权限始终尽可能严格地限定在智能体的需求范围内。这将减轻（但不能完全消除）构建模型驱动系统的风险。
</Warning>
...
有关所有 `WatsonxSQLDatabaseToolkit` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/python/integrations/langchain_ibm/WatsonxSQLDatabaseToolkit/)。
