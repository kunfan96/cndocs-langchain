...
LangChain提供了内置防护栏（例如，[PII检测](#pii-detection)、[人在回路中](#human-in-the-loop)）以及一个灵活的中间件系统，用于使用任一方法构建自定义防护栏。
...
人在回路中中间件对于以下情况很有帮助：金融交易和转账、删除或修改生产数据、向外部方发送通信，以及任何具有重大业务影响的操作。
...
<Tip>
    有关实施审批工作流的完整详情，请参阅[人在回路中文档](/oss/javascript/langchain/human-in-the-loop)。
</Tip>
...
- [人在回路中](/oss/javascript/langchain/human-in-the-loop) - 为敏感操作添加人工审查
