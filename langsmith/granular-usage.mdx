...
title: 细粒度计费使用量
sidebarTitle: 细粒度使用量
description: 获取按工作区、项目、用户或API密钥细分的详细跟踪使用数据。
---

<Note>
对于 LangSmith Cloud，细粒度计费使用数据的收集始于 2026 年 1 月 5 日。此日期之前的使用数据不可用。

对于自托管实例，数据收集在通过以下环境变量启用该功能后开始，或在[升级到默认启用该功能的版本](/langsmith/self-hosted-changelog#langsmith-0-13-12)之后开始。
...
- 您必须拥有 [`organization:read` 权限](/langsmith/organization-workspace-operations)才能访问细粒度使用数据。
- 您只能查看您拥有读取权限的工作区的使用情况。

## 在 UI 中查看

您也可以在 [LangSmith UI](https://smith.langchain.com) 中查看细粒度使用数据：
...
此端点接受与细粒度使用端点相同的查询参数，并返回一个 CSV 文件。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/langsmith/granular-usage.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>