...
请按照此处 [HanaRdfGraph](/oss/python/integrations/graphs/sap_hana_rdf_graph) 的步骤了解有关创建 `HanaRdfGraph` 实例的更多信息。
...
默认提示可在此处找到：[`prompts.py`](https://github.com/SAP/langchain-integration-for-sap-hana-cloud/blob/main/langchain_hana/chains/graph_qa/prompts.py)
...
```python
# 从数据图中提取基本图模式。此模式将指导 LLM 生成正确的 SPARQL 查询。
...
1. **SPARQL 生成**
   该链使用您的 Turtle 格式本体（`graph.get_schema`）和用户问题调用 LLM，使用 `SPARQL_GENERATION_SELECT_PROMPT`。然后 LLM 发出一个针对您的模式量身定制的有效 `SELECT` 查询。
2. **预处理与执行**
   * **提取与清理**：从 LLM 的响应中提取原始 SPARQL 文本。
   * **注入图上下文**：如果缺少 `FROM <graph_uri>`，则添加它，并确保声明了常用前缀（`rdf:`、`rdfs:`、`owl:`、`xsd:`）。
   * **在 HANA 上运行**：通过 `HanaRdfGraph.query()` 在您的命名图上执行最终确定的查询。
3. **答案表述**
   返回的 CSV（或 Turtle）结果再次输入 LLM——这次使用 `SPARQL_QA_PROMPT`。LLM 严格基于检索到的数据生成简洁、人类可读的答案，没有幻觉。
