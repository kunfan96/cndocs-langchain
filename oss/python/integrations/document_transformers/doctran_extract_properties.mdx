---
title: "Doctran 属性提取集成"
description: "使用 LangChain Python 集成 Doctran 属性提取文档转换器。"
---

我们可以使用 [Doctran](https://github.com/psychic-api/doctran) 库提取文档的有用特征，该库使用 OpenAI 的函数调用功能来提取特定元数据。

从文档中提取元数据在多种任务中非常有用，包括：

* **分类**：将文档分为不同类别
* **数据挖掘**：提取可用于数据分析的结构化数据
* **风格迁移**：改变文本的写作风格，使其更接近预期的用户输入，从而提升向量搜索结果
...
从文档中提取属性后，结果将作为新文档返回，提取的属性保存在元数据中。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/document_transformers/doctran_extract_properties.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
