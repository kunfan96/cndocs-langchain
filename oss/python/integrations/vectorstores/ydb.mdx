...
> [YDB](https://ydb.tech/) 是一个功能多样的开源分布式 SQL 数据库，它将高可用性和可扩展性与强一致性和 ACID 事务相结合。它能够同时处理事务型 (OLTP)、分析型 (OLAP) 和流处理工作负载。

本笔记本展示了如何使用与 `YDB` 向量存储相关的功能。

## 设置

首先，使用 Docker 设置一个本地 YDB：
...
本笔记本不需要凭证，只需确保你已按上述方式安装了相关包。

如果你想获得一流的模型调用自动跟踪功能，也可以通过取消注释以下内容来设置你的 [LangSmith](/langsmith/home) API 密钥：
...
你可以使用 `add_documents` 函数向向量存储添加项目。
...
你可以使用 `delete` 函数按 ID 从向量存储中删除项目。
...
创建向量存储并添加相关文档后，你很可能希望在链或智能体执行过程中对其进行查询。
