...
>[Steam（维基百科）](https://en.wikipedia.org/wiki/Steam_(service)) 是由 `Valve Corporation` 开发的视频游戏数字分发服务和商店。它为 Valve 的游戏提供自动更新，并扩展到分发第三方游戏。`Steam` 提供各种功能，例如带有 Valve 反作弊措施的游戏服务器匹配、社交网络和游戏流媒体服务。

>[Steam](https://store.steampowered.com/about/) 是游玩、讨论和创作游戏的终极目的地。
...
本指南将引导您了解如何使用 Steam API 与 LangChain，根据您当前的 Steam 游戏库存获取 Steam 游戏推荐，或收集您提供的某些 Steam 游戏的信息。
...
要使用此工具包，请准备好您的 OpenAI API 密钥、Steam API 密钥（来自 [Steam 开发者 API 密钥页面](https://steamcommunity.com/dev/apikey)），并记下您自己的 SteamID。一旦您获得了 Steam API 密钥，您可以在下面将其作为环境变量输入。
该工具包将读取 "STEAM_KEY" API 密钥作为环境变量来验证您的身份，因此请在此处设置它们。您还需要设置您的 "OPENAI_API_KEY" 和您的 "STEAM_ID"。
...
初始化 LLM、SteamWebAPIWrapper、SteamToolkit，最重要的是初始化 langchain 代理来处理您的查询！
