...
本笔记本展示了与 `Activeloop Deep Lake` 相关的基本功能。虽然 `Deep Lake` 可以存储嵌入，但它能够存储任何类型的数据。它是一个无服务器数据湖，具有版本控制、查询引擎和流式数据加载器，可连接到深度学习框架。
...
在 `./my_deeplake/` 本地创建一个数据集，然后运行相似性搜索。Deeplake+LangChain 集成在底层使用 Deep Lake 数据集，因此 `dataset` 和 `vector store` 可以互换使用。要在您自己的云端或 Deep Lake 存储中创建数据集，请[相应调整路径](https://docs.deeplake.ai/latest/getting-started/storage-and-creds/storage-options/)。
...
设置 `read_only=True` 可防止在不需要更新时意外修改向量存储。这确保了数据保持不变，除非明确有意更改。通常，指定此参数以避免意外更新是一个好习惯。
...
默认情况下，Deep Lake 数据集存储在本地。要将它们存储在内存中、Deep Lake 托管数据库中或任何对象存储中，您可以在创建向量存储时提供[相应的路径和凭据](https://docs.deeplake.ai/latest/getting-started/storage-and-creds/storage-options/)。某些路径需要在 Activeloop 注册并创建 API 令牌，可以[在此处获取](https://app.activeloop.ai/)
...
此外，similarity_search 方法支持执行查询，其中可以使用 Deep Lake 的张量查询语言（TQL）指定查询。
