...
这将帮助你开始使用 Cohere [聊天模型](/oss/javascript/langchain/models)。有关所有 `ChatCohere` 功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_cohere.ChatCohere.html)。
...
| [工具调用](/oss/javascript/langchain/tools) | [结构化输出](/oss/javascript/langchain/structured-output) | [图像输入](/oss/javascript/langchain/messages#multimodal) | 音频输入 | 视频输入 | [Token 级流式传输](/oss/javascript/langchain/streaming/) | [Token 使用](/oss/javascript/langchain/models#token-usage) | [Logprobs](/oss/javascript/langchain/models#log-probabilities) |
| :---: | :---: | :---: |  :---: | :---: | :---: | :---: | :---: |
| ✅ | ✅ | ❌ | ❌ | ❌ | ✅ | ❌ | ❌ |

## 设置

为了使用 LangChain.js Cohere 集成，你需要一个 API 密钥。
你可以在[这里](https://dashboard.cohere.com/welcome/register)注册 Cohere 账户并创建 API 密钥。

你需要先安装 [`@langchain/cohere`](https://www.npmjs.com/package/@langchain/cohere) 包。
...
如果你想获得模型调用的自动追踪，你还可以通过取消注释以下内容来设置你的 [LangSmith](/langsmith/home) API 密钥：
...
**注意：** 如果提供了自定义客户端，构造函数中的 `COHERE_API_KEY` 环境变量和 `apiKey` 参数将被忽略。
...
Cohere 也开箱即用地支持 RAG。
你可以将文档作为上下文传递给 API 请求，Cohere 的模型将在生成响应时使用它们。
...
API 还允许其他非静态文档的连接。
这方面的一个例子是他们的 `web-search` 连接器，它允许你传递一个查询，API 将在网络上搜索相关文档。
下面的示例演示了如何使用此功能。
...
我们可以在 `additional_kwargs` 对象中看到 API 请求做了几件事：

- 执行了一个搜索查询，将结果数据存储在 `searchQueries` 和 `searchResults` 字段中。在 `searchQueries` 字段中，我们看到他们为了更好的结果重写了我们的查询。
- 从搜索查询中生成了三个文档。
- 生成了一个引用列表。
- 基于上述操作和内容生成了最终响应。
...
有关所有 ChatCohere 功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_cohere.ChatCohere.html)。

---

<div className="source-links">
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/javascript/integrations/chat/cohere.mdx) 或 [提交 issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
