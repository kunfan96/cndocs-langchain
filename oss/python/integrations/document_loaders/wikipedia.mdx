---
title: "Wikipedia 集成"
description: "使用 LangChain Python 与 Wikipedia 文档加载器集成。"
---
...
本笔记本展示了如何将维基页面从 `wikipedia.org` 加载到我们下游使用的文档格式中。
...
- `lang`（可选）：默认值="en"。用于在 Wikipedia 的特定语言部分进行搜索
- `load_max_docs`（可选）：默认值=100。用于限制下载的文档数量。下载所有 100 个文档需要时间，因此在实验时请使用较小的数字。目前硬性限制为 300。
- `load_all_available_meta`（可选）：默认值=False。默认情况下，仅下载最重要的字段：`title` 和 `summary`。如果设置为 `True`，则将下载所有可用字段。
- `doc_content_chars_max`（可选）：默认值=4000。文档内容的最大字符数。
