---
title: "TSV 集成"
description: "使用 LangChain Python 与 Tsv 文档加载器集成。"
---

> [制表符分隔值 (TSV)](https://en.wikipedia.org/wiki/Tab-separated_values) 文件是一种用于存储表格数据的简单、基于文本的文件格式。[3] 记录由换行符分隔，记录内的值由制表符分隔。

## `UnstructuredTSVLoader`

您也可以使用 `UnstructuredTSVLoader` 加载表格。使用 `UnstructuredTSVLoader` 的一个优势是，如果在 `"elements"` 模式下使用，元数据中将提供表格的 HTML 表示。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/document_loaders/tsv.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
