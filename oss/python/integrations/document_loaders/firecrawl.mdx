---
title: "FireCrawl 集成"
description: "使用 LangChain Python 集成 FireCrawl 文档加载器。"
---

[FireCrawl](https://firecrawl.dev/?ref=langchain) 可以爬取任何网站并将其转换为 LLM 就绪的数据。它爬取所有可访问的子页面，并为每个页面提供干净的 Markdown 格式内容和元数据，无需站点地图。

FireCrawl 能够处理反向代理、缓存、速率限制以及 JavaScript 阻止的内容等复杂情况，由 [mendable.ai](https://mendable.ai) 团队构建。
...
- `scrape`：爬取单个 URL 并返回 Markdown 格式内容。
- `crawl`：爬取该 URL 及所有可访问的子页面，并返回各页面的 Markdown 格式内容。
...
您还可以向加载器传递 `params` 参数，这是一个传递给爬虫的选项字典。更多信息请参阅 [FireCrawl API 文档](https://github.com/mendableai/firecrawl-py)。
...
您还可以向加载器传递 `params` 参数，这是一个传递给加载器的选项字典。更多信息请参阅 [FireCrawl API 文档](https://github.com/mendableai/firecrawl-py)。
