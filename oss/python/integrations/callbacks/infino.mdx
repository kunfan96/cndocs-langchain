---
title: "Infino 集成"
description: "使用 LangChain Python 与 Infino 回调集成。"
---

>[Infino](https://github.com/infinohq/infino) 是一个可扩展的遥测存储，专为日志、指标和追踪设计。Infino 可以作为独立的可观测性解决方案，也可以作为您可观测性技术栈中的存储层。

本示例展示了如何通过 `LangChain` 和 [Infino](https://github.com/infinohq/infino) 在调用 OpenAI 和 ChatOpenAI 模型时跟踪以下内容：
...
# 创建回调处理器。这会将延迟、错误、令牌使用情况、提示以及提示响应记录到 Infino。
...
# 向 OpenAI 模型提问的问题数量。我们在此演示中限制为少量问题以节省费用。
num_questions = 10
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/callbacks/infino.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>