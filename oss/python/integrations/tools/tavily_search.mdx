---
title: "Tavily 搜索集成"
description: "使用 LangChain Python 集成 Tavily 搜索工具。"
---
...
| :---: | :---: |:--------------------------------------------------------:| :---: |
| ❌ | ✅ | 标题、URL、内容摘要、raw_content、答案、图片 | 每月 1,000 次免费搜索 |
...
这里我们展示如何实例化一个 Tavily 搜索工具实例。该工具接受各种参数以自定义搜索。实例化后，我们使用一个简单的查询来调用该工具。此工具允许您使用 Tavily 的搜索 API 端点完成搜索查询。

实例化
该工具在实例化时接受各种参数：

- max_results（可选，int）：要返回的最大搜索结果数。默认为 5。
- topic（可选，str）：搜索类别。可以是 "general"、"news" 或 "finance"。默认为 "general"。
- include_answer（可选，bool）：在结果中包含对原始查询的答案。默认为 False。
- include_raw_content（可选，bool）：包含每个搜索结果的清理和解析后的 HTML。默认为 False。
- include_images（可选，bool）：在响应中包含与查询相关的图片列表。默认为 False。
- include_image_descriptions（可选，bool）：包含每张图片的描述性文本。默认为 False。
- search_depth（可选，str）：搜索深度，可以是 "basic" 或 "advanced"。默认为 "basic"。
- time_range（可选，str）：从当前日期回溯的时间范围以过滤结果 - "day"、"week"、"month" 或 "year"。默认为 None。
- include_domains（optional, List[str]）：要特别包含的域名列表。默认为 None。
- exclude_domains（optional, List[str]）：要特别排除的域名列表。默认为 None。
...
- 出于可靠性和性能考虑，某些影响响应大小的参数在调用时无法修改：`include_answer` 和 `include_raw_content`。这些限制可防止意外的上下文窗口问题并确保结果一致。
...
我们也可以使用模型生成的 ToolCall 来调用该工具，这种情况下将返回一个 ToolMessage：
...
我们可以通过将工具绑定到代理，直接将我们的工具与代理执行器一起使用。这使代理能够动态设置 Tavily 搜索工具的可用参数。

在下面的示例中，当我们要求代理查找“哪个国家举办了 2024 年欧洲杯？仅包含维基百科来源。”时，代理将动态设置参数并调用 Tavily 搜索工具：使用 `{'query': 'Euro 2024 host nation', 'include_domains': ['wikipedia.org']` 调用 `tavily_search`
...
举办 2024 年欧洲杯的国家是德国。您可以在 [UEFA Euro 2024 的维基百科页面](https://en.wikipedia.org/wiki/UEFA_Euro_2024)上找到更多信息。
```
...
有关所有 Tavily 搜索 API 功能和配置的详细文档，请前往 API 参考：[docs.tavily.com/documentation/api-reference/endpoint/search](https://docs.tavily.com/documentation/api-reference/endpoint/search)
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/tools/tavily_search.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>