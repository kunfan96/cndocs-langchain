...
sidebarTitle: "嵌入模型"
description: "使用 LangChain JavaScript 集成 GoogleGenerativeAIEmbeddings 嵌入模型。"
---

本指南将帮助您开始使用 LangChain 集成 Google 生成式 AI [嵌入模型](/oss/javascript/integrations/embeddings)。有关 `GoogleGenerativeAIEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-google-genai/GoogleGenerativeAIEmbeddings)。
...
要访问 Google 生成式 AI 嵌入模型，您需要注册一个 Google AI 账户，获取 API 密钥，并安装 `@langchain/google-genai` 集成包。
...
```typescript
const text2 = "LangGraph 是一个用于构建有状态、多参与者 LLM 应用的库";
...
有关所有 `GoogleGenerativeAIEmbeddings` 功能和配置的详细文档，请访问 [API 参考](https://reference.langchain.com/javascript/langchain-google-genai/GoogleGenerativeAIEmbeddings)。
