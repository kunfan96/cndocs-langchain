...
>[Zapier Natural Language Actions](https://nla.zapier.com/start/) 通过自然语言 API 接口，为您提供访问 Zapier 平台上 5000 多个应用和 20000 多个操作的能力。
>
>NLA 支持 `Gmail`、`Salesforce`、`Trello`、`Slack`、`Asana`、`HubSpot`、`Google Sheets`、`Microsoft Teams` 等数千个应用：[zapier.com/apps](https://zapier.com/apps)
>`Zapier NLA` 处理所有底层 API 认证和从自然语言到底层 API 调用再到为 LLM 返回简化输出的转换过程。核心理念是：您或您的用户通过类 OAuth 的配置界面暴露一组操作，然后通过 REST API 进行查询和执行。

NLA 提供 API 密钥和 OAuth 两种方式来签署 NLA API 请求。

1. 服务端（API 密钥）：适用于快速入门、测试和生产场景，此时 LangChain 仅使用开发者 Zapier 账户中暴露的操作（并使用开发者在 Zapier.com 上连接的账户）

2. 面向用户（OAuth）：适用于生产场景，当您部署面向最终用户的应用且 LangChain 需要访问最终用户在 Zapier.com 上暴露的操作和连接账户时使用

本快速入门主要关注服务端用例。请跳转至[使用 OAuth 访问令牌示例](#oauth)查看如何为面向用户场景配置 Zapier 的简短示例。查阅[完整文档](https://nla.zapier.com/start/)以获取完整的面向用户 OAuth 开发者支持。

本示例介绍如何将 Zapier 集成与 `SimpleSequentialChain` 和 `Agent` 配合使用。
代码如下：
...
以下代码片段展示如何使用已获取的 OAuth 访问令牌初始化包装器。请注意传入的参数与设置环境变量的区别。查阅[认证文档](https://nla.zapier.com/docs/authentication/#oauth-credentials)以获取完整的面向用户 OAuth 开发者支持。
