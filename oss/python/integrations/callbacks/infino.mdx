...
>[Infino](https://github.com/infinohq/infino) 是一个可扩展的遥测存储，专为日志、指标和追踪设计。Infino 既可以作为独立的可观测性解决方案，也可以作为可观测性栈中的存储层。

本示例展示了如何通过 `LangChain` 和 [Infino](https://github.com/infinohq/infino) 调用 OpenAI 和 ChatOpenAI 模型时跟踪以下内容：
...
# 创建回调处理程序。这将记录延迟、错误、Token 使用情况、Prompt 以及 Prompt 响应到 Infino。
...
# 向 OpenAI 模型提问的问题数量。我们在此限制为较短的数量，以便在运行此演示时节省费用。
num_questions = 10
...
我们现在使用 matplotlib 创建延迟、错误和消耗 Token 的图表。
