...
description: 学习如何在 LangSmith 中使用 OpenTelemetry 追踪 Temporal 工作流和活动。
---

[Temporal](https://temporal.io/) 是一个持久化执行平台，使开发者能够构建弹性的分布式应用程序。本指南将向您展示如何使用 OpenTelemetry 在 LangSmith 中追踪 Temporal 工作流和活动。

LangSmith 支持 OpenTelemetry (OTEL) 追踪摄取，这与 Temporal 原生的 OpenTelemetry 拦截器无缝集成。这使得您可以对工作流执行、活动以及其中的任何 LLM 调用进行完整的分布式追踪。
...
| 变量名              | 必需 | 描述                                             |
| ------------------- | ---- | ------------------------------------------------ |
| `LANGSMITH_API_KEY` | 是   | 您从设置中获取的 LangSmith API 密钥。            |
...
Go 使用 `langsmith-go` SDK 与 Temporal 的 OpenTelemetry 拦截器来自动追踪工作流和活动。
...
初始化 LangSmith 追踪器，创建 Temporal 的 OpenTelemetry 拦截器，并将其注册到 Temporal 客户端和 worker：
...
Python 使用 `temporalio` SDK 与 OpenTelemetry 拦截器，通过 OTLP 将追踪导出到 LangSmith。
...
创建一个配置了 OTLP 导出器的 OpenTelemetry `TracerProvider`，用于将追踪发送到 LangSmith：
...
创建带有 `TracingInterceptor` 的 Temporal 客户端并启动 worker：
...
TypeScript 使用 `@temporalio/sdk` 与 OpenTelemetry 拦截器将追踪发送到 LangSmith。
...
创建一个配置了 OTLP 导出器的 `NodeTracerProvider`，用于将追踪发送到 LangSmith：
...
创建一个带有 OpenTelemetry 拦截器的 worker 用于活动，以及一个工作流导出器用于工作流跨度：
...
设置自定义服务名称以区分不同的 Temporal worker 或服务：
...
对于高吞吐量的工作流，配置采样以减少追踪量：

<CodeGroup>

```go Go
// 注意：langsmith.NewTracer() 使用默认采样
// 对于自定义采样，直接使用 TracerProvider
...
3. **关闭时刷新**：在应用程序退出前调用 `provider.shutdown()` 以刷新待处理的跨度
4. **检查项目**：验证追踪是否发送到正确的项目（默认为 `"default"`）

### 缺少活动跨度

确保在客户端和 worker 上都配置了追踪拦截器：
- **客户端**：需要拦截器来启动工作流
- **Worker**：需要拦截器来执行活动
...
- **Python/TypeScript**：确保 OpenTelemetry SDK 使用追踪传播器正确初始化

### Worker 关闭挂起

如果追踪未刷新，请确保使用适当的超时调用关闭方法：
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/langsmith/trace-with-temporal.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>