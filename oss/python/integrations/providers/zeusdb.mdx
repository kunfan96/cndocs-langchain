...
本文档展示了如何使用 ZeusDB 为您的 LangChain 应用程序带来企业级的向量搜索能力。
...
此示例使用 *OpenAIEmbeddings*，它需要一个 OpenAI API 密钥 - [在此处获取您的 OpenAI API 密钥](https://platform.openai.com/api-keys)
...
ZeusDB 的企业级功能已完全集成到 LangChain 生态系统中，提供量化、持久化、高级搜索功能以及许多其他企业能力。
...
ZeusDB 持久化允许您将完全填充的索引保存到磁盘，并在稍后加载时完全恢复状态。这包括向量、元数据、HNSW 图以及（如果启用）产品量化模型。
...
- 您可以继续在 loaded_store 上使用所有向量存储 API（similarity_search、检索器等）。
...
返回来自 ZeusDB 的 `(Document, raw_distance)` 对（距离越低 = 越相似）。
如果您更喜欢 `[0, 1]` 范围内的归一化相关性分数，请使用 `similarity_search_with_relevance_scores`。
...
将向量存储转换为检索器可为您提供一个标准的 LangChain 接口，链（例如 RetrievalQA）可以调用该接口来获取上下文。在底层，它使用您选择的搜索类型（similarity 或 mmr）和 search_kwargs。
...
# 与 LangChain 表达式语言 (LCEL) 一起使用 - 仅需 langchain-core
...
**何时使用异步：** Web 服务器（FastAPI/Starlette）、执行并行搜索的代理/管道，或您希望非阻塞/并发检索的笔记本。如果您编写的是简单脚本，同步方法就足够了。

这些是**异步操作** - 常规同步方法的 async/await 版本。以下是每个操作的功能：
