---
title: "OpenGauss 向量存储集成"
description: "使用 LangChain Python 与 OpenGauss 向量存储集成。"
---

本笔记本介绍如何开始使用 openGauss 向量存储。[openGauss](https://opengauss.org/en/) 是一款高性能关系型数据库，具备原生向量存储和检索能力。此集成使得 LangChain 应用能够进行符合 ACID 标准的向量操作，将传统 SQL 功能与现代 AI 驱动的相似性搜索相结合。
向量存储。
...
| `distance_strategy` | DistanceStrategy.COSINE  | 用于检索的向量相似性度量。选项：euclidean（L2 距离）、cosine（角度距离，适用于文本嵌入）、manhattan（L1 距离，适用于稀疏数据）、negative_inner_product（归一化向量的点积）。\n 默认为 cosine。 |
