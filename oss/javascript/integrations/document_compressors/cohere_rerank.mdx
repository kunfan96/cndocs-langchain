...
对文档进行重排序可以极大地提升任何 RAG 应用和文档检索系统的性能。
...
如果我们希望从方法本身返回文档，可以使用 `.compressDocuments()` 方法。
...
正如预期的那样，具有最高 `relevanceScore` 的文档是引用华盛顿特区的那个，分数为 `98.7%`！

### 与 `CohereClient` 一起使用

如果您在 Azure、AWS Bedrock 或独立实例上使用 Cohere，您可以使用 `CohereClient` 来创建一个带有您端点的 `CohereRerank` 实例。
