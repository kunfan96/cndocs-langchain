---
title: "ChatReka 集成"
description: "使用 LangChain Python 集成 ChatReka 聊天模型。"
---

本指南提供了开始使用 Reka [聊天模型](/oss/python/langchain/models)的快速概览。

Reka 有多个聊天模型。您可以在 [Reka 文档](https://docs.reka.ai/available-models)中找到关于最新模型及其费用、上下文窗口和支持输入类型的信息。
...
要访问 Reka 模型，您需要创建 Reka 开发者账户，获取 API key，并通过 `pip install reka-api` 安装 `langchain_community` 集成包和 reka Python 包。

### 凭据

前往 [platform.reka.ai/](https://platform.reka.ai/) 注册 Reka 并生成 API key。完成后，设置 REKA_API_KEY 环境变量：
...
首先需要创建要使用的工具。我们主要选择的工具是 Tavily —— 一个搜索引擎。LangChain 中内置了一个工具，可轻松将 Tavily 搜索引擎作为工具使用。
...
现在我们可以了解如何让该模型进行工具调用。为此，我们使用 .bind_tools 让语言模型了解这些工具。

```python
model_with_tools = model.bind_tools(tools)
```

现在我们可以调用该模型了。首先用普通消息调用它，看看它如何响应。我们可以查看 content 字段和 tool_calls 字段。
...
这还没有调用该工具——它只是告诉我们要调用。为了实际调用它，我们需要创建我们的智能体。

# 创建智能体

现在我们已经定义了工具和 LLM，可以创建智能体了。我们将使用 LangGraph 来构建智能体。目前，我们使用高级接口来构建智能体，但 LangGraph 的好处在于这个高级接口背后有一个低级别、高度可控的 API，以防您需要修改智能体逻辑。

现在，我们可以使用 LLM 和工具初始化智能体。

注意，我们传入的是 model 而非 model_with_tools。这是因为 [`create_agent`](https://reference.langchain.com/python/langchain/agents/factory/create_agent) 会在底层为我们调用 `.bind_tools`。
...
为了准确了解底层发生了什么（并确保它没有调用工具），我们可以查看 LangSmith 追踪记录：[smith.langchain.com/public/2372d9c5-855a-45ee-80f2-94b63493563d/r](https://smith.langchain.com/public/2372d9c5-855a-45ee-80f2-94b63493563d/r)
...
我们可以查看 LangSmith 追踪记录以确认它有效地调用了搜索工具。
...
我们已经了解了如何使用 `invoke` 调用智能体以获得最终响应。如果智能体执行多个步骤，这可能需要一些时间。为了显示中间进度，我们可以在消息发生时将其流式传输回来。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/chat/reka.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
