...
ModelScope（[主页](https://www.modelscope.cn/) | [GitHub](https://github.com/modelscope/modelscope)）建立在“模型即服务”（Model-as-a-Service，MaaS）的理念之上。它旨在汇集 AI 社区最先进的机器学习模型，并简化在实际应用中利用 AI 模型的过程。本仓库开源的核心 ModelScope 库提供了允许开发者执行模型推理、训练和评估的接口与实现。

这将帮助您开始使用 LangChain 集成 ModelScope 嵌入模型。
