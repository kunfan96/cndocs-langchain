---
title: "Agentql 集成"
description: "使用 LangChain Python 与 Agentql 工具集成。"
---

[AgentQL](https://www.agentql.com/) 工具提供网页交互和结构化数据提取功能，可从任何网页使用 [AgentQL 查询](https://docs.agentql.com/agentql-query) 或自然语言提示提取数据。AgentQL 可跨多种语言和网页使用，不会因时间推移和变化而失效。

## 概述

AgentQL 提供以下三种工具：

- **`ExtractWebDataTool`**：根据 URL 从网页提取结构化数据为 JSON，使用 [AgentQL 查询](https://docs.agentql.com/agentql-query/query-intro) 或数据的自然语言描述。

以下两种工具也打包为 `AgentQLBrowserToolkit`，必须与 `Playwright` 浏览器或通过 Chrome DevTools Protocol (CDP) 的远程浏览器实例一起使用：

- **`ExtractWebDataBrowserTool`**：从浏览器中的活动网页提取结构化数据为 JSON，使用 [AgentQL 查询](https://docs.agentql.com/agentql-query/query-intro) 或自然语言描述。
...
要运行此笔记本，需安装 `Playwright` 浏览器并配置 Jupyter Notebook 的 `asyncio` 循环。

```python
!playwright install

# 此导入仅适用于 Jupyter Notebook，因为它们有自己的事件循环
import nest_asyncio
...
要使用 AgentQL 工具，您需要从 [AgentQL Dev Portal](https://dev.agentql.com/) 获取自己的 API 密钥，并设置 AgentQL 环境变量。
...
- `is_stealth_mode_enabled`：是否启用实验性反机器人规避策略。此功能可能无法在所有网站上始终有效。启用此模式后，数据提取可能需要更长时间才能完成。**默认为 `False`。**
- `wait_for`：提取数据前等待页面加载的秒数。**默认为 `0`。**
- `is_scroll_to_bottom_enabled`：是否在提取数据前滚动到页面底部。**默认为 `False`。**
- `mode`：`"standard"` 使用深度数据分析，而 `"fast"` 以牺牲部分分析深度为代价换取速度，适用于大多数用例。[在此指南中了解有关模式的更多信息。](https://docs.agentql.com/accuracy/standard-mode) **默认为 `"fast"`。**
...
要实例化 **ExtractWebDataBrowserTool**，您需要将工具与浏览器实例连接。
...
要实例化 **GetWebElementBrowserTool**，您需要将工具与浏览器实例连接。
...
此工具在底层使用 AgentQL 的 REST API，将公开可用网页的 URL 发送到 AgentQL 的端点。这不适用于私有页面或登录会话。对于这些用例，请使用 `ExtractWebDataBrowserTool`。

- `url`：要从中提取数据的网页 URL。
- `query`：要执行的 AgentQL 查询。如果要提取精确结构化数据，请使用 AgentQL 查询。在文档中了解[如何编写 AgentQL 查询](https://docs.agentql.com/agentql-query) 或在 [AgentQL Playground](https://dev.agentql.com/playground) 中测试一个。
- `prompt`：要从页面提取数据的自然语言描述。AgentQL 将从您的提示中推断数据的结构。如果要提取由自由形式语言定义的数据而不定义特定结构，请使用 `prompt`。
...
# 禁用 'visible_only' 将允许我们单击页面上不可见的元素
...
您可以在链中使用 AgentQL 工具，首先将其绑定到[工具调用模型](/oss/python/langchain/tools/)，然后调用它：
...
您可以使用 `AgentQLBrowserToolkit` 将 AgentQL 工具与 AI 代理一起使用。此工具包包括 `ExtractDataBrowserTool` 和 `GetWebElementBrowserTool`。以下是结合 AgentQL 工具包和 Playwright 工具的代理浏览器操作示例。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/tools/agentql.mdx) 或[提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
