---
title: "Replicate 集成"
description: "使用 LangChain Python 与 Replicate LLM 集成。"
---

>[Replicate](https://replicate.com/blog/machine-learning-needs-better-tools) 在云端运行机器学习模型。我们拥有一个开源模型库，您只需几行代码即可运行。如果您正在构建自己的机器学习模型，Replicate 可以轻松地将其大规模部署。
...
要运行此笔记本，您需要创建一个 [replicate](https://replicate.com) 帐户并安装 [replicate python 客户端](https://github.com/replicate/replicate-python)。
...
在 [replicate 探索页面](https://replicate.com/explore)上找到一个模型，然后按照以下格式粘贴模型名称和版本：model_name/version。
...
再举一个例子，对于这个 [dolly 模型](https://replicate.com/replicate/dolly-v2-12b)，点击 API 标签页。模型名称/版本将是：`replicate/dolly-v2-12b:ef0e1aefc61f8e096ebe4db6b2bacc297daf2ef6899f0f7e001ec445893500e5`
...
您可以选择在生成时流式传输响应，这对于向用户展示耗时生成过程的交互性很有帮助。有关详细信息，请参阅 [流式传输](/oss/python/langchain/streaming) 的详细文档。
...
您还可以指定停止序列。如果您有一个明确的停止序列用于生成，并且无论如何都会解析它，那么最好（更便宜且更快！）在达到一个或多个停止序列时就取消生成，而不是让模型一直运行到指定的 `max_length`。无论您是否处于流式模式，停止序列都有效，并且 Replicate 只会向您收取直到停止序列的生成费用。
...
LangChain 的全部意义在于……链式调用！以下是如何做到这一点的示例。
