...
本指南将引导您解决在运行 LangSmith 自托管实例时可能遇到的常见问题。

在运行 LangSmith 时，您可能会遇到意外的 500 错误、性能缓慢或其他问题。本指南将帮助您诊断并解决这些问题。

## 获取有用信息

要诊断和解决问题，您首先需要检索一些相关信息。以下部分将说明如何针对 Kubernetes 或 Docker 设置执行此操作，以及如何获取有用的浏览器信息。
...
故障排除的第一步是收集有关您的 LangSmith 部署的重要调试信息。服务日志、kubernetes 事件和容器的资源利用率可以帮助识别问题的根本原因。

您可以运行我们的 [k8s 故障排除脚本](https://github.com/langchain-ai/helm/blob/main/charts/langsmith/scripts/get_k8s_debugging_info.sh)，该脚本将拉取所有相关的 kubernetes 信息并将其输出到一个文件夹中以供调查。该脚本还会将此文件夹压缩为 zip 文件以便共享。以下是运行此脚本的示例，假设您的 langsmith 部署是在 `langsmith` 命名空间中启动的：

```bash
bash get_k8s_debugging_info.sh --namespace langsmith
```

然后，您可以检查生成的文件夹的内容，查找任何相关的错误或信息。如果您希望 LangSmith 团队协助调试，请将此 zip 文件分享给团队。
...
如果您遇到的问题表现为浏览器错误，检查 HAR 文件也可能很有帮助，因为它可能包含关键信息。要获取 HAR 文件，您可以按照[此指南](https://support.langchain.com/articles/9042697994-how-to-generate-a-har-file-for-troubleshooting)操作，该指南解释了适用于各种浏览器的简短过程。
...
4. 更新您的 helm chart `langsmith_config.yaml` 为新大小（例如 `100 Gi`）

5. 删除 clickhouse statefulset `kubectl delete statefulset langsmith-clickhouse --cascade=orphan -n <namespace>`

6. 应用具有更新大小的 helm chart（您可以按照[升级指南](/langsmith/self-host-upgrades)操作）
...
当 ClickHouse 数据库与我们的迁移处于不一致状态时，会发生此错误。您需要重置到较早的数据库版本，然后重新运行您的升级/迁移。
...
1. 编辑您的 `langsmith_config.yaml` 并增加 `frontend.maxBodySize` [值](https://github.com/langchain-ai/helm/blob/main/charts/langsmith/values.yaml#L519)。这可能看起来像这样：
...
当您的用户没有在 Clickhouse 中创建行策略所需的权限时，会发生此错误。部署 Docker 部署时，您还需要从 github 仓库复制 `users.xml` 文件。这会将 `<access_management>` 标签添加到 `users.xml` 文件中，从而允许用户创建行策略。以下是我们期望使用的默认 `users.xml` 文件。
...
在某些环境中，AquaSec 可能会阻止 ClickHouse 正确启动。这可能表现为 ClickHouse pod 不发出任何日志且无法被标记为就绪。
通常这是由于 AquaSec 设置了 `LD_PRELOAD`，这会干扰 ClickHouse。要解决此问题，您可以将以下环境变量添加到您的 ClickHouse 部署中：
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/langsmith/troubleshooting.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>