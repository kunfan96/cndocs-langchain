...
本笔记本展示了 UpTrain 回调处理器如何无缝集成到您的管道中，以促进各种评估。我们选择了一些我们认为适合评估链的评估。这些评估会自动运行，结果显示在输出中。有关 UpTrain 评估的更多详细信息，请参阅 [GitHub](https://github.com/uptrain-ai/uptrain?tab=readme-ov-file#pre-built-evaluations-we-offer-)。
...
- **[事实准确性](https://docs.uptrain.ai/predefined-evaluations/context-awareness/factual-accuracy)**：评估 LLM 是否产生幻觉或提供不正确的信息。
...
重排序涉及根据与查询的相关性对节点进行重新排序，并选择前 n 个节点。由于重排序完成后节点数量可能会减少，我们执行以下评估：

- **[上下文重排序](https://docs.uptrain.ai/predefined-evaluations/context-awareness/context-reranking)**：检查重排序后的节点顺序是否比原始顺序更相关于查询。
- **[上下文简洁性](https://docs.uptrain.ai/predefined-evaluations/context-awareness/context-conciseness)**：检查减少的节点数量是否仍然提供所有所需信息。
...
或者，您可以使用 UpTrain 的托管服务来评估您的模型。您可以 [创建一个免费的 UpTrain 帐户](https://uptrain.ai/) 并获得免费试用积分。如果您想要更多试用积分，[可以与 UpTrain 的维护者预约通话](https://calendly.com/uptrain-sourabh/30min)。
...
笔记本将提示您输入 API 密钥。您可以通过更改下面单元格中的 `key_type` 参数，在 OpenAI API 密钥和 UpTrain API 密钥之间进行选择。
...
UpTrain 回调处理器将在生成后自动捕获查询、上下文和响应，并对响应运行以下三项评估（*评分从 0 到 1*）：
...
重排序过程涉及根据与查询的相关性对节点进行重新排序，并选择前 n 个节点。由于重排序完成后节点数量可能会减少，我们执行以下评估：
