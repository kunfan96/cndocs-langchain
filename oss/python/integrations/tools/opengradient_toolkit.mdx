...
toolkit = OpenGradientToolkit(
    # 如果您已设置环境变量 OPENGRADIENT_PRIVATE_KEY，则无需此项
...
volatility_tool = toolkit.create_run_model_tool(
    ...
    tool_description="为 ETH/USDT 交易对生成波动率测量",
...
读取工作流是定期运行存储在智能合约上的模型并使用实时预言机数据的计划推理。有关这些的更多信息可以[在此处找到](https://docs.opengradient.ai/developers/sdk/ml_workflows.html)。
...
forecast_tool = toolkit.create_read_workflow_tool(
    ...
    tool_description="从已部署的工作流读取 ETH 价格的最新预测",
