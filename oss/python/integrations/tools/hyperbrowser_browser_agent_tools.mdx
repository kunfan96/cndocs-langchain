...
[Hyperbrowser](https://hyperbrowser.ai) 是一个用于运行浏览器代理和扩展无头浏览器的平台。它允许您大规模启动和管理浏览器会话，并为任何网络抓取需求提供易于使用的解决方案，例如抓取单个页面或爬取整个网站。

主要功能：

- **即时可扩展性** - 无需基础设施麻烦，即可在几秒钟内启动数百个浏览器会话
...
Claude Computer Use、OpenAI CUA 和 Browser Use 提供以下可用参数：

- `task`：使用代理执行的任务
- `max_steps`：代理为完成任务可采取的最大交互步骤数
