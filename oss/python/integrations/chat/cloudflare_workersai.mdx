...
本指南将帮助您开始使用 CloudflareWorkersAI [聊天模型](/oss/python/langchain/models)。有关所有 ChatCloudflareWorkersAI 功能和配置的详细文档，请访问 [API 参考](https://python.langchain.com/docs/integrations/chat/cloudflare_workersai/)。
...
要访问 CloudflareWorkersAI 模型，您需要创建 CloudflareWorkersAI 账户，获取 API 密钥，并安装 `langchain-cloudflare` 集成包。

### 凭据

访问 [www.cloudflare.com/developer-platform/products/workers-ai/](https://www.cloudflare.com/developer-platform/products/workers-ai/) 注册 CloudflareWorkersAI 并生成 API 密钥。完成后，设置 CF_AI_API_KEY 环境变量和 CF_ACCOUNT_ID 环境变量：
