...
本指南提供了在 [langchain_discord](/oss/python/integrations/tools/) 中开始使用 Discord 工具的快速概览。有关每个工具和配置的更多详细信息，请参阅您仓库中的文档字符串或相关文档页面。
...
此集成要求您将 `DISCORD_BOT_TOKEN` 设置为环境变量，以向 Discord API 进行身份验证。
...
以下示例展示了如何在 `langchain_discord` 中实例化 Discord 工具。请根据您的具体用法进行调整。
...
以下是一个完整示例，展示了如何在链或代理中与 LLM 集成 `DiscordReadMessages` 和 `DiscordSendMessage` 工具。此示例假设您有一个函数（如 [`create_agent`](https://reference.langchain.com/python/langchain/agents/factory/create_agent)），该函数设置了一个 LangChain 风格的代理，能够在适当时调用工具。
