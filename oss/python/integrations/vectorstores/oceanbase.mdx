...
检查与 OceanBase 的连接并为向量数据设置内存使用比例：
...
配置嵌入模型的 API 密钥。这里我们以 `DashScopeEmbeddings` 为例。当使用上述 Docker 镜像部署 `Oceanbase` 时，只需按照以下脚本设置 `host`、`port`、`user`、`password` 和 `database name`。对于其他部署方法，请根据实际情况设置这些参数。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/vectorstores/oceanbase.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
