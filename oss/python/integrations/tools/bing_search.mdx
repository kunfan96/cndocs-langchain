---
title: "Bing搜索集成"
description: "使用LangChain Python集成Bing搜索工具。"
---

> [Bing搜索](https://learn.microsoft.com/en-us/bing/search-apis/bing-web-search/)是一项Azure服务，可提供安全、无广告、基于位置的搜索结果，从数十亿网络文档中呈现相关信息。通过利用Bing通过单次API调用即可梳理数十亿网页、图片、视频和新闻的能力，帮助您的用户从万维网上找到他们所需的内容。

## 设置

按照[说明](https://learn.microsoft.com/en-us/bing/search-apis/bing-web-search/create-bing-search-service-resource)创建Azure Bing搜索v7服务，并获取订阅密钥。
...
通过BingSearch运行查询并返回摘要、标题和链接元数据。
...
我们在此展示如何将其作为[代理](/oss/python/langchain/agents)的一部分使用。我们使用OpenAI Functions Agent，因此需要设置并安装所需的依赖项。我们还将使用[LangSmith Hub](https://smith.langchain.com/hub)来获取提示，因此也需要安装它。
...
{'input': 'What happened in the latest burning man floods?',
 'output': '在最新的火人节上，大雨导致洪水，造成数千名参与者滞留。该节日在内华达州的黑岩森林举行，约有7万人聚集参加活动。过量的降雨导致该地区部分地区发生山洪暴发。结果，营地被脚踝深的泥浆填满，使人们难以离开。当局正在调查节日现场的一起死亡事件，该事件受到了洪水的影响。然而，在随后的几天里，随着雨势减弱，数千名火人节参与者得以大规模撤离。'}
```
...
<Callout icon="edit">
    [在GitHub上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/tools/bing_search.mdx)或[提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>