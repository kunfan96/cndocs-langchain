---
title: "Hyperbrowser 浏览器代理集成"
description: "使用 LangChain Python 与 Hyperbrowser 浏览器代理工具集成。"
---

[Hyperbrowser](https://hyperbrowser.ai) 是一个用于运行、运行浏览器代理和扩展无头浏览器的平台。它允许您大规模启动和管理浏览器会话，并为任何网页抓取需求提供易于使用的解决方案，例如抓取单个页面或爬取整个网站。
...
Hyperbrowser 提供了强大的浏览器代理工具，使 AI 模型能够以编程方式与 Web 浏览器交互。这些浏览器代理可以导航网站、填写表单、点击按钮、提取数据并执行复杂的 Web 自动化任务。
...
- **Claude Computer Use Tool**：与 Anthropic 的 Claude 计算机使用功能集成
...
要访问 Hyperbrowser 工具，您需要安装 `langchain-hyperbrowser` 集成包，并创建一个 Hyperbrowser 帐户并获取 API 密钥。
...
`HyperbrowserOpenAICUATool` 是一个专门的工具，通过 Hyperbrowser 利用 OpenAI 的 Computer Use Agent (CUA) 功能。
...
`HyperbrowserClaudeComputerUseTool` 是一个专门的工具，通过 Hyperbrowser 利用 Claude 的计算机使用功能。
...
- `task`：使用代理执行的任务
- `max_steps`：代理为完成任务可以采取的最大交互步骤数
- `session_options`：浏览器会话配置

更多详细信息，请参阅各自的 API 参考：
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/tools/hyperbrowser_browser_agent_tools.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>