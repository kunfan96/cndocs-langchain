...
description: "使用 LangChain Python 与 VectorizeRetriever 检索器集成。"
---

本笔记本展示如何使用 LangChain Vectorize 检索器。

> [Vectorize](https://vectorize.io/) 帮助您更快、更轻松地构建 AI 应用程序。
> 它自动化数据提取，使用 RAG 评估找到最佳向量化策略，
> 并让您快速为非结构化数据部署实时 RAG 管道。
> 您的向量搜索索引保持最新，并与您现有的向量数据库集成，
> 因此您完全掌控您的数据。
> Vectorize 处理繁重的工作，让您专注于构建强大的 AI 解决方案，而不会被数据管理所困扰。
...
2. 在 [访问令牌](https://docs.vectorize.io/rag-pipelines/retrieval-endpoint#access-tokens) 部分生成一个访问令牌。
3. 收集您的组织 ID。从浏览器 URL 中，在 `/organization/` 之后提取 UUID。
...
如果您想从单个查询获取自动追踪，也可以通过取消注释下方来设置您的 [LangSmith](/langsmith/home) API 密钥：
