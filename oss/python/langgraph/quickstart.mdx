...
本快速入门演示如何使用 LangGraph 图 API 或函数式 API 构建一个计算器代理。

<Tip>
    **正在使用 AI 编码助手？**

    - 安装 [LangChain 文档 MCP 服务器](/use-these-docs)，让您的代理能够访问最新的 LangChain 文档和示例。
    - 安装 [LangChain 技能](https://github.com/langchain-ai/langchain-skills)，以提升您的代理在 LangChain 生态系统任务上的性能。
</Tip>

- 如果您倾向于将代理定义为节点和边的图，请[使用图 API](#use-the-graph-api)。
- 如果您倾向于将代理定义为单个函数，请[使用函数式 API](#use-the-functional-api)。

有关概念信息，请参阅 [图 API 概览](/oss/python/langgraph/graph-api) 和 [函数式 API 概览](/oss/python/langgraph/functional-api)。
...
<Note>
    在函数式 API 中，您无需显式定义节点和边，而是在单个函数内编写标准控制流逻辑（循环、条件语句）。
</Note>
