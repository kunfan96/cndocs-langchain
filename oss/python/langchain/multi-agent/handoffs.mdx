...
<Note>
**为什么包含 `ToolMessage`？** 当 LLM 调用工具时，它期望一个响应。具有匹配 `tool_call_id` 的 `ToolMessage` 完成了此请求-响应周期——没有它，对话历史将变得不完整。每当您的交接工具更新消息时，这都是必需的。
</Card>
...
<Note>
**为什么不传递所有子代理消息？** 虽然您可以在交接中包含完整的子代理对话，但这通常会产生问题。接收代理可能会被无关的内部推理混淆，并且令牌成本会不必要地增加。通过仅传递交接对，您可以保持父图的上下文专注于高层协调。如果接收代理需要额外的上下文，请考虑在 `ToolMessage` 内容中总结子代理的工作，而不是传递原始消息历史。
</Note>
