...
按照如下方式安装 `@langchain/community` 包：
...
通过此集成，您可以使用 DeepInfra 嵌入模型为您的文本数据获取嵌入。以下是嵌入模型的[链接](https://deepinfra.com/models/embeddings)。

首先，您需要在 DeepInfra 网站上注册，并从 [DeepInfra 仪表板获取 API 令牌](https://deepinfra.com/dash/api_keys)。您可以从模型卡片中复制名称，并开始在代码中使用它们。

要使用 `DeepInfraEmbeddings` 类，您需要一个来自 DeepInfra 的 API 令牌。您可以将此令牌直接传递给构造函数，或将其设置为环境变量（`DEEPINFRA_API_TOKEN`）。
