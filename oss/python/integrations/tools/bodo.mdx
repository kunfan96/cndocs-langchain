...
本笔记本概述了如何使用 [langchain-bodo](https://pypi.org/project/langchain-bodo/) 集成包创建代理并对大型数据集执行问答，该包底层使用 [Bodo DataFrames](https://github.com/bodo-ai/Bodo) 和 `Python` 代理。

Bodo DataFrames 是一个高性能的 DataFrame 库，只需简单更改导入即可自动加速和扩展 Pandas 代码（参见下面的示例）。由于其强大的 Pandas 兼容性，Bodo DataFrames 使通常擅长生成 Pandas 代码的 LLM 能够更高效地回答关于更大数据集的问题，并将生成的代码扩展到超出 Pandas 的限制。

**注意：`Python` 代理执行 LLM 生成的 Python 代码 - 如果 LLM 生成的 Python 代码有害，这可能会很糟糕。请谨慎使用。**

## 设置

在运行示例之前，请复制 [titanic 数据集](https://raw.githubusercontent.com/pandas-dev/pandas/main/doc/data/titanic.csv) 并将其本地保存为 `titanic.csv`。
...
这展示了如何使用 OPENAI_FUNCTIONS 代理类型初始化代理。请注意，这是上述方法的替代方案。
...
此示例展示了将 Bodo DataFrame 传递给 `create_bodo_dataframes_agent` 并进行一些额外预处理的稍微复杂一些的用例。
由于 Bodo DataFrames 是惰性求值的，如果回答问题不需要所有列，您可能可以节省计算资源。请注意，传递给代理的 DataFrame 也可以大于可用内存。
...
请注意，虽然 Bodo DataFrames 支持 Pandas 中大多数常见的计算密集型操作，但如果代理生成的代码当前不受支持（参见下面的警告），DataFrame 将被转换回 Pandas 以防止错误。
...
默认情况下，DataFrame 的头部作为 markdown 表格嵌入到提示中。
由于 Bodo DataFrames 是惰性求值的，此头部操作可以被优化，但在某些情况下仍然可能很慢。作为一种优化，您可以将头部的行数设置为 0，以便在提示期间不进行任何求值。
...
您也可以将一个或多个 Pandas DataFrames 传递给 `create_bodo_dataframes_agent`。DataFrame 将在传递给代理之前被转换为 Bodo。
