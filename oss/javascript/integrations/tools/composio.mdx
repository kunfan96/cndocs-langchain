---
title: "Composio 集成"
description: 通过 Composio 的统一 API 平台为 AI 代理访问 500 多个工具和集成，支持 OAuth 处理、事件驱动的工作流和多用户支持。
---

[Composio](https://composio.dev) 是一个集成平台，可以访问 GitHub、Slack、Notion 等流行应用程序中的 500 多个工具。它使 AI 代理能够通过统一的 API 与外部服务进行交互，处理身份验证、权限和事件驱动的工作流。
...
- **500+ 工具访问**：针对 GitHub、Slack、Gmail、Jira、Notion 等的预构建集成
- **身份验证管理**：处理 OAuth 流程、API 密钥和身份验证状态
- **事件驱动的工作流**：根据外部事件（新的 Slack 消息、GitHub 问题等）触发代理
- **细粒度权限**：控制每个用户的工具访问和数据暴露
...
设置 [LangSmith](/langsmith/home) 进行跟踪也很有帮助：
...
使用 LangChain 提供程序初始化 Composio，并从特定工具包中获取工具。每个工具包代表一个具有多个工具（您可以执行的操作）的服务（例如 GitHub、Slack）。
...
这是一个使用 Composio 工具和 LangGraph 代理与 HackerNews 交互的完整示例：
...
Composio 支持基于外部事件触发代理。当连接的应用程序中发生事件（如新的 GitHub 提交或 Slack 消息）时，触发器会自动向您的应用程序发送结构化有效负载。
...
对于生产，请在 [Composio 仪表板](https://platform.composio.dev/settings/events)中配置 webhooks：
