...
description: "使用 LangChain JavaScript 与 CohereEmbeddings 嵌入模型进行集成。"
---

本文档将帮助您开始使用 LangChain 的 CohereEmbeddings [嵌入模型](/oss/javascript/integrations/embeddings)。有关 `CohereEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-cohere/CohereEmbeddings)。

## 概述

### 集成详情

| 类 | 包 | 本地 | [Python 支持](https://python.langchain.com/docs/integrations/embeddings/cohere/) | 下载量 | 版本 |
| :--- | :--- | :---: | :---: |  :---: | :---: |
...
const embeddings = new CohereEmbeddings({
  apiKey: "YOUR-API-KEY", // 在 Node.js 中默认为 process.env.COHERE_API_KEY
...
在底层，向量存储和检索器实现会调用 `embeddings.embedDocument(...)` 和 `embeddings.embedQuery(...)`，分别为 `fromDocuments` 和检索器的 `invoke` 操作中使用的文本创建嵌入。
...
有关所有 `CohereEmbeddings` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-cohere/CohereEmbeddings)。

---

<div className="source-links">
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/javascript/integrations/embeddings/cohere.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
