---
title: "PGVector 集成"
description: "使用 LangChain Python 与 PGVector 向量存储进行集成。"
---

> 这是一个 LangChain 向量存储抽象的实现，使用 `postgres` 作为后端并利用 `pgvector` 扩展。
...
* `langchain-postgres` 仅与 psycopg3 兼容。请将您的连接字符串从 `postgresql+psycopg2://...` 更新为 `postgresql+psycopg://langchain:langchain@...`（是的，驱动程序名称是 `psycopg` 而不是 `psycopg3`，但它将使用 `psycopg3`）。
* 嵌入存储和集合的架构已更改，以使 `add_documents` 能与用户指定的 ID 正确配合工作。
* 现在必须传递一个显式的连接对象。

目前，**没有机制**支持在架构更改时轻松进行数据迁移。向量存储中的任何架构更改都需要用户重新创建表并重新添加文档。
如果这是一个问题，请使用不同的向量存储。如果不是，此实现应该适合您的用例。
...
运行此笔记本不需要任何凭据，只需确保您已下载 `langchain-postgres` 包并正确启动了 postgres 容器。

如果您希望获得最佳级别的模型调用自动跟踪，您也可以通过取消注释下方来设置您的 [LangSmith](/langsmith/home) API 密钥：
...
# 请参阅上面的 docker 命令以启动启用了 pgvector 的 postgres 实例。
...
有关可以在 `PGVector` 向量存储上执行的不同搜索的完整列表，请参阅 [API 参考](https://reference.langchain.com/python/langchain-postgres/vectorstores/PGVector)。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/vectorstores/pgvector.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
