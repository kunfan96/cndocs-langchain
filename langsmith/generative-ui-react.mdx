...
生成式用户界面（Generative UI）允许智能体超越文本，生成丰富的用户界面。这使得创建更具交互性和上下文感知能力的应用程序成为可能，其中用户界面能够根据对话流程和 AI 响应进行适配。

![Agent Chat 展示了一个关于预订/住宿的提示，以及一组内联渲染为 UI 组件的酒店列表卡片（图片、标题、价格、位置）。](/langsmith/images/generative-ui-sample.jpg)

LangSmith 支持将您的 React 组件与图代码放在一起。这使您可以专注于为图构建特定的 UI 组件，同时轻松插入现有的聊天界面（如 [Agent Chat](https://agentchat.vercel.app)），并且仅在实际需要时加载代码。
...
`ui` 部分指向图将使用的 UI 组件。默认情况下，我们建议使用与图名称相同的键，但您可以根据自己的喜好拆分组件，有关更多详细信息，请参阅[自定义 UI 组件的命名空间](#自定义-ui-组件的命名空间)。
...
在底层，`LoadExternalComponent` 将从 LangSmith 获取 UI 组件的 JS 和 CSS，并在 shadow DOM 中渲染它们，从而确保与应用程序其余部分的样式隔离。

## 操作指南

### 在客户端提供自定义组件

如果您已经在客户端应用程序中加载了组件，您可以提供一个这样的组件映射，以便直接渲染，而无需从 LangSmith 获取 UI 代码。
...
您可以通过向 `LoadExternalComponent` 组件提供 `meta` 属性来向客户端组件传递额外上下文。
...
您可以在节点执行完成之前，通过使用 `useStream()` hook 的 `onCustomEvent` 回调来流式传输 UI 消息。当 LLM 生成响应时更新 UI 组件，这尤其有用。
...
然后，您可以通过调用 `ui.push()` / `push_ui_message()` 并使用与您希望更新的 UI 消息相同的 ID 来推送更新到 UI 组件。
...
类似于可以通过附加 RemoveMessage 从状态中移除消息，您可以通过调用 `remove_ui_message` / `ui.delete` 并使用 UI 消息的 ID 来从状态中移除 UI 消息。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/langsmith/generative-ui-react.mdx) 或 [提交问题](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>