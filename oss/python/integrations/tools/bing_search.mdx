---
title: "Bing 搜索集成"
description: "使用 LangChain Python 集成 Bing 搜索工具。"
---

> [Bing Search](https://learn.microsoft.com/en-us/bing/search-apis/bing-web-search/) 是一项 Azure 服务，可提供安全、无广告、位置感知的搜索结果，从数十亿网页文档中呈现相关信息。通过 Bing 强大的搜索能力，只需一次 API 调用即可搜索数十亿网页、图片、视频和新闻，帮助用户找到所需内容。

## 设置

按照[说明](https://learn.microsoft.com/en-us/bing/search-apis/bing-web-search/create-bing-search-service-resource)创建 Azure Bing Search v7 服务并获取订阅密钥。
...
以下展示如何将其作为[智能体](/oss/python/langchain/agents)的一部分使用。我们使用 OpenAI Functions Agent，因此需要安装相关依赖。我们还将使用 [LangSmith Hub](https://smith.langchain.com/hub) 拉取提示词，因此也需要安装该包。
