...
通过嵌入比较文档的优势在于可以跨多种语言工作。"Harrison says hello" 和 "Harrison dice hola" 在向量空间中会占据相似的位置，因为它们在语义上具有相同的含义。

然而，在向量化之前使用 LLM **将文档翻译成其他语言** 仍然很有用。当用户期望用不同语言查询知识库，或者对于特定语言没有最先进的嵌入模型时，这一点尤其有帮助。
...
翻译文档后，结果将作为新文档返回，其 page_content 已翻译为目标语言。异步版本在文档被分成多个部分时可以提高性能。它还将确保以正确的顺序返回输出。
