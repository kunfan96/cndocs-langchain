...
> [AskNews](https://asknews.app) 通过单个自然语言查询，为任何 LLM 注入最新的全球新闻（或历史新闻）。具体而言，AskNews 每天对超过 30 万篇文章进行翻译、摘要提取、实体提取和索引处理，并存入热冷向量数据库。AskNews 为你提供低延迟端点访问这些向量数据库。当你查询 AskNews 时，你会获得一个经过提示优化的字符串，其中包含所有最相关的增强信息（例如实体、分类、翻译、摘要）。这意味着你不需要管理自己的新闻 RAG，也不需要担心如何以精简的方式向 LLM 传达新闻信息。
> AskNews 还致力于透明度，这就是为什么我们的覆盖范围受到监控，并分布在数百个国家、13 种语言和 5 万个来源。如果你想追踪我们的来源覆盖情况，可以访问我们的[透明度仪表板](https://asknews.app/en/transparency)。
...
设置 [LangSmith](https://smith.langchain.com/) 以获得最佳可观测性（有用但非必须）
