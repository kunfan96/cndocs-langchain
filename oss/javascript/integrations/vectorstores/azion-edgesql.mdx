...
description: "使用 LangChain JavaScript 与 AzionVectorStore 进行集成。"
---

`AzionVectorStore` 用于管理和搜索文档集合，通过向量嵌入直接在 Azion 的边缘平台（Edge Platform）上使用 Edge SQL 进行操作。

本指南提供了快速入门 `AzionVectorStore` [向量存储](/oss/javascript/integrations/vectorstores) 的概览。有关所有 `AzionVectorStore` 功能和配置的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-community/vectorstores/azion_edgesql/AzionVectorStore)。
...
有关所有 `AzionVectorStore` 功能和配置的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-community/vectorstores/azion_edgesql/AzionVectorStore)。
