...
此示例使用 OpenAIEmbeddings，需要 OpenAI API key：[在此获取您的 OpenAI API key](https://platform.openai.com/api-keys)
...
*方案 1：🔑 每次输入 API key*
在 Jupyter 中使用 getpass 安全地为当前会话输入 key：
...
*方案 2：🗂️ 使用 .env 文件*
将 key 保存在本地 .env 文件中并使用 python-dotenv 自动加载
